...
function DeliveryInfo({ order }) {
  ...
  return <section className={`orderDeliveryInfo ${state.status.toLowerCase()}`} aria-live="polite">
...
export default function UnifiedOrdersCenter({ center, children }) {
  ...
  return <section className="unifiedOrdersCenter">
    <section className="unifiedOrdersHero"><div><span>PHASE 11-3 · PACK & SHIP</span><h1>통합 주문·배송센터</h1><p>네이버·쿠팡·Cafe24 판매자배송 주문을 한곳에서 보고, 오늘 포장·출고할 주문을 안전하게 처리합니다.</p></div><div><small>지금 처리 필요</small><b>{count(center.summary.actionRequired)}건</b><em>판매자배송 {count(center.summary.total)}건</em><em>로켓그로스 {count(center.summary.rocketGrowthStored)}건 · 별도 저장</em></div></section>
