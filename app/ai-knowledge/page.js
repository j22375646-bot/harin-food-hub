...
import knowledgeAdapter from '../../lib/ui/phase28-adapters/knowledge.js';
...
export default async function Page({searchParams}){
  ...
  if(!phase28Runtime.activePages.includes('knowledge')){
    const {renderDashboardRoute}=await import('../dashboard-route.js');
    return renderDashboardRoute('knowledge',searchParams);
  }
  try{
    const snapshot=await knowledgeSnapshot.loadPhase28KnowledgeSnapshot({db:supabaseModule.getSupabase(),now:new Date()});
    return <Phase28KnowledgePage model={knowledgeAdapter.buildPhase28KnowledgeModel(snapshot)}/>;
  }catch(error){
    return <Phase28KnowledgePage model={knowledgeAdapter.buildPhase28KnowledgeModel({generatedAt:null,error:error.message})}/>;
  }
}
