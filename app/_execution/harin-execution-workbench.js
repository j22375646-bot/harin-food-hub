...
export default function HarinExecutionWorkbench({view,data={},aiPanel,children}){
  ...
  return <section className={`executionV8 executionV8-${meta.tone}`}>
    <section className="executionHero"><div className="executionHeroCopy"><span>PHASE 14-7 · {meta.eyebrow}</span><div><i><FlowIcon view={view}/></i><section><h1>{meta.title}</h1><p>{meta.description}</p></section></div><small>AI는 설명만 · 실제 플랫폼 변경은 사장님 승인 뒤 · 자료가 부족하면 판단 보류</small></div><div className="executionHeroMetrics">{heroMetrics.map(([label,value])=><span key={label}><small>{label}</small><b>{value}</b></span>)}</div></section>
