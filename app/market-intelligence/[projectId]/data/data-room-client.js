  ...
  return <section className="marketDataRoom">
    ...
    <section className="marketDataGrid">
      ...
      <HarinCard className="marketSourceLibrary">
        ...
        {loading?<div className="marketDataLoading">자료실을 불러오는 중이에요…</div>:sources.length?<div className="marketSourceList">{sources.map(item=>{const meta=statusMeta[item.ingest_status]||[item.ingest_status,'neutral'];return <button type="button" className={selectedSourceId===item.id?'active':''} onClick={()=>setSelectedSourceId(item.id)} key={item.id}><HarinPictogram icon={item.mime_type?.startsWith('image/')?'image':'document'} tone="blue" size={18}/><span><b>{item.display_name}</b><small>{bytes(item.size_bytes)} · {new Date(item.created_at).toLocaleDateString('ko-KR')}</small></span><HarinBadge tone={meta[1]}>{meta[0]}</HarinBadge></button>;})}</div>:<HarinEmptyState icon="folder" title="아직 보관된 자료가 없어요" description="왼쪽에서 첫 원본을 올려주세요."/>}
      </HarinCard>
    </section>
    <section className="marketReviewGrid">
      <HarinCard className="marketOcrReview">
        ...
        {selected?<form onSubmit={saveReview}><header><span><b>{selected.display_name}</b><small>{selected.ocr_engine==='TEXT_UTF8'?'텍스트 자동 추출':'이미지·PDF는 판독 내용을 직접 붙여넣어 확인'}</small></span><HarinButton variant="ghost" icon="download" disabled={Boolean(working)} onClick={()=>openSource(selected)}>원본 열기</HarinButton></header><label><span>판독 내용</span><textarea required maxLength="200000" value={review.ocr_text} onChange={event=>setReview({...review,ocr_text:event.target.value})} placeholder="이미지나 PDF에서 확인한 내용을 붙여넣어주세요."/></label><div className="marketReviewControls"><label><span>판독 신뢰도</span><div><input type="number" min="0" max="100" step="1" value={review.ocr_confidence} onChange={event=>setReview({...review,ocr_confidence:event.target.value})}/><em>%</em></div></label><label className="marketOwnerCheck"><input type="checkbox" checked={review.owner_confirmed} onChange={event=>setReview({...review,owner_confirmed:event.target.checked})}/><span><b>원본과 같음을 확인했어요</b><small>95% 이상과 이 확인이 모두 있어야 검수 완료</small></span></label></div><HarinButton variant="primary" icon="shield" disabled={working==='REVIEW'} type="submit">{working==='REVIEW'?'검수 저장 중…':'판독 검수 저장'}</HarinButton></form>:<HarinEmptyState icon="search" title="검수할 원본을 선택해주세요" description="보관된 원본 목록을 누르면 판독 검수 화면이 열려요."/>}
      </HarinCard>
