...
export async function renderMarketWorkspace({projectId,workspace}){
  ...
  return <HarinPageFrame kind="analysis" className={`marketWorkspace marketWorkspace-${workspace}`}>
    ...
    {workspace==='data'?<><MarketProductBaseline projectId={project.id} productName={productName}/><MarketDataRoom projectId={project.id} productName={productName}/></>:<HarinCard className="marketStageEmpty">
