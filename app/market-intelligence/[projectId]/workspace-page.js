...
const STAGES={
  data:{number:'01',label:'자료실',context:'선택 상품 전용 자료실',icon:'database',tone:'blue',description:'파일과 출처를 모으고 OCR 결과를 사장님이 확인하는 공간입니다.',empty:'아직 검수된 근거가 없습니다.',next:'자료 업로드·OCR·근거 연결'},
  market:{number:'02',label:'시장 분석',context:'실제 수요와 시장 범위',icon:'analysis',tone:'lavender',description:'사장님이 승인한 시장범위와 실제 수요 신호를 확인합니다.',empty:'아직 승인된 시장범위가 없습니다.',next:'시장범위·수요·고객 연결'},
  competition:{number:'03',label:'경쟁 분석',context:'경쟁상품과 고객 불편',icon:'search',tone:'pink',description:'경쟁 상품 리뷰의 불편과 우리 상품이 해결하는 근거를 함께 봅니다.',empty:'아직 검수된 경쟁 근거가 없습니다.',next:'경쟁상품·리뷰·차별점 연결'},
  conversion:{number:'04',label:'구매 전환',context:'구매 장벽과 전환안',icon:'target',tone:'mint',description:'구매 장벽, 상세페이지, 판매구성, 재구매와 실험을 연결합니다.',empty:'아직 저장된 전환안이 없습니다.',next:'상세페이지·구성·재구매 연결'},
  b2b:{number:'05',label:'B2B·조달',context:'납품 준비와 공공조달',icon:'store',tone:'amber',description:'선택 상품의 납품 준비도와 공공조달 연결 조건을 확인합니다.',empty:'아직 B2B 준비표가 없습니다.',next:'상품·서류·공고 검토'}
};
...
export async function renderMarketWorkspace({projectId,workspace}){
  ...
  const procurementPromise=workspace==='b2b'?db.from('optional_provider_snapshots').select('provider,status,fetched_at,error_code,error_message,metric_summary').eq('provider','PUBLIC_PROCUREMENT').order('fetched_at',{ascending:false}).limit(5):Promise.resolve({data:[],error:null});
  let data,procurementResult;
  try{[data,procurementResult]=await Promise.all([projectsModule.loadProject({db,projectId}),procurementPromise]);}
  ...
  const procurementSnapshots=procurementResult?.error?[]:procurementResult?.data||[];
  ...
  return <HarinPageFrame kind="analysis" className={`marketWorkspace marketWorkspace-${workspace}`}>
    <HarinPageHeader className="marketWorkspaceHeader" eyebrow={stage.context} title={project.project_name} description={stage.description} icon={stage.icon} tone={stage.tone} note={`${product?.name||project.product_snapshot?.name||'선택 상품'} 전용 프로젝트 · 다른 상품 자료와 분리 저장`} metrics={[
    ...
    <nav className="marketWorkspaceTabs" aria-label="시장·전환 분석 단계">{tabs.map(([id,item])=><Link href={projectsModule.projectHref(project.id,id)} className={workspace===id?'active':''} aria-current={workspace===id?'page':undefined} data-tone={item.tone} key={id}><i>{item.number}</i><HarinPictogram icon={item.icon} tone={item.tone} size={18}/><b>{item.label}</b></Link>)}</nav>
    <section className="marketWorkspaceContent" aria-label={`${stage.label} 작업 영역`}>
      <div className="marketWorkspaceStage">{workspace==='data'?<><MarketProductBaseline projectId={project.id} productName={productName}/><MarketOfficialEvidence projectId={project.id} productName={productName}/><MarketLabelEvidence projectId={project.id} productName={productName}/><MarketResearchEvidence projectId={project.id} productName={productName}/><MarketNaverEvidenceSearch projectId={project.id} productName={productName}/><MarketDataRoom projectId={project.id} productName={productName}/></>:workspace==='market'?<><MarketContextEvidence projectId={project.id} productName={productName}/><RawMarketEvidence projectId={project.id} productName={productName}/><MarketNaverTrend projectId={project.id} productName={productName}/><MarketNaverAdResearch projectId={project.id} productName={productName}/><MarketNaverCommerceOrders projectId={project.id} productName={productName}/><MarketProfileWorkbench projectId={project.id} productName={productName}/></>:workspace==='competition'?<CompetitionWorkbench projectId={project.id} productName={productName}/>:workspace==='conversion'?<ConversionStage projectId={project.id} productName={productName}/>:workspace==='b2b'?<B2BOpportunityWorkbench center={b2bReadinessModule.buildB2BReadiness({project,product,snapshots:procurementSnapshots,env:process.env})}/>:<HarinCard className="marketStageEmpty">
      ...
      <details className="marketProjectIdentity"><summary><span><b>프로젝트 저장 정보</b><small>상품 격리·템플릿·최근 저장 기록</small></span><i aria-hidden="true">+</i></summary><div><article><small>상품 격리 키</small><b>{project.master_product_id}</b><p>상품을 바꾸면 별도 프로젝트를 엽니다.</p></article><article><small>재사용 템플릿</small><b>{project.template_id}</b><p>화면 구조만 같고 근거는 복사하지 않습니다.</p></article><article><small>최근 저장</small><b>{project.updated_at?new Date(project.updated_at).toLocaleString('ko-KR'):'기록 없음'}</b><p>모든 변경은 버전으로 남깁니다.</p></article></div></details>
