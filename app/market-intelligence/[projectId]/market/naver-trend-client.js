...
export default function MarketNaverTrend({projectId,productName}){
  ...
  if(!data||!draft)return <HarinCard className="marketTrendError"><HarinPictogram icon="warning" tone="pink"/><span><b>네이버 수요 신호를 불러오지 못했어요</b><small>{message||'잠시 뒤 다시 시도해주세요.'}</small></span><HarinButton icon="sync" onClick={()=>load()}>다시 확인</HarinButton></HarinCard>;
  ...
  return <section className="marketNaverTrendWorkbench">
    ...
    <HarinCard className="marketTrendControl"><HarinSectionHeading eyebrow="OWNER-SELECTED PROFILE" title="상품별 수집 조건" description="추천 검색어는 시작점일 뿐이며, 사장님이 직접 수정한 값으로만 수집합니다." icon="settings"/>
      ...
      <footer><span><HarinPictogram icon="shield" tone="mint" size={17}/><small>수집은 읽기 전용입니다. 광고·상품·입찰가는 변경하지 않아요.</small></span><HarinButton icon="check" disabled={Boolean(working)} onClick={()=>run('SAVE')}>{working==='SAVE'?'조건 저장 중…':'조건 저장'}</HarinButton><HarinButton variant="primary" icon="sync" disabled={Boolean(working)||!data.readiness.configured} onClick={()=>run('COLLECT')}>{working==='COLLECT'?'수요 신호 수집 중…':'지금 새로 수집'}</HarinButton></footer>
    </HarinCard>
