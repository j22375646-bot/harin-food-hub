...
export default async function Layout({children}){
  ...
  if(!session)redirect('/login?next=%2Fapprovals');
  const phase28Runtime=featureFlagsModule.phase28RuntimeConfig(process.env,{routeId:'changes'});
  if(phase28Runtime.activePages.includes('changes'))return <Phase28Shell routeId="changes" generatedAt={null}>{children}</Phase28Shell>;
  return children;
}
