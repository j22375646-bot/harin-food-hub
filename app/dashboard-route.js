...
import coupangOperationalInventoryModule from '../lib/coupang/operational-inventory.js';
...
import naverBizmoneyModule from '../lib/naver/bizmoney.js';
...
import executionTopologyModule from '../lib/infrastructure/execution-topology.js';
...
import customerServiceOperationModule from '../lib/customer-service/operation-summary.js';
import coupangInquiryThreadModule from '../lib/coupang/inquiry-thread.js';
...
async function optionalTableQuery(query){
  const result=await query;
  return result?.error?.code==='PGRST205'?{...result,data:[],error:null,status:200,statusText:'Optional table pending migration'}:result;
}
...
const MAIN_OVERVIEW_TABLES = [
  ...
  'coupang_orders','coupang_rg_orders','coupang_returns','coupang_inquiries','coupang_exchanges',
...
async function buildFocusedShellData({
  ...
  cafe24Token=null, cafe24Counts={}, coupangCounts={}, summaries={}
}) {
...
async function buildMainDashboardData({
  ...
  coupangInventoryResult,coupangRgOrdersResult,coupangInquiriesResult,coupangExchangesResult,customerServiceAvailable,
  ...
  const customerService=customerServiceOperationModule.buildCustomerServiceOperationSummary({
    customerServiceRows,coupangInquiries:coupangInquiriesResult.data,
    coupangReturns:coupangReturnsResult.data,coupangExchanges:coupangExchangesResult.data,
    available:customerServiceAvailable
  });
  ...
  const riskProducts=rgInventory.filter(item=>['OUT_OF_STOCK','CRITICAL','LOW'].includes(String(item.stock_status||'').toUpperCase())).slice(0,3).map(item=>({
    key:`COUPANG:${item.vendor_item_id}`,name:item.external_sku_id||`SKU ${item.vendor_item_id}`,platform:'COUPANG',
    currentRevenue:0,growthAmount:0,growthRate:null,stockStatus:item.stock_status,daysOfStock:item.days_of_stock,
  ...
  const salesCommandCenter=salesCommandCenterModule.buildSalesCommandCenter({
    pacing,priorityCenter,dataHealth:shell.dataHealth,
    productSignals:{growth:[],risk:riskProducts,period:null},profitability:{},financialTrust:{},
  ...
  return {
    ...
    coupang:{
      ...
      inventoryMarketing,unansweredInquiries:coupangInquiriesResult.unavailable?null:customerService.active.filter(item=>item.platform==='COUPANG'&&item.kind==='INQUIRY').length,
...
async function buildInventoryDashboardData({
  ...
  inventoryLots=[],cafe24Token,latestAiPageResults
}) {
  const rawInventory=coupangInventoryResult.data||[];
  const shell=await buildFocusedShellData({
    queryIssues,syncResult,alertsResult,generatedAt,cafe24Token,
  ...
  const {active:operationalInventory,excluded:excludedInventory}=coupangOperationalInventoryModule.splitOperationalInventory(inventoryBase);
  const {items:rgInventory,summary:inventoryMarketing}=coupangMarketingModule.buildInventoryMarketing(operationalInventory);
  ...
  return {
    ...
    coupang:{
      ...
      rgInventoryExcludedCount:excludedInventory.length,
      rgTotalOrderable:rgInventory.reduce((sum,item)=>sum+number(item.total_orderable_quantity),0),
      rgSalesLast30Days:rgInventory.reduce((sum,item)=>sum+number(item.sales_last_30_days),0),
      rgOutOfStock:rgInventory.filter(item=>item.stock_status==='OUT_OF_STOCK').length,
      rgLowStock:rgInventory.filter(item=>['CRITICAL','LOW'].includes(item.stock_status)).length,
...
async function buildSearchTermsDashboardData({
  ...
  const searchTermCenter=naverSearchTermCenterModule.buildSearchTermCenter({
    rows:searchTermRows,
    registeredKeywords,
  ...
  searchTermCenter.latest_sync=searchTermSync;
  const shell=await buildFocusedShellData({
    queryIssues,syncResult,alertsResult,generatedAt,cafe24Token,
    summaries:{NAVER:`${number(searchTermCenter.summary?.total).toLocaleString('ko-KR')}개 실제 검색어`,CAFE24:'채널 연결 상태',COUPANG:'선택하지 않은 플랫폼'}
  });
  const builtPanels=aiPagePanelsModule.buildAiPagePanels({
    dataHealth:shell.dataHealth,collectionCenter:shell.collectionCenter,alerts:shell.alerts,
    searchTermCenter,aiConfiguration:openaiClientModule.configuration(),generatedAt,
    period:searchTermPeriod?.period_end||kstScheduleModule.kstDateKey(generatedAt)
  });
  const aiPagePanels=finalizeAiPagePanels({keyword:builtPanels.keyword},latestAiPageResults,generatedAt,['keyword']);
  return {
    loadedView:'keyword',loadedWorkspace:'search-terms',loaderPerformance:loaderSession.snapshot(),generatedAt,
    dataHealth:shell.dataHealth,channelConnections:shell.channelConnections,collectionCenter:shell.collectionCenter,
    aiPagePanels,financialTrust:{},productAdTargets:{summary:{},items:[]},naverBidWorkbench:{summary:{},candidates:[]},
    masterProducts:[],channelProducts:[],productCosts:[],products:[],syncs:shell.syncs,reports:[],actions:[],alerts:shell.alerts,
    automationRuns:[],qualityChecks:[],metricSnapshots:[],
    kpis:{sales:0,orders:0,visitors:0,pageviews:0,conversion:0,averageOrder:0,products:0},
    naver:{
      campaigns:0,adgroups:0,keywords:registeredKeywords.length,keywordPeriod:null,keywordTop:[],keywordWaste:[],searchTermCenter
    },
    coupang:{}
  };
}
...
async function buildProductCatalogDashboardData({
  ...
  const sellableCafe24Ids=new Set(sourceProducts
  ...
  const commerceChannels=[
    ...sourceChannels.filter(item=>item.platform==='CAFE24'),
    ...productMapping.links
  ];
  const productOperations=productOperationsModule.buildUnifiedProductOperations({
    masterProducts:sellableMasterProducts,
    channelProducts:commerceChannels,
  ...
  return {
    ...
    selectedProductPlatform:platform,
    masterProducts:sellableMasterProducts,
    channelProducts:commerceChannels,
    productMapping,
    productOperations
...
async function buildRegisteredKeywordDashboardData({
  ...
  const marketingDiagnosis=workspace==='diagnosis'&&isNaver?marketingDiagnosisModule.buildMarketingDiagnosis({
    ...
    period:keywordPeriod,
    targetRoas:Number(process.env.NAVER_TARGET_ROAS_PERCENT||250)
  }):null;
  ...
  const aiPagePanels=finalizeAiPagePanels({keyword:builtPanels.keyword},latestAiPageResults,generatedAt,['keyword']);
  return {
    loadedView:'keyword',loadedWorkspace:workspace,loaderPerformance:loaderSession.snapshot(),generatedAt,
    dataHealth:shell.dataHealth,channelConnections:shell.channelConnections,collectionCenter:shell.collectionCenter,
    ...
    naver:{
      ...
      searchTermCenter:{summary:{total:0,terms:0},items:[]},marketingDiagnosis
    },
    coupang:{
...
async function buildCsDashboardData({
  loaderSession,generatedAt,queryIssues,
  ordersResult,itemsResult,syncResult,alertsResult,
  coupangOrdersResult,coupangItemsResult,coupangReturnsResult,coupangExchangesResult,coupangInquiriesResult,
  channelCsItems,csOperationAudits,cafe24Token,latestAiPageResults
}) {
  const shell=await buildFocusedShellData({
    queryIssues,syncResult,alertsResult,generatedAt,cafe24Token,
    cafe24Counts:{orders:ordersResult.data?.length||0},
    ...
    summaries:{
      NAVER:`${(channelCsItems||[]).filter(item=>String(item.platform).toUpperCase()==='NAVER').length.toLocaleString('ko-KR')}건 CS 저장`,
      CAFE24:`${(channelCsItems||[]).filter(item=>String(item.platform).toUpperCase()==='CAFE24').length.toLocaleString('ko-KR')}건 CS 저장`,
      COUPANG:`${((coupangInquiriesResult.data?.length||0)+(coupangReturnsResult.data?.length||0)+(coupangExchangesResult.data?.length||0)).toLocaleString('ko-KR')}건 문의·클레임`
    }
  });
  const customerService=unifiedCustomerServiceModule.buildUnifiedCustomerService({
    cafe24Orders:ordersResult.data||[],cafe24OrderItems:itemsResult.data||[],
    coupangOrders:coupangOrdersResult.data||[],coupangOrderItems:coupangItemsResult.data||[],
    coupangReturns:(coupangReturnsResult.data||[]).map(returnCaseView),
    coupangExchanges:(coupangExchangesResult.data||[]).map(exchangeCaseView),
    coupangInquiries:(coupangInquiriesResult.data||[]).map(item=>coupangInquiryThreadModule.hydrateInquiry(item)),
    channelItems:channelCsItems||[],operationAudits:csOperationAudits||[],
    ...
    channelConnections:shell.channelConnections.channels||[],now:generatedAt
  });
  const builtPanels=aiPagePanelsModule.buildAiPagePanels({
    dataHealth:shell.dataHealth,customerService,collectionCenter:shell.collectionCenter,
    alerts:shell.alerts,aiConfiguration:openaiClientModule.configuration(),generatedAt,
    period:kstScheduleModule.kstDateKey(generatedAt)
  });
  const aiPagePanels=finalizeAiPagePanels({cs:builtPanels.cs},latestAiPageResults,generatedAt,['cs']);
  return {
    loadedView:'cs',loadedWorkspace:null,loaderPerformance:loaderSession.snapshot(),generatedAt,
    dataHealth:shell.dataHealth,channelConnections:shell.channelConnections,collectionCenter:shell.collectionCenter,
    customerService,aiPagePanels,
    kpis:{sales:0,orders:ordersResult.data?.length||0,visitors:0,pageviews:0,conversion:0,averageOrder:0,products:0},
    products:[],syncs:shell.syncs,reports:[],actions:[],alerts:shell.alerts,automationRuns:[],qualityChecks:[],metricSnapshots:[]
  };
}

async function buildReportsDashboardData({
  loaderSession,generatedAt,queryIssues,syncResult,alertsResult,
  ...
  const shell=await buildFocusedShellData({
    queryIssues,syncResult,alertsResult,generatedAt,
    summaries:{
      NAVER:'저장된 진단 근거',CAFE24:'저장된 진단 근거',COUPANG:'저장된 진단 근거'
    }
  });
  ...
  const aiPagePanels=finalizeAiPagePanels({reports:builtPanels.reports},latestAiPageResults,generatedAt,['reports']);
  return {
    loadedView:'reports',loadedWorkspace:null,loaderPerformance:loaderSession.snapshot(),generatedAt,
    dataHealth:shell.dataHealth,channelConnections:shell.channelConnections,collectionCenter:shell.collectionCenter,
...
async function buildChangesDashboardData({
  loaderSession,generatedAt,queryIssues,syncResult,alertsResult,
  reportsResult,actionsResult,evaluationsResult,automationResult,
  financialChanges=[],financialAudits=[],experiments=[],
  masterResult,costsResult,naverCampaignResult,naverGroupResult,
  keywordPeriod,marketingKeywordStats=[],marketingKeywordCatalog=[],
  productAdTargetRows=[],naverKeywordProductLinks=[],latestAiPageResults
}) {
  const shell=await buildFocusedShellData({
    queryIssues,syncResult,alertsResult,generatedAt,
    summaries:{NAVER:`${marketingKeywordCatalog.length.toLocaleString('ko-KR')}개 키워드`,CAFE24:'선택하지 않은 플랫폼',COUPANG:'플랫폼별 변경 경로 분리'}
  });
  const reports=reportsResult.data||[];
  const actions=actionsResult.data||[];
  const evaluations=evaluationsResult.data||[];
  const automationRuns=automationResult.data||[];
  const activeMasterProducts=(masterResult.data||[]).filter(item=>item.is_active!==false);
  const costIds=new Set((costsResult.data||[])
    .filter(item=>number(item.unit_cost)+number(item.packaging_cost)+number(item.other_unit_cost)>0)
    .map(item=>String(item.master_product_id)));
  const costCoverageRate=activeMasterProducts.length
    ? activeMasterProducts.filter(item=>costIds.has(String(item.id))).length/activeMasterProducts.length*100
    : null;
  const financialTrust=financialTrustModule.evaluateFinancialTrust({
    costCoverageRate,
    missingCostProducts:activeMasterProducts.filter(item=>!costIds.has(String(item.id))).length,
  ...
  const naverBidWorkbenchRaw=naverBidWorkbenchModule.buildNaverBidWorkbench({
    keywords:marketingKeywordCatalog,stats:marketingKeywordStats,
    adgroups:naverGroupResult.data||[],campaigns:naverCampaignResult.data||[],
    productTargets:productAdTargets.items||[],keywordProductLinks:naverKeywordProductLinks,
    masterProducts:activeMasterProducts,
    financialTrust:{
      allowed_cpc:financialTrust.allowed?.allowed_cpc===true,
      financial_actions:financialTrust.allowed?.bid_increase===true
    },
    period:keywordPeriod||{},executionEnabled:naverBidExecutionModule.configuration().write_enabled
  });
  const naverBidWorkbench={...naverBidWorkbenchRaw,candidates:naverBidWorkbenchRaw.candidates.map(candidate=>{
    const snapshot=naverBidWorkbenchModule.proposalSnapshot(candidate);
    return {...candidate,snapshot_token:snapshot?authModule.signBidProposalSnapshot(snapshot):null};
  ...
  const builtPanels=aiPagePanelsModule.buildAiPagePanels({
    dataHealth:shell.dataHealth,collectionCenter:shell.collectionCenter,alerts:shell.alerts,
    retentionValidation,experiments,aiConfiguration:openaiClientModule.configuration(),generatedAt,
    period:kstScheduleModule.kstDateKey(generatedAt)
  });
  const aiPagePanels=finalizeAiPagePanels({changes:builtPanels.changes},latestAiPageResults,generatedAt,['changes']);
  return {
    loadedView:'changes',loadedWorkspace:null,loaderPerformance:loaderSession.snapshot(),generatedAt,
    dataHealth:shell.dataHealth,channelConnections:shell.channelConnections,collectionCenter:shell.collectionCenter,
    reports,actions,retentionValidation,experiments,automationRuns,aiPagePanels,
    financialTrust,financialTrustToken:authModule.signFinancialTrust(financialTrust),
    productAdTargets,naverBidWorkbench,
    kpis:{sales:0,orders:0,visitors:0,pageviews:0,conversion:0,averageOrder:0,products:activeMasterProducts.length},
    products:[],syncs:shell.syncs,alerts:shell.alerts,qualityChecks:[],metricSnapshots:[]
  };
}
...
async function getDashboardData(state) {
  ...
  const queryMetadata = [
    ...
    ['NAVER','naver_campaigns'],['NAVER','naver_adgroups'],['NAVER','naver_keywords'],['NAVER','sync_logs'],['NAVER','naver_stats_daily'],['NAVER','naver_bizmoney_daily'],['NAVER','raw_api_responses_bizmoney'],
  ...
  const supplementalQueries={
    ...
    executionPaths:view==='collection'&&state?.workspace==='execution-paths' ? Promise.allSettled([
      db.from('execution_path_controls').select('lane_key,label,current_trigger,current_executor,queue_backend,desired_trigger,desired_executor,desired_queue_backend,mode,migration_authorized,schedule_label,source_path,safety_rules,notes,updated_at').order('lane_key'),
      db.from('coupang_sync_requests').select('id,request_type,status,requested_at,started_at,finished_at,idempotency_key,scheduled_for,kst_execution_date').order('requested_at',{ascending:false}).limit(800),
      db.from('coupang_operation_requests').select('id,operation_type,target_type,status,created_at,started_at,executed_at,idempotency_key').order('created_at',{ascending:false}).limit(1200),
      db.from('automation_runs').select('id,job_name,trigger_type,status,started_at,finished_at,idempotency_key,scheduled_for,kst_execution_date').order('started_at',{ascending:false}).limit(500),
      db.from('worker_heartbeats').select('worker_id,service_name,status,source_ip,last_seen_at,last_success_at,updated_at').order('last_seen_at',{ascending:false}).limit(20),
  ...
  const reportsQuery=view==='changes'
    ...
    : view==='insight'
      ? pageLoaderProfilesModule.scopeInsightReportQuery(db.from('reports').select(reportFields,{count:'exact'}))
  ...
  const settledQueries = await Promise.allSettled([
    ...
    db.from('coupang_inquiries').select('inquiry_key,inquiry_type,inquiry_id,status,answered,product_id,seller_product_id,vendor_item_id,order_id,question_text,parent_answer_id,inquired_at,updated_at,raw_data').order('inquired_at',{ascending:false}).limit(100),
  ...
  if(view==='main'){
    ...
    const targetSettled=dataHealthModule.settleQueries(targetsRaw,[
      {platform:'SHARED',dataset:'business_targets'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const channelCsSettled=dataHealthModule.settleQueries(channelCsRaw,[
      {platform:'SHARED',dataset:'customer_service_items'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    ...
    return buildMainDashboardData({
      ...
      coupangInquiriesResult,coupangExchangesResult,customerServiceAvailable:![channelCsSettled.results[0],coupangInquiriesResult,coupangReturnsResult,coupangExchangesResult].some(result=>result.unavailable),
  ...
  if(focusedSearchTerms){
    ...
    const searchTermRowsSettled=dataHealthModule.settleQueries(searchTermRowsRaw,[{platform:'NAVER',dataset:'naver_search_terms'}],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const searchTermSyncSettled=dataHealthModule.settleQueries(searchTermSyncRaw,[{platform:'NAVER',dataset:'naver_search_terms_sync'}],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const registeredKeywordsSettled=dataHealthModule.settleQueries(registeredKeywordsRaw,[{platform:'NAVER',dataset:'naver_search_term_registered_keywords'}],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    ...
    queryIssues.push(...searchTermRowsSettled.issues,...searchTermSyncSettled.issues,...registeredKeywordsSettled.issues,...aiResultsSettled.issues,...cafe24TokenSettled.issues);
    const allSearchTermRows=searchTermRowsSettled.results[0].data||[];
    const newestSearchTerm=allSearchTermRows[0]||null;
    const searchTermPeriod=newestSearchTerm?{period_start:newestSearchTerm.period_start,period_end:newestSearchTerm.period_end}:null;
    const searchTermRows=searchTermPeriod?allSearchTermRows.filter(item=>item.period_start===searchTermPeriod.period_start&&item.period_end===searchTermPeriod.period_end):[];
    return buildSearchTermsDashboardData({
      loaderSession,generatedAt,queryIssues,syncResult,alertsResult,
      searchTermRows,searchTermPeriod,searchTermSync:searchTermSyncSettled.results[0].data||null,
      registeredKeywords:registeredKeywordsSettled.results[0].data||[],
  ...
  if(focusedProductWorkspace){
    const [aiResultsRaw,cafe24TokenRaw,mappingHistoryRaw]=await Promise.all([
      supplementalQueries.aiResults,supplementalQueries.cafe24Token,supplementalQueries.productMappingHistory
    ]);
    ...
    const mappingHistorySettled=dataHealthModule.settleQueries(mappingHistoryRaw,[
      {platform:'SHARED',dataset:'product_mapping_history'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    queryIssues.push(...aiResultsSettled.issues,...cafe24TokenSettled.issues,...mappingHistorySettled.issues);
    return buildProductCatalogDashboardData({
  ...
  if(view==='inventory'||focusedInsightReport||(view==='product'&&state?.workspace==='costs')){
    const [aiResultsRaw,cafe24TokenRaw,inventoryLotsRaw]=await Promise.all([supplementalQueries.aiResults,supplementalQueries.cafe24Token,supplementalQueries.inventoryLots]);
    const aiResultsSettled=dataHealthModule.settleQueries(aiResultsRaw,[
      {platform:'SHARED',dataset:'ai_analysis_results'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const cafe24TokenSettled=dataHealthModule.settleQueries(cafe24TokenRaw,[
    ...
    const inventoryLotsSettled=dataHealthModule.settleQueries(inventoryLotsRaw,[
      {platform:'SHARED',dataset:'inventory_lots'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    queryIssues.push(...aiResultsSettled.issues,...cafe24TokenSettled.issues,...inventoryLotsSettled.issues);
    const cafe24Token=cafe24TokenSettled.results[0].data?.token_data||null;
    ...
    if(view==='inventory'){
      return buildInventoryDashboardData({
        loaderSession,generatedAt,queryIssues,syncResult,alertsResult,
        coupangProductsResult,coupangProductItemsResult,coupangInventoryResult,
        inventoryLots:inventoryLotsSettled.results[0].data||[],cafe24Token,latestAiPageResults
      });
    }
    if(view==='product'){
  ...
  if(view==='reports'){
    ...
    queryIssues.push(...phase7Settled.issues,...aiResultsSettled.issues);
    return buildReportsDashboardData({
  ...
  if(view==='cs'){
    const [channelCsRaw,csAuditsRaw,aiResultsRaw,cafe24TokenRaw]=await Promise.all([
      supplementalQueries.channelCs,supplementalQueries.csAudits,
      supplementalQueries.aiResults,supplementalQueries.cafe24Token
    ]);
    const channelCsSettled=dataHealthModule.settleQueries(channelCsRaw,[
      {platform:'SHARED',dataset:'customer_service_items'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const csAuditSettled=dataHealthModule.settleQueries(csAuditsRaw,[
      {platform:'COUPANG',dataset:'coupang_operation_requests_cs'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    const aiResultsSettled=dataHealthModule.settleQueries(aiResultsRaw,[
      {platform:'SHARED',dataset:'ai_analysis_results'}
    ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
    ...
    queryIssues.push(...channelCsSettled.issues,...csAuditSettled.issues,...aiResultsSettled.issues,...cafe24TokenSettled.issues);
    return buildCsDashboardData({
      loaderSession,generatedAt,queryIssues,ordersResult,itemsResult,syncResult,alertsResult,
      coupangOrdersResult,coupangItemsResult,coupangReturnsResult,coupangExchangesResult,coupangInquiriesResult,
      channelCsItems:customerServiceStore.hydrateRows(channelCsSettled.results[0].data||[]),
      csOperationAudits:csAuditSettled.results[0].data||[],
      cafe24Token:cafe24TokenSettled.results[0].data?.token_data||null,
      latestAiPageResults:aiPageResultsModule.latestByPage(aiResultsSettled.results[0].data||[])
    });
  }
  ...
  const reliabilitySettled=dataHealthModule.settleQueries(await supplementalQueries.reliability,[
    {platform:'COUPANG',dataset:'worker_heartbeats'},
    {platform:'COUPANG',dataset:'coupang_operation_requests_dead_letters'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  ...
  const ownedSiteSettled=dataHealthModule.settleQueries(await supplementalQueries.ownedSiteReadiness,[
    {platform:'GOOGLE',dataset:'owned_site_api_snapshots'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  queryIssues.push(...ownedSiteSettled.issues);
  const ownedSiteSnapshots=ownedSiteSettled.results[0].data||[];
  const shippingReferenceSettled=dataHealthModule.settleQueries(await supplementalQueries.shippingReference,[
    {platform:'PUBLIC',dataset:'shipping_reference_snapshots'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  queryIssues.push(...shippingReferenceSettled.issues);
  const shippingReferenceSnapshots=shippingReferenceSettled.results[0].data||[];
  const operationsHealthSettled=dataHealthModule.settleQueries(await supplementalQueries.operationsHealth,[
    {platform:'SHARED',dataset:'operations_health_snapshots'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  queryIssues.push(...operationsHealthSettled.issues);
  const operationsHealthSnapshots=operationsHealthSettled.results[0].data||[];
  const optionalProviderSettled=dataHealthModule.settleQueries(await supplementalQueries.optionalProviders,[
    {platform:'SHARED',dataset:'optional_provider_snapshots'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  ...
  const executionPathSettled=dataHealthModule.settleQueries(await supplementalQueries.executionPaths,[
    {platform:'SHARED',dataset:'execution_path_controls'},
    {platform:'COUPANG',dataset:'execution_path_sync_requests'},
    {platform:'SHARED',dataset:'execution_path_operation_requests'},
    {platform:'SHARED',dataset:'execution_path_automation_runs'},
    {platform:'SHARED',dataset:'execution_path_worker_heartbeats'},
    {platform:'SHARED',dataset:'execution_path_sync_logs'}
  ],(error,issue)=>console.error(`[dashboard] ${issue.platform}/${issue.dataset} unavailable`,error));
  queryIssues.push(...executionPathSettled.issues);
  const [executionPathControlsResult,executionPathSyncResult,executionPathOperationResult,executionPathAutomationResult,executionPathHeartbeatResult,executionPathSyncLogsResult]=executionPathSettled.results;
  ...
  const { active:coupangOperationalInventory, excluded:coupangExcludedInventory } = coupangOperationalInventoryModule.splitOperationalInventory(coupangInventoryBase);
  const { items: coupangInventory, summary: inventoryMarketing } = coupangMarketingModule.buildInventoryMarketing(coupangOperationalInventory);
  ...
  const settlementCenterInput = {
    ...
    naverBizmoneyDaily:naverBizmoneyResult.data?.length?naverBizmoneyResult.data:naverBizmoneyModule.normalizeBizmoneyRawSnapshots(naverBizmoneyRawResult.data || []),
  ...
  const unifiedInventory = view==='main'
    ? coupangOperationalInventoryModule.buildOperationalInventoryCenter(coupangInventory)
    : unifiedInventoryModule.buildUnifiedInventoryCenter({
  ...
  const reliabilityCenter=reliabilityModule.buildReliabilityCenter({heartbeats:workerHeartbeats,operationRequests:operationDeadLetters,syncRequests:coupangRequestsResult.data||[],now:generatedAt});
  ...
  const advertisingChannelCenter=advertisingChannelModule.buildAdvertisingChannelCenter({
    ...
    cafe24:{
      attribution:cafe24AdAttributionResult.data||[],syncs:syncResult.data||[]
    },
    coupang:{
  ...
  const executionPathCenter=executionTopologyModule.buildExecutionTopology({
    controls:executionPathControlsResult.data||[],
    syncRequests:executionPathSyncResult.data||[],
    operationRequests:executionPathOperationResult.data||[],
  ...
  const unifiedOrders = unifiedOrdersModule.buildUnifiedOrders({
    ...
    businessCalendar:{holidayDates:shippingReferenceCenter.calendar.holidays.map(item=>item.date),holidayReady:shippingReferenceCenter.calendar.ready}
  });
  ...
  return {
    ...
    coupang: {
      ...
      rgInventoryExcludedCount:coupangExcludedInventory.length,
      ...
      inquiries: (coupangInquiriesResult.data || []).map(item=>coupangInquiryThreadModule.hydrateInquiry(item)),
...
export default async function LegacyDashboardRoute({ searchParams }) {
