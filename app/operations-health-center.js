...
export default function OperationsHealthCenter({center}){
  const [working,setWorking]=useState('');const [message,setMessage]=useState('');const [services,setServices]=useState(center?.services||[]);const infrastructure=services.filter(item=>item.group==='infrastructure');const releaseAlerts=services.filter(item=>item.group==='release-alerts');const summary=serviceSummary(services);
  async function probe(service){setWorking(service.key);setMessage(`${service.label} 읽기 신호를 확인하고 있어요.`);try{const response=await fetch(service.action.endpoint,{method:'POST',headers:{Accept:'application/json'}});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'운영 상태를 확인하지 못했습니다.');const result=payload.result||payload;setServices(current=>replaceProbeService(current,service.key,mergeProbeService(service,result,{summary:result.status==='PARTIAL'?`${service.label}에서 일부 신호를 확인했어요.`:`${service.label} 운영 신호를 방금 확인했어요.`})));setMessage(result.status==='PARTIAL'?`${service.label}에서 확인 가능한 신호만 안전하게 저장했어요.`:`${service.label} 운영 신호를 확인해 이 카드에 바로 반영했어요.`);}catch(error){setServices(current=>replaceProbeService(current,service.key,failProbeService(service,error)));setMessage(`확인 필요 · ${error.message}`);}finally{setWorking('');}}
  return <section className="naverApiCenter operationsHealthCenter">
    <header className="naverApiHero"><div className="naverApiHeroIcon"><HarinIcon name="server" size={30}/></div><div><span>PHASE {center?.phase||'20-4'} · OPERATIONS HEALTH</span><h1>서버·릴리스·운영 알림 상태</h1><p>허브 내부 서버부터 GitHub 릴리스, 외부 가동 감시와 알림 준비 상태까지 서로 섞지 않고 확인해요.</p></div><aside><span><b>{summary.ready}</b>개 정상</span><span><b>{summary.attention}</b>개 확인</span></aside></header>
