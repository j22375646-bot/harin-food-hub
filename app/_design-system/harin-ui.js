...
const { buildChartModel, buildDonutModel, buildWaterfallModel }=visualizationModule;
...
function chartValueY(value,domain,height,padding) {
  return padding+((domain.max-value)/domain.span)*(height-padding*2);
}

function chartPointX(index,count,width,padding) {
  return padding+(count===1?(width-padding*2)/2:index*(width-padding*2)/(count-1));
}

function chartSegments(values,domain,width,height,padding) {
  const count=Math.max(values.length,1);
  const points=values.map((value,index)=>value==null?null:{
    x:chartPointX(index,count,width,padding),
    y:chartValueY(value,domain,height,padding),
    value,
  });
  const segments=[];
  let current=[];
...
function HarinChartDataTable({ title, labels, series, valueFormatter }) {
  return <table className="v8ChartDataTable">
    <caption>{title||'차트 자료'}</caption>
    <thead><tr><th scope="col">구분</th>{series.map(item=><th scope="col" key={item.id}>{item.label}</th>)}</tr></thead>
    <tbody>{labels.map((label,index)=><tr key={`${label}-${index}`}><th scope="row">{label}</th>{series.map(item=><td key={`${item.id}-${index}`}>{item.values[index]==null?'확인 필요':valueFormatter(item.values[index])}</td>)}</tr>)}</tbody>
  </table>;
}
...
export function HarinMetricChart({ kind='line', title, description, labels=[], series=[], compact=false, valueFormatter=(value)=>Number(value).toLocaleString('ko-KR'), className='' }) {
  ...
  const baselineY=chartValueY(0,model.domain,height,padding);
  ...
  return <figure className={join('v8MetricChart',`v8MetricChart-${kind}`,compact&&'compact',model.hasMissingEvidence&&'hasMissingEvidence',className)} data-chart-kind={kind} data-chart-status={model.status} data-chart-evidence={model.hasMissingEvidence?'partial':'complete'}>
    <figcaption id={`${chartId}-caption`}><span><b>{title}</b>{description?<small>{description}</small>:null}</span>{model.hasMissingEvidence?<em>{model.missingPointCount}개 자료 확인 필요</em>:null}</figcaption>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={chartLabel} aria-describedby={`${chartId}-caption`} preserveAspectRatio="none">
      {[.25,.5,.75].map(ratio=><line className="v8ChartGrid" x1={padding} x2={width-padding} y1={padding+(height-padding*2)*ratio} y2={padding+(height-padding*2)*ratio} key={ratio}/>)}
      <line className="v8ChartBaseline" x1={padding} x2={width-padding} y1={baselineY} y2={baselineY}/>
      {kind==='bar'?model.series.flatMap((item,seriesIndex)=>item.values.map((value,index)=>{
        ...
        if(value==null)return <circle className="v8ChartMissingPoint" cx={x+barWidth/2} cy={baselineY} r="4" data-chart-tooltip={`${model.labels[index]} · ${item.label} 확인 필요`} key={`${item.id}-missing-${index}`}/>;
        const valueY=chartValueY(value,model.domain,height,padding);
        return <rect className={`v8ChartSeries-${item.tone}`} x={x} y={Math.min(valueY,baselineY)} width={Math.max(2,barWidth-2)} height={Math.max(2,Math.abs(baselineY-valueY))} rx="3" data-chart-tooltip={chartPointLabel(model.labels[index],item.label,value,valueFormatter)} key={`${item.id}-${index}`}/>;
      })):model.series.flatMap(item=>{
        const chart=chartSegments(item.values,model.domain,width,height,padding);
        return <g className={`v8ChartSeries-${item.tone}`} key={item.id}>{chart.segments.map((segment,index)=><polyline points={segment.map(point=>`${point.x},${point.y}`).join(' ')} key={`${item.id}-line-${index}`}/>)}{chart.points.map((point,index)=>point?<circle cx={point.x} cy={point.y} r={compact?3:4} data-chart-tooltip={chartPointLabel(model.labels[index],item.label,point.value,valueFormatter)} key={`${item.id}-point-${index}`}/>:<circle className="v8ChartMissingPoint" cx={chartPointX(index,item.values.length,width,padding)} cy={baselineY} r="4" data-chart-tooltip={`${model.labels[index]} · ${item.label} 확인 필요`} key={`${item.id}-missing-${index}`}/>)}</g>;
      })}
    </svg>
    ...
    <HarinChartDataTable title={title} labels={model.labels} series={model.series} valueFormatter={valueFormatter}/>
  </figure>;
}
...
export function HarinWaterfallChart({ title, description, items=[], footer, valueFormatter=(value)=>`${Math.round(Number(value)).toLocaleString('ko-KR')}원`, className='' }) {
  ...
  return <section className={join('v8WaterfallChart',model.hasMissingEvidence&&'hasMissingEvidence',className)} data-chart-status={model.status} data-chart-evidence={model.hasMissingEvidence?'partial':'complete'}>
...
export function HarinDonutChart({ title, description, items=[], valueFormatter=(value)=>`${Math.round(Number(value)).toLocaleString('ko-KR')}원`, className='' }) {
  ...
  const normalized=model.items;
  const total=model.total;
  if(model.status==='UNCOLLECTED')return <HarinEmptyState state="uncollected" title={`${title||'채널 비중'} 자료가 아직 없어요`} description="비교 가능한 확정 또는 예상 금액이 들어오면 비중을 표시합니다." className={className}/>;
  const radius=52,circumference=2*Math.PI*radius;
  let cursor=0;
  return <figure className={join('v8DonutChart',model.hasMissingEvidence&&'hasMissingEvidence',className)} data-chart-status={model.status} data-chart-evidence={model.hasMissingEvidence?'partial':'complete'}>
    <figcaption><span><b>{title}</b>{description?<small>{description}</small>:null}</span>{model.hasMissingEvidence?<em>{model.missingItemCount}개 자료 확인 필요</em>:null}</figcaption>
    <div><svg viewBox="0 0 140 140" role="img" aria-label={`${title||'채널 비중'} · ${normalized.map(item=>`${item.label} ${item.value===null?'확인 필요':valueFormatter(item.value)}`).join(' · ')}`}>
      <circle className="v8DonutTrack" cx="70" cy="70" r={radius}/>
      {normalized.filter(item=>item.value!==null).map(item=>{const length=total>0?item.value/total*circumference:0;const offset=-cursor;cursor+=length;return <circle className={`v8DonutSegment v8ChartSeries-${item.tone}`} cx="70" cy="70" r={radius} strokeDasharray={`${length} ${circumference-length}`} strokeDashoffset={offset} data-chart-tooltip={`${item.label} · ${valueFormatter(item.value)}`} key={item.id}/>;})}
      <text x="70" y="64" textAnchor="middle">확인 합계</text><text className="value" x="70" y="84" textAnchor="middle">{valueFormatter(total)}</text>
    </svg><ul>{normalized.map(item=><li className={item.displayStatus==='CHECK_REQUIRED'?'checkRequired':''} key={item.id}><i className={`v8ChartSeries-${item.tone}`}/><span><b>{item.label}</b><small>{item.value===null?'자료 확인 필요':total>0?`${(item.value/total*100).toFixed(1)}%`:'0.0%'}</small></span><strong>{item.value===null?'확인 필요':valueFormatter(item.value)}</strong></li>)}</ul></div>
    <HarinChartDataTable title={title} labels={normalized.map(item=>item.label)} series={[{id:'donut-value',label:'확인 값',values:normalized.map(item=>item.value)}]} valueFormatter={valueFormatter}/>
  </figure>;
}
