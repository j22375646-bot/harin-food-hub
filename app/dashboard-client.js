...
function NaverBidGuardedAutomation({workbench={},evaluation={summary:{},policy:{},rows:[]},working,onDraft,onEvaluate}){
  ...
  return <section className="naverAutomationPanel">
    <header><div><span>GUARDED AUTOMATION</span><h2>안전 자동운영 · 성과검증</h2><p>서버가 안전 후보와 평가 날짜를 자동 계산합니다. 실제 반영은 사장님 확인 팝업 뒤에만 실행합니다.</p></div><button disabled={Boolean(working)||!drafts.length} onClick={()=>onDraft(drafts)}>{working==='AUTO_DRAFT'?'초안 만드는 중…':`안전 후보 ${drafts.length}건 초안 만들기`}</button></header>
    <div className="naverAutomationRules"><article><b>감액 최대 10%</b><small>목표 CPA만큼 표본이 모인 후보만</small></article><article><b>증액 최대 5%</b><small>재고 근거가 없으면 자동 차단</small></article><article><b>하루 최대 3건</b><small>초안 생성 · 사장님 확인 후 실행</small></article><article><b>7일·14일 비교</b><small>한 번에 한 변수만 검증</small></article></div>
...
function ExperimentLab(){
  ...
  const requestedProduct=String(searchParams.get('master_product_id')||'');
  const requestedProject=String(searchParams.get('market_project_id')||'');
  const baseDate=new Date().toISOString().slice(0,10),weekLater=new Date(Date.now()+6*86400000).toISOString().slice(0,10);
  const emptyTest={action:'CREATE_TEST',name:'',master_product_id:requestedProduct,market_project_id:requestedProject,platform:'NAVER',hypothesis:'',target_type:'CAMPAIGN',source_type:'NAVER_ENTITY',metric:'CVR',start_date:baseDate,end_date:weekLater,minimum_sample_size:30,confidence_level:90,minimum_detectable_lift:10,variants:[{name:'기존안',entity_id:''},{name:'실험안',entity_id:''}]};
  ...
  const [data,setData]=useState(null),[selectedProduct,setSelectedProduct]=useState(requestedProduct),[testForm,setTestForm]=useState(emptyTest),[benchmarkForm,setBenchmarkForm]=useState(emptyBenchmark),[busy,setBusy]=useState(''),[message,setMessage]=useState(''),[showCreate,setShowCreate]=useState('TEST');
  async function load(productId=selectedProduct){try{const query=productId?`?master_product_id=${encodeURIComponent(productId)}`:'';const response=await fetch(`/api/experiments${query}`);const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error||'실험실 조회 실패');setData(result);}catch(error){setMessage(`확인 필요 · ${error.message}`);}}
  useEffect(()=>{load(selectedProduct);},[selectedProduct]);
  function chooseProduct(productId){setSelectedProduct(productId);setTestForm(current=>({...current,master_product_id:productId,market_project_id:productId===requestedProduct?requestedProject:''}));setMessage('');}
  async function submit(payload,key){setBusy(key);setMessage('서버에서 저장·계산하는 중입니다.');try{const response=await fetch('/api/experiments',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error||'저장 실패');setMessage(key==='TEST'?'실험을 등록했습니다. 매일 자동 평가되며 지금 평가도 가능합니다.':'벤치마크를 저장하고 최근 7일 실적과 비교했습니다.');if(key==='TEST')setTestForm({...emptyTest,master_product_id:selectedProduct,market_project_id:selectedProduct===requestedProduct?requestedProject:''});else setBenchmarkForm(emptyBenchmark);await load();}catch(error){setMessage(`확인 필요 · ${error.message}`);}finally{setBusy('');}}
  ...
  return <>
    <section className="pageIntro experimentIntro"><div><span className="eyebrow">GROWTH EXPERIMENT LAB</span><h1>{data.selected_product?`${data.selected_product.name} 실험실`:'A/B 테스트·벤치마크'}</h1><p>{data.selected_product?'이 상품에 연결된 실험만 보여주며 다른 상품의 결과는 섞지 않습니다.':'실험 실적과 플랫폼 데이터를 서버가 계산하고, 표본 신뢰도와 목표 대비 격차로 승자와 위험을 판정합니다.'}</p></div><div className="pagePrimaryAction"><button className="primary" onClick={()=>setShowCreate('TEST')}>새 A/B 테스트</button><details className="headerMore"><summary>기타</summary><button onClick={()=>setShowCreate('BENCHMARK')}>기준값 추가</button></details></div></section>
    {message&&<div className="syncToast">{message}</div>}
    <section className="experimentProductScope"><div><HarinIcon name="product" size={21}/><span><small>상품별 실험 보기</small><b>{data.selected_product?.name||'모든 실험'}</b></span></div><label><span>판매 중 기준상품</span><select value={selectedProduct} onChange={event=>chooseProduct(event.target.value)}><option value="">모든 실험</option>{(data.products||[]).map(product=><option value={product.id} key={product.id}>{product.name}</option>)}</select></label>{data.selected_product?<Link href="/market-intelligence" className="secondary">다른 상품 개발하기</Link>:null}</section>
