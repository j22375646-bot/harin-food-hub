...
const DEFAULT={mode:'PAUSED',weekdays:[1,2,3,4,5],start_minute:540,end_minute:1080,interval_minutes:60,max_changes_per_run:3,daily_change_limit:6,time_slots:null,allow_increase:false};
...
export default function KeywordBidSchedulePanel({adgroupId,adgroupName='',rules=[]}){
  ...
  return <section className={`bidScheduleCard mode-${String(draft.mode).toLowerCase()} ${open?'open':''} ${control.emergency_paused?'emergency':''}`}>
    ...
    {open?<div className="bidScheduleBody">
      ...
      <div className="bidScheduleGuard"><i><HarinIcon name="target" size={19}/></i><span><b>시간대 설정은 기존 안전선을 넓히지 않아요</b><small>시간대 최대가는 키워드 상한보다 낮을 때만 적용하며, 목표 순위 예상값은 실제 노출을 보장하지 않습니다.</small></span><em>{automationEnabled?'서버 실행 준비':'서버 적용 잠금'}</em></div>
