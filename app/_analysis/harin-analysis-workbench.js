...
import { HarinEmptyState, HarinMetricChart, HarinPageAiRegion, HarinPageContent, HarinPageFrame, HarinPageHeader, HarinPageToolbar, HarinQuickAction, HarinWaterfallChart } from '../_design-system/harin-ui.js';
...
function InsightOverviewDesk({data={},platform='all'}){
  ...
  const periodMetrics=periodReports.map(report=>reportMetric(report,platform));
...
function InsightChannelDesk({data={}}){
  ...
  return <section className="insightChannelDesk" id="insight-channels">
    <header><div><span>채널 비교표</span><h2>세 채널을 같은 기준으로 비교해요</h2><p>한 채널 자료가 없어도 나머지 채널은 그대로 보여줍니다.</p></div><Link href="/data-collection">데이터 상태 확인</Link></header>
