...
function KeywordDetailPlaceholder(){
  return <aside className="keywordOpsDetail keywordOpsDetailEmpty" aria-label="선택 키워드 상세 안내">
    <i aria-hidden="true"><KeywordPictogram/></i>
    <b>키워드를 선택하면 상세가 열려요</b>
    <p>성과·입찰가·순위·변경 기록을 표 옆에서 확인하고 바로 작업할 수 있습니다.</p>
    <small>오른쪽 패널은 표 폭을 흔들지 않고 같은 자리를 유지합니다.</small>
  </aside>;
}
...
export default function KeywordOperationsTable({workspace='registered',platform='naver',data={},ownerShellVisible=true}){
  ...
  const workbenchLayout=keywordWorkbenchLayout({platform,workspace,hasDetail:Boolean(detail)});
  const detailSearchUrl=detail?.platform==='NAVER'?naverKeywordSearchUrl(detail.keyword):null;
  ...
  function downloadNaverView(){
    if(isCoupang||!rows.length)return;
    const blob=new Blob([buildNaverKeywordCsv(rows)],{type:'text/csv;charset=utf-8'});
    const url=URL.createObjectURL(blob);const anchor=document.createElement('a');
    anchor.href=url;anchor.download=`naver-keywords-current-view-${new Date().toISOString().slice(0,10)}.csv`;anchor.click();
    URL.revokeObjectURL(url);setExportNotice(`현재 보기 ${rows.length.toLocaleString('ko-KR')}개를 CSV로 내려받았습니다.`);
  }
  ...
  return <section className={`keywordOps workspace-${workspace} platform-${platform} layout-${workbenchLayout.mode.toLowerCase().replaceAll('_','-')} ${listSignalEnabled?'hasRankSignal':''}`} data-workbench-layout={workbenchLayout.mode} id="keyword-operations-table">
    ...
    {groupEnabled?<details open className="keywordOpsAdgroupWorkspace keywordOpsScopePane" aria-label="네이버 캠페인과 광고그룹 선택">
      <summary className="keywordOpsScopeSummary"><i aria-hidden="true"><KeywordPictogram/></i><span><b>캠페인·광고그룹 범위</b><small>{selectedCampaignName||'전체 캠페인'} · {selectedAdgroupName||'전체 광고그룹'}</small></span><em aria-hidden="true">⌄</em></summary>
