...
import KeywordDetailWorkbench from './keyword-detail-workbench.js';
  ...
  return <section className={`keywordOps workspace-${workspace} platform-${platform} layout-${workbenchLayout.mode.toLowerCase().replaceAll('_','-')} ${listSignalEnabled?'hasRankSignal':''}`} data-workbench-layout={workbenchLayout.mode} id="keyword-operations-table">
    ...
    <div className={`keywordOpsLayout ${detail?'hasDetail':''}`}>
      ...
      {detail?<KeywordDetailWorkbench
        ...
        evidencePanel={detail.platform==='NAVER'&&detail.source==='REGISTERED'?<KeywordBidInlineTrend keywordId={String(detail.id).replace(/^NAVER:/,'')} currentBid={detail.currentBid} recommendedBid={detail.recommendedBid}/>:null}
        historyPanel={detail.platform==='NAVER'&&['REGISTERED','HISTORY'].includes(detail.source)?<KeywordBidHistoryPanel keywordId={String(detail.id).replace(/^NAVER:/,'')} adgroupId={detail.adgroupId}/>:null}
      />:null}
    </div>
