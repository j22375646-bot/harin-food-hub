...
const ACTIONS = {
  LOWER_BID: { label: '입찰가 감액 검토', recommendation: '입찰가를 낮춘 뒤 7일간 재검토' },
  PAUSE: { label: '키워드 중지 검토', recommendation: '키워드 중지 전 최종 확인' },
  WATCH: { label: '성장 키워드 관찰', recommendation: '현재 운영을 유지하고 7일간 추적' }
};
...
export async function POST(request) {
  ...
  try {
    ...
    const items = rawItems.slice(0, 20).map(item => ({
      keyword_id: String(item.keyword_id || '').trim(), keyword: String(item.keyword || '').trim(), action_type: String(item.action_type || '').toUpperCase(),
      cost: number(item.cost), conversion_revenue: number(item.conversion_revenue), clicks: number(item.clicks), conversions: number(item.conversions)
    ...
    const rows = pending.map(item => ({
      platform:'NAVER', target_type:'KEYWORD', target_id:item.keyword_id, target_name:item.keyword, action_type:item.action_type, status:'PLANNED', review_after:reviewAfter,
    ...
    if (rows.length) { const { error } = await db.from('actions').insert(rows); if (error) throw error; }
