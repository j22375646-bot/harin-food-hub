...
export async function GET(request){
  ...
  try{
    ...
    const analysis=await bidPerformanceAnalysis.loadBidOperatingScope({
      db:supabaseModule.getSupabase(),campaignId:params.get('campaignId'),adgroupId:params.get('adgroupId')
    });
    return apiSafety.json({ok:true,analysis},{headers});
  }catch(error){
    const status=Number(error.status)||500;
    return apiSafety.json({
      ok:false,code:error.code||'NAVER_BID_OPERATING_SCOPE_FAILED',
      error:status<500?error.message:'네이버 지역·기기 성과를 불러오지 못했습니다.'
    },{status,headers});
  }
}
