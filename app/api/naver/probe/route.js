...
import naverProbe from '../../../../lib/naver/probe.js';
...
export async function POST(request) {
  ...
  try {
    const result = await naverProbe.probeReadAccess({ db:supabaseModule.getSupabase() });
    return Response.json({ ok:true, result }, { headers:{ 'Cache-Control':'no-store' } });
  } catch (error) {
    return Response.json({ ok:false, code:error.code || 'NAVER_SEARCH_AD_PROBE_FAILED', error:error.message }, {
