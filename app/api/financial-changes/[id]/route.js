...
const ACTIONS = new Set(['CONFIRM_EXECUTE', 'APPROVE', 'EXECUTE', 'VERIFY', 'ROLLBACK', 'REJECT']);
...
export async function POST(request, context) {
  ...
  try {
    ...
    if (['CONFIRM_EXECUTE', 'APPROVE', 'EXECUTE', 'ROLLBACK', 'REJECT'].includes(action) && body.confirm !== true) return apiSafety.json({ ok:false, error:'명시적 확인이 필요합니다.', code:'CONFIRMATION_REQUIRED' }, { status:400 });
    const options = { actor:authModule.requestActor(request), note:body.note };
    const result = action === 'CONFIRM_EXECUTE' ? await financialChanges.confirmAndExecute(id, options)
      : action === 'APPROVE' ? await financialChanges.approve(id, options)
