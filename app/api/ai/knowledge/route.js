...
export async function GET(request) {
  ...
  try {
    const found=await supabaseModule.getSupabase().from('ai_knowledge_documents')
      .select('id,title,category,version_label,status,scope_pages,source_type,source_label,notes,privacy_status,vector_status,approved_by,approved_at,created_at,updated_at')
    ...
    return apiSafety.json({
      ...
      guard:{ execution_enabled:config.execution_enabled, file_search_configured:config.file_search_configured, uploads_enabled:false, reason:'크레딧 충전 전에는 OpenAI 파일 업로드와 분석 호출을 실행하지 않습니다.' }
    });
...
export async function POST(request) {
  ...
  try {
    ...
    const saved=await supabaseModule.getSupabase().from('ai_knowledge_documents').insert(payload)
      .select('id,title,category,version_label,status,scope_pages,source_type,source_label,notes,privacy_status,vector_status,approved_by,approved_at,created_at,updated_at').single();
...
export async function PATCH(request) {
  ...
  try {
    ...
    const found=await db.from('ai_knowledge_documents').select('id,status,privacy_status,vector_status').eq('id',id).maybeSingle();
    if(found.error)throw found.error;
