...
function errorResponse(error) {
  ...
  return apiSafety.inputErrorResponse(error) || apiSafety.json({ ok:false, error:error.message || '비용 변경 미리보기 생성 실패' }, { status:500 });
}
...
export async function PUT(request) {
  ...
  try {
    ...
    const result = await financialChanges.createPreview(body, {
