...
export async function POST(request) {
  ...
  try {
    ...
    if (body.confirm !== true) return Response.json({ok:false,error:'실제 주문 변경 확인이 필요합니다.'},{status:400});
    return Response.json({ok:true,...await actionsModule.executeOrderAction(body.action,body)});
