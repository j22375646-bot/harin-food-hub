...
export async function POST(request) {
  ...
  try {
    ...
    if (extension === 'xlsx') buffer = apiSafety.assertXlsx(buffer, file.name);
    const result = await importModule.importFile({ buffer, fileName: file.name, fileType: file.type, dataset: form.get('dataset') });
    let report = null;
    ...
    return apiSafety.inputErrorResponse(error) || Response.json({ ok: false, error: error.message || '쿠팡 파일 가져오기에 실패했습니다.' }, { status: 500 });
  }
}
