...
export async function GET(request){
  ...
  try{const now=new Date();return Response.json({ok:true,...await runnerModule.runJob({jobName:'WEEKLY_PLATFORM_REPORTS',triggerType:'CRON',maxAttempts:2,...scheduleKeys.cronExecution('WEEKLY_PLATFORM_REPORTS',{now,hour:6,minute:0}),work:()=>weeklyModule.generateWeekly({now,triggerType:'CRON'})})});}
