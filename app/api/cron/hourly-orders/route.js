...
export async function GET(request){
  ...
  const tracking=await Promise.resolve().then(async()=>{
    const center=await unifiedOrdersModule.loadUnifiedOrders({db});
    const orders=center.orders.filter(order=>order.stage!=='DELIVERED');
    return trackingQueue.queueTrackingForOrders(db,orders,{kind:'hourly',at:startedAt});
  ...
  const jobs=[
    ...
    tracking.status==='fulfilled'?{platform:'EPOST_TRACKING',ok:true,data:{queued:tracking.value.length}}:{platform:'EPOST_TRACKING',ok:false,error:tracking.reason?.message||'배송추적 요청 실패'},
