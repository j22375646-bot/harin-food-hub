...
export async function POST(request){
  ...
  try{
    ...
    const [masterResult,channelResult,costResult,settingResult,ruleResult]=await Promise.all([
    ...
    const [cafeOrdersResult,coupangOrdersResult,rgOrdersResult,productItemsResult,keywordLinksResult]=await Promise.all([
      db.from('cafe24_orders').select('order_id,order_date').gte('order_date',period.start).lte('order_date',`${period.end}T23:59:59+09:00`).limit(10000),
    ...
    const [cafeItemsResult,coupangItemsResult,rgItemsResult,keywordsResult,keywordStatsResult,coupangAdsResult]=await Promise.all([
