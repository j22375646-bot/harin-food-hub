...
async function xlsxResponse(candidates,events){
  ...
  return new Response(output,{status:200,headers:{'Content-Type':XLSX_MIME,'Content-Disposition':`attachment; filename*=UTF-8''${encodeURIComponent('모아온_주문.xlsx')}`,'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff','X-Moaon-Search-Contract':'1','X-Moaon-Export-Count':String(candidates.length)}});
}
...
export async function GET(request){
  ...
  try{
    ...
    if(options.format==='xlsx')return xlsxResponse(candidates,events);
