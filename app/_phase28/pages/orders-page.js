...
const CALENDAR_EVENT_REFRESH_INTERVAL_MS=15000;
...
function receiverCopy(order={}){
  const receiver=order.receiver||{};
  if(['LOADING','RETRYING'].includes(order.receiverHydrationStatus))return {
    status:order.receiverHydrationStatus,
    primary:'수취정보 자동 조회 중',
    secondary:order.receiverHydrationStatus==='RETRYING'?'채널 응답이 늦어 안전하게 다시 조회하고 있어요.':'채널 주문 상세에서 받는 분·연락처·주소를 불러오고 있어요.'
  };
  if(order.receiverHydrationStatus==='ERROR')return {
    status:'ERROR',primary:'수취정보 조회 지연',secondary:'자동 재시도 후에도 채널 응답이 늦습니다. 전체 수집으로 다시 확인해주세요.'
  };
  return {
    status:hasReceiverDetails(receiver)?'READY':'UNAVAILABLE',
    primary:`${receiver.name||'받는 분 확인 필요'} · ${receiver.contact||'연락처 확인 필요'}`,
    secondary:`${receiverAddress(receiver)} · ${receiver.message||'배송메모 없음'}`
  };
}
...
function OrderGiftNotice({order}){
  ...
  return <section className="orderGiftNotice" aria-label="사은품 증정 안내">
    ...
    <div className="orderGiftAmount"><span>실결제 주문금액</span><strong>{money(order.amount)}</strong></div>
    {order.gifts.map(gift=><article key={`${gift.eventId}-${gift.minimumAmount}`}><div><span>{gift.eventTitle}</span><small>{gift.eventStartDate} ~ {gift.eventEndDate}</small></div><p><b>{money(gift.minimumAmount)} 이상 충족</b><strong>{gift.giftName} {gift.quantity}개</strong></p></article>)}
  </section>;
}
...
export function OrderRow({order,selected,previewed,onSelect,onPreview}){
  ...
  const receiverView=receiverCopy(order);
  const selectionReason=order.selectionBlockedReason||'현재 주문은 허브 출고 작업에서 선택할 수 없습니다.';
  function previewFromKeyboard(event){
    if(event.target!==event.currentTarget||!['Enter',' '].includes(event.key))return;
    event.preventDefault();onPreview(order);
  }
  return <article className={`orderRow${selected?' selected':''}${previewed?' previewed':''}${order.cancellationRequested?' cancellation':''}`} data-selection-locked={selectable?undefined:'true'} data-previewed={previewed?'true':undefined} data-gift-required={order.giftRequired?'true':undefined} tabIndex={0} aria-label={`${order.productName} 주문 상세 보기${order.giftRequired?' · 사은품 증정 대상':''}`} onClick={()=>onPreview(order)} onKeyDown={previewFromKeyboard}>
    ...
    <div className="orderProduct"><OrderProductThumbnail order={order}/><span><span className="productMeta"><Phase28ChannelLogo brand={order.platform} size="compact"/>{order.channelLabel} · 판매자배송{order.platform==='NAVER'?<em className="selectionLock">네이버 송장 발급</em>:null}</span><strong>{order.productName}</strong><small>{productOption(order)}</small>{order.invoice?<span className="orderInvoice" data-status={order.invoice.status} aria-label={`${order.invoice.label} 송장번호 ${order.invoice.number}`}><HarinIcon name="document" size={14}/><b>{order.invoice.label}</b><span>{order.invoice.number}</span></span>:null}{order.fulfillmentStatus&&!(order.epostTrackingBadge&&order.fulfillmentStatus.phase==='TRACKING')?<span className="orderFulfillmentStatus" data-active={order.fulfillmentStatus.active?'true':'false'} data-attention={order.fulfillmentStatus.needsAttention?'true':'false'}><i/><b>{order.fulfillmentStatus.label}</b></span>:null}{order.gifts?.length?<span className="orderEventGift"><HarinIcon name="sparkles" size={15}/><b>사은품 증정 대상</b><span>실결제 주문금액 {money(order.amount)} · {order.gifts.map(item=>`${money(item.minimumAmount)} 이상 → ${item.giftName} ${item.quantity}개`).join(' · ')}</span></span>:null}</span></div>
    <div className="shippingPerson" data-receiver-status={receiverView.status}><strong>{receiverView.primary}</strong><span>{receiverView.secondary}</span></div>
    <div className="orderTime"><strong>{dateTime(order.orderedAt)}</strong><span>{order.timingBadge?.detail||'출고 일정 확인'}</span></div>
    <div className="orderBadges">{order.giftRequired?<span className="orderGiftAttention"><HarinIcon name="sparkles" size={15}/>사은품 증정</span>:null}{order.listDeliveryBadge?<span className="epostTrackingBadge" data-status={order.listDeliveryBadge.status} title={`${order.listDeliveryBadge.source==='EPOST'?'우체국':'쇼핑몰'} 배송상태 · ${order.listDeliveryBadge.detail}`} aria-label={`${order.listDeliveryBadge.source==='EPOST'?'우체국':'쇼핑몰'} 배송상태 ${order.listDeliveryBadge.label}`}><i/>{order.listDeliveryBadge.label}</span>:<span data-tone={rowTone(order)}>{order.stageLabel}</span>}{order.timingBadge?<span data-tone={order.timingBadge.type==='DELAYED'?'delay':'schedule'}>{order.timingBadge.label||'일정 확인'}</span>:null}</div>
    <div className="orderAmount"><strong>{money(order.amount)}</strong><span>실결제 · {Number(order.quantity||0).toLocaleString('ko-KR')}개</span></div>
  </article>;
}
...
function OrdersRail({activeTab,setActiveTab,selectedOrders,previewOrder,channels,activeStage,busy,delayedCount,giftOrderCount,giftAutomation,fulfillmentFeed,fulfillmentError,fulfillmentCheckedAt,onPrimaryAction,onSync}){
  const tabs=[{id:'selected',label:'선택 주문'},{id:'actions',label:'출고 작업'},{id:'sync',label:'수집 상태'}];
  const stage=STAGES.find(item=>item.id===activeStage)||STAGES[0];
  const receiver=previewOrder?.receiver||{};
  const receiverView=receiverCopy(previewOrder||{});
  ...
  return <div className="ordersRail">
    ...
    <section className="ordersRailSummary"><h3>오늘 출고 기준</h3><div><span>당일출고 마감</span><strong>오후 3시</strong></div><div><span>사은품 자동화</span><strong data-status={giftAutomation?.status||'SETUP_REQUIRED'}>{giftAutomation?.status==='READY'?`조건 ${Number(giftAutomation.ruleCount||0).toLocaleString('ko-KR')}개 · 대상 ${Number(giftOrderCount||0).toLocaleString('ko-KR')}건`:'캘린더 조건 없음'}</strong></div><div><span>배송지연</span><strong>{Number(delayedCount||0).toLocaleString('ko-KR')}건 확인</strong></div><div><span>로켓그로스</span><strong>자동처리 · 제외</strong></div></section>
  </div>;
}
...
export default function Phase28OrdersPage({model={}}){
  ...
  useEffect(()=>{
    calendarEventRevisionRef.current=String(giftAutomation.revision||'');
  },[giftAutomation.revision]);
  useEffect(()=>{
    let stopped=false;
    let running=false;
    async function checkCalendarEvents(){
      ...
      running=true;
      try{
        const response=await fetch('/api/calendar/events/revision',{cache:'no-store'});
        const result=await response.json();
        if(!response.ok||!result.ok)return;
        const nextRevision=String(result.revision||'');
        if(nextRevision!==calendarEventRevisionRef.current){
          calendarEventRevisionRef.current=nextRevision;
          router.refresh();
        }
      }catch{
        // The server-rendered setup state remains visible while the next check retries.
      }finally{
        running=false;
      }
    }
    const initial=window.setTimeout(checkCalendarEvents,1500);
    const timer=window.setInterval(checkCalendarEvents,CALENDAR_EVENT_REFRESH_INTERVAL_MS);
    function resume(){if(document.visibilityState!=='hidden')checkCalendarEvents();}
    document.addEventListener('visibilitychange',resume);
    return()=>{stopped=true;window.clearTimeout(initial);window.clearInterval(timer);document.removeEventListener('visibilitychange',resume);};
  },[router]);
  useEffect(()=>{
    let active=true;
  ...
  function selectOrder(order,checked){
    ...
    setPreviewOrderId(order.hubOrderId);setActiveRailTab('selected');
  ...
  function previewOrderInRail(order){setPreviewOrderId(order.hubOrderId);setActiveRailTab('selected');}
  function changeStage(id){setActiveStage(id);setPreviewOrderId(null);setDelayOnly(false);setStatusMessage(`${STAGES.find(item=>item.id===id)?.label||'선택 단계'} 주문만 모았어요.`);}
