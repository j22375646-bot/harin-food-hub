...
function AnalysisRunner({products,selectedId,setSelectedId,period,setPeriod,onRun,running}){
  ...
  return <section className="paRunner" data-state={running?'running':selected?'ready':'waiting'} aria-labelledby="paRunnerTitle">
    ...
    {selected?<div className="paSelectedProduct"><span className="paProductGlyph">{selected.name.slice(0,1)}</span><div><span className="paBrands">{CHANNELS.map(brand=><Phase28ChannelLogo brand={brand} size="compact" key={brand}/>)}<b>{selected.connectedChannels}/3채널 연결</b></span><strong>{selected.name}</strong><small>최근 수집 매출 {money(selected.metrics?.revenue)} · 광고 노출 {count(selected.metrics?.searchDemand,'회')}</small></div><em>선택 완료<br/><b>{time(selected.sources?.sales?.asOf)}</b></em></div>:null}
...
function ReportHero({report}){
  ...
  const headline=metrics.search_demand!=null?`${report.product?.name||'선택 상품'}은 연결 광고 노출 ${count(metrics.search_demand,'회')} 및 실제 매출 ${money(metrics.revenue)}이 확인됐어요.`:`${report.product?.name||'선택 상품'}의 판매 근거는 확인됐지만 광고 노출 근거를 더 확인해야 해요.`;
  return <section className="paReportHero"><div><span>현재 결론</span><h2>{headline}</h2><p>{report.calculationVersion==='PRODUCT_ANALYSIS_V2'?'전체 연결 광고 노출 집계 · 시장 검색량 아님':'이전 계산 보고서: 상위 12개 제한·누락값 0 처리 영향 확인 필요. 저장값은 유지되며 새 분석으로 비교하세요.'}</p><div>{(report.signals||[]).slice(0,3).map((item,index)=><em data-tone={item.tone} key={`${item.title}-${index}`}>{item.title}</em>)}</div></div><EvidenceRadar report={report}/><dl><div><dt>근거 연결</dt><dd>{Object.values(report.sources||{}).filter(item=>['READY','CALCULATED'].includes(item.status)).length} / {Object.keys(report.sources||{}).length}</dd></div><div><dt>분석 기간</dt><dd>{PERIOD_LABEL[report.periodDays]||`${report.periodDays}일`}</dd></div><div><dt>계산 시각</dt><dd>{time(report.createdAt)}</dd></div></dl></section>;
}
...
function KeywordChart({keywords=[]}){const rows=keywords.slice(0,7),max=Math.max(1,...rows.map(item=>Number(item.impressions)||0));return <figure className="paKeywordChart"><figcaption><span>키워드 경쟁</span><strong>상품에 연결된 광고 노출</strong><small>실제 연결 키워드만 표시</small></figcaption>{rows.length?<div>{rows.map(item=><article key={item.keyword}><span><b>{item.keyword}</b><small>클릭 {count(item.clicks,'회')} · 주문 {count(item.orders)}</small></span><i><em style={{'--value':`${Math.max(5,(Number(item.impressions)||0)/max*100)}%`}}/></i><strong>{count(item.impressions,'회')}</strong></article>)}</div>:<p>상품에 직접 연결된 네이버 키워드 자료가 없습니다.</p>}</figure>;}

function AnalysisChapters({report}){return <section className="paChapters"><details open><summary><b>01</b><span><strong>수요와 판매</strong><small>연결 광고 노출과 실제 주문이 같은 기간에 이어지는지 봅니다.</small></span><i>⌄</i></summary><div><ChannelChart channels={report.channels}/><KeywordChart keywords={report.keywords}/></div></details><details><summary><b>02</b><span><strong>시장·가격·고객</strong><small>외부 근거가 연결된 항목만 비교합니다.</small></span><i>⌄</i></summary><div className="paSetupGrid">{['competition','audience','reviews'].map(key=>{const item=report.sources?.[key]||{};return <article key={key}><HarinIcon name="search" size={22}/><span><strong>{SOURCE_LABEL[key]}</strong><small>{item.detail||'근거 연결 필요'}</small></span><span className="paSetupAction"><em>{STATUS_LABEL[item.status]||'연결 필요'}</em>{item.href?<Link href={item.href} prefetch={false}>관리 화면 열기</Link>:null}</span></article>;})}</div></details><details><summary><b>03</b><span><strong>키워드와 다음 행동</strong><small>연결 광고 노출과 판매 실적 사이의 확인할 지점을 정리합니다.</small></span><i>⌄</i></summary><div className="paSignalList">{(report.signals||[]).map((item,index)=><article data-tone={item.tone} key={`${item.title}-${index}`}><b>{String(index+1).padStart(2,'0')}</b><span><strong>{item.title}</strong><small>{item.body}</small></span></article>)}</div></details></section>;}
