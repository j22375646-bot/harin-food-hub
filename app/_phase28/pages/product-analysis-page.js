...
function Provenance({sources={}}){return <section className="paProvenance"><header><span>DATA PROVENANCE</span><h3>결론에 사용한 데이터의 출처와 성격</h3></header><div>{Object.entries(SOURCE_LABEL).map(([key,label])=>{const item=sources[key]||{status:'SETUP_REQUIRED'};return <article key={key}><span>{label}</span><strong>{item.label||label}</strong><small>{item.detail||'연결 상태 확인 필요'}</small><em data-status={item.status}>{STATUS_LABEL[item.status]||item.status}</em></article>;})}</div></section>;}
