...
function ChannelDeck({model,router}){
  return <section className="kpChannelDeck" aria-label="키워드 운영 채널 선택"><div className="kpChannelSwitch">{(model.channels||[]).map(channel=><button type="button" aria-pressed={channel.active} data-active={channel.active} key={channel.id} onClick={()=>router.push(channel.href)}><Phase28ChannelLogo brand={channel.brand}/><span><strong>{channel.label}</strong><small>{channel.description}</small></span><em>{channel.id==='naver'?'API 운영':'WING 수동'}</em></button>)}</div><article className="kpChannelMode"><Phase28ChannelLogo brand={model.platform==='coupang'?'COUPANG':'NAVER'}/><span><small>{model.platform==='coupang'?'쿠팡':'네이버'} 운영 방식</small><strong>{model.mode?.label||'운영 방식 확인 필요'}</strong><p>{model.mode?.description||'채널 연결 상태를 확인해주세요.'}</p></span></article></section>;
}
...
export default function Phase28KeywordsPage({model,aiPanel}){
  ...
  const visibleRows=useMemo(()=>sourceRows.filter(row=>{
    ...
    const matchesQuery=!needle||`${row.keyword} ${row.campaign} ${row.adgroup||''} ${row.product}`.toLocaleLowerCase('ko').includes(needle);
  ...
  return <main className="p28Keywords" data-phase28-root="true" data-phase28-page="keywords">
    <Phase28PageHeading context="네이버·쿠팡 채널 분리 · 실제 운영" title="오늘 조정할 키워드 " accent={`${model.hero?.checkCount||0}건`} suffix="이 있어요." summary={model.hero?.summary}/>
    ...
    <Phase28RightRailLayout label="키워드 판단 패널" rail={<DecisionDesk row={activeRow} draft={activeRow?drafts[activeRow.id]:''} onDraft={changeDraft} onPreview={openPreview} model={model}/>}>
      <section className="kpWorkbench" aria-labelledby="kpWorkbenchTitle"><header><div><i><HarinIcon name="keyword" size={22}/></i><span><small>KEYWORD WORKBENCH</small><h2 id="kpWorkbenchTitle">{model.platform==='coupang'?'쿠팡 WING 작업표':'네이버 키워드 운영표'}</h2></span></div><em>{model.platform==='coupang'?'WING 입찰가 확인 필요':`네이버 검색광고 · ${referenceTime(model.generatedAt)}`}</em></header><WorkspaceTabs model={model} router={router}/><div className="kpScope"><label><span>현재 채널 키워드 찾기</span><input type="search" value={query} onChange={event=>setQuery(event.currentTarget.value)} placeholder="키워드·캠페인·상품 검색"/></label><div>{FILTERS.map(([id,label])=><button type="button" aria-pressed={filter===id} key={id} onClick={()=>setFilter(id)}>{label}</button>)}</div><small>{model.platform==='naver'?'현재 입찰가·추천가 서버 기준':'입찰가는 WING 확인 필요'}</small></div>{selected.size?<div className="kpBulk"><strong>{selected.size.toLocaleString('ko-KR')}개 선택</strong><span>{model.platform==='naver'?`수정 입찰가 ${changedRows.length.toLocaleString('ko-KR')}건 입력 · 실제 반영 전 최신값 재조회`:'네이버 선택과 분리된 쿠팡 전용 작업표'}</span><button type="button" onClick={openPreview}>{model.platform==='naver'?'변경안 미리보기':'WING 작업 확인'}</button></div>:null}<KeywordTable model={model} rows={visibleRows} selected={selected} activeId={activeId} drafts={drafts} onToggle={toggle} onInspect={inspect} onDraft={changeDraft}/><footer><span>현재 채널에서 <strong>{visibleRows.length.toLocaleString('ko-KR')}개</strong>를 보는 중</span><small>채널별 데이터·선택·실행 경로 분리</small></footer></section>
