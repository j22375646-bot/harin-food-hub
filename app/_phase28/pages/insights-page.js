...
function ActionPlan({actions={}}){
  ...
  return <section className="inOwnerSection inActionPlan"><header><div><span>OWNER ACTION LOOP</span><h4>오늘 검토하고, 7일 뒤 같은 기준으로 다시 봅니다.</h4></div><b>자동 변경 없음</b></header><div><article><span>01 · 지금 확인</span>{now.length?now.map(item=><section key={item.id}><strong>{item.title}</strong><p>{item.reason}</p></section>):<p className="inOwnerEmpty">즉시 변경할 항목 없이 수집 상태를 확인합니다.</p>}</article><article><span>02 · 7일 확인</span>{sevenDays.length?sevenDays.map(item=><section key={item.id}><strong>{item.title}</strong><p>{item.review}</p></section>):<p className="inOwnerEmpty">7일 뒤 비교할 행동 근거가 아직 없습니다.</p>}</article><article data-tone="guard"><span>03 · 안전장치</span><strong>{actions.guardrail||'사장님 확인 전에는 광고·상품을 변경하지 않습니다.'}</strong></article></div></section>;
}
