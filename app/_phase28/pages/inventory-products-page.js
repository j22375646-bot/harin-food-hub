...
function mappingAssignment(item,assignments){
  if(Object.prototype.hasOwnProperty.call(assignments,item.externalProductId))return assignments[item.externalProductId];
  return item.noLink?NO_LINK_VALUE:item.masterProductId||'';
}
function costComplete(row={}){return COST_FIELDS.every(([field])=>row[field]!==''&&row[field]!=null&&Number.isFinite(Number(row[field]))&&Number(row[field])>=0);}
function costTotal(row={}){return costComplete(row)?COST_FIELDS.reduce((sum,[field])=>sum+Number(row[field]),0):null;}
async function executeConfirmedFinancialPreview(previewResult){
  ...
  const response=await fetch(`/api/financial-changes/${requestId}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'CONFIRM_EXECUTE',confirm:true,note:'상품 원가 작업대에서 확인 후 즉시 실행'})});
  const result=await response.json();
  if(!response.ok||!result.ok)throw new Error(result.error||'원가 변경을 적용하지 못했습니다.');
  if(result.blocked||result.applied===false)throw new Error(result.request?.error_message||'자료가 바뀌어 실행을 멈췄습니다. 다시 확인해주세요.');
  if(!result.verified)throw new Error(result.request?.error_message||'변경 후 실제 저장값 재확인이 필요합니다.');
  return result;
}
...
function ProductMappingWorkspace({model}){
  ...
  const readyAssignments=selectedRows.map(item=>({external_product_id:item.externalProductId,master_product_id:mappingAssignment(item,assignments)})).filter(item=>item.master_product_id);
  const recommendableCount=selectedRows.filter(item=>!mappingAssignment(item,assignments)&&item.suggestions?.[0]?.masterProductId).length;
  ...
  function fillSuggestions(){const fillable=selectedRows.filter(item=>!mappingAssignment(item,assignments)&&item.suggestions?.[0]?.masterProductId);setAssignments(current=>{const next={...current};for(const item of fillable)next[item.externalProductId]=item.suggestions[0].masterProductId;return next;});const unresolved=selectedRows.length-readyAssignments.length-fillable.length;setMessage(fillable.length?`추천값 ${count(fillable.length)}개 채움 · ${count(Math.max(0,unresolved))}개 직접 선택 필요`:'선택한 상품에는 아직 안전하게 채울 추천값이 없어요.');}
  ...
  async function connectSelected(){if(!selectedRows.length)return;const missing=selectedRows.length-readyAssignments.length;if(missing){setMessage(`연결 결정을 ${count(missing)}개 더 선택해주세요.`);return;}const noLinkCount=readyAssignments.filter(item=>item.master_product_id===NO_LINK_VALUE).length;if(!window.confirm(`${CHANNEL_NAMES[platform]} 실상품 ${selectedRows.length}개의 연결 결정을 저장할까요?\n기준상품 연결 ${selectedRows.length-noLinkCount}개 · 연결제품 없음 ${noLinkCount}개`))return;await postMapping({action:'BULK_ASSIGN',platform,assignments:readyAssignments},'연결 결정 저장 완료');}
  ...
  return <Phase28RightRailLayout label="상품 연결 보조석" rail={<ProductMappingRail mapping={mapping} platform={platform} view={view} selectedIds={selectedIds} assignments={Object.fromEntries(selectedRows.map(item=>[item.externalProductId,mappingAssignment(item,assignments)]))}/> }><><ProductHero hero={model.hero||{}}/><section className="ipMappingGuide"><div><span>CHANNEL PRODUCT MAPPING</span><h2>채널 상품을 골라 기준상품에 한 번에 연결해요</h2><p>판매중·노출중·재고 보유가 확인된 상품만 표시하고, 행마다 연결할 Cafe24 기준상품을 지정합니다.</p></div><div className="ipPlatformSwitch" aria-label="연결할 채널 선택">{['NAVER','COUPANG'].map(brand=><button type="button" key={brand} aria-pressed={platform===brand} onClick={()=>choosePlatform(brand)}><Phase28ChannelLogo brand={brand}/><span><strong>{CHANNEL_NAMES[brand]}</strong><small>{brand==='NAVER'?'스마트스토어 판매·재고 확인':'쿠팡 판매·재고 확인'}</small></span><em>{count((mapping.candidates||[]).filter(item=>item.platform===brand).length)}개 미연결</em></button>)}</div></section><section className="ipWorkbench"><header className="ipWorkbenchHead"><div><span>PLATFORM-SAFE WORKBENCH</span><h2>{CHANNEL_NAMES[platform]} 채널 상품</h2><p>선택한 플랫폼의 판매·노출·재고 확인 상품만 표시하고 다른 채널 상품은 섞지 않아요.</p></div><span className="ipMappingChannelBadge"><Phase28ChannelLogo brand={platform} size="compact"/>{CHANNEL_NAMES[platform]} 전용</span></header><div className="ipUtility"><label><HarinIcon name="search" size={18}/><input value={query} onChange={event=>{setQuery(event.target.value);setVisibleLimit(model.visibleLimit||50);setSelectedIds([]);setAssignments({});}} placeholder="채널 상품명·상품 ID·기준상품 검색"/></label><span>Cafe24 판매중 기준상품 {count(mapping.masterProducts.length)}개</span></div><nav className="ipTabs" aria-label="상품 운영 보기">{PRODUCT_TABS.map(tab=><button type="button" aria-selected={(model.workspace||'catalog')===tab.id} key={tab.id} onClick={()=>pushPhase28Route(router,tab.href)}>{tab.label}</button>)}</nav><div className="ipMappingToolbar"><div>{[['UNLINKED','미연결 상품'],['LINKED','연결 완료']].map(([id,label])=><button type="button" key={id} className={view===id?'active':''} onClick={()=>chooseView(id)}>{label}</button>)}</div><span>{CHANNEL_NAMES[platform]} {visibleRows.length.toLocaleString('ko-KR')} / {filteredRows.length.toLocaleString('ko-KR')}개 표시</span></div><div className="ipMappingSelection"><label><input type="checkbox" checked={visibleRows.length>0&&selectedIds.length===Math.min(visibleRows.length,100)} onChange={event=>event.target.checked?selectVisible():setSelectedIds([])}/><span>현재 목록 선택</span></label><strong>{selectedIds.length}개 선택</strong>{view==='UNLINKED'?<><button type="button" onClick={fillSuggestions} disabled={!recommendableCount}>추천값 채우기 {recommendableCount?`${count(recommendableCount)}개`:''}</button><button type="button" className="primary" onClick={connectSelected} disabled={busy||!selectedIds.length}>선택 {selectedIds.length}개 결정 저장</button></>:<button type="button" className="danger" onClick={unlinkSelected} disabled={busy||!selectedIds.length}>선택 {selectedIds.length}개 연결 해제</button>}</div><div className="ipMappingBoundary"><div className="ipMappingColumns"><span>선택</span><span>{CHANNEL_NAMES[platform]} 상품</span><span>{view==='UNLINKED'?'연결할 기준상품':'연결된 기준상품'}</span><span>연결 근거</span></div><div className="ipRows">{visibleRows.map(item=>{const suggested=item.suggestions?.[0];const selectedMasterId=mappingAssignment(item,assignments);const linkedMaster=mapping.masterProducts.find(product=>product.id===item.masterProductId);return <article className="ipMappingRow" data-selected={selectedIds.includes(item.externalProductId)} key={`${item.platform}-${item.externalProductId}`}><input type="checkbox" aria-label={`${item.name} 선택`} checked={selectedIds.includes(item.externalProductId)} onChange={event=>toggle(item.externalProductId,event.target.checked)}/><span className="ipMappingProduct"><Phase28ChannelLogo brand={platform}/><span><strong>{item.name}</strong><small>상품 ID {item.externalProductId} · {money(item.price)}</small></span></span>{view==='UNLINKED'?<label className="ipMasterSelect"><span className="srOnly">연결할 기준상품</span><select value={selectedMasterId} onChange={event=>setAssignments(current=>({...current,[item.externalProductId]:event.target.value}))}><option value="">기준상품 선택</option><option value={NO_LINK_VALUE}>연결제품 없음 · 플랫폼 단독판매</option>{mapping.masterProducts.map(product=><option value={product.id} key={product.id}>{product.name} · {money(product.price)}</option>)}</select></label>:<span className="ipLinkedMaster"><strong>{linkedMaster?.name||'기준상품 확인 필요'}</strong><small>{item.method==='AUTO'?'자동 연결':'수동 연결'}</small></span>}<span className="ipMappingReason">{view==='UNLINKED'?(selectedMasterId===NO_LINK_VALUE?<><strong>연결제품 없음</strong><small>이 플랫폼에서만 판매하는 상품으로 저장</small></>:suggested?<><strong>추천 {count(suggested.confidence??Math.round((suggested.score||0)*100))}%</strong><small>{suggested.reasons?.join(' · ')||'상품명·규격 비교'}</small></>:<><strong>직접 선택 필요</strong><small>확정할 추천 근거가 부족해요.</small></>):<><strong>{item.confidence==null?'확인 필요':`일치도 ${Math.round(Number(item.confidence)*100)}%`}</strong><small>변경 이력 보존</small></>}</span></article>;})}{!visibleRows.length?<div className="ipEmpty"><HarinIcon name="link" size={24}/><strong>{view==='UNLINKED'?`${CHANNEL_NAMES[platform]} 미연결 상품이 없어요.`:`${CHANNEL_NAMES[platform]} 연결 기록이 없어요.`}</strong><span>선택한 플랫폼의 최신 판매·재고 확인 상품만 표시합니다.</span></div>:null}</div>{visibleRows.length<filteredRows.length?<button type="button" className="ipMappingMore" onClick={()=>setVisibleLimit(current=>current+(model.visibleLimit||50))}>상품 {Math.min(model.visibleLimit||50,filteredRows.length-visibleRows.length)}개 더 보기</button>:null}</div>{message?<p className="ipWorkbenchMessage" role="status">{message}</p>:null}<footer className="ipWorkbenchFoot"><div className="ipWorkbenchFootStatus"><i className="ipWorkbenchFootIcon"><HarinIcon name="link" size={19}/></i><span><small>채널 분리</small><strong>{CHANNEL_NAMES[platform]} 판매·재고 확인 상품만 선택·저장</strong></span></div><div className="ipWorkbenchFootSource"><i className="ipWorkbenchFootIcon"><HarinIcon name="database" size={19}/></i><span><small>연결 기준</small><strong>Cafe24 판매중 기준상품</strong></span></div></footer></section></></Phase28RightRailLayout>;
}

function ProductCostRail({workbench,selectedIds,rows}){
  const selectedRows=workbench.rows.filter(row=>selectedIds.includes(row.id));
  const readySelected=selectedRows.filter(row=>costComplete(rows[row.id])).length;
  return <div className="ipRailBody"><header className="ipRailHeader"><span>원가 입력 보조석</span><h2>판매 상품 원가</h2><p>모르는 비용은 비워두고 확인된 금액만 저장합니다.</p></header><div className="ipProductJudgment" data-ready={workbench.pendingCount===0}><i><HarinIcon name={workbench.pendingCount===0?'shield':'price'} size={22}/></i><span><small>전체 입력 상태</small><strong>{count(workbench.readyCount)} / {count(workbench.rows.length)}개</strong><em>{workbench.pendingCount?`${count(workbench.pendingCount)}개 확인 필요`:'모두 입력 완료'}</em></span></div><dl className="ipRailFacts"><div><dt>선택 상품</dt><dd>{count(selectedIds.length)}개</dd></div><div><dt>저장 준비</dt><dd>{count(readySelected)}개</dd></div><div><dt>저장 방식</dt><dd>상품별 기록·즉시 검증</dd></div></dl><section className="ipMappingSafety"><span>COST SAFETY</span><h3>빈칸을 0원으로 만들지 않아요.</h3><p>상품 원가·포장비·기타 단위비 세 값을 모두 확인한 상품만 서버에 저장하고, 변경 뒤 실제 값을 다시 확인합니다.</p></section><small className="ipRailSafety">판매중 Cafe24 기준상품만 수정하며 변경 전후 값과 실행 이력을 보존합니다.</small></div>;
}

function ProductCostWorkspace({model}){
  const router=useRouter();
  const workbench=model.costWorkbench||{rows:[],readyCount:0,pendingCount:0};
  const [rows,setRows]=useState(()=>Object.fromEntries(workbench.rows.map(row=>[row.id,{unitCost:row.unitCost??'',packagingCost:row.packagingCost??'',otherUnitCost:row.otherUnitCost??''}])));
  const [selectedIds,setSelectedIds]=useState([]);
  const [query,setQuery]=useState('');
  const [filter,setFilter]=useState('ALL');
  const [bulk,setBulk]=useState({unitCost:'',packagingCost:'',otherUnitCost:''});
  const [busy,setBusy]=useState(false);
  const [message,setMessage]=useState('');
  const visibleRows=useMemo(()=>workbench.rows.filter(row=>{
    if(query&&!`${row.name} ${row.sku}`.toLocaleLowerCase('ko-KR').includes(query.toLocaleLowerCase('ko-KR')))return false;
    const ready=costComplete(rows[row.id]);
    if(filter==='READY'&&!ready)return false;
    if(filter==='PENDING'&&ready)return false;
    return true;
  }),[workbench.rows,rows,query,filter]);
  function toggle(id,checked){setSelectedIds(current=>checked?[...new Set([...current,id])]:current.filter(value=>value!==id));}
  function selectVisible(){setSelectedIds(visibleRows.slice(0,100).map(row=>row.id));}
  function changeCost(id,field,value){setRows(current=>({...current,[id]:{...(current[id]||{}),[field]:value}}));setSelectedIds(current=>current.includes(id)?current:[...current,id]);}
  function applyBulk(){const updates=Object.fromEntries(COST_FIELDS.filter(([field])=>bulk[field]!==''&&Number.isFinite(Number(bulk[field]))&&Number(bulk[field])>=0).map(([field])=>[field,bulk[field]]));if(!selectedIds.length){setMessage('일괄 입력할 판매중 상품을 먼저 선택해주세요.');return;}if(!Object.keys(updates).length){setMessage('일괄 적용할 금액을 한 칸 이상 입력해주세요.');return;}setRows(current=>{const next={...current};for(const id of selectedIds)next[id]={...(next[id]||{}),...updates};return next;});setMessage(`${count(selectedIds.length)}개 상품에 입력값을 적용했습니다. 저장 전 표에서 다시 확인해주세요.`);}
  async function executeCostPayload(row){const response=await fetch('/api/costs',{method:'PUT',headers:{'content-type':'application/json','idempotency-key':crypto.randomUUID()},body:JSON.stringify({type:'PRODUCT',master_product_id:row.id,unit_cost:Number(rows[row.id].unitCost),packaging_cost:Number(rows[row.id].packagingCost),other_unit_cost:Number(rows[row.id].otherUnitCost)})});const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.error||'원가 변경 준비 실패');await executeConfirmedFinancialPreview(result);}
  async function saveSelected(){const selected=workbench.rows.filter(row=>selectedIds.includes(row.id));if(!selected.length)return;const incomplete=selected.filter(row=>!costComplete(rows[row.id]));if(incomplete.length){setMessage(`선택 상품 중 ${count(incomplete.length)}개의 원가·포장비·기타 비용을 모두 확인해주세요.`);return;}if(!window.confirm(`판매중 상품 ${selected.length}개의 원가를 한 번에 저장할까요?\n상품마다 변경 전후 값과 실제 저장값 재확인 결과가 기록됩니다.`))return;setBusy(true);setMessage(`원가 ${count(selected.length)}개를 순서대로 저장·검증하고 있어요…`);const completed=[],failures=[];for(const row of selected){try{await executeCostPayload(row);completed.push(row.id);}catch(error){failures.push(`${row.name}: ${error.message}`);}}setBusy(false);setSelectedIds(current=>current.filter(id=>!completed.includes(id)));setMessage(failures.length?`원가 ${count(completed.length)}개 저장 완료 · ${count(failures.length)}개 확인 필요 (${failures[0]})`:`원가 ${count(completed.length)}개 저장 완료 · 실제 저장값 재확인까지 끝났습니다.`);router.refresh();}
...
function RocketGrowthInboundWorkspace({rows}){
  ...
  async function loadInbound(preserve=true){
    try{
      ...
      const result=await response.json();
      if(!response.ok||!result.ok)throw new Error(result.error||'입고 송장 자료 조회 실패');
      setData({destinations:result.destinations||[],products:result.products||[],history:result.history||[]});
      if(!preserve&&!selectedCenterCode&&result.destinations?.length){
        const first=result.destinations[0];
        setSelectedCenterCode(first.centerCode);
        setDestinationForm({...emptyDestination,...first});
      }
    }catch(error){setMessage(`확인 필요 · ${error.message}`);}finally{setLoading(false);}
  }
  useEffect(()=>{loadInbound(false);},[]);
  useEffect(()=>{
    if(!data.history.some(item=>['QUEUED','ISSUING'].includes(item.status)))return undefined;
    const timer=window.setInterval(()=>loadInbound(true),2000);
    return ()=>window.clearInterval(timer);
  },[data.history]);

  function chooseCenter(code){
    const found=data.destinations.find(item=>item.centerCode===code)||null;
    setSelectedCenterCode(code);
    setDestinationForm(found?{...emptyDestination,...found}:{...emptyDestination,centerCode:code,label:code});
  ...
  async function saveDestination(){
    ...
    try{
      const response=await fetch('/api/inventory/rocket-growth/inbound',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'SAVE_DESTINATION',confirm:true,destination:destinationForm})});
      const result=await response.json();
      if(!response.ok||!result.ok)throw new Error(result.error||'물류센터 저장 실패');
      setSelectedCenterCode(result.destination.centerCode);setShowDestinationForm(false);setMessage('물류센터 주소를 저장했습니다. 이제 일괄 송장에 사용할 수 있어요.');await loadInbound(true);
  ...
  const allSelected=shipmentRows.length>0&&shipmentRows.slice(0,50).every(row=>drafts[row.id]?.selected);
  return <div className="ipInboundWorkbench">
    <section className="ipInboundDestination">
      <header><div><span>ROCKET GROWTH DESTINATION</span><h3>로켓그로스 물류센터 선택</h3><p>쿠팡 저장 자료의 센터 코드를 불러오고, 주소는 최초 한 번만 저장합니다.</p></div><button type="button" onClick={()=>setShowDestinationForm(value=>!value)}><HarinIcon name="edit" size={17}/>센터 주소 등록·수정</button></header>
      <div className="ipInboundDestinationSelect"><label><span>받는 물류센터</span><select value={selectedCenterCode} onChange={event=>chooseCenter(event.target.value)} disabled={loading}><option value="">물류센터 선택</option>{data.destinations.map(item=><option value={item.centerCode} key={item.centerCode}>{item.label} · {item.centerCode} · {item.statusLabel}</option>)}</select></label><article data-ready={selectedDestination?.ready||false}><i><HarinIcon name={selectedDestination?.ready?'shield':'warning'} size={20}/></i><span><small>우체국 발급 상태</small><strong>{selectedDestination?.statusLabel||'센터 선택 필요'}</strong></span><em>{selectedDestination?.source==='COUPANG_API_HINT'?'쿠팡 코드 자동 발견':'암호화 저장'}</em></article></div>
...
function ProductsWorkspace({model}){
  return model.workspace==='mappings'?<ProductMappingWorkspace model={model}/>:model.workspace==='costs'?<ProductCostWorkspace model={model}/>:<ProductCatalogWorkspace model={model}/>;
}
