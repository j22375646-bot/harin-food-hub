...
function waterfallLayout(items){
  ...
  const actual=Math.max(0,Number(values.actual)||0);
  const max=Math.max(gross,expected,actual,1);
  const layout={
    ...
    expected:{height:expected,bottom:0,connector:expected},
    actual:{height:actual,bottom:0,connector:actual}
  };
...
function DecisionBoard({model,period,setPeriod,selectedId,onSelect}){
  ...
  return <section className="spDecisionBoard" aria-labelledby="spDecisionTitle">
    ...
    <div className="spSpineGrid"><figure className="spWaterfall"><figcaption><div><span>정산 대조 스파인</span><strong>총매출에서 실제 지급까지</strong></div><div className="spLegend"><i data-tone="blue"/>지급액<i data-tone="expense"/>공제<i data-tone="actual"/>차이</div></figcaption><div className="spWaterfallChart" role="img" aria-label="총매출에서 취소 환불, 판매 수수료, 배송 물류비를 거쳐 예상 정산액과 실제 지급액을 비교한 그래프">{(model.waterfall||[]).map(item=><article className="spWaterfallStep" data-tone={item.tone} data-known={item.value!=null} key={item.id}><div className="spWaterfallTrack"><i className="spWaterfallBar" style={styles[item.id]}/></div><strong>{item.value==null?'확인 필요':money(['refunds','fees','logistics'].includes(item.id)?-Math.abs(item.value):item.value)}</strong><span>{item.label}</span></article>)}</div><p>채널별 원본은 섞지 않고, 같은 기간의 서버 계산 합계만 한 축에서 비교합니다.</p></figure>
