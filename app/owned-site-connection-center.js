...
export default function OwnedSiteConnectionCenter({center}){
  const [working,setWorking]=useState('');const [message,setMessage]=useState('');const [services,setServices]=useState(center?.services||[]);const summary=serviceSummary(services);
  async function probe(service){setWorking(service.key);setMessage(`${service.label} 읽기 자료를 확인하고 있어요.`);try{const response=await fetch(service.action.endpoint,{method:'POST',headers:{Accept:'application/json'}});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.error||'연결 확인 요청을 처리하지 못했습니다.');const result=payload.result||payload;setServices(current=>replaceProbeService(current,service.key,mergeProbeService(service,result)));setMessage(result.status==='NO_DATA'?`${service.label} 연결은 됐지만 아직 충분한 표본이 없어요.`:`${service.label} 읽기 연결을 확인해 이 카드에 바로 반영했어요.`);}catch(error){setServices(current=>replaceProbeService(current,service.key,failProbeService(service,error)));setMessage(`확인 필요 · ${error.message}`);}finally{setWorking('');}}
  return <section className="naverApiCenter ownedSiteApiCenter">
    <header className="naverApiHero"><div className="naverApiHeroIcon"><HarinIcon name="growth" size={30}/></div><div><span>PHASE {center?.phase||'19-1'} · OWNED SITE SIGNALS</span><h1>자사몰 유입·전환 API 센터</h1><p>검색 유입, 방문, 모바일 속도, 실제 사용성을 한곳에서 확인하되 공급자 자료는 서로 섞지 않아요.</p></div><aside><span><b>{summary.ready}</b>개 정상</span><span><b>{summary.attention}</b>개 확인</span></aside></header>
