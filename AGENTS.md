...
- 0.103.0부터 유료 인증서 대신 앱에 고정된 Ed25519 공개키로 업데이트를 검증한다. 사용자 요청에 따른 다음 앱 버전 배포는 테스트/빌드 뒤 `desktop/scripts/build-distribution.ps1`, `desktop/scripts/publish-signed-release.cjs <산출물 폴더>`로 GitHub `moaon-stable` 배포까지 진행한다. 코드 푸시/로컬 바로가기 갱신만으로 고객 배포 완료라고 하지 않는다.
- 개인키는 저장소 밖 Windows DPAPI 저장본을 사용한다. 재생성/삭제하거나 설치파일에 넣지 않는다. 기존 서명 검증을 단순 해제하지 않는다. 상세 절차는 P4-117 보고서를 따른다.
