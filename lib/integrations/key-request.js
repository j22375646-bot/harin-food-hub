...
function createHandler({database=require('../cafe24/supabase.js').getSupabase,validate=auth.validateSession,env=process.env}={}){return async request=>{
 try{
  ...
  if(input.action==='LIST'){
   ...
   const inventory=require('./connection-inventory.js'),collections=Object.fromEntries(await Promise.all(Object.keys(keys.definitions).map(async p=>[p,await inventory.collection(db,p)])));
   const aiChecks=Object.fromEntries(await Promise.all(Object.entries(keys.definitions).filter(([,d])=>d.ai).map(async([p])=>{try{return [p,keys.aiMetadata(p,rows.some(r=>r.provider===p)?await keys.readRow(p,db):null,env)];}catch{return [p,{status:'SETUP_REQUIRED',enabled:false,ready:false,reasonCodes:['KEY_UNAVAILABLE'],checkedAt:null}];}})));
   const allCards=Object.entries(keys.definitions).map(([provider,d])=>{const row=rows.find(r=>r.provider===provider),check=checks.data.find(r=>r.provider===provider&&r.revision===(row?.revision||0));return {provider,collection:collections[provider],name:d.name,revision:row?.revision||0,expiresAt:row?.expiresAt||(provider==='COUPANG'?env.COUPANG_KEY_EXPIRES_AT||null:null),updatedAt:row?.updatedAt||null,editable:env.MOAON_MANAGED_KEYS_ENABLED==='1',check:d.ai?aiChecks[provider]:check?.result||null,fields:Object.keys(d.fields),identity:d.identity};});
   return reply(200,{ok:true,services:inventory.services(env),cards:allCards.filter(c=>!keys.definitions[c.provider].ai),aiCards:allCards.filter(c=>keys.definitions[c.provider].ai)});
  }
  if(input.action==='REVEAL'){
   if(keys.definitions[input.provider].ai)return reply(400,{ok:false,code:'KEYS_INVALID'});
  ...
  if(input.action==='SAVE'){
   ...
   const d=keys.definitions[input.provider];
   const existing=await rpc({action:d.ai?'CHECK':'REVEAL',provider:input.provider});
   const base=d.ai?{}:keys.decode(input.provider,existing,env);
   ...
   const fields=d.ai&&input.provider!=='OPENAI'?{...input.fields,managedPolicyVersion:require('./managed-ai-config.js').POLICY_VERSION}:input.fields;
   const envelope=keys.cipher(env).seal({tenantId:keys.TENANT,provider:input.provider,revision:input.revision+1},fields);
   const saved=await rpc({action:'SAVE',provider:input.provider,revision:input.revision,envelope,expiresAt:input.expiresAt});
   if(d.ai)return reply(200,{ok:true,revision:saved.revision,...keys.aiMetadata(input.provider,{revision:saved.revision,envelope,updatedAt:saved.updatedAt},env)});
   return reply(200,{ok:true,...saved});
  }
  const checked=await rpc(input);
  if(keys.definitions[input.provider].ai)return reply(200,{ok:true,...keys.aiMetadata(input.provider,checked?await keys.readRow(input.provider,db):null,env)});
