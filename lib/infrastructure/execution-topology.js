...
function buildExecutionTopology({controls=[],syncRequests=[],operationRequests=[],automationRuns=[],heartbeats=[],syncLogs=[],nativeQueueEnabled=false,now=new Date()}={}){
  ...
  const lanes=rows.map(control=>{
    ...
    return {...control,state,lastEvidenceAt:stamp(evidence),evidenceStatus:evidence?.status||null,evidenceLabel:evidence?`${evidence.operation_type||evidence.request_type||evidence.job_type||evidence.job_name||evidence.service_name||'실행 신호'} · ${evidence.status||'확인'}`:'운영 설정만 확인됨'};
  });
  return {
    phase:'21-1',generatedAt:new Date(now).toISOString(),mode:'OBSERVE',lanes,
    summary:{lanes:lanes.length,activeLanes:lanes.filter(item=>item.state==='ACTIVE').length,collisionKeys:collisionKeys.length,activeQueue:activeSync+activeOperations+activeAutomation,legacyWithoutKey:legacyWithoutKey(syncRequests)+legacyWithoutKey(operationRequests)+legacyWithoutKey(automationRuns),workerReady,nativeQueueEnabled:Boolean(nativeQueueEnabled)},
    ...
    protections:[
      '현재 systemd·Vercel Cron·Supabase 사용자 정의 큐의 소유권을 바꾸지 않습니다.',
...
module.exports={ACTIVE_STATUSES,FALLBACK_CONTROLS,buildExecutionTopology,duplicateKeys,legacyWithoutKey};
