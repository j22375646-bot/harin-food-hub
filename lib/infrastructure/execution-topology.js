...
function buildExecutionTopology({controls=[],syncRequests=[],operationRequests=[],automationRuns=[],heartbeats=[],syncLogs=[],nativeQueueEnabled=false,now=new Date()}={}){
  ...
  return {
    phase:'21-2',generatedAt:new Date(now).toISOString(),mode:'AUTO_GUARD',lanes,
    ...
    protections:[
      '별도 잠금 절차 없이 현재 systemd·Vercel Cron·Supabase 실행 경로를 자동 확인합니다.',
