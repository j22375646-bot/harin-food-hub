...
const {buildWorkerHealth}=require('../operations/reliability-center.js');
...
function evidenceFor(laneKey,{syncRequests,operationRequests,automationRuns,heartbeats,syncLogs}){
  // A delayed queue completion is not evidence that the hourly trigger ran again.
  if(laneKey==='HOURLY_ORDERS')return latest(laneAutomationRuns(laneKey,automationRuns),['started_at']);
...
function buildExecutionTopology({controls=[],syncRequests=[],operationRequests=[],automationRuns=[],heartbeats=[],syncLogs=[],issues=[],env=process.env,nativeQueueEnabled=false,now=new Date()}={}){
  ...
  const workerReady=Boolean(heartbeat&&buildWorkerHealth([heartbeat],now).workers[0]?.ready);
  ...
  const lanes=rows.map(control=>{
    ...
    const hourly=control.lane_key==='HOURLY_ORDERS';
    const evidenceAt=hourly?evidence?.started_at||null:stamp(evidence);
    const runAge=time(now)-time(evidenceAt);
    const scheduleMissing=hourly&&(!evidenceAt||runAge>75*60*1000);
    const scheduleFailed=hourly&&(['FAILED','PARTIAL'].includes(evidence?.status)||(evidence?.status==='RUNNING'&&runAge>15*60*1000));
    const state=collisionKeys.length?'COLLISION_RISK':unauthorizedMode?'BLOCKED':scheduleMissing||scheduleFailed||workerLane&&!workerReady?'CHECK':evidence?'ACTIVE':'OBSERVE';
  ...
  const switchReady=collisionKeys.length===0&&workerReady&&activeQueue===0&&protectedLanes===lanes.length&&lanes.every(item=>item.mode==='OBSERVE');
  const blockers=[];
  ...
  if(lanes.some(item=>item.lane_key==='HOURLY_ORDERS'&&item.state==='CHECK'))blockers.push('시간별 주문 수집 실행 공백 확인 필요');
