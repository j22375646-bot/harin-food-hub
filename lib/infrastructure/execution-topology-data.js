...
const querySpecs=[
  ...
  ['automationRuns',db=>db.from('automation_runs').select('id,job_name,trigger_type,status,started_at,finished_at,idempotency_key,scheduled_for,kst_execution_date').order('started_at',{ascending:false}).limit(500)],
...
async function loadExecutionTopology(db,{now=new Date(),nativeQueueEnabled=process.env.SUPABASE_NATIVE_QUEUE_ENABLED==='true'}={}){
  ...
  return {...buildExecutionTopology(input),issues};
}
