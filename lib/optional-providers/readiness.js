...
function publicError(error){const message=String(error?.message||'선택형 API 확인에 실패했습니다.').replace(/\s+/g,' ').slice(0,180);if(/403|auth|key|credential/i.test(message))return 'DeepL API Free 키 또는 계정 상태를 확인해주세요.';if(/456|quota/i.test(message))return '이번 달 무료 번역 한도를 모두 사용했습니다. 자동 번역은 계속 잠금 상태입니다.';return message;}
...
module.exports={DEFINITIONS,buildOptionalProviderCenter,probeDeepL,publicError,statusFor};
