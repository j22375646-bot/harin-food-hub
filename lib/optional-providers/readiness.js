...
async function probeProcurement({db,env=process.env,fetchImpl=fetch,now=new Date()}={}){const config=configModule.providerConfig('PUBLIC_PROCUREMENT',env);if(!config.businessActive){const error=new Error('B2B·공공조달 사업 시작 확인이 필요합니다.');error.code='BUSINESS_NOT_ACTIVE';error.status=423;throw error;}if(!config.enabled){const error=new Error('나라장터 읽기 공급자는 현재 사용 중지 상태입니다.');error.code='PROVIDER_DISABLED';error.status=423;throw error;}if(!config.apiKey){const error=new Error('필요한 서버 설정: 공공데이터포털 서비스 키');error.code='CONFIG_REQUIRED';error.status=412;throw error;}try{const result=await procurement.probe({config,fetchImpl});const stored=await saveSnapshot(db,{provider:'PUBLIC_PROCUREMENT',status:result.status,metric_summary:{item_count:result.totalCount},fetched_at:new Date(now).toISOString(),metadata:{read_only:true,sample_probe:true,sample_notice_number:result.sampleNoticeNumber,source_timestamp:result.sourceTimestamp,contains_customer_data:false,automatic_submission:false}});return {provider:'PUBLIC_PROCUREMENT',status:result.status,snapshot:stored,count:result.totalCount};}catch(error){await saveSnapshot(db,{provider:'PUBLIC_PROCUREMENT',status:'FAILED',metric_summary:{},fetched_at:new Date(now).toISOString(),error_code:error.code||'PROCUREMENT_READ_FAILED',error_message:publicError(error,'PUBLIC_PROCUREMENT'),metadata:{read_only:true,sample_probe:true,contains_customer_data:false}}).catch(()=>{});throw error;}}
