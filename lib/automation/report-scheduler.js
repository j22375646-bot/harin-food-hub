...
const { cleanupDuplicateDiagnosisReports } = require('../reports/deduplicate.js');
...
async function generatePlatformSet({period,reportType,mode,deduplicate,revisionNote}){
  ...
  const deduplication=await cleanupDuplicateDiagnosisReports().then(result=>({ok:true,...result})).catch(error=>({ok:false,error:error.message||'자동진단 중복 정리 실패'}));
  return {ok:reports.every(item=>item.ok),reports,delivery,deduplication};
}
...
async function generateWeekly({ triggerType = 'SYSTEM', deduplicate = true, runOptions = {}, now = new Date() } = {}) {
  return runJob({ jobName:'WEEKLY_PLATFORM_REPORTS',triggerType,maxAttempts:2,...runOptions,work:()=>generatePlatformSet({period:reportModule.previousWeek(now),reportType:'WEEKLY',mode:triggerType==='MANUAL'?'WEEKLY_MANUAL':'WEEKLY_AUTO',deduplicate}) });
}
...
module.exports = { dailyPeriod, previousMonth, isFirstDayKst, monthlyStage, generatePlatformSet, generateDaily, generateWeekly, generateMonthly };
