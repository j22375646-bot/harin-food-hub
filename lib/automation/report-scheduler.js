...
async function generateMonthly({ triggerType = 'SYSTEM', deduplicate = true, runOptions = {} } = {}) {
  return runJob({ jobName: 'MONTHLY_PLATFORM_REPORTS', triggerType, maxAttempts: 2, ...runOptions, work: () => generatePlatformSet({period:previousMonth(),reportType:'MONTHLY',mode:triggerType==='MANUAL'?'MONTHLY_MANUAL':'MONTHLY_AUTO',deduplicate}) });
}
