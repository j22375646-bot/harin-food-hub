...
function buildMainCashflow({
  ...
  const naverAdSpend=available.ads?round(sum(naverAdRows,'cost')):null;
  const coupangAdSpend=available.ads?round(sum(coupangAdRows,'ad_spend')):null;
  const adSpend=available.ads?round(naverAdSpend+coupangAdSpend):null;
  ...
  return Object.freeze({
    ...
    adSpendByPlatform:Object.freeze({ALL:adSpend,NAVER:naverAdSpend,CAFE24:available.ads?0:null,COUPANG:coupangAdSpend}),
