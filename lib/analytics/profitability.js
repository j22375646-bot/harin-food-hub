...
function calculateProfitability({ items = [], productLinks = [], productCosts = [], channelSetting = {}, shippingRule = {}, adSpend = 0 } = {}) {
  ...
  const result = {
    ...
    break_even_roas: coverageRate > 0 && marginRate > 0 ? round(100 / marginRate) : null, cost_coverage_rate: round(coverageRate), cost_status: coverageRate >= financialTrust.MIN_COST_COVERAGE_RATE ? 'COMPLETE' : coverageRate > 0 ? 'PARTIAL' : 'COST_DATA_REQUIRED',
