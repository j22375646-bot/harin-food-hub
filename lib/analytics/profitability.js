...
function calculateProfitability({ items = [], productLinks = [], productCosts = [], channelSetting = {}, shippingRule = {}, adSpend = 0 } = {}) {
  ...
  const products = [...productMap.values()].map(row => {
    ...
    return { ...row, revenue: round(row.revenue), product_cost: round(row.product_cost), fees: round(row.fees), base_shipping_cost:round(row.base_shipping_cost), return_reserve:round(row.return_reserve), remote_area_reserve:round(row.remote_area_reserve), shipping_cost: round(row.shipping_cost), contribution_before_ads: round(contributionBeforeAds), contribution_margin_rate: marginRate == null ? null : round(marginRate * 100), break_even_roas: row.cost_configured && marginRate > 0 ? round(100 / marginRate) : null };
