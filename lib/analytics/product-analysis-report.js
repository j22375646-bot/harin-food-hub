...
const sum=(rows,key)=>rows.some(item=>finite(item?.[key])==null)?null:rows.reduce((total,item)=>total+finite(item[key]),0);
...
function buildProductAnalysisSummary({product={},performance={},keywords=[],customerEvidence={},marketEvidence={},periodDays=30,periodStart=null,periodEnd=null,generatedAt=new Date().toISOString()}={}){
  ...
  const keywordRows=(keywords||[]).map(item=>({
    ...
    ctr:finite(item.impressions)>0&&finite(item.clicks)!=null?Number((finite(item.clicks)/finite(item.impressions)*100).toFixed(2)):null,
    roas:finite(item.cost)>0&&finite(item.conversion_revenue)!=null?Number((finite(item.conversion_revenue)/finite(item.cost)*100).toFixed(1)):null
  })).sort((left,right)=>(right.impressions||0)-(left.impressions||0));
  const channelAdEvidence=channels.NAVER?.ad_evidence_status==='READY';
  const searchDemand=keywordRows.length?sum(keywordRows,'impressions'):channelAdEvidence?finite(channels.NAVER?.impressions):null;
  const searchClicks=keywordRows.length?sum(keywordRows,'clicks'):channelAdEvidence?finite(channels.NAVER?.clicks):null;
  ...
  const metrics={
    revenue:rounded(revenue),orders:rounded(orders),units:rounded(units),order_value:orders>0&&revenue!=null?rounded(revenue/orders):null,
    ...
    click_rate:searchDemand>0&&searchClicks!=null?Number((searchClicks/searchDemand*100).toFixed(2)):null,
  ...
  const sources={
    ...
    search:evidence(status(metrics.search_demand),'네이버 연결 광고 노출',metrics.search_demand==null?'연결 광고 노출 자료 누락 · 확인 필요':`연결 광고 전체 집계 · 시장 검색량 아님 · 키워드 ${keywordRows.length}개 중 최대 12개 표시`,generatedAt,sourceLinks.search),
  ...
  const signals=[];
  if(metrics.search_demand!=null)signals.push({tone:'good',title:'연결 광고 노출 확인',body:`선택 기간에 연결 광고 노출 ${metrics.search_demand.toLocaleString('ko-KR')}회가 확인됐습니다. 시장 검색량과는 다릅니다.`});
  ...
  return {
    kind:'PRODUCT_ANALYSIS',schema_version:'1.1',calculation_version:'PRODUCT_ANALYSIS_V2',generated_at:generatedAt,period_days:Number(periodDays),period_start:periodStart,period_end:periodEnd,
    product:{id:String(product.id||''),name:String(product.name||'상품 확인 필요'),sku:String(product.sku||'')},
    metrics,channels:channelRows,keywords:keywordRows.slice(0,12),customer,market,sources,signals,
