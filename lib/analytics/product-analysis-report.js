...
function buildProductAnalysisSummary({product={},performance={},keywords=[],customerEvidence={},marketEvidence={},periodDays=30,periodStart=null,periodEnd=null,generatedAt=new Date().toISOString()}={}){
  ...
  const marketBase=market.project_id?`/market-intelligence/${encodeURIComponent(market.project_id)}`:'/market-intelligence';
  ...
  const sources={
    ...
    profit:evidence(performance.cost_status==='CALCULATED'?'CALCULATED':'CHECK_REQUIRED','원가·공헌이익',performance.cost_status==='CALCULATED'?'원가 장부 계산 완료':'상품 원가 확인 필요',generatedAt,`/products/costs?master_product_id=${encodeURIComponent(String(product.id||''))}`),
    ...
    competition:evidence(competitionStatus,'경쟁 상품 가격',competitionStatus==='READY'?`검증 경쟁상품 ${market.verified_competitors}개 · 가격 표본 ${market.competitor_price_samples}개`:competitionStatus==='CHECK_REQUIRED'?`검증 경쟁상품 ${market.verified_competitors}개 · 가격 근거 확인 필요`:'상품개발의 검증 경쟁상품 근거 연결 필요',market.as_of,`${marketBase}/competition`),
    audience:evidence(audienceStatus,'고객 구성',customer.identified_customers>0?`Cafe24 구매고객 ${customer.identified_customers}명 · 재구매 ${customer.repeat_customers}명`:market.verified_personas>0?`검증 고객상 ${market.verified_personas}개 연결`:'구매 고객 집계 또는 검증 고객상 연결 필요',market.as_of||generatedAt,market.project_id?`${marketBase}/market`:marketBase),
    reviews:evidence(reviewsStatus,'검증 리뷰',reviewsStatus==='READY'?`검증 리뷰 ${market.verified_review_sets}묶음 · 표본 ${market.review_sample_size}건`:'상품개발의 검증 리뷰 Evidence 연결 필요',market.as_of,`${marketBase}/competition`)
  };
  const signals=[];
...
module.exports={buildCustomerPurchaseEvidence,buildProductAnalysisSummary,productAnalysisReportType};
