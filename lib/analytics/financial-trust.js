...
function evaluateFinancialTrust({
  ...
  } else if (coverage < MIN_COST_COVERAGE_RATE) {
    reasons.push({
      code: 'COST_COVERAGE_LOW',
      label: '상품 원가 보완 필요',
      message: `원가 반영률 ${coverage.toFixed(1)}%로 기준 ${MIN_COST_COVERAGE_RATE}%에 미달합니다.`,
  ...
  const costReady = coverage !== null && coverage >= MIN_COST_COVERAGE_RATE;
  const assignmentReady = !requireAdAssignment || (unassigned !== null && unassigned === 0);

  return {
    status: reasons.length ? 'BLOCKED' : 'READY',
    formula_version: FORMULA_VERSION,
    thresholds: { minimum_cost_coverage_rate: MIN_COST_COVERAGE_RATE },
