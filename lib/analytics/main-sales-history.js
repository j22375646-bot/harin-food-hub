...
function historyMonthKeys(asOf=new Date(),days=7){
...
function buildMainSalesHistory({
  cafe24Orders=[],naverOrders=[],coupangOrders=[],coupangRgOrders=[],asOf=new Date(),days=7,sourceComplete=true
}={}){
  const end=seoulDateKey(asOf);
  ...
  for(const order of cafe24Orders||[]){
    ...
    if(!cafe24OrderOrigin.isCafe24StorefrontOrder(order))continue;
    add({
  ...
  const status=sourceComplete===false||missingRevenueOrders>0?'BLOCKED':totalOrders>0&&totalRevenue>0?'READY':'NO_DATA';
  return Object.freeze({
    ...
    basis:sourceComplete===false?'최근 7일 수집 범위를 모두 확인한 뒤 표시합니다.':status==='READY'?'최근 7일 채널별 결제 주문 실제값':'최근 7일 결제 주문 금액 확인 필요'
  });
}

module.exports={buildMainSalesHistory,historyMonthKeys};
