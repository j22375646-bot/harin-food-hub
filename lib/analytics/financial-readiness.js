...
function buildFinancialReadiness({
  performance = {}, financialTrust = {}, productCosts = [], channelCostSettings = [],
  channelShippingRules = [], targetCoverageRate = TARGET_COST_COVERAGE_RATE
} = {}) {
  const items = performance.items || [];
  const summary = performance.summary || {};
  const totalRevenue = number(summary.revenue);
  ...
  const coveredRevenue = items
    .filter(item => configuredCosts.has(item.master_product_id) || item.cost_status === 'CALCULATED')
