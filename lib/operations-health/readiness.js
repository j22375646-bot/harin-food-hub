...
function workerService(heartbeats=[],now=new Date()){
  ...
  const status=!worker?'VERIFY_REQUIRED':worker.ready?'READY':'FAILED';
  const failedSummary=worker?.silence_minutes==null?'워커 신호 시각을 확인해야 합니다.':worker.stale?'워커 생존 신호가 오래되었습니다.':'워커가 작업 가능한 상태가 아닙니다. 오류·종료 상태를 확인해주세요.';
  return {...DEFINITIONS[0],status,summary:{READY:'고정 IP 워커가 정상 신호를 보내고 있어요.',FAILED:failedSummary,VERIFY_REQUIRED:'워커 생존 신호가 아직 없어요.'}[status],detail:worker?`${worker.silence_minutes==null?'신호 시각 확인 필요':`${worker.silence_minutes}분 전 신호`} · ${worker.status} · ${worker.current_job_type||'대기 중'}`:'worker_heartbeats 첫 신호를 기다리고 있어요.',lastAttemptAt:worker?.last_seen_at||null,lastSuccessAt:worker?.last_success_at||null,previousSuccess:false,errorMessage:worker?.last_error||null,checks:[{key:'heartbeat',label:'생존 신호',status},{key:'fixed-ip',label:'서울 고정 IP 작업',status},{key:'privacy',label:'고객정보 없는 상태 집계',status:'READY'}],capabilities:[{key:'queue',label:'수집·출고 작업 생존 상태',readStatus:status,writeStatus:'NOT_APPLICABLE'}],action:null};
}
