...
async function handleProbe(request,provider){
  ...
  try{const db=supabase.getSupabase();const result=await readiness.probeProvider(provider,{db});return Response.json({ok:true,result},{headers:{'Cache-Control':'no-store'}});}
