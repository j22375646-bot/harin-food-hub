...
async function handleProbe(request,provider){
  ...
  try{const db=supabase.getSupabase();const config=providerConfig.providerConfig(provider),missingFields=provider==='VERCEL'?[]:providerConfig.missingFields(provider,config);const result=await requestGuard.protectedRead({db,provider,requestInput:{probe:'health'},ttlMs:5*60*1000,killSwitchEnabled:config.enabled,missingFields,execute:()=>readiness.probeProvider(provider,{db})});return Response.json({ok:true,result},{headers:{'Cache-Control':'no-store'}});}
