...
function requestUrl({apiKey,startYymm,endYymm,hsCode,countryCode}){
  const url=new URL(BASE_URL);url.searchParams.set('serviceKey',apiKey);url.searchParams.set('strtYymm',startYymm);url.searchParams.set('endYymm',endYymm);url.searchParams.set('cntyCd',countryCode);if(hsCode)url.searchParams.set('hsSgn',hsCode);return url.toString();
}
