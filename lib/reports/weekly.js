...
const naverOwnerInsightModule = require('./naver-owner-insight.js');
const operatingRuleModule = require('../ai/operating-rules.js');
const NAVER_STANDARD_VERSION = 'HARIN-NAVER-ROAS-700-V1.0';
...
function comparison(current, previous) {
  return {
    ...
    naver_impressions: { current: current.naver.impressions, previous: previous.naver.impressions, change_rate: change(current.naver.impressions, previous.naver.impressions) },
    naver_clicks: { current: current.naver.clicks, previous: previous.naver.clicks, change_rate: change(current.naver.clicks, previous.naver.clicks) },
    naver_ctr: { current: current.naver.ctr, previous: previous.naver.ctr, change_rate: change(current.naver.ctr, previous.naver.ctr) },
    naver_average_order_value: { current: current.naver.average_order_value, previous: previous.naver.average_order_value, change_rate: change(current.naver.average_order_value, previous.naver.average_order_value) },
...
async function generateReport(options = {}) {
  ...
  const baseProfitability=engine.profitability({ cafe24:platform==='NAVER'?current.cafe24:(cafe24||{}), naver:naver||{} });
  const contribution=current.contribution||{};
  const business_context=platform==='NAVER'?{
    standardVersion:NAVER_STANDARD_VERSION,
    store:{
      source:'CAFE24_STORE_AGGREGATE',netRevenue:current.cafe24.net_revenue,orders:current.cafe24.orders,
      averageOrderValue:current.cafe24.average_order_value,conversionRate:current.cafe24.conversion_rate,
      customers:{
        status:current.cafe24.customers?.status||'NOT_COLLECTED',returningRate:current.cafe24.customers?.returningRate??null,
        repeatPurchaseCustomers:current.cafe24.customers?.repeatPurchaseCustomers??null,identifiedCustomers:current.cafe24.customers?.identifiedCustomers??null
      }
    },
    profitability:{
      contributionProfit:current.financialTrust?.status==='READY'?contribution.contribution_profit:null,
      contributionMarginRate:current.financialTrust?.status==='READY'?contribution.contribution_margin_rate:null,
      costCoverageRate:contribution.cost_coverage_rate??null
    },
    attribution:{settlementRevenue:null,settlementRoas:null,orderLinkStatus:'CHECK_REQUIRED'},
    inventory:{status:'CHECK_REQUIRED'}
  }:null;
  const summary = { generated_at: new Date().toISOString(), generation_mode: mode, period, previous_period: previousPeriod, diagnostic_standard:{version:NAVER_STANDARD_VERSION,calculation_owner:'SERVER',automatic_write:false}, operating_rule:{source:configuredThresholds.enabled?'SAVED':'DEFAULT_FALLBACK',versions:thresholds.versions,thresholds}, score: score(cafe24, naver, selectedKeywords, platform, current.coverage, coupang, thresholds), cafe24, naver, coupang, keywords: selectedKeywords, comparison: compare, comparison_guard: guard, platform_events: eventResult.data || [], data_coverage: current.coverage, financial_trust:current.financialTrust, business_context, anomalies, profitability: { ...baseProfitability, ...contribution, net_sales:baseProfitability.net_sales, paid_roas:baseProfitability.paid_roas, mer:baseProfitability.mer }, insights: diagnosticRows, recommendations: recommendationRows };
  ...
  if(platform==='NAVER'&&reportType==='WEEKLY')summary.owner_brief=naverOwnerInsightModule.buildNaverOwnerInsight(summary);
  ...
  if (options.notify !== false && alertResult.alerts.length) alert_delivery = await notificationService.deliverAlerts(alertResult.alerts,{db,triggerType:mode.includes('AUTO')||mode==='SCHEDULED'?'CRON':'MANUAL'}).catch(error=>({status:'FAILED',error:error.message}));
