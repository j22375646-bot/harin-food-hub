...
async function periodSummary(db, start, end) {
  const [ordersResult, itemsResult, trafficResult, refsResult, customerHistoryResult, statsResult, campaignResult, linksResult, costsResult, channelCostResult, coupangOrdersResult, coupangItemsResult, coupangSettlementsResult, coupangAdResult] = await Promise.all([
  ...
  const failed = [ordersResult, itemsResult, trafficResult, refsResult, customerHistoryResult, statsResult, campaignResult, linksResult, costsResult, channelCostResult, coupangOrdersResult, coupangItemsResult, coupangSettlementsResult, coupangAdResult].find(result => result.error);
...
async function syncAnomalyAlerts(db, anomalies, platform, reportId, resolveMissing) {
  ...
  for (const anomaly of anomalies) {
    ...
    const inserted = await db.from('alerts').insert({ source_type:'ANOMALY', source_id:reportId, platform:anomaly.platform, severity:anomaly.severity, title:anomaly.title, message:`${anomaly.message} ${anomaly.recommended_action}`, fingerprint:anomaly.fingerprint, status:'OPEN' });
...
async function generateReport(options = {}) {
  ...
  const alertResult = await syncAnomalyAlerts(db, anomalies, platform, inserted.data.id, guard.safe && coverageReady);
  return { created: true, report: inserted.data, actions_created: actionsCreated, alerts_created:alertResult.created, alerts_resolved:alertResult.resolved, period, summary };
}
