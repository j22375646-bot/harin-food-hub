...
async function loadPhase28DiagnosisSnapshot({db,now=new Date(),latestLimit=24,versionLimit=80}={}){
  ...
  const latestPromise=db.from('reports').select(LATEST_FIELDS).eq('is_latest',true).order('created_at',{ascending:false}).limit(safeLatestLimit);
