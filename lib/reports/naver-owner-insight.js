...
const FORMULA_VERSION='NAVER-OWNER-DECISION-V3';
...
function stateFor(value,{minimum=null,maximum=null,required=true}={}){
  if(value==null||(minimum==null&&maximum==null))return required?'CHECK_REQUIRED':'NEUTRAL';
...
function bottleneckSnapshot({naver,comparison,keywords,businessContext,financialReady,coverageReady,levers}){
  ...
  return [
    ...
    {id:'click',step:'02',label:'클릭',state:clicks==null||ctr==null?'CHECK_REQUIRED':cpcLever.state,value:ctr,unit:'PERCENT',changeRate:comparisonRate(comparison,'naver_ctr'),question:'보인 뒤 클릭으로 이어졌나요?',evidence:clicks==null?'클릭·CTR 확인 필요':`클릭 ${Math.round(clicks).toLocaleString('ko-KR')}회 · CTR ${rounded(ctr)}%`,next:'소재 매력과 CPC를 함께 확인'},
...
function buildNaverOwnerInsight(summary={}){
  ...
  const levers=leverSnapshot(naver,comparison,targetRoas).map(lever=>lever.target==null?{...lever,state:'CHECK_REQUIRED',diagnosis:'목표값 계산에 필요한 운영 기준·성과를 확인해야 합니다.'}:lever);
