...
function requestUrl({apiKey,pageNo=1,numOfRows=20,filters={}}){
  const url=new URL(BASE_URL);url.searchParams.set('serviceKey',normalizeServiceKey(apiKey));url.searchParams.set('pageNo',String(Math.max(1,Number(pageNo)||1)));url.searchParams.set('numOfRows',String(Math.min(100,Math.max(1,Number(numOfRows)||20))));url.searchParams.set('type','json');
