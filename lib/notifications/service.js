...
async function centerData(db = supabaseModule.getSupabase()) {
  const [settings, alerts, deliveries] = await Promise.all([
    getSettings(db),
    db.from('alerts').select('id,source_type,source_id,platform,severity,title,message,status,created_at,acknowledged_at,resolved_at').order('created_at',{ascending:false}).limit(100),
  ...
  return { settings, alerts:alerts.data || [], deliveries:deliveries.data || [], email_provider_configured:Boolean(process.env.RESEND_API_KEY && process.env.REPORT_FROM_EMAIL) };
}
