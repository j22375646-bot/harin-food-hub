...
async function readYear({config,year,fetchImpl=fetch}={}){
  ...
  const url=new URL(config.endpoint);url.searchParams.set('ServiceKey',config.apiKey);url.searchParams.set('solYear',String(numericYear));url.searchParams.set('numOfRows','100');url.searchParams.set('pageNo','1');
