...
const seoulPartsFormatter=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});
const seoulWeekdayFormatter=new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',weekday:'long'});
...
function seoulParts(value){
  ...
  const parts=Object.fromEntries(seoulPartsFormatter.formatToParts(date).filter(part=>part.type!=='literal').map(part=>[part.type,part.value]));
...
function calculateCutoffSchedule({asOf=new Date(),holidayDates=[],holidayReady=false,cutoffHour=15}={}){
  ...
  const weekday=seoulWeekdayFormatter.format(deadline);
  return {
    label:'오후 3시',
