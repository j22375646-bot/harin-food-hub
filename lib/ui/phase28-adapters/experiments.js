...
function testModel(test,productMap){
  ...
  const winnerId=evaluation.winner?.id||null;
  ...
  const status=String(evaluation.status||'NOT_EVALUATED');
  ...
  return Object.freeze({
    ...
    resultSummary:`${evaluation.summary||'평가 결과 확인 필요'}${test.evaluation_status&&test.evaluation_status!==evaluation.status?' · 이전 저장 판정과 다름: 현재 계산 기준 재평가':''}`,
    savedEvaluationStatus:test.evaluation_status||null,calculationVersion:evaluation.calculationVersion||null,
