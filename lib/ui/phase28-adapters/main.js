...
function buildPhase28MainModel(data={}){
  ...
  const taskCount=Number.isFinite(Number(daily.total))?Number(daily.total):0;
  const exceptionCount=Number.isFinite(Number(daily.exception_total))?Number(daily.exception_total):0;
  ...
  return Object.freeze({
    hero:Object.freeze({
      ...
      taskCount,
      exceptionCount,
      tone:exceptionCount>0?'watch':taskCount>0?'focus':'steady',
      headline:exceptionCount>0?'확인이 필요한 흐름이 있어요.':taskCount>0?'오늘 처리할 일을 정리했어요.':'오늘 회사는 순항 중이에요.',
      summary:taskCount>0?`운영 근거를 기준으로 처리할 일 ${taskCount}건을 우선순위대로 모았어요.`:'새로 처리할 업무가 없습니다. 채널 상태와 매출 흐름만 확인하세요.'
    }),
