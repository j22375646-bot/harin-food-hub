...
function buildPhase28MainModel(data={}){
  ...
  const cashflowValue=(key,fallback)=>Object.hasOwn(mainCashflow,key)?numeric(mainCashflow[key]):fallback;
  const cashflowSales=cashflowValue('sales',numeric(metrics.current)??numeric(profitability.revenue));
  const cashflowProfit=cashflowValue('profit',numeric(profitability.contribution_profit));
  ...
  return Object.freeze({
    ...
    metrics:Object.freeze({
      ...
      profit:moneyMetric({value:cashflowProfit,source:Object.hasOwn(mainCashflow,'profit')?'mainCashflow.profit':'liveProfitability.contribution_profit',metricKind:'calculated',status:cashflowProfit!=null?'READY':'BLOCKED',asOf,reasons:cashflowProfit!=null?[]:['FINANCIAL_TRUST_REQUIRED']}),
    ...
    cashflow:Object.freeze({
      ...
      rows:Object.freeze([
        Object.freeze({key:'sales',label:'결제 매출',value:cashflowSales,status:cashflowSales!=null?'READY':'CHECK_REQUIRED'}),
        Object.freeze({key:'operating',label:'원가·배송',value:cashflowValue('operatingCost',finite(profitability.product_cost)&&finite(profitability.shipping_cost)?profitability.product_cost+profitability.shipping_cost:null)}),
        Object.freeze({key:'fees',label:String(mainCashflow.feesAndAdsLabel||'수수료·광고'),value:cashflowValue('feesAndAds',finite(profitability.fees)&&finite(profitability.ad_spend)?profitability.fees+profitability.ad_spend:null),status:String(mainCashflow.feesAndAdsStatus||'CHECK_REQUIRED')}),
        Object.freeze({key:'profit',label:'실제 이익',value:cashflowValue('profit',numeric(profitability.contribution_profit))})
      ])
    }),
