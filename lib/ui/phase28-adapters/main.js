...
function buildCashCalendar(settlement={},asOf=null){
  ...
  const schedules=(settlement.schedules||[]).filter(item=>dateOnly(item.date)&&finite(item.amount)&&item.amount!==0);
  ...
  return Object.freeze({
    ...
    meta:Object.freeze({
      ...
      next:next?Object.freeze({platform:String(next.platform||'SHARED'),date:dateOnly(next.date),status:String(next.status||'확인 필요'),amount:next.amount}):null
    })
  });
}
