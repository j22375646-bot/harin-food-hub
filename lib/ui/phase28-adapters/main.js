...
function buildCashCalendar(settlement={},asOf=null){
  const start=seoulDateKey(asOf);
  if(!start)return Object.freeze({items:Object.freeze([]),meta:Object.freeze({status:'CHECK_REQUIRED',next:null,sourceCount:0})});
  const end=addDays(start,7);
  const grouped=new Map();
  for(const item of settlement.schedules||[]){
    const date=dateOnly(item.date);
    const amount=numeric(item.amount);
    if(!date||amount==null||amount===0)continue;
    const platform=String(item.platform||'SHARED');
    const type=String(item.type||'');
    const key=`${platform}:${date}:${type}`;
    const current=grouped.get(key)||{platform,date,type,status:String(item.status||'확인 필요'),amount:0};
    current.amount+=amount;
    grouped.set(key,current);
  }
  const schedules=[...grouped.values()].filter(item=>item.amount!==0).map(item=>({
    ...item,direction:item.amount<0?'OUTFLOW':'INFLOW'
  }));
  const items=Object.freeze(schedules
    ...
    .map(item=>Object.freeze({
      ...
      direction:String(item.direction||'INFLOW')
    })));
  const next=schedules
    .filter(item=>dateOnly(item.date)>end)
    .sort((left,right)=>dateOnly(left.date).localeCompare(dateOnly(right.date)))[0]||null;
  return Object.freeze({
    items,
    meta:Object.freeze({
      status:items.length?'READY':Array.isArray(settlement.schedules)&&settlement.schedules.length?'NO_SCHEDULE':'CHECK_REQUIRED',
      sourceCount:Array.isArray(settlement.schedules)?settlement.schedules.length:0,
      next:next?Object.freeze({platform:String(next.platform||'SHARED'),date:dateOnly(next.date),status:String(next.status||'확인 필요'),amount:next.amount,direction:String(next.direction||'INFLOW')}):null
    })
  });
}
...
function moneyMetric({value,source,metricKind='actual',status='READY',period='MONTH',asOf=null,sampleSize=null,reasons=[]}){
...
function buildPhase28MainModel(data={}){
  ...
  const mainCashflow=data.mainCashflow||{};
  ...
  const cashCalendar=buildCashCalendar(data.unifiedSettlement||{},asOf);
  ...
  return Object.freeze({
    ...
    metrics:Object.freeze({
      ...
      current:moneyMetric({value:finite(metrics.current)?metrics.current:null,source:'salesCommandCenter.metrics.current',metricKind:'actual',period:center.month||'MONTH',asOf,sampleSize:metrics.currentSampleSize}),
      forecast:moneyMetric({value:finite(metrics.forecast)?metrics.forecast:null,source:'salesCommandCenter.metrics.forecast',metricKind:'estimate',status:finite(metrics.forecast)?'PARTIAL':'BLOCKED',period:center.month||'MONTH',asOf,sampleSize:metrics.currentSampleSize,reasons:finite(metrics.forecast)?['PACE_ESTIMATE']:[metrics.forecastStatus==='SAMPLE_REQUIRED'?'SAMPLE_REQUIRED':'VALUE_MISSING']}),
      profit:moneyMetric({value:finite(mainCashflow.profit)?mainCashflow.profit:finite(profitability.contribution_profit)?profitability.contribution_profit:null,source:finite(mainCashflow.profit)?'mainCashflow.profit':'liveProfitability.contribution_profit',metricKind:'calculated',status:finite(mainCashflow.profit)||finite(profitability.contribution_profit)?'READY':'BLOCKED',asOf,reasons:finite(mainCashflow.profit)||finite(profitability.contribution_profit)?[]:['FINANCIAL_TRUST_REQUIRED']}),
    ...
    cashflow:Object.freeze({
      status:String(mainCashflow.status||cashflow.status||'CHECK_REQUIRED'),
      description:String(mainCashflow.description||cashflow.description||'재무 근거를 확인한 뒤 예상 잔액을 계산합니다.'),
      rows:Object.freeze([
        Object.freeze({key:'sales',label:'결제 매출',value:numeric(mainCashflow.sales)??numeric(metrics.current)??numeric(profitability.revenue),status:numeric(mainCashflow.sales)!=null||numeric(metrics.current)!=null||numeric(profitability.revenue)!=null?'READY':'CHECK_REQUIRED'}),
        Object.freeze({key:'operating',label:'원가·배송',value:numeric(mainCashflow.operatingCost)??(finite(profitability.product_cost)&&finite(profitability.shipping_cost)?profitability.product_cost+profitability.shipping_cost:null)}),
        Object.freeze({key:'fees',label:String(mainCashflow.feesAndAdsLabel||'수수료·광고'),value:numeric(mainCashflow.feesAndAds)??(finite(profitability.fees)&&finite(profitability.ad_spend)?profitability.fees+profitability.ad_spend:null),status:String(mainCashflow.feesAndAdsStatus||'CHECK_REQUIRED')}),
        Object.freeze({key:'profit',label:'실제 이익',value:numeric(mainCashflow.profit)??numeric(profitability.contribution_profit)})
      ])
    }),
    forecast:buildSevenDayForecast(data.salesHistory?.daily?.length?data.salesHistory:(data.cafe24Analytics||{}),asOf),
    cashCalendar:cashCalendar.items,
    cashCalendarMeta:cashCalendar.meta,
