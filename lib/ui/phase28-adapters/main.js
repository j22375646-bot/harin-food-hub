...
function buildSevenDayForecast(analytics={},asOf=null){
  ...
  if(!recent.length||!start||analytics.status==='BLOCKED'||analytics.status==='NO_DATA'||recentOrders<=0||recentRevenue<=0)return Object.freeze({status:'BLOCKED',days:Object.freeze([]),expectedOrders:null,expectedRevenue:null,basis:analytics.basis||'최근 7일 판매 근거 확인 필요'});
