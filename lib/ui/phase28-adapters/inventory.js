...
function inventoryMarkerLabel(value){
  ...
  if(productWord)return productWord[1];
  const cleaned=normalized
    .replace(/<[^>]*>/g,' ')
    .replace(/(?:하린식품|haccp|해썹인증|해썹|친환경무농약인증|무농약인증|국내산|국산|공식몰|단독|최신본|전통|수제|건강식품)/gi,' ')
    .replace(/20\d{2}\s*년?/g,' ')
    .replace(/[^0-9A-Za-z가-힣]+/g,' ')
    .replace(/\s+/g,' ')
    .trim();
  const meaningful=cleaned.split(' ').find(part=>/[A-Za-z가-힣]/u.test(part));
  return Array.from(meaningful||'재')[0]||'재';
}
...
function buildPhase28InventoryModel(data={}){
  ...
  const rows=frozenRows(source.map(item=>{
    ...
    const name=text(item.productItem?.item_name||item.external_sku_id)||'상품 정보 확인 필요';
    return {
      ...
      name,
      markerLabel:inventoryMarkerLabel(name),
...
module.exports={buildPhase28InventoryModel,inventoryMarkerLabel};
