...
function mappedSource(source={}){
  return Object.freeze({
    ...
    questionText:text(source.question_text),
    conversation:frozenRows((Array.isArray(source.conversation)?source.conversation:[]).map(entry=>({
      answerId:text(entry.answerId)||null,parentAnswerId:text(entry.parentAnswerId)||null,
      answerType:text(entry.answerType),partnerTransferStatus:text(entry.partnerTransferStatus),
      needAnswer:entry.needAnswer===true,replyAt:entry.replyAt||null,content:text(entry.content)
    }))),
    replyRequired:typeof source.reply_required==='boolean'?source.reply_required:null,
    canReply:source.can_reply===true,
    statusVerified:source.status_verified===true,
    verificationError:text(source.verification_error)||null,
...
function mappedRow(item){
  ...
  return {
    ...
    content:text(item.source?.question_text)||text(item.content)||'문의 원문 확인 필요',
