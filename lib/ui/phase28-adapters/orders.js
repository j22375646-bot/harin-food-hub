...
function cutoffState(asOf){
  const date=new Date(asOf||'');
  if(Number.isNaN(date.getTime()))return Object.freeze({label:'오후 3시',remainingMinutes:null,status:'CHECK_REQUIRED'});
  const parts=Object.fromEntries(new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).formatToParts(date).filter(part=>part.type!=='literal').map(part=>[part.type,part.value]));
  const elapsed=number(parts.hour)*60+number(parts.minute);
  return Object.freeze({label:'오후 3시',remainingMinutes:Math.max(0,15*60-elapsed),status:'READY'});
}
