...
function orderGifts(order,events=[]){
  if(!order?.orderedAt)return Object.freeze([]);
  const date=calendarCenter.seoulDateKey(order.orderedAt);
  return frozenRows(events.map(event=>calendarCenter.resolveEventGift(event,{orderAmount:order.amount,date})).filter(Boolean));
}
