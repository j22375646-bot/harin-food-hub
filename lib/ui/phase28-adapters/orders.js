...
function giftAutomationView(events=[]){
  ...
  const revision=events.map(event=>event.updatedAt||null).filter(Boolean).sort().at(-1)||null;
...
function buildPhase28OrdersModel(data={}){
  ...
  const compact=compactOrders(sellerOrders,events);
  ...
  return Object.freeze({
    ...
    hero:Object.freeze({
      ...
      cancellationCount,
      giftOrderCount:compact.filter(item=>item.giftRequired).length,
