...
const AUTOMATION_SLOTS=Object.freeze([
  Object.freeze({id:'daily',label:'일간 자동진단',schedule:'매일 07:10',matches:['DAILY_PLATFORM_REPORTS']}),
  Object.freeze({id:'weekly',label:'주간 자동진단',schedule:'매주 월요일 07:30',matches:['WEEKLY_PLATFORM_REPORTS']}),
  Object.freeze({id:'monthly',label:'월간 자동진단',schedule:'매월 1일·5일 08:00',prefix:'MONTHLY_PLATFORM_REPORTS_'})
]);
...
function savedInsightReports(reports,platform){
  return (reports||[])
    .filter(report=>text(report.platform).toUpperCase()===platform&&['WEEKLY','MONTHLY'].includes(text(report.report_type).toUpperCase())&&report.summary_json)
    .sort((left,right)=>reportTime(right)-reportTime(left));
}
...
function buildPhase28InsightsModel(data={},options={}){
  ...
  return Object.freeze({
    ...
    schedule:Object.freeze({label:'주간 월요일 07:30 · 월간 1일·5일 08:00',weeklyLabel:'매주 월요일 07:30',monthlyLabel:'매월 1일·5일 08:00',timezone:'Asia/Seoul',mode:'채널별 주간·월간 자동 생성·서버 저장'}),
