...
function buildChannel(channel,reports,dataHealth){
  ...
  const ownerBrief=current?(current.summary_json.owner_brief?.snapshotVersion?current.summary_json.owner_brief:naverOwnerInsightModule.buildNaverOwnerInsight(current.summary_json)):null;
...
function normalizeInsightReportDetail(report={}){
  ...
  const ownerBrief=summary.owner_brief?.snapshotVersion?summary.owner_brief:naverOwnerInsightModule.buildNaverOwnerInsight(summary);
  const caveats=[];
