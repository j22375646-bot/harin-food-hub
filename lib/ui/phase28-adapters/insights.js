...
const naverOwnerInsightModule=require('../../reports/naver-owner-insight.js');

const CHANNELS=Object.freeze([
  Object.freeze({platform:'NAVER',id:'naver',name:'네이버',source:'검색광고 API · 캠페인 · 키워드'})
]);
...
const AUTOMATION_SLOTS=Object.freeze([
  Object.freeze({id:'weekly',label:'네이버 주간 자동진단',schedule:'매주 월요일 07:30',matches:['WEEKLY_PLATFORM_REPORTS']})
]);

function ownerBriefFor(summary={}){
  const stored=summary.owner_brief;
  if(stored?.snapshotVersion===naverOwnerInsightModule.SNAPSHOT_VERSION&&stored?.evidence?.formulaVersion===naverOwnerInsightModule.FORMULA_VERSION)return stored;
  const current=naverOwnerInsightModule.buildNaverOwnerInsight(summary);
  if(stored)current.caveats.push('이전 저장 보고서의 원천값으로 현재 판정식을 적용한 조회입니다. 저장된 보고서는 변경하지 않았습니다.');
  return current;
}

function automationRunTime(run){return new Date(run?.finished_at||run?.started_at||0).getTime()||0;}
function automationRunState(run){
  if(!run)return 'CHECK_REQUIRED';
  const status=text(run.status).toUpperCase();
  if(['SUCCESS','COMPLETED'].includes(status))return 'READY';
  if(['FAILED','DEAD_LETTER'].includes(status))return 'FAILED';
  if(['RUNNING','PENDING','RETRYING'].includes(status))return 'RUNNING';
  return 'CHECK_REQUIRED';
}
function buildAutomation(runs=[]){
  const rows=Array.isArray(runs)?runs:[];
  const items=AUTOMATION_SLOTS.map(slot=>{
    const matches=rows.filter(run=>slot.prefix?text(run.job_name).toUpperCase().startsWith(slot.prefix):slot.matches.includes(text(run.job_name).toUpperCase())).sort((a,b)=>automationRunTime(b)-automationRunTime(a));
    const latest=matches[0]||null;
    return Object.freeze({
      id:slot.id,label:slot.label,schedule:slot.schedule,state:automationRunState(latest),
      lastRunId:latest?.id||null,lastRunAt:latest?.finished_at||latest?.started_at||null,
      error:text(latest?.error_message)||null
    });
  });
  return Object.freeze({items:frozenRows(items),ready:items.filter(item=>item.state==='READY').length,total:items.length});
}
...
function savedInsightReports(reports,platform){
  return (reports||[])
    .filter(report=>text(report.platform).toUpperCase()===platform&&text(report.report_type).toUpperCase()==='WEEKLY'&&report.summary_json)
    .sort((left,right)=>reportTime(right)-reportTime(left));
}
...
function reportHeader(report,channel,change=null){
  const reportType=text(report.report_type).toUpperCase();
  return Object.freeze({
    ...
    reportType,reportTypeLabel:reportType==='MONTHLY'?'월간':'주간'
  });
}
...
function buildChannel(channel,reports,dataHealth){
  ...
  const ownerBrief=current?ownerBriefFor(current.summary_json):null;
  ...
  return Object.freeze({
    ...
    signals:frozenRows(signals),ownerBrief:ownerBrief?Object.freeze(ownerBrief):null
  });
}
...
function normalizeInsightReportDetail(report={}){
  ...
  const ownerBrief=ownerBriefFor(summary);
  const caveats=[];
  ...
  return Object.freeze({
    ...
    ownerBrief:Object.freeze(ownerBrief),
...
function buildPhase28InsightsModel(data={},options={}){
  ...
  const savedReports=Object.fromEntries(CHANNELS.map(channel=>{
    const rows=savedInsightReports(reports,channel.platform);
    const headers=rows.map((report,index)=>reportHeader(
      report,
      channel,
      changeRate(channelRevenue(report,channel),channelRevenue(rows.slice(index+1).find(candidate=>candidate.report_type===report.report_type),channel))
    ));
  ...
  return Object.freeze({
    ...
    initialChannel:'naver',
    hero:Object.freeze({count:insightCount,summary:'네이버 광고 성과를 사장님 관점의 결정·위험·캠페인·키워드·7일 확인 순서로 봅니다.'}),
    ...
    automation:buildAutomation(data.automationRuns||[]),
    schedule:Object.freeze({label:'주간 · 매주 월요일 07:30',weeklyLabel:'매주 월요일 07:30',timezone:'Asia/Seoul',mode:'네이버 주간 자동 생성·서버 저장'}),
    policy:Object.freeze({channelSeparated:true,missingAsZero:false,automaticWrites:false,detailLoading:'ON_DEMAND'})
  });
}

module.exports={CHANNELS,AUTOMATION_SLOTS,buildAutomation,buildPhase28InsightsModel,normalizeInsightReportDetail};
