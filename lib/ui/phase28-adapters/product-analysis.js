...
function normalizeSavedReport(report={}){
  ...
  return Object.freeze({
    ...
    calculationVersion:summary.calculation_version||'LEGACY',
    calculationAssessment:Object.freeze({status:summary.calculation_version==='PRODUCT_ANALYSIS_V2'?'READY':'CHECK_REQUIRED',message:summary.calculation_version==='PRODUCT_ANALYSIS_V2'?'전체 연결 광고 노출 집계 · 시장 검색량 아님':'이전 계산 보고서: 상위 12개 제한·누락값 0 처리 영향 확인 필요. 저장값은 유지되며 새 분석으로 비교하세요.'}),
...
function buildProduct(row={},master={},performance={},asOf=null){
  ...
  const metrics=Object.freeze({
    ...
    searchDemand:naver.ad_evidence_status==='READY'?numberOrNull(naver.impressions):null,searchClicks:naver.ad_evidence_status==='READY'?numberOrNull(naver.clicks):null,adSpend:numberOrNull(performance.ad_spend)
  });
...
function buildPhase28ProductAnalysisModel(data={}){
  ...
  return Object.freeze({
    ...
    hero:Object.freeze({productCount:products.length,savedCount:history.length,summary:'연결 광고 노출, 고객층, 경쟁 가격과 실제 판매 실적을 같은 분석 시점으로 묶어 봅니다.'})
  });
}
