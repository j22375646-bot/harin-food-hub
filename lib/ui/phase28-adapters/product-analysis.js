...
function source(status,label,detail,asOf=null){
  return Object.freeze({status,label,detail,asOf});
}
...
function normalizeSavedReport(report={}){
  ...
  return Object.freeze({
    ...
    sources:Object.freeze(summary.sources||{}),channels:Object.freeze(summary.channels||{}),
    keywords:frozenRows(summary.keywords||[]),signals:frozenRows(summary.signals||[]),
    report:Object.freeze(summary)
  });
}
...
function buildProduct(row={},master={},performance={},asOf=null){
  ...
  return Object.freeze({
    id:text(row.master_product_id||performance.master_product_id||master.id),
    ...
    sources:Object.freeze({
      sales:source(ready(metrics.revenue),'판매 실적','채널 주문·상품 매칭 기준',asOf),
      profit:source(performance.cost_status==='CALCULATED'?'CALCULATED':'CHECK_REQUIRED','원가·공헌이익',performance.cost_status==='CALCULATED'?'원가 장부 계산 완료':'상품 원가 확인 필요',asOf),
      search:source(ready(metrics.searchDemand),'네이버 검색광고',metrics.searchDemand==null?'상품 연결 검색 근거 없음':'노출·클릭 실제 집계',asOf),
      competition:source('SETUP_REQUIRED','경쟁 상품','네이버 쇼핑 검색 표본 연결 필요'),
      audience:source('SETUP_REQUIRED','고객 구성','상품별 채널 분석 자료 연결 필요'),
      reviews:source('SETUP_REQUIRED','검증 리뷰','리뷰 Evidence 연결 필요')
    })
  });
}
...
function buildPhase28ProductAnalysisModel(data={}){
  ...
  const operationRows=data.productOperations?.items||[];
  const seen=new Set();
  const products=[];
  ...
  return Object.freeze({
    writePolicy:'READ_ONLY',generatedAt:data.generatedAt||null,defaultPeriod:30,periodOptions:Object.freeze([30,90,365]),
    products:frozenRows(products),history,activeReport:history[0]||null,
