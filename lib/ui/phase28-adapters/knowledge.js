...
function operatingRuleModel(item={}){
  ...
  return Object.freeze({
    ...
    metrics:Object.freeze({
      targetRoasPercent:Number(config.target_roas_percent),
      conversionWarningPercent:Number(config.conversion_rate_warning_percent),
      changeWarningPercent:Number(config.change_warning_percent),
      minimumCostCoveragePercent:Number(config.minimum_cost_coverage_percent),
      freshnessHours:Number(config.freshness_hours),
...
function buildPhase28KnowledgeModel(snapshot={}){
  ...
  const operatingRuleItems=['insight','auto_diagnosis'].map(key=>ruleCurrent[key]).filter(Boolean).map(operatingRuleModel);
