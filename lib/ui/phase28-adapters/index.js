...
const {buildPhase28CalendarModel}=require('./calendar.js');
const {buildPhase28OrdersModel}=require('./orders.js');
const {buildPhase28CsModel}=require('./cs.js');
const {buildPhase28InventoryModel,inventoryMarkerLabel}=require('./inventory.js');
...
const {buildPhase28ChangesModel}=require('./changes.js');
const {buildPhase28ValidationModel}=require('./validation.js');
const {buildPhase28ExperimentsModel}=require('./experiments.js');
const {buildPhase28KnowledgeModel}=require('./knowledge.js');

const PHASE28_AVAILABLE_ADAPTERS=Object.freeze(['main','calendar','orders','cs','inventory','products','settlement','keywords','product-analysis','insights','development','system','notifications','diagnoses','changes','validation','experiments','knowledge']);

module.exports={
  PHASE28_AVAILABLE_ADAPTERS,
  buildPhase28MainModel,
  buildPhase28CalendarModel,
  ...
  buildPhase28ChangesModel,
  buildPhase28ValidationModel,
  buildPhase28ExperimentsModel,
  buildPhase28KnowledgeModel
};
