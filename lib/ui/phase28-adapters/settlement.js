...
function stateFor(channel){
  ...
  if(channel.ledger_status==='FAMILY_REQUIRED')return Object.freeze({code:'FAMILY_REQUIRED',label:'배송유형 원장 확인',tone:'warning'});
  if(channel.payout_complete===false&&numberOrNull(channel.actual_payout)!=null)return Object.freeze({code:'PARTIAL',label:channel.pending_payout!=null?'일부 지급 · 예정액 확인':'지급 일부 확인 · 기간 대조',tone:'warning'});
...
function compactChannel(channel={},schedules=[]){
  ...
  return Object.freeze({
    ...
    basis:[text(channel.basis)||'자료 확인 필요',channel.sales_api_complete===false?'매출통계 일부 날짜 · 주문 추정 포함':null,channel.advertising_complete===false?'광고비 일부 날짜 · 부분합계':null].filter(Boolean).join(' · '),
    payoutBasis:text(channel.payout_basis)||null,
    afterOperatingCosts:numberOrNull(channel.after_operating_costs),
    pendingPayout:numberOrNull(channel.pending_payout),
    ...
    evidence:Object.freeze({
      ...
      payout:channel.payout_evidence||null,
      matchedItems:numberOrNull(channel.settlement_item_count),
      totalItems:numberOrNull(channel.settlement_item_total),
      coverage
    })
  });
}
...
function compactPeriod(center={},days=30){
  ...
  const baseWaterfall=[
    {id:'gross',label:waterfall.gross_sales_complete===false?'확인된 매출 · 부분합계':'총매출',value:numberOrNull(waterfall.gross_sales)??numberOrNull(waterfall.known_gross_sales),tone:'blue',partial:waterfall.gross_sales_complete===false},
    ...
    {id:'expected',label:waterfall.payout_basis==='CHANNEL_SPECIFIC'?'채널별 예상 지급':'예상 정산',value:numberOrNull(waterfall.expected_payout),tone:'expected'},
  ...
  let chainConnected=hasNumber(baseWaterfall[0].value)&&!baseWaterfall[0].partial;
  const connectedWaterfall=baseWaterfall.map((item,index)=>{
    if(index===0)return {...item,chainConnected};
    chainConnected=chainConnected&&hasNumber(item.value)&&(item.id!=='actual'||actualComplete)&&!(item.id==='expected'&&waterfall.payout_basis==='CHANNEL_SPECIFIC');
  ...
  return Object.freeze({
    ...
    payoutBasis:text(waterfall.payout_basis)||null,
    afterOperatingCosts:numberOrNull(waterfall.after_operating_costs),
