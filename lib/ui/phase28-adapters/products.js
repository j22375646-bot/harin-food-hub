...
const EMPTY_MAPPING=Object.freeze({summary:Object.freeze({}),masterProducts:EMPTY_ROWS,candidates:EMPTY_ROWS,links:EMPTY_ROWS});
const EMPTY_COST_WORKBENCH=Object.freeze({rows:EMPTY_ROWS,readyCount:0,pendingCount:0});
const KOREAN_PRODUCT_COLLATOR=new Intl.Collator('ko-KR',{numeric:true,sensitivity:'base'});
const compareProductNames=(left,right)=>KOREAN_PRODUCT_COLLATOR.compare(text(left?.name),text(right?.name))||text(left?.id).localeCompare(text(right?.id));

function safeMapping(data={}){
  const mapping=data.productMapping||{};
  const masterSource=Array.isArray(mapping.masterProducts)?mapping.masterProducts:(data.masterProducts||[]);
  const masterProducts=frozenRows(masterSource.filter(item=>item?.id&&item.is_active===true).map(item=>({
    id:text(item.id),name:text(item.name)||'기준 상품 확인 필요',price:numberOrNull(item.selling_price)
  })).sort(compareProductNames));
  const sellingMasterIds=new Set(masterProducts.map(item=>item.id));
  const candidates=frozenRows((mapping.candidates||[]).filter(item=>item?.is_active===true&&['NAVER','COUPANG'].includes(text(item.platform).toUpperCase())).map(item=>({
    ...
    noLink:item.no_link===true,
    autoEligible:Boolean(item.auto_eligible),
    suggestions:frozenRows((item.candidates||[]).filter(candidate=>sellingMasterIds.has(text(candidate.master_product_id))).map(candidate=>({
  ...
  const links=frozenRows((mapping.links||[]).filter(item=>item?.is_active===true&&sellingMasterIds.has(text(item.master_product_id))&&['NAVER','COUPANG'].includes(text(item.platform).toUpperCase())).map(item=>({
    platform:text(item.platform).toUpperCase(),
    externalProductId:text(item.external_product_id),
...
function buildPhase28ProductsModel(data={}){
  const workspace=text(data.loadedWorkspace||'catalog');
  ...
  const costRows=workspace==='costs'?frozenRows(rows.map(row=>({
    id:row.id,name:row.name,sku:row.sku,basePrice:row.basePrice,
    unitCost:row.cost.unit,packagingCost:row.cost.packaging,otherUnitCost:row.cost.other,
    total:row.cost.total,ready:row.cost.status==='READY'
  }))):EMPTY_ROWS;
  return Object.freeze({
    kind:'products',
    workspace,
    ...
    rows:workspace==='catalog'?rows:EMPTY_ROWS,
    costWorkbench:workspace==='costs'?Object.freeze({rows:costRows,readyCount:costRows.filter(row=>row.ready).length,pendingCount:costRows.filter(row=>!row.ready).length}):EMPTY_COST_WORKBENCH,
    mapping:workspace==='mappings'?safeMapping(data):EMPTY_MAPPING,
