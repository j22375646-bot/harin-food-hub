...
function buildPhase28ProductsModel(data={}){
  ...
  const costRows=frozenRows(rows.map(row=>({
  ...
  return Object.freeze({
    ...
    rows,
    costWorkbench:Object.freeze({rows:costRows,readyCount:costRows.filter(row=>row.ready).length,pendingCount:costRows.filter(row=>!row.ready).length}),
