...
function compactRow(row={}){
  ...
  const canDraft=channel==='NAVER'&&row.canDraft===true&&Boolean(row.snapshotToken)&&numberOrNull(row.currentBid)!=null;
  const campaign=text(row.campaignName||row.campaign)||'캠페인 확인 필요';
  return Object.freeze({
    id:text(row.id)||`${channel}:${text(row.keyword)||'unknown'}`,
    channel,
    source:text(row.source)||'REGISTERED',
    keyword:text(row.keyword)||'키워드 확인 필요',
    campaignId:text(row.campaignId)||`NAME:${campaign}`,
    campaign,
    ...
    roas7d:numberOrNull(row.roas7d??row.roas),
    roasToday:numberOrNull(row.roasToday),
    decision:decision.code,
    tone:decision.tone,
    statusLabel:decision.label,
    canDraft,
    snapshotToken:canDraft?row.snapshotToken:null,
...
function clientRow(row={},reasonId){
  return {
    id:row.id,channel:row.channel,keyword:row.keyword,campaignId:row.campaignId,campaign:row.campaign,
    currentBid:row.currentBid,recommendedBid:row.recommendedBid,minimumBid:row.minimumBid,maximumBid:row.maximumBid,
    clicks:row.clicks,cost:row.cost,orders:row.orders,roas:row.roas,roas7d:row.roas7d,roasToday:row.roasToday,
    decision:row.decision,tone:row.tone,statusLabel:row.statusLabel,
    canDraft:row.canDraft,snapshotToken:row.snapshotToken,applicationMode:row.applicationMode,
    reasonIds:Object.freeze(row.reasons.map(reasonId))
  };
}
...
function buildPhase28KeywordsModel(data={},options={}){
  ...
  const operationalRows=frozenRows(normalized.map(compactRow));
  const cost=sumKnown(operationalRows,'cost');
  const clicks=sumKnown(operationalRows,'clicks');
  const orders=sumKnown(operationalRows,'orders');
  const revenue=sumKnown(operationalRows,'revenue');
  const noOrderSpend=operationalRows.filter(row=>row.orders===0&&row.cost!=null).reduce((sum,row)=>sum+row.cost,0);
  const actions={
    lower:operationalRows.filter(row=>row.decision==='LOWER').length,
    raise:operationalRows.filter(row=>row.decision==='RAISE').length,
    hold:operationalRows.filter(row=>['KEEP','HISTORY'].includes(row.decision)).length,
    blocked:operationalRows.filter(row=>row.decision==='BLOCKED').length
  };
  const reasonCatalog=[];
  const reasonIndex=new Map();
  const reasonId=reason=>{
    if(!reasonIndex.has(reason)){
      reasonIndex.set(reason,reasonCatalog.length);
      reasonCatalog.push(reason);
    }
    return reasonIndex.get(reason);
  };
  const rows=frozenRows(operationalRows.map(row=>clientRow(row,reasonId)));
  const campaignMap=new Map();
  for(const row of operationalRows){
    const current=campaignMap.get(row.campaignId)||{id:row.campaignId,name:row.campaign,count:0};
    current.count+=1;
    campaignMap.set(row.campaignId,current);
  }
  const campaigns=frozenRows([...campaignMap.values()].sort((left,right)=>left.name.localeCompare(right.name,'ko-KR')||left.id.localeCompare(right.id)));
  const keywordView=keywordOperations.normalizeKeywordView(platform,{});
  const sortOptions=frozenRows(keywordOperations.KEYWORD_SORT_OPTIONS.filter(option=>platform==='naver'||!option.naverOnly).map(option=>({value:option.value,label:option.label,field:option.field,direction:option.direction})));
  ...
  return Object.freeze({
    ...
    rows,campaigns,reasonCatalog:Object.freeze(reasonCatalog),
    view:Object.freeze({sort:keywordView.sort,sortOptions}),
    summary:Object.freeze({total:operationalRows.length,cost,clicks,orders,revenue,noOrderSpend,actions})
  });
}
