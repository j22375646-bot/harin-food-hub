...
const GROUPS=Object.freeze([
  {id:'operations',label:'오늘의 운영',items:['home','calendar','orders','cs','inventory','products','settlement','keywords']},
  {id:'growth',label:'성장과 관리',items:['product-analysis','analysis','development']},
  {id:'system',label:'시스템',items:['system','notifications']},
  {id:'records',label:'기록과 검증',items:['changes','validation','experiments','knowledge']}
]);

const COPY=Object.freeze({
  home:['오늘','가장 먼저 볼 판단'],
  calendar:['캘린더','일정·메모 관리'],
  ...
  const groups=GROUPS.map(group=>Object.freeze({
    ...
    items:Object.freeze(group.items.map(id=>{
      ...
      return Object.freeze({
        ...
        prefetch:route.loadPolicy==='ON_DEMAND'?false:null
      });
    }))
  }));
  return Object.freeze({
...
function buildPhase28Vitality(badges){
  ...
  const requiredKeys=['orders','cs','inventory','notifications'];
  const values=requiredKeys.map(key=>badges[key]);
  if(values.some(value=>!Number.isFinite(value)||value<0))return Object.freeze({known:false,attention:null,score:null,label:'확인 필요'});
  const attention=values.reduce((total,value)=>total+value,0);
