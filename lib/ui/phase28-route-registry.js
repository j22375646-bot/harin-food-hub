...
const PHASE28_ROUTES=Object.freeze([
  ...
  {id:'calendar',href:'/calendar',legacyView:'calendar',workspace:null,adapterId:'calendar',writePolicy:'GUARDED',preserveWorkspaces:false},
  ...
  {id:'product-analysis',href:'/product-analysis',legacyView:'product-analysis',workspace:null,adapterId:'product-analysis',writePolicy:'GUARDED',preserveWorkspaces:false},
  ...
  {id:'diagnoses',href:'/diagnoses',legacyView:'reports',workspace:null,adapterId:'diagnoses',writePolicy:'READ_ONLY',preserveWorkspaces:false,loadPolicy:'ON_DEMAND'},
  {id:'changes',href:'/approvals',legacyView:'changes',workspace:null,adapterId:'changes',writePolicy:'GUARDED',preserveWorkspaces:false,loadPolicy:'ON_DEMAND'},
  {id:'validation',href:'/execution-validation',legacyView:'validation',workspace:null,adapterId:'validation',writePolicy:'READ_ONLY',preserveWorkspaces:false,loadPolicy:'ON_DEMAND'},
  {id:'experiments',href:'/ab-tests',legacyView:'experiments',workspace:null,adapterId:'experiments',writePolicy:'GUARDED',preserveWorkspaces:false,loadPolicy:'ON_DEMAND'},
  {id:'knowledge',href:'/ai-knowledge',legacyView:'knowledge',workspace:null,adapterId:'knowledge',writePolicy:'GUARDED',preserveWorkspaces:false,loadPolicy:'ON_DEMAND'}
]);
...
function validatePhase28Registry(routes=PHASE28_ROUTES){
  ...
  for(const route of routes){
    ...
    if(route?.loadPolicy!=null&&!['ON_DEMAND'].includes(route.loadPolicy))issues.push({code:'INVALID_LOAD_POLICY',id:route?.id||null});
  }
  return issues;
}
