...
function detectAnomalies({ comparison = {}, coverage = {}, platform = 'ALL', comparisonSafe = true } = {}) {
  ...
  return RULES.filter(rule => scope === 'ALL' || rule.platform === scope).flatMap(rule => {
    ...
    if (magnitude < rule.warning) return [];
    ...
    const severity = magnitude >= rule.critical ? 'ERROR' : 'WARNING';
...
module.exports = { RULES, detectAnomalies, toInsight, toRecommendation };
