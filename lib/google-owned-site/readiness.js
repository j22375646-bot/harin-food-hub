...
const LEGACY_SNAPSHOT_FIELDS='id,provider,site_url,status,metric_summary,quota_summary,source_timestamp,fetched_at,error_code,error_message,metadata';
function legacySnapshotsQuery(db){
  return db.from('owned_site_api_snapshots').select(LEGACY_SNAPSHOT_FIELDS)
    .or('metadata->>kind.is.null,metadata->>kind.neq.GA4_ECOMMERCE_V1')
...
function publicError(error){
  ...
  if(error?.code==='PAGESPEED_PUBLIC_QUOTA_EXHAUSTED'||/pagespeed.*quota|공용 호출 한도/i.test(message))return 'Google 공용 호출 한도가 소진되었습니다. PageSpeed API 키를 추가한 뒤 다시 확인해주세요.';
...
function buildOwnedSiteReadiness({snapshots=[],env=process.env,now=new Date()}={}){
  const services=DEFINITIONS.map(def=>{
    ...
    const rows=snapshots.filter(row=>row.provider===def.provider&&row.metadata?.kind!=='GA4_ECOMMERCE_V1');const attempt=latest(rows);const success=latest(rows.filter(row=>row.status==='SUCCESS'));
    ...
    return {...def,status,credentialReady:missing.length===0,killSwitchEnabled:config.enabled,missingFields:missing,
      optionalNotice:def.provider==='PAGESPEED'&&!config.apiKey?'지금은 키 없이 수동 확인합니다. 잦은 자동 수집에는 PageSpeed API 키를 권장해요.':null,
...
module.exports={ DEFINITIONS, buildOwnedSiteReadiness, legacySnapshotsQuery, probeProvider, publicError, metricSummary };
