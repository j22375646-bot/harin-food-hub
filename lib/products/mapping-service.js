...
function createProductSources({ channelProducts = [], coupangProducts = [], coupangProductItems = [] }) {
  ...
  const naver = channelProducts.filter(item => item.platform === 'NAVER' && String(item.raw_data?.source_type || '').toUpperCase() === 'NAVER_COMMERCE_PRODUCT').map(item => ({
    ...
    is_active:item.is_active !== false, source_type:'NAVER_COMMERCE_PRODUCT',
...
function buildMappingDashboard({ masterProducts = [], channelProducts = [], coupangProducts = [], coupangProductItems = [], history = [] }) {
  const sources = createProductSources({ channelProducts, coupangProducts, coupangProductItems });
  ...
  return {
    ...
    masterProducts:masterProducts.filter(item => item && item.id && item.is_active !== false),
    candidates,
...
async function loadMappingDashboard({ db }) {
  const [masters, links, cafe24Products, coupangProducts, coupangItems, history] = await Promise.all([
    ...
    db.from('channel_products').select('id,master_product_id,platform,external_product_id,external_product_name,selling_price,is_active,match_method,match_confidence,matched_at,matched_by,raw_data').order('updated_at',{ascending:false}).limit(1000),
...
function planBulkManualMappingOperations({ dashboard, platform, assignments = [] }) {
  ...
  const sourceMap = new Map((dashboard.candidates || [])
    .filter(item => item.platform === requestedPlatform)
  ...
  const operations = rows.map(item => {
    ...
    if (!source) throw new Error('선택한 플랫폼의 현재 실상품만 연결할 수 있습니다.');
...
function planBulkMappingOperations({ dashboard, action, platform, externalProductIds = [] }) {
  ...
  const rows = sourceRows.filter(item => item.platform === requestedPlatform && requestedIds.has(String(item.external_product_id)));
  const operations = [];
