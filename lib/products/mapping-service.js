...
async function loadMappingDashboard({ db }) {
  const [masters, links, cafe24Products, coupangProducts, coupangItems, naverGroups, naverCampaigns, history] = await Promise.all([
  ...
  const firstError = [masters,links,cafe24Products,coupangProducts,coupangItems,naverGroups,naverCampaigns,history].find(result => result.error)?.error;
