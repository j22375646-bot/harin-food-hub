...
function createProductSources({ channelProducts = [], coupangProducts = [], coupangProductItems = [] }) {
  ...
  for (const item of coupangProductItems) {
    if (!item.seller_product_id || !(number(item.sale_price) > 0) || (item.status && !isCoupangSellingStatus(item.status))) continue;
  ...
  const coupang = coupangProducts.map(item => {
    ...
    return {
      ...
      is_active:isCoupangSellingStatus(item.status),
  ...
  const naver = channelProducts.filter(item => item.platform === 'NAVER' && String(item.raw_data?.source_type || '').toUpperCase() === 'NAVER_COMMERCE_PRODUCT').map(item => ({
    ...
    is_active:item.is_active === true, source_type:'NAVER_COMMERCE_PRODUCT',
...
function buildMappingDashboard({ masterProducts = [], channelProducts = [], coupangProducts = [], coupangProductItems = [], history = [] }) {
  ...
  return {
    summary:{ ...dashboardSummary(eligibleMasterProducts, channelProducts, candidates, eligibleSources), inactive_sources:sources.length-eligibleSources.length },
