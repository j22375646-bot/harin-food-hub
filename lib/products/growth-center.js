...
function buildGrowthCenter({ masterProducts = [], profiles = [], offers = [], checklists = [], productCosts = [], channelSettings = [], shippingRules = [] }) {
  ...
  const items = masterProducts.map(product => {
    ...
    const aPilot = isPilotProduct(a.master_product.name);
    const bPilot = isPilotProduct(b.master_product.name);
    return Number(bPilot) - Number(aPilot) || a.master_product.name.localeCompare(b.master_product.name, 'ko');
  });
  return {
    checklist_items:CHECKLIST_ITEMS.map(([key,label]) => ({ key, label })),
    pilot_product_id:items.find(item => isPilotProduct(item.master_product.name))?.master_product.id || items[0]?.master_product.id || null,
