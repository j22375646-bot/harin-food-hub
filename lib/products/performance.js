...
function hasNaverCoverage(evidence,start,end){
 return evidence?.status==='SUCCESS'&&evidence.complete===true&&evidence.source==='NAVER_COMMERCE'
   ...
   &&Number.isFinite(Date.parse(evidence.collected_at||''))&&dateKey(evidence.collected_at)>=end;
}
...
async function loadUnifiedProductPerformance({ db, periodStart, periodEnd, ...input }) {
  const [keywords, stats, coupangAds,naverCommerce,coverageLogs] = await Promise.all([
    ...
    input.naverCollectionEvidence?Promise.resolve({data:[]}):db.from('sync_logs').select('status,finished_at,metadata').eq('platform','NAVER').eq('job_type','COMMERCE_SYNC').order('finished_at',{ascending:false}).limit(30)
  ]);
  const firstError = [keywords,stats,coupangAds].find(result=>result.error)?.error;
  if (firstError) throw firstError;
  const naverCollectionEvidence=input.naverCollectionEvidence||(!coverageLogs.error?(coverageLogs.data||[]).filter(row=>row.status==='SUCCESS').map(row=>row.metadata?.order_coverage).find(row=>hasNaverCoverage(row,periodStart,periodEnd)):null);
