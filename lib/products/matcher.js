...
function specCompatibility(left, right) {
  ...
  const common = a.filter(item => b.includes(item)).length;
  const coverage = common / Math.min(a.length, b.length);
  const aCounts = a.filter(item => /(?:개|입|포|병)$/.test(item));
  const bCounts = b.filter(item => /(?:개|입|포|병)$/.test(item));
...
function scoreProductMatch(master, source) {
  const masterText = compact(master.name), sourceText = compact(source.name);
  const masterName = nameCompact(master.name), sourceName = nameCompact(source.name);
  ...
  const contained = Math.min(masterName.length, sourceName.length) >= 3 && (masterName.includes(sourceName) || sourceName.includes(masterName)) ? 1 : 0;
  const tokens = tokenOverlap(master.name, source.name);
  const specs = specCompatibility(master.name, source.name);
  ...
  const masterCounts = extractSpecs(master.name).filter(item => /(?:개|입|포|병)$/.test(item));
  const sourceCounts = extractSpecs(source.name).filter(item => /(?:개|입|포|병)$/.test(item));
  if (masterCounts.length && sourceCounts.length && !masterCounts.some(item => sourceCounts.includes(item))) score *= 0.62;
