...
function summarizeCostProgress(products=[],rows={}){
  const ready=products.filter(product=>costStatus(rows[product.id]).ready).length;
  const total=products.length;
  return {
    total,
    ready,
    pending:Math.max(0,total-ready),
    rate:total?Math.round(ready/total*100):0
  };
}

module.exports={PAGE_SIZE,COST_FIELDS,hasCostValue,costStatus,filterCostProducts,paginateCostProducts,summarizeCostProgress};
