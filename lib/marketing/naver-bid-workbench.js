...
function candidateFor({ keyword, stats, productTarget, linkedMasterProductId, adgroup, campaign, financialTrust = {}, period = {}, executionEnabled = false }) {
...
function buildNaverBidWorkbench({ keywords = [], stats = [], adgroups = [], campaigns = [], productTargets = [], keywordProductLinks = [], masterProducts = [], financialTrust = {}, period = {}, executionEnabled = false } = {}) {
  ...
  const candidates = keywords.map(keyword => { const linkedMasterProductId=productByKeyword.get(String(keyword.ncc_keyword_id)); const adgroup=adgroupsById.get(String(keyword.ncc_adgroup_id || '')); const campaign=campaignsById.get(String(adgroup?.ncc_campaign_id || '')); return candidateFor({
    ...
    period,
    executionEnabled
  }); }).sort((left, right) => right.metrics.cost - left.metrics.cost || right.metrics.clicks - left.metrics.clicks).slice(0, 500);
