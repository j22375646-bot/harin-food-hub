...
const FORMULA_VERSION = 'n1-product-target-v2';
...
function calculateProductTarget({ item, target, financialTrust, periodEnd, asOf }) {
  ...
  const conversions = finite(naver.attributed_orders);
  ...
  const cvr = clicks > 0 && conversions != null ? conversions / clicks : null;
  const allowableCpc = cvr == null ? null : allowableCpa * cvr;
  const sample = sampleDecision({ cost:adSpend, conversions, allowableCpa });
  const status = allowableCpc == null || conversions<=0 || sample.code === 'NO_AD_SAMPLE' ? 'OBSERVE' : 'READY';
