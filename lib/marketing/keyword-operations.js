...
function groupedMetric(rows=[],idKey,nameKey){
  const grouped=new Map();
  for(const item of rows){
    const id=text(item[idKey])||'UNMAPPED';
    const current=grouped.get(id)||{id,name:text(item[nameKey])||'확인 필요',keywordCount:0,clicks:0,cost:0,orders:0,revenue:0,adgroupIds:new Set()};
...
function buildNaverAdgroupWorkspace(rows=[],{campaignId='ALL',adgroupId='ALL'}={}){
  ...
  const campaigns=groupedMetric(naverRows,'campaignId','campaignName');
  const campaignRows=selectedCampaign==='ALL'?naverRows:naverRows.filter(item=>(text(item.campaignId)||'UNMAPPED')===selectedCampaign);
  const adgroups=groupedMetric(campaignRows,'adgroupId','adgroupName').map(({adgroupCount,...item})=>item);
...
function keywordOwnerWorkspace({platform='naver',workspace='registered',naverBidWorkbench={},searchTermCenter={},coupang={},financialChanges=[]}={}){
  ...
  const platformWorkspace=workspace==='search-terms'?'registered':workspace;
  const platforms=[
...
module.exports={KEYWORD_PAGE_SIZES,normalizeKeywordRows,filterKeywordRows,paginateKeywordRows,keywordOperationSummary,keywordOwnerWorkspace,buildNaverAdgroupWorkspace,naverRegisteredRows,naverSearchTermRows,coupangKeywordRows,historyRows};
