...
function naverRegisteredRows(workbench={}){
  return (workbench.candidates||[]).map(candidate=>{
    ...
    roas7d:number(candidate.metrics?.roas_7d??candidate.metrics?.roas),
    roasToday:number(candidate.metrics?.roas_today),
    actualProfit:null,
...
function naverSearchTermRows(center={}){
  return (center.items||[]).map(item=>({
    ...
    revenue:numeric(item.conversion_revenue),roas:number(item.roas),roas7d:null,roasToday:null,actualProfit:null,
...
function coupangKeywordMetricKey(item={}){
  return [item.campaign_id,item.advertised_option_id,item.keyword].map(text).join(':');
}

function coupangKeywordRoasWindows(coupang={}){
  const windows=new Map();
  const today=text(coupang.keywordToday);
  const add=(bucket,item)=>{
    const cost=number(item.ad_spend),revenue=number(item.revenue_1d);
    if(cost!==null){bucket.cost+=cost;bucket.costKnown=true;}
    if(revenue!==null){bucket.revenue+=revenue;bucket.revenueKnown=true;}
  };
  for(const item of Array.isArray(coupang.adKeywordDaily)?coupang.adKeywordDaily:[]){
    const key=coupangKeywordMetricKey(item);
    if(!windows.has(key))windows.set(key,{week:{cost:0,revenue:0,costKnown:false,revenueKnown:false},today:{cost:0,revenue:0,costKnown:false,revenueKnown:false}});
    const window=windows.get(key);
    add(window.week,item);
    if(today&&text(item.date)===today)add(window.today,item);
  }
  const roas=bucket=>bucket.costKnown&&bucket.revenueKnown&&bucket.cost>0?Math.round(bucket.revenue/bucket.cost*1000)/10:null;
  return new Map([...windows].map(([key,window])=>[key,{roas7d:roas(window.week),roasToday:roas(window.today)}]));
}

function coupangKeywordRows(coupang={}){
  const keyed=new Map();
  const roasWindows=coupangKeywordRoasWindows(coupang);
  for(const item of [...(coupang.adKeywordTop||[]),...(coupang.adKeywordWaste||[])]){
    ...
    const window=roasWindows.get(key)||{roas7d:null,roasToday:null};
    keyed.set(key,{
      id:`COUPANG:${key||keyed.size}`,
      platform:'COUPANG',source:'REGISTERED',keyword:text(item.keyword)||'키워드 없음',
      campaignId:text(item.campaign_id),campaignName:text(item.campaign_name)||'캠페인 확인 필요',
      ...
      roas:window.roas7d??number(item.roas??item.roas_14d)??(cost>0?revenue/cost*100:null),roas7d:window.roas7d,roasToday:window.roasToday,actualProfit:null,
...
function historyRows(changes=[]){
  return changes.filter(item=>upper(item.change_type)==='NAVER_BID'&&upper(item.platform)==='NAVER').map((item,index)=>({
    ...
    minimumBid:null,maximumBid:null,impressions:0,clicks:0,cost:0,orders:0,revenue:0,roas:null,roas7d:null,roasToday:null,actualProfit:null,
...
function keywordFinderScore(item,query){
  const needle=upper(query);
  const keyword=upper(item.keyword);
  const campaign=upper(item.campaignName||item.campaign);
  const adgroup=upper(item.adgroupName);
  const product=upper(item.product);
  let score=Number.POSITIVE_INFINITY;
  if(keyword===needle)score=0;
  else if(keyword.startsWith(needle))score=10;
  else if(keyword.includes(needle))score=20;
  else if(campaign.includes(needle))score=30;
  else if(adgroup.includes(needle))score=40;
  else if(product.includes(needle))score=50;
  if(!Number.isFinite(score))return score;
  if(item.adCategoryState==='INACTIVE')score+=5;
  return score;
}

function findGlobalKeywordRows(rows=[],{query='',platform='naver',limit=8}={}){
  const needle=text(query);
  if(!needle)return [];
  const scopedPlatform=upper(platform)==='COUPANG'?'COUPANG':'NAVER';
  const resultLimit=Math.max(1,Math.min(20,Number(limit)||8));
  return rows
    .filter(item=>upper(item.platform)===scopedPlatform)
    .map(item=>({item,score:keywordFinderScore(item,needle)}))
    .filter(result=>Number.isFinite(result.score))
    .sort((a,b)=>a.score-b.score||numeric(b.item.cost)-numeric(a.item.cost)||text(a.item.keyword).localeCompare(text(b.item.keyword),'ko')||text(a.item.id).localeCompare(text(b.item.id),'ko'))
    .slice(0,resultLimit)
    .map(result=>result.item);
}

function globalKeywordJump(result,{platform='naver'}={}){
  if(!result)return null;
  const scopedPlatform=upper(platform)==='COUPANG'?'COUPANG':'NAVER';
  if(upper(result.platform)!==scopedPlatform)return null;
  return {
    id:text(result.id),
    query:text(result.keyword),
    campaignId:text(result.campaignId)||'ALL',
    adgroupId:text(result.adgroupId)||'ALL',
    page:1,
    quickFilter:'ALL',
...
function isEditableNaverKeyword(item){
  return upper(item?.platform)==='NAVER'
    && upper(item?.source)==='REGISTERED'
    && item?.adCategoryState!=='INACTIVE'
    && item?.canDraft===true
    && number(item?.currentBid)!=null;
}

function selectKeywordRowsByCondition(rows=[],{condition='CHANGEABLE'}={}){
  const normalized=upper(condition);
  if(!['CHANGEABLE','RECOMMENDED','NO_ORDER_COST','LOW_ROAS'].includes(normalized))return [];
  return rows.filter(item=>{
    if(!isEditableNaverKeyword(item))return false;
    if(normalized==='RECOMMENDED')return number(item.recommendedBid)!=null;
    if(normalized==='NO_ORDER_COST')return numeric(item.cost)>0&&numeric(item.orders)<=0;
    if(normalized==='LOW_ROAS')return number(item.roas)!=null&&number(item.roas)<700;
    return true;
  });
}

function planKeywordBulkDrafts(rows=[],currentDrafts={},options={}){
  const mode=upper(options.mode);
  const supported=['PERCENT','AMOUNT','TARGET','RECOMMENDED'];
  const requestedValue=mode==='RECOMMENDED'?null:number(options.value);
  const drafts={},appliedIds=[],skippedIds=[],clampedIds=[];
  if(!supported.includes(mode)||(mode!=='RECOMMENDED'&&requestedValue==null))return {drafts,appliedIds,skippedIds,clampedIds};

  for(const item of rows){
    const id=text(item?.id);
    if(!id||!isEditableNaverKeyword(item)){if(id)skippedIds.push(id);continue;}
    const currentBid=number(item.currentBid);
    const base=number(currentDrafts?.[id])??currentBid;
    let requested=mode==='RECOMMENDED'?number(item.recommendedBid)
      :mode==='PERCENT'?base*(1+requestedValue/100)
      :mode==='AMOUNT'?base+requestedValue
      :requestedValue;
    if(requested==null||!Number.isFinite(requested)){skippedIds.push(id);continue;}
    requested=Math.round(requested/10)*10;
    const minimum=number(item.minimumBid)??70;
    const maximum=number(item.maximumBid)??100000;
    const isIncrease=requested>currentBid;
    if(isIncrease&&(item.manualDecreaseOnly===true||(['PERCENT','AMOUNT'].includes(mode)&&requested>maximum))){skippedIds.push(id);continue;}
    const planned=Math.min(maximum,Math.max(minimum,requested));
    if(planned!==requested)clampedIds.push(id);
    drafts[id]=planned;
    appliedIds.push(id);
  }
  return {drafts,appliedIds,skippedIds,clampedIds};
}
