...
function buildCafe24Channel({ orders, salesDaily, settings, startMs, endMs, unavailable, lastSyncAt, scopeRequired, approvalRequired }) {
  ...
  if (!amounts.length) return {
    platform:'CAFE24', label:'Cafe24', status:approvalRequired?'APPROVAL_REQUIRED':scopeRequired?'SCOPE_REQUIRED':'NO_DATA', basis:approvalRequired?'주문 자료 없음 · 매출통계 개발자 승인 필요':scopeRequired?'주문 자료 없음 · 매출통계 권한 필요':'주문 자료 없음',
    gross_sales:null, refunds:null, fees:null, logistics:null, expected_payout:null, actual_payout:null,
    order_count:periodOrders.length, last_updated_at:lastSyncAt || latestDate(periodOrders,['order_date']),
    action:approvalRequired?cafe24FinanceCapability.APPROVAL_ACTION:scopeRequired?'Cafe24 매출통계 읽기 권한을 다시 연결하세요.':'최근 30일 결제 주문이 없다면 정상입니다. 주문이 있다면 Cafe24 수집 상태를 확인하세요.',
    action_href:approvalRequired?cafe24FinanceCapability.DOCS_URL:scopeRequired?'/oauth/cafe24/start':null
  };
  ...
  return {
    platform:'CAFE24', label:'Cafe24', status:approvalRequired ? 'APPROVAL_REQUIRED' : !hasSalesApi&&scopeRequired ? 'SCOPE_REQUIRED' : hasCostSetting ? 'ESTIMATED' : 'COST_REQUIRED',
    basis:hasSalesApi
      ? approvalRequired ? '저장된 Cafe24 매출통계 · 새 수집 개발자 승인 필요' : hasCostSetting ? 'Cafe24 매출통계 API · 예상 정산' : 'Cafe24 매출통계 API · 비용 설정 필요'
      : approvalRequired ? '주문 기반 추정 · 매출통계 개발자 승인 필요' : scopeRequired ? '주문 기반 추정 · 매출통계 권한 필요' : hasCostSetting ? '주문·설정 기반 예상' : '비용 설정 필요',
    ...
    action_href:approvalRequired ? cafe24FinanceCapability.DOCS_URL : !hasSalesApi&&scopeRequired ? '/oauth/cafe24/start' : null
  };
}
...
function buildCoupangChannel({
  ...
  return {
    ...
    action:strictCostCoverage && !completeSettlementCoverage
      ? '로켓그로스 주문과 연결되지 않은 정산 범위가 있어 확정 지급액을 보류했습니다. 쿠팡 수집을 다시 실행하세요.'
      : expectedPayout == null
...
function buildUnifiedSettlementCenter({
  ...
  const cafe24ApprovalRequired=liveCafe24Capability
    ?liveCafe24Capability.status==='APPROVAL_REQUIRED'
    :legacySettlementCapability==='APPROVAL_REQUIRED';
  const cafe24ScopeRequired=!cafe24ApprovalRequired&&legacySettlementCapability==='SETUP_REQUIRED';
  ...
  const channels = [
    buildCafe24Channel({ orders:cafe24Orders, salesDaily:cafe24SalesDaily, settings:channelCostSettings, startMs, endMs, unavailable:Boolean(unavailable.CAFE24), lastSyncAt:lastSync('CAFE24'), scopeRequired:cafe24ScopeRequired, approvalRequired:cafe24ApprovalRequired }),
