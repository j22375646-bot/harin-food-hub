...
function buildCafe24Channel({ orders, salesDaily, settings, startMs, endMs, unavailable, lastSyncAt, reconnectRequired, scopeRequired, approvalRequired }) {
  ...
  const periodOrders = (orders || []).filter(order => isCafe24StorefrontOrder(order)&&inPeriod(order.order_date, startMs, endMs));
  ...
  const fallbackOrders=periodOrders.filter(row=>!apiDates.has(dateOnly(row.order_date)));
  ...
  const refunds = hasSalesApi
    ? apiRefunds.every(value=>value!=null) ? apiRefunds.reduce((sum,value)=>sum+Math.max(0,value),0)+fallbackOrders.reduce((sum,row)=>sum+Math.max(0,number(row.cancel_amount)),0) : null
  ...
  return {
    ...
    sales_api_complete:apiDates.size>=Math.ceil((endMs-startMs)/DAY_MS),
...
function buildUnifiedSettlementCenter({
  ...
  const cafe24ApprovalRequired=!cafe24ReconnectRequired&&(
    ...
    ||Number(latestCafe24SalesError?.status)===403
    ||latestCafe24SalesError?.code==='APPROVAL_REQUIRED'
  );
  ...
  const actualRevenueChannels=revenueChannels.filter(item=>item.actual_payout!=null);
