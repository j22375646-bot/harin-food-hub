...
function buildUnifiedSettlementCenter({
  ...
  const actualPayout=actualRevenueChannels.length
    ...
    :null;
  const actualPayoutComplete=Boolean(revenueChannels.length)&&actualRevenueChannels.length===revenueChannels.length;
  const actualPayoutCoverage=revenueChannels.length
    ?Math.round(actualRevenueChannels.length/revenueChannels.length*1000)/10
    :null;
  ...
  return {
    ...
    waterfall:{
      ...
      revenue_channel_count:revenueChannels.length,
      variance,
