...
function buildCoupangChannel({
  ...
  const actualPayout = strictCostCoverage && !completeSettlementCoverage ? null : summaryPayout ?? rowPayout;
...
function buildUnifiedSettlementCenter({
  ...
  const rocketGrowthCosts=(coupangCostTransactions||[]).filter(isRocketGrowthRow);
  const sellerCosts=(coupangCostTransactions||[]).filter(row=>!isRocketGrowthRow(row));
  ...
  const componentChannels = key => {
    ...
    if (key === 'advertising') return revenueChannels.filter(item=>['COUPANG','COUPANG_RG'].includes(item.platform));
