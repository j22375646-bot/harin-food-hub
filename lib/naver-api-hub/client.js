...
async function readBoundedJson(response, maxResponseBytes) {
  const tooLarge = () => Object.assign(new Error('NAVER API HUB response too large'), {code:'NAVER_API_HUB_RESPONSE_TOO_LARGE'});
  ...
  try {
    while (true) {
      ...
      if (size > maxResponseBytes) { void reader.cancel().catch(() => {}); throw tooLarge(); }
...
async function request(pathname, { body, config = getConfig(), signal, maxResponseBytes, fetchImpl = globalThis.fetch } = {}) {
  if (maxResponseBytes !== undefined && (!Number.isSafeInteger(maxResponseBytes) || maxResponseBytes < 1)) throw new TypeError('Invalid response limit');
  ...
  // Existing callers keep their response behavior; public AI opts into a stream cap.
  const payload = maxResponseBytes === undefined ? await response.json().catch(() => null) : await readBoundedJson(response,maxResponseBytes);
  if (!response.ok) {
...
async function fetchSearchTrend({ startDate, endDate, timeUnit, keywordGroups, config = getConfig(), signal, maxResponseBytes, fetchImpl }) {
  return request(SEARCH_TREND_PATH, { config, signal, maxResponseBytes, fetchImpl, body:{ startDate, endDate, timeUnit, keywordGroups } });
}
