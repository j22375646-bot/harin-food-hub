...
function evaluate(test, variants) {
  ...
  if (test.metric === 'CTR') {
    ...
    confidence = null;
    confidenceMethod = 'UNSUPPORTED';
  }
  const requiredConfidence = number(test.confidence_level) || 90;
  const detectable = liftPercent >= number(test.minimum_detectable_lift);
  const winner = confidence != null && sufficient && detectable && bestScore > controlScore && confidence >= requiredConfidence ? best : null;
  ...
  const summary = !sufficient
    ...
    : winner
      ...
      : confidence == null ? `판정 보류 · ${test.metric} 통계 검정 미지원 · 표본 준비는 통계 신뢰도가 아닙니다.`
      : `판정 보류 · 개선폭 ${liftPercent.toFixed(1)}% · 신뢰도 ${confidence.toFixed(1)}%`;
  return { calculationVersion: 'EXPERIMENT_DECISION_V2', status, confidence, requiredConfidence, confidenceMethod, liftPercent, winner, control, best, variants: enriched, summary, controlValue, bestValue, samples: { control: controlSample, challenger: bestSample, minimum, sufficient } };
}
