...
function optionalUuid(value,label){
  ...
  if(!id)return null;
  if(!UUID_RE.test(id))throw new Error(`${label}을 다시 선택해주세요.`);
  return id;
}
...
async function evaluateTest(id, { automatic = false } = {}) {
  ...
  for (const variant of result.variants) {
    check(await db.from('ab_test_variants').update({
      ...
      source_snapshot: { source_type: test.source_type, entity_id: variant.entity_id, collected_at: new Date().toISOString(), calculation_version: result.calculationVersion, confidence_method: result.confidenceMethod, required_confidence: result.requiredConfidence }
  ...
  return { test_id: id, status: result.status, winner_variant_id: result.winner?.id || null, summary: result.summary, confidence: result.confidence, confidence_method: result.confidenceMethod, required_confidence: result.requiredConfidence, calculation_version: result.calculationVersion, samples: result.samples, lift_percent: result.liftPercent, completed: shouldComplete };
}
...
async function createTest(input) {
  ...
  const masterProductId=optionalUuid(input.master_product_id,'상품');
  const marketProjectId=optionalUuid(input.market_project_id,'상품개발 프로젝트');
  if (!String(input.name || '').trim()) throw new Error('실험 이름을 입력하세요.');
  if (variants.length < 2) throw new Error('대조군과 실험군을 각각 입력하세요.');
  if(marketProjectId&&!masterProductId)throw new Error('상품개발 프로젝트에는 기준상품이 필요합니다.');
  if(masterProductId){
    const product=await db.from('master_products').select('id,is_active').eq('id',masterProductId).eq('is_active',true).maybeSingle();
    if(product.error)throw product.error;
    if(!product.data)throw new Error('판매 중인 기준상품을 찾을 수 없습니다.');
  }
  if(marketProjectId){
    const project=await db.from('market_projects').select('id,master_product_id').eq('id',marketProjectId).maybeSingle();
    if(project.error)throw project.error;
    if(!project.data||String(project.data.master_product_id)!==masterProductId)throw new Error('선택 상품과 상품개발 프로젝트가 일치하지 않습니다.');
  }
  const test = check(await db.from('ab_tests').insert({
    ...
    master_product_id:masterProductId,market_project_id:marketProjectId,
...
async function listLab({masterProductId=null}={}) {
  const db = supabaseModule.getSupabase();
  const productId=optionalUuid(masterProductId,'상품');
  let testsQuery=db.from('ab_tests').select('*,ab_test_variants(*)').order('created_at',{ascending:false}).limit(100);
  if(productId)testsQuery=testsQuery.eq('master_product_id',productId);
  const [testsResult,benchmarksResult,productsResult]=await Promise.all([
    testsQuery,
    db.from('performance_benchmarks').select('*').eq('is_active',true).order('created_at',{ascending:false}).limit(100),
    db.from('master_products').select('id,name,selling_price').eq('is_active',true).order('name')
  ]);
  const tests=check(testsResult,'실험 목록');
  const benchmarks=check(benchmarksResult,'벤치마크 목록');
  const products=check(productsResult,'판매 중 상품 목록');
  const productMap=new Map(products.map(item=>[String(item.id),item]));
  const scopedTests=tests.map(item=>({...item,product:item.master_product_id?productMap.get(String(item.master_product_id))||null:null}));
  const comparisons = [];
  ...
  return {tests:scopedTests,benchmarks,comparisons,products,selected_product:productId?productMap.get(productId)||null:null};
}
