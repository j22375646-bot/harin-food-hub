...
const CLASSIFICATION_LABELS=Object.freeze({
  BRAND:'브랜드',GENERAL_PURCHASE:'일반 구매',PRODUCT_DETAIL:'상품 형태·용량',
...
function classifySearchTerm(value,options={}){
  ...
  if(containsAny(term,['무료게임','실시간tv','대출','코인시세','주식추천','구인구직','채용공고','중고차매매','연예인']))return {classification:'IRRELEVANT',confidence:.96,reason:'상품 구매와 관계없는 명확한 검색 의도입니다.'};
  if(containsAny(term,['효능','효과','부작용','먹는법','마시는법','끓이는법','볶이는법','만드는법','성분','칼로리','차이','뜻','이유','원산지']))return {classification:'INFORMATION',confidence:.9,reason:'효능·사용법·성분을 알아보는 검색입니다.'};
  if(containsAny(term,['비염','기관지','기침','목관리','목아픔','붓기','혈당','당뇨','쓰레기','감기','염증','다이어트','물대신','임산부','고혈압','루틴','탈모']))return {classification:'PROBLEM_SITUATION',confidence:.86,reason:'고객의 문제 또는 생활 상황이 드러난 검색입니다.'};
  const hasProductDetail=containsAny(term,['티백','스틱','분말','원물','깍지','껍질','대용량'])
    ||term.endsWith('환')
    ||term.endsWith('청')
    ||/\btb\b/i.test(display)
    ||/\d+\s*(개|포|봉|팩|g|kg|ml|입|박스)/i.test(display);
  if(hasProductDetail)return {classification:'PRODUCT_DETAIL',confidence:.88,reason:'상품 형태·용량·수량이 포함됐습니다.'};
  if(containsAny(term,['구매','추천','가격','최저가','파는곳','할인','주문','후기','인기','좋은','어디서']))return {classification:'GENERAL_PURCHASE',confidence:.88,reason:'구매·비교 의도가 드러난 검색입니다.'};
  return {classification:'GENERAL_PURCHASE',confidence:.55,reason:'상품명 중심의 일반 탐색으로 분류됐습니다. 필요하면 직접 고쳐주세요.'};
}
...
function recommendAction(row,options={}){
  ...
  if(!conversions&&targetCpa>0&&cost>=targetCpa*2)return {action:'NEGATIVE_REVIEW',reason:`전환 없이 목표 CPA의 2배 이상(${Math.round(cost).toLocaleString('ko-KR')}원) 사용됐습니다.`};
  ...
  return {action:'OBSERVE',reason:'현재 성과를 더 지켜보는 편이 안전합니다.'};
}
...
function buildSearchTermCenter({rows=[],registeredKeywords=[],period=null,lastError=null,collectionTotal=null}={}){
  ...
  const totals=items.reduce((sum,item)=>({
    impressions:sum.impressions+number(item.impressions),
    clicks:sum.clicks+number(item.clicks),
    cost:sum.cost+number(item.cost),
    conversions:sum.conversions+number(item.conversions),
    revenue:sum.revenue+number(item.conversion_revenue)
  ...
  return {
    ...
    summary:{
      total:Math.max(items.length,number(collectionTotal)),
      displayed:items.length,
      unregistered:items.filter(item=>!item.is_registered_exact).length,
      negative_candidates:items.filter(item=>item.recommended_action==='NEGATIVE_REVIEW').length,
      content_candidates:items.filter(item=>item.recommended_action==='CONTENT_FAQ').length,
      ...totals
    },
