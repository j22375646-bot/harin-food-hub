...
const FORMULA_VERSION='phase24-6-bid-performance-v1';
...
function pointMetrics(point={}){
  return {
    ...
    average_rank:metric(point,'avgRnk','averageRank','rank')
  };
}
...
function weightedRank(points){
  const ranked=points.filter(item=>number(item?.average_rank)!==null);
...
function buildBidPerformanceAnalysis({keyword={},rule=null,dailyPayload=[],devicePayload=[],weekdayPayload=[],hourPayload=[],estimates=null,bidHistory=[],errors=[],now=new Date()}={}){
  ...
  return {
    ...
    rank:{target,average,attainment_percent:targetAttainment(average,target),volatility:volatility(daily),status:average===null?'NO_DATA':target===null?'TARGET_REQUIRED':'READY'},
...
module.exports={FORMULA_VERSION,WINDOW_DAYS,WEEKDAYS,responsePoints,pointMetrics,summarize,dailySeries,dimensionMetrics,volatility,targetAttainment,buildBidPerformanceAnalysis,estimateRows,loadBidPerformanceAnalysis};
