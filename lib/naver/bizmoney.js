...
function normalizeBizmoneyDaily({charges=[],exhausts=[],periods=[],balance=null,now=new Date()}={}){
  ...
  for(const item of Array.isArray(exhausts)?exhausts:[]){
    ...
    if(!row)continue;
    add(row,'used_purchased',item?.useRefundableAmt??item?.useRefundableamt);
    add(row,'used_free',item?.useNonrefundableAmt??item?.useNonRefundableAmt);
  ...
  for(const item of Array.isArray(periods)?periods:[]){
    ...
    if(row.used_purchased===null)add(row,'used_purchased',item?.useRefundableAmt);
    if(row.used_free===null)add(row,'used_free',item?.useNonRefundableAmt??item?.useNonrefundableAmt);
