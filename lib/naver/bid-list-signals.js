...
async function loadBidListSignals({db,api=naverClient,keywordIds,now=new Date()}={}){
  ...
  const until=kstDateKey(now),since=shiftDate(until,-6);
  const response=await api.request('GET','/stats',{ids,fields:['avgRnk','recentAvgRnk'],timeRange:{since,until},timeIncrement:1});
  return {
    platform:'NAVER',
    formula_version:FORMULA_VERSION,
    period:{since,until},
    signals:buildBidListSignals({entities:response.data,rules:ruleResult.data||[],keywordIds:ids})
  };
}

module.exports={FORMULA_VERSION,MAX_KEYWORDS,validateKeywordIds,buildBidListSignals,loadBidListSignals};
