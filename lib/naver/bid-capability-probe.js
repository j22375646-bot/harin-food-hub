...
async function attempt(checks,key,execute,{emptyStatus='NO_SAMPLE',failureStatus='VERIFY_REQUIRED',failureNote='',readyNote='실계정 읽기 확인 완료'}={}){
  try{
    ...
    const item=check(key,failureStatus,{note:failureNote||safeError(error),errorCode:error?.code||error?.status||'PROBE_FAILED'});
    checks.push(item);
...
async function probeBidCapabilities({db,api=defaultApi,env=process.env,now=new Date()}={}){
  ...
  try{
    ...
    await attempt(checks,'position_15',()=>api.request('POST','/estimate/average-position-bid/keyword',null,{device:'PC',items:[{key:keywordText,position:15}]}),{emptyStatus:'NOT_SUPPORTED',failureStatus:'NOT_SUPPORTED',failureNote:'현재 계정의 공식 예상 API는 15위 목표값을 제공하지 않습니다.',readyNote:'15위 목표 예상 응답 확인 완료'});
