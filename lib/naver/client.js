...
async function request(method,uri,query) {
  ...
  const response=await fetch(url,{method,headers:{'X-Timestamp':timestamp,'X-API-KEY':apiKey,'X-Customer':customerId,'X-Signature':signature(timestamp,method,uri,secretKey)},cache:'no-store'});
