...
function config(env=process.env) {
  const customerId=env.NAVER_CUSTOMER_ID?.trim(), apiKey=env.NAVER_API_KEY?.trim(), secretKey=env.NAVER_SECRET_KEY?.trim();
  if(!customerId||!apiKey||!secretKey) {
    const error=new Error('네이버 광고 API 서버 환경변수가 아직 저장되지 않았습니다.');
    error.code='NAVER_SEARCH_AD_CONFIG_REQUIRED';
    error.status=503;
    throw error;
  }
...
async function request(method,uri,query,body) {
  const {customerId,apiKey,secretKey}=config(await require('../integrations/managed-keys.js').environment('NAVER'));
