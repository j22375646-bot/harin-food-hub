...
const SCHEDULE_FIELDS='ncc_adgroup_id,mode,weekdays,start_minute,end_minute,interval_minutes,max_changes_per_run,daily_change_limit,cooldown_minutes,time_slots,allow_increase,activation_confirmed_at,last_run_slot,last_run_at,last_run_status,updated_at';
...
async function listNaverBidRuns({db=getSupabase(),adgroupId,limit=50}={}){
  const id=String(adgroupId||'').trim();
  if(!id)throw new NaverBidScheduleStoreError('네이버 광고그룹을 다시 선택해주세요.','ADGROUP_REQUIRED',400);
  const safeLimit=Math.min(100,Math.max(1,Number(limit)||50));
  const result=await db.from(RUN_TABLE)
    .select('id,ncc_adgroup_id,run_slot,mode,status,planned_count,executed_count,blocked_count,details,error_message,started_at,finished_at,created_at')
    .eq('ncc_adgroup_id',id).order('started_at',{ascending:false}).limit(safeLimit);
  if(result.error)fail(result.error,'네이버 자동입찰 실행 기록을 불러오지 못했습니다.');
  return result.data||[];
}
...
async function listRecentNaverBidChanges({db=getSupabase(),keywordIds=[]}={}){
  const ids=[...new Set((keywordIds||[]).map(value=>String(value||'').trim()).filter(Boolean))].slice(0,500);
  if(!ids.length)return [];
  const result=await db.from('financial_change_requests')
    .select('target_key,platform,status,executed_at,verified_at,created_at')
    .eq('platform','NAVER').eq('change_type','NAVER_BID').in('target_key',ids)
    .not('executed_at','is',null).order('executed_at',{ascending:false}).limit(Math.min(500,ids.length*5));
  if(result.error)fail(result.error,'최근 네이버 입찰 변경 시각을 확인하지 못했습니다.');
  return result.data||[];
}
...
module.exports={
  ...
  listNaverBidRuns,listNaverBidSchedules,listRecentNaverBidChanges,saveNaverBidSchedule,setNaverBidAutomationPaused
};
