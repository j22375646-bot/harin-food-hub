...
async function loadGroupContextDefault({db,schedule}){
  ...
  const recentChanges=typeof scheduleStore.listRecentNaverBidChanges==='function'
    ?await scheduleStore.listRecentNaverBidChanges({db,keywordIds:ids})
    :[];
  return {keywords:keywords.data||[],rules,links,recentChanges};
}
...
async function runOne({db,api,store,financialChanges,loadGroupContext,schedule,now,automationEnabled}){
  ...
  const timeSlot=schedules.activeTimeSlot(schedule,now);
  ...
  try{
    ...
    const candidates=await schedules.buildEstimateCandidates({api,keywords:context.keywords,rules:context.rules,timeSlot,now});
    const dailyCount=await store.dailyExecutedCount({db,adgroupId:schedule.ncc_adgroup_id,now});
    const plan=schedules.buildNaverBidSchedulePlan({schedule,candidates,rules:context.rules,timeSlot,recentChanges:context.recentChanges||[],now,dailyExecutedCount:dailyCount,automationEnabled});
...
async function runDueNaverBidSchedules({
  ...
  const control=typeof store.getNaverBidAutomationControl==='function'
    ?await store.getNaverBidAutomationControl({db})
    :{emergency_paused:false,paused_reason:null};
  if(control?.emergency_paused===true){
    return {
      platform:'NAVER',checked_at:now.toISOString(),automation_enabled:automationEnabled,
      emergency_paused:true,pause_reason:control.paused_reason||'사장님 긴급 정지',due_count:0,runs:[]
    };
  }
  ...
  return {platform:'NAVER',checked_at:now.toISOString(),automation_enabled:automationEnabled,emergency_paused:false,due_count:due.length,runs};
}
