...
const bizmoney=require('./bizmoney.js');
...
async function syncBizmoney(db,days=90){
  const until=new Date();
  const since=new Date(until);
  since.setDate(until.getDate()-Math.max(1,Math.min(days,365))+1);
  const query={searchStartDt:date(since).replaceAll('-',''),searchEndDt:date(until).replaceAll('-','')};
  const requests={
    balance:get('/billing/bizmoney'),
    charges:get('/billing/bizmoney/histories/charge',query),
    exhausts:get('/billing/bizmoney/histories/exhaust',query),
    periods:get('/billing/bizmoney/histories/period',query)
  };
  ...
  settled.forEach((result,index)=>{
    ...
    else errors.push({source:key,code:result.reason?.code||null,status:result.reason?.status||null,message:result.reason?.message||'네이버 비즈머니 수집 실패'});
  });
  if(settled.every(item=>item.status==='rejected'))throw Object.assign(new Error(errors[0]?.message||'네이버 비즈머니 자료를 가져오지 못했습니다.'),{code:'NAVER_BIZMONEY_SYNC_FAILED',errors});
  const rows=bizmoney.normalizeBizmoneyDaily({
    balance:values.balance||null,
    charges:values.charges||[],
    exhausts:values.exhausts||[],
    periods:values.periods||[],
    now:until
  });
  for(let i=0;i<rows.length;i+=500){
    const result=await db.from('naver_bizmoney_daily').upsert(rows.slice(i,i+500),{onConflict:'date'});
    if(result.error&&result.error.code!=='PGRST205')throw result.error;
    if(result.error?.code==='PGRST205')return {
      status:errors.length?'PARTIAL':'SUCCESS_RAW',rows:rows.length,
      chargeRows:rows.filter(row=>bizmoney.advertisingCharged(row)!==null).length,
      deductionRows:rows.filter(row=>bizmoney.advertisingUsed(row)!==null).length,
      currentBalance:bizmoney.numberOrNull(values.balance?.bizmoney),errors,persistedTo:'raw_api_responses'
    };
  }
  return {
    status:errors.length?'PARTIAL':'SUCCESS',rows:rows.length,
    chargeRows:rows.filter(row=>bizmoney.advertisingCharged(row)!==null).length,
    deductionRows:rows.filter(row=>bizmoney.advertisingUsed(row)!==null).length,
    currentBalance:bizmoney.numberOrNull(values.balance?.bizmoney),errors
  };
}
...
async function syncKeywordStats(db,days=7){
  ...
  const until=new Date();const since=new Date(until);since.setDate(until.getDate()-Math.max(1,Math.min(days,30))+1);const periodStart=kstDate(since),periodEnd=kstDate(until);const fields=['impCnt','clkCnt','salesAmt','ccnt','convAmt'];const rows=[];
...
async function syncKeywordPerformanceWindows(db){
  const weekly=await syncKeywordStats(db,7);
  const today=await syncKeywordStats(db,1);
  return {weekly,today};
}
...
async function syncAll(){
  ...
  try{
    ...
    let bizmoneyResult;
    try{bizmoneyResult=await syncBizmoney(db,90);}
    catch(error){bizmoneyResult={status:'FAILED',rows:0,chargeRows:0,deductionRows:0,currentBalance:null,errors:[{code:error.code||null,status:error.status||null,message:error.message}]};}
    const counts={campaigns:campaigns.length,adgroups:groups.length,keywords:keywords.length,stats,bizmoney:bizmoneyResult};
    ...
    await db.from('sync_logs').update({status:['SUCCESS','SUCCESS_RAW'].includes(bizmoneyResult.status)?'SUCCESS':'PARTIAL',finished_at:new Date().toISOString(),rows_received:counts.campaigns+counts.adgroups+counts.keywords+bizmoneyResult.rows,metadata:{counts}}).eq('id',log.data.id);
    return counts;
...
module.exports={syncAll,syncStats,syncBizmoney,syncKeywordStats,syncKeywordPerformanceWindows,syncSearchTerms,syncSearchTermsLogged,searchTermValue,searchTermPoints};
