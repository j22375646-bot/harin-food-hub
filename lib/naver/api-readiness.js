...
const apiHubReliability=require('../naver-api-hub/reliability.js');
...
const DEFINITIONS = [
  ...
  },
  {
    ...
    jobs:['SEARCH_AD_CONNECTION_TEST','SEARCH_AD_BID_CAPABILITY_TEST','FETCH_ALL','SEARCH_TERMS'],
...
function apiHubSearchUsage(syncs,env,now){
  const rows=syncs.filter(row=>row.platform==='NAVER'&&row.job_type==='API_HUB_SEARCH_EVIDENCE_COLLECT');
  return apiHubReliability.quotaState({rows,env,now});
}
...
function buildService(definition, syncs, env, now) {
  ...
  if (definition.key === 'apiHub') {
    ...
    const searchQuota=apiHubSearchUsage(syncs,env,now);
    service.searchQuota={
      label:'공개 검색 하루 호출 예산',used:searchQuota.used,limit:searchQuota.budget,
      officialLimit:searchQuota.official_limit,remaining:searchQuota.remaining,warning:searchQuota.warning,
      blocked:searchQuota.blocked,unit:'회',source:'허브 공개검색 기록 기준',consoleExcluded:true
    };
    service.killSwitch={
      key:'api-hub-search',enabled:apiHubReliability.searchEnabled(env),
      label:'공개 검색 수집',cacheMinutes:apiHubReliability.searchCacheMinutes(env)
    };
    service.checks.push({
      key:'search-switch',label:'공개 검색 수집',
      status:service.killSwitch.enabled?(searchQuota.blocked?'LOCKED':'READY'):'LOCKED'
    });
  }
  return service;
}
...
function buildNaverApiReadiness({ syncs = [], env = process.env, now = new Date() } = {}) {
  ...
  return {
    phase:'18-6', generatedAt:new Date(now).toISOString(), services,
    summary:{
    ...
    rules:[
      ...
      'NAVER 공개 검색은 짧은 캐시를 재사용하고 하루 호출 예산에 도달하면 이전 성공 자료만 표시합니다.'
    ]
  };
}

module.exports = { buildNaverApiReadiness, safeError, kstMonthKey, apiHubSearchUsage };
