...
function encodeEventBody(entry={}){
  const payload={
    ...
    platforms:normalizeEventPlatforms(entry.platforms),plan:normalizeEventPlan(entry.plan)
  };
...
function decodeEventBody(value){
  ...
  try{
    ...
    return Object.freeze({
      ...
      platforms:normalizeEventPlatforms(payload?.platforms),plan:normalizeEventPlan(payload?.plan)
    });
  }catch{
...
function normalizeEntryInput(input={}){
  ...
  if(type==='EVENT'){
    result.platforms=normalizeEventPlatforms(input.platforms);result.plan=normalizeEventPlan(input.plan);
...
function decorateEntry(row={}){
  ...
  if(eventBody){entry.eventColor=eventBody.eventColor;entry.giftMode=eventBody.giftMode;entry.giftTiers=eventBody.giftTiers;entry.platforms=eventBody.platforms;entry.plan=eventBody.plan;entry.eventConfigInvalid=Boolean(eventBody.invalid);entry.eventState=eventState(entry);}
