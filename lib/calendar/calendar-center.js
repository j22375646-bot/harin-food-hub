...
function normalizeGiftTiers(value){
  ...
  const tiers=value.map((item,index)=>{
    ...
    if(!Number.isInteger(minimumAmount)||minimumAmount<1||minimumAmount>100000000)throw new CalendarInputError(`${index+1}번째 사은품 기준 금액을 확인해주세요.`);
    if(!Number.isInteger(quantity)||quantity<1||quantity>99)throw new CalendarInputError(`${index+1}번째 사은품 수량을 확인해주세요.`);
    return Object.freeze({minimumAmount,giftName:text(item?.giftName,120,'사은품 이름',{required:true}),quantity});
...
function resolveEventGift(entry,{orderAmount,date=seoulDateKey()}={}){
  ...
  const tier=[...(entry.giftTiers||[])].filter(item=>item.minimumAmount<=amount).sort((left,right)=>right.minimumAmount-left.minimumAmount)[0];
