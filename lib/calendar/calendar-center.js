...
const EVENT_PLATFORMS=Object.freeze(['NAVER','COUPANG','CAFE24']);
function normalizeEventPlatforms(value){
  if(value===undefined)return [...EVENT_PLATFORMS];
  if(!Array.isArray(value)||!value.length||value.length>3||new Set(value).size!==value.length||value.some(v=>!EVENT_PLATFORMS.includes(v)))throw new CalendarInputError('진행 플랫폼을 하나 이상 선택해주세요.');
  return EVENT_PLATFORMS.filter(v=>value.includes(v));
}
function normalizeEventPlan(value){
  if(value===undefined)return {stage:'READY',goal:'',audience:'',benefit:'',preparation:'',review:''};
  if(!value||typeof value!=='object'||Array.isArray(value)||!['DRAFT','READY'].includes(value.stage)||Object.keys(value).some(k=>!['stage','goal','audience','benefit','preparation','review'].includes(k)))throw new CalendarInputError('이벤트 기획 내용을 확인해주세요.');
  return {stage:value.stage,...Object.fromEntries(['goal','audience','benefit','preparation','review'].map(k=>[k,text(value[k],300,'기획 항목')]))};
}
function normalizeEventCampaign(value){
  if(value===undefined)return {discountType:'NONE',discountValue:0,discountConditions:'',messageStatus:'NOT_PLANNED',messageChannel:'',messagePlannedDate:'',messageSentDate:''};
  const keys=['discountType','discountValue','discountConditions','messageStatus','messageChannel','messagePlannedDate','messageSentDate'];
  if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(k=>!keys.includes(k))||!['NONE','PERCENT','AMOUNT'].includes(value.discountType)||!['NOT_PLANNED','PLANNED','SENT'].includes(value.messageStatus))throw new CalendarInputError('할인과 마케팅 발송 기록을 확인해주세요.');
  if(['discountConditions','messageChannel'].some(k=>value[k]!==undefined&&typeof value[k]!=='string'))throw new CalendarInputError('할인 조건과 발송 채널은 글자로 입력해주세요.');
  const amount=value.discountValue;
  if(!Number.isInteger(amount)||amount<0||amount>(value.discountType==='PERCENT'?100:100000000)||(value.discountType==='NONE'?amount!==0:amount===0))throw new CalendarInputError('할인율은 1~100%, 할인액은 1원 이상 정수로 입력해주세요.');
  for(const k of ['messagePlannedDate','messageSentDate'])if(value[k]!==undefined&&(typeof value[k]!=='string'||value[k]&&!validDateKey(value[k])))throw new CalendarInputError('발송 날짜를 확인해주세요.');
  if(value.messageStatus==='SENT'&&!value.messageSentDate||value.messageStatus!=='SENT'&&value.messageSentDate)throw new CalendarInputError('발송 완료 상태와 실제 발송일을 함께 확인해주세요.');
  return {...value,discountConditions:text(value.discountConditions,300,'할인 조건'),messageChannel:text(value.messageChannel,100,'발송 채널'),messagePlannedDate:value.messagePlannedDate||'',messageSentDate:value.messageSentDate||''};
}
function normalizeEventExecution(value){try{return require('../../desktop/ui/event-tools.js').normalize(value);}catch(e){throw new CalendarInputError(e.message);}}
function encodeEventBody(entry={}){
  const payload={
    ...
    platforms:normalizeEventPlatforms(entry.platforms),plan:normalizeEventPlan(entry.plan),campaign:normalizeEventCampaign(entry.campaign),...(entry.execution!==undefined?{execution:normalizeEventExecution(entry.execution)}:{})
  };
  const stored=`${EVENT_BODY_PREFIX}${JSON.stringify(payload)}`;
  if(stored.length>4000)throw new CalendarInputError('이벤트 설명·기획·메시지 내용을 조금 줄여주세요.');
  return stored;
}
...
function decodeEventBody(value){
  ...
  try{
    ...
    return Object.freeze({
      ...
      platforms:normalizeEventPlatforms(payload?.platforms),plan:normalizeEventPlan(payload?.plan),campaign:normalizeEventCampaign(payload?.campaign),...(payload?.execution!==undefined?{execution:normalizeEventExecution(payload.execution)}:{})
    });
  }catch{
...
function eventState(entry,dateKey=seoulDateKey()){
  ...
  if(!entry||entry.type!=='EVENT'||entry.status==='ARCHIVED'||entry.eventConfigInvalid||entry.plan?.stage==='DRAFT')return 'INACTIVE';
  if(date<entry.date)return 'UPCOMING';
  if(date>(entry.endDate||entry.date))return 'ENDED';
  return 'ACTIVE';
}

function resolveEventGift(entry,{orderAmount,date=seoulDateKey(),platform}={}){
  const amount=Number(orderAmount);
  if(entry?.platforms!==undefined&&(!Array.isArray(entry.platforms)||!entry.platforms.includes(platform)))return null;
...
function normalizeEntryInput(input={}){
  ...
  if(type==='EVENT'){
    result.platforms=normalizeEventPlatforms(input.platforms);result.plan=normalizeEventPlan(input.plan);result.campaign=normalizeEventCampaign(input.campaign);if(input.execution!==undefined)result.execution=normalizeEventExecution(input.execution);
...
function decorateEntry(row={}){
  ...
  if(eventBody){entry.eventColor=eventBody.eventColor;entry.giftMode=eventBody.giftMode;entry.giftTiers=eventBody.giftTiers;entry.platforms=eventBody.platforms;entry.plan=eventBody.plan;entry.campaign=eventBody.campaign;entry.execution=eventBody.execution;entry.eventConfigInvalid=Boolean(eventBody.invalid);entry.eventState=eventState(entry);}
  return Object.freeze(entry);
}

function eventRevision(entries=[]){
  return JSON.stringify(entries.map(item=>[String(item.id||''),String(item.updatedAt||item.updated_at||'')]).sort((a,b)=>a[0].localeCompare(b[0])));
}
