...
async function shop({query,sort,env=process.env,fetchImpl=fetch}){
 if(!env.NAVER_SHOPPING_CLIENT_ID||!env.NAVER_SHOPPING_CLIENT_SECRET)throw Object.assign(Error('Setup'),{code:'CONFIG_REQUIRED'});
 const params=new URLSearchParams({query,sort,display:'40',start:'1'});
 const response=await fetchImpl('https://openapi.naver.com/v1/search/shop.json?'+params,{headers:{'X-Naver-Client-Id':env.NAVER_SHOPPING_CLIENT_ID,'X-Naver-Client-Secret':env.NAVER_SHOPPING_CLIENT_SECRET},cache:'no-store',signal:AbortSignal.timeout(15000)});
 if(!response.ok)throw Error('Shopping unavailable');return response.json();
}
function createResearch({ads=require('../naver/client.js'),hub=require('../naver-api-hub/client.js'),shopping=shop,now=()=>new Date()}={}){
 return async input=>{
  ...
  const [keywords,trend,products]=await Promise.all([
   ...
   (async()=>{try{const r=await shopping(input);if(!Array.isArray(r.items)||!Number.isSafeInteger(r.total)||r.total<0)throw Error('Invalid');const rows=r.items.slice(0,40).map(v=>({id:contract.text(v.productId,80),title:contract.text(v.title,240),store:contract.text(v.mallName,100),brand:contract.text(v.brand,100),category:[v.category1,v.category2,v.category3,v.category4].filter(Boolean).join(' > '),price:metric(v.lprice)>0?metric(v.lprice):null}));return {status:rows.length?'READY':'EMPTY',total:r.total,rows};}catch(e){return {...failure(e,'rows'),total:null};}})()
  ]);
