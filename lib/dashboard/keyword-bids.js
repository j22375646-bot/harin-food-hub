...
function createKeywordBidOperations({db,api=client,changes=financial,live=execution,loadScope=loadActiveAdScope,research=require('./market-research.js').research}={}){
 const groupOperate=require('./group-bids.js').createGroupBidOperations({db,api,loadScope,configuration:live.configuration});
 ...
 return async(input,{actor,check})=>{
  if(input.action==='RESEARCH'){const result=await research(input);await check();return result;}
  if(input.action.startsWith('GROUP_'))return groupOperate(input,{actor,check});
