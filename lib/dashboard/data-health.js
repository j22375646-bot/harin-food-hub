...
function latestByTime(rows = []) {
  return [...rows].sort((a,b) => new Date(b.finished_at || b.started_at || 0).getTime() - new Date(a.finished_at || a.started_at || 0).getTime())[0] || null;
}
...
function buildDataHealth({ issues = [], syncs = [], automationRuns = [], coupangRequests = [], summaries = {}, now = new Date() }) {
  ...
  const channels = CHANNELS.map(platform => {
    ...
    const latest = latestByTime(syncs.filter(item => item.platform === platform));
    ...
    const lastSuccess = latestByTime(syncs.filter(item => item.platform === platform && item.status === 'SUCCESS'));
    const hasPreviousData = Boolean(lastSuccess?.finished_at);
    const calculationStatus = ['PARTIAL', 'FAILED', 'STALE'].includes(status) ? 'CHECK_REQUIRED' : status === 'READY' ? 'READY' : 'WAITING';
    return {
      platform,
      status,
      dataMode:channelIssues.length ? 'UNAVAILABLE' : calculationStatus === 'CHECK_REQUIRED' && hasPreviousData ? 'PREVIOUS' : status === 'READY' ? 'LIVE' : 'WAITING',
      calculationStatus,
