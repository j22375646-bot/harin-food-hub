...
async function loadWorkspaceInventory({db,now=()=>new Date()}={}){
 ...
 const links=await read('channel_products','id,master_product_id,platform,external_product_id,is_active,raw_data,updated_at','master_product_id',active.map(r=>r.id),5000,'id');
 ...
 const [cafe,options]=await Promise.all([
  ...
  read('coupang_product_items','vendor_item_id,seller_product_id','seller_product_id',forPlatform('COUPANG'),5000,'vendor_item_id')
 ]);
 ...
 const items=active.map(master=>{
  ...
  for(const channel of channels){const mapped=({CAFE24:c,NAVER:n,COUPANG:cp})[channel.platform];channel.externalId=mapped?.external_product_id==null?null:String(mapped.external_product_id).slice(0,160);}
