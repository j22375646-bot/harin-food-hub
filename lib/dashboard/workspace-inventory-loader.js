...
async function loadWorkspaceInventory({db,now=()=>new Date()}={}){
 ...
 const items=active.map(master=>{
  ...
  for(const channel of channels){const mapped=({CAFE24:c,NAVER:n,COUPANG:cp})[channel.platform];channel.externalId=mapped?.external_product_id==null?null:String(mapped.external_product_id).slice(0,160);channel.product=buildWorkspaceProductDetails({platform:channel.platform,link:mapped,cafeProduct:source,options:expected,now:at});}
