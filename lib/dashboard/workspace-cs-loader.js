...
async function loadWorkspaceCs({db,now=()=>new Date()}={}){
 const specs=[
  ['customerServiceRows','customer_service_items','source_key,platform,kind,completed,status,occurred_at','occurred_at'],
  ['coupangInquiries','coupang_inquiries','inquiry_key,inquiry_id,answered,status,inquired_at','inquired_at'],
 ...
 results.forEach((rows,index)=>rows.forEach(row=>{
  ...
  items.push({...active,status:typeof row.status==='string'?row.status.slice(0,80):'확인 필요',occurredAt:rawDate&&Number.isFinite(Date.parse(rawDate))?new Date(rawDate).toISOString():null});
 }));
