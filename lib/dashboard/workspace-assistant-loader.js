...
async function loadWorkspaceAssistant({db,context,now=new Date(),ordersReader=loadUnifiedOrders,csReader=loadWorkspaceCs,scopes=["orders","tasks","cs","reports"]}={}){
 ...
 const entries=await Promise.all(Object.entries(readers).filter(([key])=>scopes.includes(key)).map(async([key,read])=>{try{return [key,await read()];}catch{return [key,unknown()];}}));
