...
function targetLikelihood(item = {}) {
  ...
  if (item.forecastStatus === 'SAMPLE_REQUIRED') return { code:'CHECK_REQUIRED', label:'계산 대기', description:'이번 달 결제 주문이 발생하면 월말 예상 매출을 자동으로 계산해요.' };
...
function buildCashflow(pacingItem = {}, profitability = {}, financialTrust = {}) {
  if (!pacingItem.platform || pacingItem.status === 'NO_DATA' || typeof pacingItem.revenueActual !== 'number' || !Number.isFinite(pacingItem.revenueActual)) return { status:'CHECK_REQUIRED', expectedInflow:null, expectedAdOutflow:null, expectedOperatingOutflow:null, expectedBalance:null, description:'월 매출 자료가 없어 30일 예상치를 계산하지 않았어요.' };
...
function buildSmartSchedule(now = new Date()) {
  ...
  const operatingMinute = clock.minuteOfDay < 360 ? clock.minuteOfDay + 1440 : clock.minuteOfDay;
  const state = (start, end = null) => operatingMinute < start ? 'UPCOMING' : end != null && operatingMinute >= end ? 'DONE' : 'NOW';
...
function buildSalesCommandCenter({ pacing = {}, priorityCenter = {}, dataHealth = {}, productSignals = {}, profitability = {}, financialTrust = {}, unifiedOrders = {}, customerService = {}, unifiedInventory = {}, reliabilityCenter = {}, alerts = [], now = new Date() } = {}) {
  ...
  const hasPacing = Boolean(item.platform) && item.status !== 'NO_DATA' && typeof item.revenueActual === 'number' && Number.isFinite(item.revenueActual);
  const target = number(item.revenueTarget);
  const current = number(item.revenueActual);
  const hasForecast = typeof item.revenueForecast === 'number' && Number.isFinite(item.revenueForecast);
  const forecast = hasForecast ? item.revenueForecast : null;
  const shortage = target ? Math.max(0, target - current) : null;
  const forecastShortage = target && hasForecast ? Math.max(0, target - forecast) : null;
  const metrics = { target:target || null, current:hasPacing?round(current):null, forecast:hasPacing&&hasForecast?round(forecast):null, shortage:hasPacing&&shortage != null ? round(shortage) : null, forecastShortage:hasPacing&&forecastShortage != null ? round(forecastShortage) : null, requiredDailyRevenue:hasPacing&&target ? round(item.requiredDailyRevenue) : null, progressRate:hasPacing&&item.revenueProgressRate != null ? Number(number(item.revenueProgressRate).toFixed(1)) : null };
  if (Number.isFinite(Number(item.sampleSize))) metrics.currentSampleSize=Number(item.sampleSize);
  if (item.forecastStatus) metrics.forecastStatus=String(item.forecastStatus);
  return {
    month:pacing.month || dateOnly(pacing.asOf).slice(0,7), asOf:pacing.asOf || null,
    revenueStatus:pacing.status || (hasPacing ? 'READY' : 'NO_DATA'),
    metrics,
    targetSettings:{ month:pacing.month || dateOnly(pacing.asOf).slice(0,7), platform:'ALL', revenueTarget:target || null, adBudget:Number.isFinite(Number(item.adBudget))?number(item.adBudget):null, targetRoas:Number.isFinite(Number(item.targetRoas))?number(item.targetRoas):null },
