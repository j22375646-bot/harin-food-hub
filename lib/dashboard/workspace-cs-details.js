...
function csDetails(row,{secret}={}){
 ...
 const id=v=>/^\d{1,20}$/.test(String(v||''))?String(v):null;
 const product=row.inquiry_type?{name:text(row.product_name).slice(0,200),productId:id(row.product_id),vendorItemId:id(row.vendor_item_id),sellerProductId:id(row.seller_product_id)}:null;
 return {product,reply:{available:row.inquiry_type==='ONLINE'&&row.answered===false,sourceUpdatedAt:date(row.updated_at)},status:unavailable?'UNAVAILABLE':body||title||history.length?'AVAILABLE':'MISSING',title:title.slice(0,200),body:body.slice(0,2000),history,truncated,updatedAt:date(row.source_updated_at||row.updated_at)};
}
