...
const MAIN_REMOTE_QUERY_BUDGET=37;
...
async function buildMainDashboardData({
  ...
  naverAdsResult,coupangAdsResult,businessTargetsResult,monthlyRevenueResult,customerServiceRows,coupangInquiriesResult,coupangExchangesResult,customerServiceAvailable,cafe24Token,reportsResult,calendarEntries=[],
  ...
  const customerService=customerServiceOperationModule.buildCustomerServiceOperationSummary({
    customerServiceRows,coupangInquiries:coupangInquiriesResult.data,coupangReturns:coupangReturnsResult.data,
    coupangExchanges:coupangExchangesResult.data,available:customerServiceAvailable
  });
  ...
  const unifiedSettlement=unifiedSettlementModule.buildUnifiedSettlementCenter({
    cafe24Orders:ordersResult.data||[],cafe24SalesDaily:cafe24SalesResult.data||[],cafe24Token,
    ...
    coupangRgOrders:coupangRgOrdersResult.data||[],coupangRgOrderItems:coupangRgItemsResult.data||[],
    ...
    unavailable:{
      ...
      COUPANG_RG:Boolean(coupangRgOrdersResult.unavailable&&coupangRgItemsResult.unavailable)
    }
  });
  ...
  return {
    ...
    financialAutomation:{
      ...
      cafe24ScopeRequired:['SCOPE_REQUIRED','APPROVAL_REQUIRED'].includes(unifiedSettlement.channels.find(item=>item.platform==='CAFE24')?.status)
    },
    coupang:{
...
async function loadPhase28MainDashboard({db,now=new Date()}={}){
  ...
  const querySpecs=[
    ...
    ['coupang_inquiries',db.from('coupang_inquiries').select('inquiry_key,inquiry_id,answered').or('answered.eq.false,answered.is.null').limit(1000)],
    ['coupang_exchanges',db.from('coupang_exchanges').select('exchange_id,status').order('requested_at',{ascending:false}).limit(1000)]
  ];
  ...
  return buildMainDashboardData({
    ...
    coupangInquiriesResult,coupangExchangesResult,
    customerServiceAvailable:![channelCsResult,coupangInquiriesResult,coupangReturnsResult,coupangExchangesResult].some(result=>result.unavailable),
    cafe24SalesResult,naverSettlementsResult,coupangSettlementsResult,coupangCostsResult,coupangAdSettlementsResult,coupangSettlementSummariesResult,costCalibrationResult,
    historyMonthlyRevenueResults
  });
}
