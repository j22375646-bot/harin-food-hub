...
function buildDeadLetters(operationRows=[],syncRows=[]){
  const operation=(operationRows||[]).filter(row=>row.status==='FAILED'&&!(row.operation_type==='EPOST_TRACKING'&&/조회결과가 없습니다/.test(String(row.error_message||'')))).map(row=>({kind:'OPERATION',id:row.id,title:row.operation_type,target:`${row.target_type||''} ${row.target_id||''}`.trim(),failed_at:iso(row.dead_lettered_at||row.executed_at||row.created_at),attempt_count:Number(row.attempt_count||0),manual_retry_count:Number(row.manual_retry_count||0),error:row.error_message||'작업 실패'}));
