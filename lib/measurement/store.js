...
function decodeCursor(after) {
  ...
  try{
    ...
    // Date.parse normalizes impossible dates (e.g. February 30) and 24:00.
    // Compare UTC calendar/time components, keeping original fractional seconds for the cursor.
    if(new Date(value.createdAt).toISOString().slice(0,19)!==value.createdAt.slice(0,19))throw new Error();
    return value;
