...
const ENCODED_CONTROL_CHARACTER = /%(?:0[0-9a-f]|1[0-9a-f]|7f|8[0-9a-f]|9[0-9a-f]|c2%(?:8[0-9a-f]|9[0-9a-f]))/iu;
...
function normalizeLandingUrl(value) {
  ...
  if (ENCODED_CONTROL_CHARACTER.test(landingUrl)) {
    fail('landingUrl 필드에 인코딩된 제어 문자를 사용할 수 없습니다.', 'CONTROL_CHARACTER');
  }
  let parsed;
  try {
...
function inspectExistingQuery(parsed, normalized) {
  ...
  for (const [rawKey, value] of parsed.searchParams) {
    ...
    if (!Object.hasOwn(PARAMETER_TO_FIELD, key)) continue;
