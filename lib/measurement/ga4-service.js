...
function gatedState({config,missingFields}){
  ...
  return {
    status,scopeHash:null,missingFields,canRefresh:false,report:null,lastAttemptAt:null,lastSuccessAt:null,
...
function configuredBase(missingFields=[],scopeHash){
  return {
    status:'VERIFY_REQUIRED',scopeHash,missingFields,canRefresh:true,report:null,lastAttemptAt:null,lastSuccessAt:null,
...
async function getState({db,env=process.env,now=new Date()}={}){
  ...
  const base=configuredBase(ready.missingFields,ready.scopeHash);
  try{
...
async function refresh({db,env=process.env,now=new Date(),fetchImpl=fetch}={}){
  ...
  const promise=performRefresh({db,env,now,fetchImpl},ready)
    .catch(()=>({...configuredBase(ready.missingFields,ready.scopeHash),status:'FAILED',error:'저장된 측정 자료를 불러오지 못했습니다.'}))
