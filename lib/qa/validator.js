...
async function runDataQualityChecks({ automationRunId = null } = {}) {
  ...
  const [logs, traffic, cafeOrders, naverStats, coupangOrders, coupangProducts, coupangInventory, duplicates] = await Promise.all([
    db.from('sync_logs').select('platform,job_type,status,started_at,finished_at,error_message,rows_received,metadata').in('job_type', ['FETCH_ALL','COMMERCE_SYNC','ORDERS_REALTIME','CUSTOMER_SERVICE','RG_INVENTORY']).order('started_at', { ascending: false }).limit(120),
  ...
  for (const platform of ['CAFE24', 'NAVER', 'COUPANG']) {
    const allowed = platform === 'NAVER'
      ? new Set(['FETCH_ALL','COMMERCE_SYNC'])
      : platform === 'CAFE24'
        ? new Set(['FETCH_ALL','ORDERS_REALTIME'])
        : new Set(['FETCH_ALL','ORDERS_REALTIME','CUSTOMER_SERVICE','RG_INVENTORY']);
    const latest = (logs.data || []).filter(item => item.platform === platform && allowed.has(item.job_type))
