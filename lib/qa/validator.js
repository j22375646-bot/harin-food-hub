...
async function runDataQualityChecks({ automationRunId = null } = {}) {
  ...
  return { checked: checks.length, ok: checks.filter(item => ['OK', 'REAL_ZERO'].includes(item.status_code)).length, warnings: checks.filter(item => item.severity === 'WARNING').length, errors: errors.length, checks, new_alerts:newAlerts.length, alert_delivery:alertDelivery };
}
