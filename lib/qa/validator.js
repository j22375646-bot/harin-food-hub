...
async function runDataQualityChecks({ automationRunId = null } = {}) {
  ...
  for (const item of errors) {
    const fingerprint = `QA:${item.platform}:${item.dataset}:${item.status_code}`;
    const existing = await db.from('alerts').select('id').eq('fingerprint', fingerprint).eq('status', 'OPEN').limit(1).maybeSingle();
    if (!existing.error && !existing.data) await db.from('alerts').insert({ source_type: 'DATA_QUALITY', platform: item.platform, severity: item.severity, title: `${item.platform} ${item.dataset} 점검 필요`, message: item.message, fingerprint, status: 'OPEN' });
  }
  return { checked: checks.length, ok: checks.filter(item => ['OK', 'REAL_ZERO'].includes(item.status_code)).length, warnings: checks.filter(item => item.severity === 'WARNING').length, errors: errors.length, checks };
}
