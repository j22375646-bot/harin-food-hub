...
function createRead({database=require('../cafe24/supabase.js').getSupabase,load=require('../dashboard/workspace-assistant-loader.js').loadWorkspaceAssistant,timeoutMs=25000}={}){return async request=>{
 ...
 const data=await load({db,context:before,scopes:before.scopes});current();
 ...
 return reply(200,{ok:true,writePolicy:'READ_ONLY',retrievedAt:data.retrievedAt,sourcePolicy:'STORED_DATA',status:data.status,sources:Object.fromEntries(before.scopes.map(s=>[s,data.sources[s]])),caveats:data.caveats});
