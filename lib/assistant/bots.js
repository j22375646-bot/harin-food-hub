...
async function command({input,worker,db,session,digest,env=process.env,fetcher=fetch}){
 ...
 if(input.action==='BOT_SAVE'){
  ...
  const expected=input.slot==='WORK'?'moaon_hub_bot':'moaon_solo_bot';
 ...
 if(claim.claimed){let status='UNKNOWN';try{const row=claim.row,key=cipher(env).open(scope(row.slot,row.revision),row.envelope).token;await telegram(key,'sendMessage',{chat_id:row.settings.chatId,text:(input.slot==='WORK'?'모아온 업무비서':'모아온 개인비서')+' 연결 시험\n이 대화로 알림을 받을 수 있습니다. AI 대화 연결 상태는 모아온에서 별도로 확인해 주세요.'},fetcher);status='SENT';}catch(e){status=e.code==='BOT_REJECTED'?'FAILED':'UNKNOWN';}await rpc({...input,action:'TEST_RESULT',status});}
