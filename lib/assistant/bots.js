...
async function telegram(token,method,payload={},fetcher=fetch){
 if(!validToken(token)||!['getMe','sendMessage','sendPhoto'].includes(method))throw fail('BOT_INVALID');
 let body=JSON.stringify(payload),headers={'content-type':'application/json'};
 if(method==='sendPhoto'){
  if(!Buffer.isBuffer(payload.photo)||payload.photo.length>2000000)throw fail('BOT_INVALID');
  body=new FormData();for(const [key,value] of Object.entries(payload))body.append(key,key==='photo'?new Blob([value],{type:'image/png'}):typeof value==='object'?JSON.stringify(value):String(value));headers={};
 }
 try{const r=await fetcher('https://api.telegram.org/bot'+token+'/'+method,{method:'POST',redirect:'error',signal:AbortSignal.timeout(25000),headers,body});const raw=await r.text();if(raw.length>24000)throw Error();const data=JSON.parse(raw);if(!r.ok||data.ok!==true)throw fail(r.status>=400&&r.status<500?'BOT_REJECTED':'BOT_UNKNOWN');return data.result;}catch(e){throw fail(['BOT_REJECTED','BOT_INVALID'].includes(e.code)?e.code:'BOT_UNKNOWN');}
}
async function command({input,worker,db,session,digest,env=process.env,fetcher=fetch}){
 ...
 if(input.action==='BOT_CONFIG'){
  const rows=await rpc(input);const members=await db.rpc('moaon_assistant_chat_config',{p_worker_hash:digest});if(members.error||!Array.isArray(members.data))throw fail('ASSISTANT_UNAVAILABLE');return {bots:rows.map(r=>({chatUsers:members.data.filter(u=>u.slot===r.slot&&u.enabled),slot:r.slot,revision:r.revision,username:r.username,settings:{...r.settings,allowedUsers:r.settings.allowedUsers.filter(id=>!members.data.some(u=>u.slot===r.slot&&u.chatId===id&&!u.enabled))},token:r.settings.enabled?cipher(env).open(scope(r.slot,r.revision),r.envelope).token:null}))};
 }
 ...
 if(input.action==='BOT_SAVE'){
  ...
  const expected=({WORK:'moaon_hub_bot',SOLO:'moaon_solo_bot',STUDY:'moaon_study_bot',SUP:'moaon_sup_bot',AD:'moaon_ad_bot'})[input.slot];
 ...
 if(claim.claimed){let status='UNKNOWN';try{const row=claim.row,key=cipher(env).open(scope(row.slot,row.revision),row.envelope).token;await telegram(key,'sendMessage',{chat_id:row.settings.chatId,text:(({WORK:'모아온 업무비서',SOLO:'모아온 개인비서',STUDY:'모아온 지식비서',SUP:'모아온 관리비서',AD:'모아온 광고비서'})[input.slot])+' 연결 시험\n이 대화로 알림을 받을 수 있습니다. AI 대화 연결 상태는 모아온에서 별도로 확인해 주세요.'},fetcher);status='SENT';}catch(e){status=e.code==='BOT_REJECTED'?'FAILED':'UNKNOWN';}await rpc({...input,action:'TEST_RESULT',status});}
