...
async function command({input,worker,db,session,digest,collect=reports.collect,send=telegram,env=process.env}){
 ...
 if(j.id){
  try{const summary=await collect({start:j.start_date,end:j.end_date,fresh:j.fresh,db});
   if(j.dedupe?.startsWith('change:')){const shift=d=>new Date(Date.parse(d)-7*86400000).toISOString().slice(0,10);const previous=await collect({start:shift(j.start_date),end:shift(j.end_date),fresh:true,db});summary.change=require('./ads-change.js').compare(summary,previous,j.changeSettings);}
 ...
 if(d.job){let status='UNKNOWN';try{const b=d.bot,token=cipher(env).open({tenantId:TENANT,provider:'TELEGRAM_'+b.slot,revision:b.revision},b.envelope).token;const j=d.job,m=j.summary?.metrics;const show=v=>v===null||v===undefined?'확인 필요':Math.round(v*100)/100;
  ...
  await send(token,'sendMessage',{chat_id:b.settings.chatId,text:message,reply_markup:{inline_keyboard:[[{text:'모아온에서 확인',url:'https://harin-cafe24-sync.vercel.app'}]]}});status='SENT';
