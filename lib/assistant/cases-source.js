...
function orderTime(v){if(typeof v!=='string'||!/^20\d{2}-\d{2}-\d{2}[T ]\d{2}:\d{2}/.test(v))return null;const normalized=v.replace(' ','T')+(/[zZ]$|[+-]\d{2}:?\d{2}$/.test(v)?'':'+09:00');const time=Date.parse(normalized);return Number.isFinite(time)?new Date(time).toISOString():null;}
async function load({db,ordersReader=require('../orders/unified-orders.js').loadUnifiedOrders}){
 ...
 try{const center=await ordersReader({db,summaryRead:true});for(const platform of ['NAVER','CAFE24','COUPANG']){
  ...
  for(const o of center.orders.filter(o=>o.platform===platform&&o.fulfillment!=='ROCKET_GROWTH')){
   ...
   if(stages.includes('ACTIVE'))observations.push({platform,kind:'ORDER',sourceId:o.hubOrderId,state:'PENDING',orderedAt:orderTime(o.orderedAt)});
