...
const names={WORK:'업무비서',SOLO:'개인비서',AD:'광고비서',SUP:'관리비서',STUDY:'학습비서'};
const colors={WORK:'#6A52BC',SOLO:'#386E82',AD:'#3F6A54',SUP:'#A56435',STUDY:'#526ABE'};
...
function model(text,slot){
 ...
 for(const line of lines){
  if(/^(조회:|모아온 저장 자료|광고 전환매출은|자료 상태:|API 조회:|봇 실행 신호와|Hermes 중단 시|모아온에 저장된 공유 지식)/.test(line))foot.push(line);
  else rows.push(line);
 }
 ...
 for(const line of visible){
  ...
  if(['SUP','STUDY'].includes(slot)){const metric=line.match(/^(연결 확인 필요|발송 실패 · 24시간|결과 확인 필요 · 24시간|검토 대기|공유 지식) · (\d+건|확인 필요)$/);if(metric){metrics.push({label:metric[1],value:metric[2]});continue;}}
  if(slot==='AD'){
  ...
  if(/^모아온 (업무비서|개인비서|관리비서|학습비서) 브리핑$/.test(line))continue;
  extra.push(line);
 }
...
function element(m){
 const accent=colors[m.slot],ink='#242439',muted='#64657C',pale=({AD:'#EAF4EE',SOLO:'#EAF3F7',SUP:'#FCF0E6',STUDY:'#EDF0FC'})[m.slot]||'#F0ECFC';
 ...
 const tile=(label,v,warn)=>box({width:468,height:222,borderRadius:26,padding:30,flexDirection:'column',justifyContent:'space-between',background:warn&&v!=='0건'&&v!=='확인 필요'?'#FFF1E8':pale},box({fontSize:label.length>10?32:38,color:muted},label),value(v,84));
 ...
 return box({width:'100%',height:'100%',background:'#FCFBFF',fontFamily:'Moaon',color:ink,padding:54,flexDirection:'column'},
  ...
  box({fontSize:64,marginTop:32,lineHeight:1.2},(m.title.includes('신규 주문')?'새 주문이 들어왔어요':({AD:'네이버 광고 리포트',SUP:'오늘의 운영 점검',STUDY:'오늘의 지식 노트',SOLO:'오늘 챙길 업무'})[m.slot]||'오늘의 업무 브리핑')),
