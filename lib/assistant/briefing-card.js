...
function model(text,slot){
 ...
 for(const line of lines){
  if(/^(조회:|모아온 저장 자료|광고 전환매출은|자료 상태:|API 조회:|봇 실행 신호와|Hermes 중단 시|승인·저장된 공유 지식)/.test(line))foot.push(line);
  else rows.push(line);
 }
...
function element(m){
 ...
 return box({width:'100%',height:'100%',background:'#FCFBFF',fontFamily:'Moaon',color:ink,padding:54,flexDirection:'column'},
  ...
  box({fontSize:64,marginTop:32,lineHeight:1.2},({AD:'네이버 광고 리포트',SUP:'오늘의 운영 점검',STUDY:'오늘의 지식 노트',SOLO:'오늘 챙길 업무'})[m.slot]||'오늘의 업무 브리핑'),
