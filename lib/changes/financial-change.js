...
async function createNaverBidPreview(snapshot, ownerDesiredBid, { db = supabaseModule.getSupabase(), idempotencyKey, actor } = {}) {
  ...
  if (existing.error) throw existing.error;
  if (existing.data) return { request:existing.data, reused:true };
  if (!snapshot || snapshot.scope !== 'naver-bid-proposal' || snapshot.external_execution_locked !== true) {
  ...
  if (inserted.error?.code === '23505') inserted = await db.from('financial_change_requests').select('*').eq('idempotency_key', key).single();
  if (inserted.error) throw inserted.error;
  await audit(db, inserted.data.id, 'NAVER_BID_PREVIEW_CREATED', null, 'PREVIEWED', actor, { preview_hash:inserted.data.preview_hash, recommended_bid:Number(snapshot.recommended_bid), owner_desired_bid:desired, external_execution_locked:true });
  return { request:inserted.data, reused:false };
}
...
async function applySnapshot(db, request, snapshot) {
  if (request.change_type === 'NAVER_BID') throw new FinancialChangeError('12-6A에서는 네이버 실제 입찰 변경을 잠가두었습니다.', 409, 'NAVER_BID_EXECUTION_LOCKED');
...
async function execute(id, { db = supabaseModule.getSupabase(), actor } = {}) {
  const request = await getRequest(db, id);
  if (request.change_type === 'NAVER_BID') throw new FinancialChangeError('12-6A에서는 승인 기록만 저장하며 실제 입찰 변경은 실행하지 않습니다.', 409, 'NAVER_BID_EXECUTION_LOCKED');
  if (FINAL_EXECUTION_STATUSES.has(request.status)) return { request, reused:true };
  if (request.status === 'EXECUTING') {
    const current = await readCurrent(db, request);
    if (!same(current, request.proposed_value)) await applySnapshot(db, request, request.proposed_value);
    ...
    await audit(db, id, 'EXECUTION_RECOVERED', 'EXECUTING', 'EXECUTED', actor);
    return { request:completed, reused:true, recovered:true };
  }
  if (request.status !== 'APPROVED') throw new FinancialChangeError('승인된 요청만 실행할 수 있습니다.', 409, 'INVALID_STATE');
  const current = await readCurrent(db, request);
  if (!same(current, request.before_value)) {
    const stale = await updateFrom(db, request, 'APPROVED', { status:'STALE', error_message:'승인 이후 원본 값이 변경됨' });
    await audit(db, id, 'EXECUTION_BLOCKED_STALE', 'APPROVED', 'STALE', actor, { current });
    return { request:stale, stale:true };
  }
  const executing = await updateFrom(db, request, 'APPROVED', { status:'EXECUTING', executed_by:cleanActor(actor), execution_started_at:new Date().toISOString(), error_message:null });
  await audit(db, id, 'EXECUTION_STARTED', 'APPROVED', 'EXECUTING', actor);
  try {
    await applySnapshot(db, executing, executing.proposed_value);
    ...
    await audit(db, id, 'EXECUTED', 'EXECUTING', 'EXECUTED', actor);
    return { request:completed, reused:false };
  } catch (error) {
    const actual = await readCurrent(db, executing).catch(() => null);
...
async function rollback(id, { db = supabaseModule.getSupabase(), actor } = {}) {
  ...
  if (request.status === 'ROLLBACK_REQUESTED') {
    const current = await readCurrent(db, request);
    if (!same(current, request.rollback_value)) await applySnapshot(db, request, request.rollback_value);
    const actual = await readCurrent(db, request);
  ...
  try {
    await applySnapshot(db, claimed, claimed.rollback_value);
    const actual = await readCurrent(db, claimed);
    ...
    await audit(db, id, 'ROLLED_BACK', 'ROLLBACK_REQUESTED', 'ROLLED_BACK', actor, { actual });
    return { request:completed, reused:false };
  } catch (error) {
    const actual = await readCurrent(db, claimed).catch(() => null);
...
module.exports = { FinancialChangeError, approve, createNaverBidPreview, createPreview, execute, impactPreview, listRequests, normalizeRequest, reject, rollback, same, verify };
