...
async function createNaverBidPreview(snapshot, ownerDesiredBid, { db = supabaseModule.getSupabase(), idempotencyKey, actor } = {}) {
  ...
  const recommendation=snapshot.recommended_bid==null?null:Number(snapshot.recommended_bid);
  const metadata = {
    keyword:String(snapshot.keyword || '').slice(0, 200),
    recommended_bid:recommendation, owner_desired_bid:desired,
    ...
    recommendation_ready:snapshot.recommendation_ready===true,
    manual_decrease_only:snapshot.manual_decrease_only===true,
  ...
  if (inserted.error?.code === '23505') inserted = await db.from('financial_change_requests').select('*').eq('idempotency_key', key).single();
  if (inserted.error) throw inserted.error;
  await audit(db, inserted.data.id, 'NAVER_BID_PREVIEW_CREATED', null, 'PREVIEWED', actor, { preview_hash:inserted.data.preview_hash, recommended_bid:recommendation, owner_desired_bid:desired, external_execution_locked:metadata.external_execution_locked, execution_phase:snapshot.execution_phase||'12-7' });
  return { request:inserted.data, reused:false };
}
