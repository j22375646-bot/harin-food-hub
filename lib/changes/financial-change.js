...
async function normalizeRequest(input, db) {
  ...
  if (type === 'PRODUCT' || type === 'PRODUCT_COST') {
    ...
    return { changeType:'PRODUCT_COST', targetType:'product_costs', targetKey:id, platform:null, proposed:{ master_product_id:id, unit_cost:amount(input.unit_cost, '상품 원가'), packaging_cost:amount(input.packaging_cost, '포장비'), other_unit_cost:amount(input.other_unit_cost, '기타 단위비'), notes:cleanText(input.notes) }, metadata:{} };
  }
...
async function readCurrent(db, spec) {
  ...
  let query;
  if (changeType === 'PRODUCT_COST') query = db.from('product_costs').select('master_product_id,unit_cost,packaging_cost,other_unit_cost,notes').eq('master_product_id', targetKey).maybeSingle();
...
module.exports = { FinancialChangeError, approve, createPreview, execute, impactPreview, listRequests, normalizeRequest, reject, rollback, same, verify };
