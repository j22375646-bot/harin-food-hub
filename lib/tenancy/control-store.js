 ...
 * - Callers supply the restricted PostgreSQL/Supabase connection adapter.
...
const { resolveTenantContext, TenantContextError } = require('./context.js');
...
function safeError(error) {
  if (error instanceof TenantControlStoreError || error instanceof TenantContextError) throw error;
  fail('CONTROL_STORE_UNAVAILABLE', 503, 'Membership store is unavailable.');
}
...
function createTenantControlStore({ database, verifySession } = {}) {
  ...
  async function findMembership({ sessionCredential, tenantId } = {}) {
    ...
    try {
      ...
      return await readVerifiedMembership(session, tenantId);
    } catch (error) {
      safeError(error);
    }
  }

  async function readVerifiedMembership(session, tenantId) {
  ...
  // Server-only, per-request capability. Never accept a caller-provided identity
  // or role; never retain this context for later requests or queued writes.
  async function resolveContext({ sessionCredential, tenantId } = {}) {
    if (!validUuid(tenantId)) fail('TENANT_REQUIRED', 400, 'Tenant is required.');
    try {
      const session = await authenticate(sessionCredential);
      const membership = await readVerifiedMembership(session, tenantId);
      const checkedAt = await databaseClock();
      return await resolveTenantContext({session, requestedTenantId: tenantId}, {
        now: () => checkedAt,
  ...
  // Server-only picker data, not an authorization grant. Recheck membership
  // through findMembership/resolveTenantContext for each subsequent operation.
  async function listBusinesses({ sessionCredential } = {}) {
    try {
      const session = await authenticate(sessionCredential);
      const result = await database.query(`
        select t.id as tenant_id, t.display_name, m.role, m.version
        from moaon_control.memberships m
        join moaon_control.tenants t on t.id = m.tenant_id
        where m.user_id = $1::uuid and m.status = 'ACTIVE' and t.status = 'ACTIVE'
        order by t.id
        limit 201
      `, [session.userId]);
      ensureSessionFresh(session, await databaseClock());
      if (rows(result).length > 200) {
        fail('BUSINESS_LIST_LIMIT', 409, 'Business list exceeds supported size.');
      }
