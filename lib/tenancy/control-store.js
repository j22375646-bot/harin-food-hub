...
function createTenantControlStore({ database, verifySession } = {}) {
  ...
  async function lockActiveTenant(client, tenantId) {
    const result = await client.query(`
      ...
      for update
    `, [tenantId]);
    if (rows(result)[0]?.status !== 'ACTIVE') {
  ...
  async function createInvitation({
    ...
    try {
      ...
      const created = await database.transaction(async client => {
        await lockActiveTenant(client, tenantId);
    ...
    try {
      ...
      return await database.transaction(async client => {
        const tenant = await client.query(`
          ...
          for update
        `, [tenantId]);
        if (rows(tenant)[0]?.status !== 'ACTIVE') invalidInvitation();
