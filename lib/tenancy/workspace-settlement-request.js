...
const {buildWorkspaceSettlementSummary}=require('./workspace-settlement-summary.js');
const PATH=/^\/api\/moaon\/businesses\/([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\/settlement$/;
const HEADERS={'cache-control':'private, no-store',vary:'Cookie','x-content-type-options':'nosniff'};
const failure=(status,code)=>Response.json({ok:false,code},{status,headers:HEADERS});
function guardWorkspaceSettlementRequest(request){
 const url=new URL(request.url),match=PATH.exec(url.pathname);
 if(!match||!['','?days=7','?days=30','?days=90'].includes(url.search))return {response:failure(400,'INVALID_REQUEST')};
 const days=url.search?Number(url.searchParams.get('days')):30;
 url.search='';
 const guarded=guardBusinessListRequest({url:url.href,method:request.method,headers:request.headers});
 return guarded.response?guarded:{...guarded,tenantId:match[1],days};
}
function createWorkspaceSettlementRequest({resolveContext,readSettlement,timeoutMs=25000}={}){
 if(typeof resolveContext!=='function'||typeof readSettlement!=='function')throw TypeError('Trusted dependencies required');
 if(!Number.isInteger(timeoutMs)||timeoutMs<1||timeoutMs>30000)throw TypeError('Invalid deadline');
 return async request=>{
  const input=guardWorkspaceSettlementRequest(request);if(input.response)return input.response;
  ...
  try{return await Promise.race([
   (async()=>{
    ...
    const data=await readSettlement({days:input.days});current();
    ...
    return Response.json({ok:true,...buildWorkspaceSettlementSummary(data,input.days)},{headers:HEADERS});
   })(),
...
module.exports={createWorkspaceSettlementRequest,guardWorkspaceSettlementRequest};
