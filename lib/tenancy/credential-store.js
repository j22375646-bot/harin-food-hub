...
function createCredentialStore({database,cipher,verifySession}={}){
 if(typeof database?.transaction!=='function'||typeof cipher?.seal!=='function'||typeof verifySession!=='function')throw TypeError('Trusted credential adapters required');
 async function identity(credential,tx){
  ...
  if(!session||typeof session.id!=='string'||!session.id||typeof session.userId!=='string'||!UUID.test(session.userId)||!Number.isFinite(Date.parse(session.expiresAt))||!Number.isFinite(new Date(now).getTime())||Date.parse(session.expiresAt)<=new Date(now).getTime())fail('CREDENTIAL_ACCESS_DENIED');
  return {id:session.id,userId:session.userId.toLowerCase()};
 }
 return Object.freeze({async save(credential,input){
  try{
   ...
   return await database.transaction(async tx=>{
    ...
    await tx.query(`insert into moaon_control.provider_credentials(tenant_id,provider,revision,envelope,updated_by)
     values($1,$2,$3,$4::jsonb,$5) on conflict(tenant_id,provider) do update set revision=excluded.revision,envelope=excluded.envelope,updated_by=excluded.updated_by,updated_at=clock_timestamp()`,[tenantId,provider,revision,JSON.stringify(envelope),actor.userId]);
  ...
  }catch(error){if(error instanceof CredentialStoreError)throw error;throw new CredentialStoreError('CREDENTIAL_STORAGE_UNAVAILABLE');}
 }});
}
