...
function createDeadline(timeoutMs, now) {
  ...
  function checkpoint() {
    ...
    if (closed || value < lastObservedAt || value >= expiresAt) throw unavailable();
    lastObservedAt = value;
    return value;
  }
  function remaining() {
    return Math.max(1, Math.ceil(expiresAt - checkpoint()));
  }
  return Object.freeze({
    checkpoint,
...
function createStepUpStorage(configuration) {
  ...
  async function issue(candidate) {
    ...
    try {
      ...
      const beginNow = deadline.checkpoint();
      ...
      const providerNow = deadline.checkpoint();
      ...
      if (deadline.checkpoint() >= beginExpiresMs) throw unavailable();
      ...
      const proof = validateProof(committed, {userId: input.userId, sessionId: input.sessionId}, deadline.checkpoint());
  ...
  async function read(identityCandidate, includeSession) {
    ...
    try {
      ...
      return readResult(value, identity, includeSession, deadline.checkpoint());
    } finally { deadline.close(); }
  }
