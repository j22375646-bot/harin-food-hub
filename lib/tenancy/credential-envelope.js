...
function binding(scope,keyId){
 if(!exact(scope,['tenantId','provider','revision'])||typeof scope.tenantId!=='string'||!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(scope.tenantId)||!['NAVER','CAFE24','COUPANG','EPOST','CLOVA','GEMINI','OPENAI'].includes(scope.provider)||!Number.isSafeInteger(scope.revision)||scope.revision<1)throw unavailable();
