  ...
  return async function handleRecoveryReviewRequest(request) {
    ...
    try {
      ...
      function sessionCheckpoint() {
        deadline.checkpoint();
        if (readClock(now) >= Math.min(initialSessionExpiry, currentSessionExpiry)) {
          fail(401, 'AUTH_REQUIRED');
        }
      }
      sessionCheckpoint();
      const rawProof = await deadline.run(() => verifyStepUp(Object.freeze({
        userId: trusted.userId,
        sessionId: trusted.id,
      })));
      const trustedProof = stepUpSnapshot(rawProof, trusted, now);
      function securityCheckpoint() {
        sessionCheckpoint();
        const current = readClock(now);
        const verifiedAt = canonicalUtcMilliseconds(trustedProof.verifiedAt);
        const expiresAt = canonicalUtcMilliseconds(trustedProof.expiresAt);
      ...
      const fresh = identitySnapshot(reverified, now);
