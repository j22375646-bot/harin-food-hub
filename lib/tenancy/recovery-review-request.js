...
const {createRecoveryReviewAdmission} = require('./recovery-review-admission.js');
const {DashboardIdentityError} = require('./dashboard-identity.js');
const {StepUpStorageError} = require('./step-up-storage.js');
...
function exactOptions(value) {
  ...
  const allowed = ['verifySession', 'verifyStepUp', 'rpcClient', 'allowedOrigin', 'timeoutMs', 'now'];
  let keys;
  try { keys = Reflect.ownKeys(value); } catch (_error) { return null; }
  if (keys.some(key => typeof key !== 'string' || !allowed.includes(key))
    || !['verifySession', 'verifyStepUp', 'rpcClient', 'allowedOrigin'].every(key => keys.includes(key))) return null;
  const result = {};
...
function createDeadline(signal, timeoutMs, now) {
  ...
  function read() {
    ...
    if (current < lastObservedAt || current >= expiresAt) {
      stop();
      throw unavailable();
    }
  ...
  return Object.freeze({
    ...
    async run(work) {
      checkpoint();
      let pending;
      try { pending = work(); } catch (error) { throw error; }
      const result = await Promise.race([Promise.resolve(pending), stoppedPromise]);
      checkpoint();
      return result;
    },
    close() {
...
function stepUpSnapshot(value, trusted, now) {
  if (value === null || value === undefined || value === false) fail(403, 'STEP_UP_REQUIRED');
  let copy;
  try { copy = exactBody(value, ['userId', 'sessionId', 'method', 'verifiedAt', 'expiresAt']); }
  catch (_error) { throw unavailable(); }
  if (!copy) throw unavailable();
  const userId = normalizedUuid(copy.userId);
  const sessionId = normalizedUuid(copy.sessionId);
  if (!userId || !sessionId || typeof copy.method !== 'string') throw unavailable();
  const verifiedAt = canonicalUtcMilliseconds(copy.verifiedAt);
  const expiresAt = canonicalUtcMilliseconds(copy.expiresAt);
  if (!Number.isFinite(verifiedAt) || !Number.isFinite(expiresAt)
    || expiresAt <= verifiedAt || expiresAt - verifiedAt > 300_000) throw unavailable();
  if (userId !== trusted.userId || sessionId !== trusted.id || copy.method !== 'mfa') {
    fail(403, 'STEP_UP_REQUIRED');
  }
  const current = readClock(now);
  if (verifiedAt > current || expiresAt <= current || current - verifiedAt >= 300_000) {
    fail(403, 'STEP_UP_REQUIRED');
  }
  return Object.freeze({
    userId,
...
async function verifyTrustedStepUp(deadline, verifyStepUp, trusted, now) {
  let rawProof;
  try {
    rawProof = await deadline.run(() => verifyStepUp(Object.freeze({
      userId: trusted.userId,
      sessionId: trusted.id,
    })));
  } catch (error) {
    if (error instanceof StepUpStorageError && error.code === 'STEP_UP_REQUIRED') {
      fail(403, 'STEP_UP_REQUIRED');
    }
    throw error;
  }
  return stepUpSnapshot(rawProof, trusted, now);
}

function createRecoveryReviewRequestHandler(options) {
  const config = exactOptions(options);
  let rpc;
  try { rpc = config?.rpcClient?.rpc; } catch (_error) {}
  if (typeof window !== 'undefined' || typeof document !== 'undefined'
    || !config || typeof config.verifySession !== 'function'
    || typeof config.verifyStepUp !== 'function'
    || !config.rpcClient || typeof rpc !== 'function'
  ...
  const verifyStepUp = config.verifyStepUp;
  const invokeRpc = rpc.bind(config.rpcClient);
  ...
  return async function handleRecoveryReviewRequest(request) {
    ...
    try {
      ...
      const trusted = identitySnapshot(verified, deadline.read);
      const initialSessionExpiry = canonicalUtcMilliseconds(trusted.expiresAt);
      let currentSessionExpiry = initialSessionExpiry;
      function sessionCheckpoint() {
        const current = deadline.read();
        if (current >= Math.min(initialSessionExpiry, currentSessionExpiry)) {
          fail(401, 'AUTH_REQUIRED');
        }
        return current;
      }
      sessionCheckpoint();
      const trustedProof = await verifyTrustedStepUp(deadline, verifyStepUp, trusted, deadline.read);
      let freshProof;
      function securityCheckpoint() {
        const current = sessionCheckpoint();
        for (const candidate of freshProof ? [trustedProof, freshProof] : [trustedProof]) {
          const verifiedAt = canonicalUtcMilliseconds(candidate.verifiedAt);
          const expiresAt = canonicalUtcMilliseconds(candidate.expiresAt);
          if (verifiedAt > current || current >= expiresAt || current - verifiedAt >= 300_000) {
            fail(403, 'STEP_UP_REQUIRED');
          }
        }
      }
      securityCheckpoint();
      const requestRpcClient = Object.freeze({
        rpc(name, args) {
          try { securityCheckpoint(); }
          catch (error) {
            if (error instanceof RequestFailure) checkpointFailure = error;
            throw error;
          }
          return invokeRpc(name, args);
        },
      });
      const admission = createRecoveryReviewAdmission({rpcClient: requestRpcClient,
        timeoutMs: deadline.checkpoint()});
      const decision = await deadline.run(() => admission({
        operatorId: trusted.userId,
        sessionId: trusted.id,
        mode: body.mode,
      }));
      if (!decision.allowed) fail(429, 'RECOVERY_REVIEW_RATE_LIMITED');

      securityCheckpoint();
      let reverified;
      try { reverified = await deadline.run(() => verifySession(credential)); }
      catch (error) {
        if (error instanceof DashboardIdentityError && error.code === 'AUTH_REQUIRED') fail(401, 'AUTH_REQUIRED');
        throw error;
      }
      const fresh = identitySnapshot(reverified, deadline.read);
      if (fresh.id !== trusted.id || fresh.userId !== trusted.userId || fresh.email !== trusted.email
        || fresh.emailVerified !== trusted.emailVerified) fail(401, 'AUTH_REQUIRED');
      currentSessionExpiry = canonicalUtcMilliseconds(fresh.expiresAt);
      securityCheckpoint();
      freshProof = await verifyTrustedStepUp(deadline, verifyStepUp, fresh, deadline.read);
      securityCheckpoint();

      const resolver = createRecoveryReviewResolver({rpcClient: requestRpcClient, operatorId: trusted.userId,
        timeoutMs: deadline.checkpoint()});
      ...
      const failure = checkpointFailure || (error instanceof RequestFailure ? error : null);
      if (failure) {
        const headers = failure.status === 429 ? {'retry-after': '60'} : undefined;
        return errorResponse(failure.status, failure.code, headers);
      }
