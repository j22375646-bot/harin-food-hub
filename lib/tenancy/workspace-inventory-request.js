...
const PATH=/^\/api\/moaon\/businesses\/([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\/inventory$/;
const HEADERS={'cache-control':'private, no-store',vary:'Cookie','x-content-type-options':'nosniff'};
const failure=(status,code)=>Response.json({ok:false,code},{status,headers:HEADERS});
function guardWorkspaceInventoryRequest(request){
...
function createWorkspaceInventoryRequest({resolveContext,readInventory,timeoutMs=25000}={}){
 if(typeof resolveContext!=='function'||typeof readInventory!=='function')throw TypeError('Trusted dependencies required');
 if(!Number.isInteger(timeoutMs)||timeoutMs<1||timeoutMs>30000)throw TypeError('Invalid deadline');
 return async request=>{
  const input=guardWorkspaceInventoryRequest(request);if(input.response)return input.response;
  ...
  try{return await Promise.race([
   (async()=>{
    ...
    const data=await readInventory();current();
...
module.exports={createWorkspaceInventoryRequest,guardWorkspaceInventoryRequest};
