...
function createCredentialRequestAdmission({rpcClient,hmacKey,trustedClientIp,verifySession,timeoutMs=10000}={}) {
 ...
 return async (credential,{signal,deadline}={})=>boundedAdmission(async (check,childSignal,childDeadline)=>{
  ...
  const session=await verifySession(credential,{signal:childSignal,deadline:childDeadline});check();
