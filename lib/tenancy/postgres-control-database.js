...
const ROLE_VALIDATION_SQL = `
  select
    ...
    exists (
      select 1 from pg_namespace n
      where n.nspname = 'moaon_auth' and n.nspowner = r.oid
    ) as owns_auth_schema,
    exists (
      select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace
      where n.nspname = 'moaon_auth' and c.relowner = r.oid
    ) as owns_auth_tables,
    exists (
      select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace
      where n.nspname = 'public' and c.relname in ('dashboard_users', 'dashboard_sessions')
        and c.relowner = r.oid
    ) as owns_protected_public_tables,
    exists (
...
function roleIsSafe(row) {
  return Boolean(row)
    ...
    && row.owns_auth_schema === false
    && row.owns_auth_tables === false
    && row.owns_protected_public_tables === false
