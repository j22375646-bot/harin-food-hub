...
function createSupabaseRecoveryProvider({url,publishableKey,serviceRoleKey,callbackUrl,fetch,timeoutMs=10_000,now=Date.now,allowInsecureLoopback=false}={}){
  ...
  function sdkClient(key){
    ...
    const abortableFetch=async(input,init={})=>{try{return await fetch(input,{...init,signal:state.signal||init.signal});}catch{return new Response(JSON.stringify({code:'recovery_transport_failed',message:'Recovery transport failed.'}),{status:503,headers:{'content-type':'application/json','x-supabase-api-version':'2024-01-01'}});}};
  ...
  async function bounded(run,mode='unavailable'){
  ...
  async function verify(type,tokenHash){
    ...
    try{
      const result=await bounded(signal=>{scoped.setSignal(signal);return client.auth.verifyOtp({token_hash:tokenHash,type});},'rejected');
      ...
      ownsSession=false;
      return {client,setSignal:scoped.setSignal,accessToken:result.data.session.access_token,identity:current};
    }finally{
      if(ownsSession){
        await bounded(signal=>{scoped.setSignal(signal);return client.auth.signOut({scope:'local'});}).catch(()=>{});
  ...
  return Object.freeze({
    ...
    async confirmEmail(tokenHash){
      ...
      finally{ if(state){await bounded(signal=>{state.setSignal(signal);return state.client.auth.signOut({scope:'local'});}).catch(()=>{});state.client.auth.stopAutoRefresh();} }
    },
    async openRecovery(tokenHash){
      const state=await verify('recovery',tokenHash); let disposed=false,global=false;
      return Object.freeze({
        identity:state.identity,
        async updatePassword(password){ if(disposed)unavailable(); const r=await bounded(signal=>{state.setSignal(signal);return state.client.auth.updateUser({password});}); if(r?.error)unavailable(); const changed=identity(r?.data?.user,currentTime(now)); if(!same(changed,state.identity))rejected(); return state.identity; },
        async signOutGlobal(){ if(disposed)unavailable(); const scoped=sdkClient(serviceRoleKey); const r=await bounded(signal=>{scoped.setSignal(signal);return scoped.client.auth.admin.signOut(state.accessToken,'global');}); if(r?.error)unavailable(); global=true; },
        async currentIdentity(){ if(disposed)unavailable(); return authoritative(state.identity.id); },
        async dispose(){ if(disposed)return; disposed=true; if(!global)await bounded(signal=>{state.setSignal(signal);return state.client.auth.signOut({scope:'local'});}).catch(()=>{}); state.client.auth.stopAutoRefresh(); },
      });
    },
  });
}
