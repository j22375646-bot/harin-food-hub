...
function readControlDatabaseConfig(env = process.env) {
  ...
  const ca = env.MOAON_CONTROL_DB_CA;
  if (ca !== undefined) {
    if (typeof ca !== 'string' || ca.length > 16384
      || !/^-----BEGIN CERTIFICATE-----\r?\n[A-Za-z0-9+/=\r\n]+\r?\n-----END CERTIFICATE-----\s*$/.test(ca)) throw invalid();
    try { if (!new X509Certificate(ca).ca) throw invalid(); }
    catch { throw invalid(); }
  }
  ...
  return Object.freeze({
    connection: Object.freeze({host, port: Number(port), database, password,
      ...
      ssl: Object.freeze({rejectUnauthorized: true, ...(ca === undefined ? {} : {ca})})}),
    maxConnections: pooler ? 1 : 2,
    connectionTimeoutMillis: 5000,
  });
}

function createConfiguredControlDatabase(env = process.env, {diagnostic} = {}) {
