...
function validateClaims(value,{url,userId,sessionId,currentMs,renewed=false,challengeStartedSeconds}){
  ...
  if(typeof iss!=='string'||typeof aud!=='string'||typeof role!=='string'||typeof sub!=='string'||typeof providerSessionId!=='string'||typeof isAnonymous!=='boolean'||!validSecond(iat)||!validSecond(exp)||!Number.isSafeInteger(iat*1000)||!Number.isSafeInteger(exp*1000)||(nbf!==undefined&&(!validSecond(nbf)||!Number.isSafeInteger(nbf*1000))))throw unavailable();
  if(iss!==`${url}/auth/v1`||aud!=='authenticated'||role!=='authenticated'||sub!==userId||!UUID.test(providerSessionId)||isAnonymous!==false||exp<=iat||iat*1000>currentMs||(nbf!==undefined&&nbf*1000>currentMs)||exp*1000<=currentMs+MIN_TOKEN_LIFETIME_MS)throw rejected();
...
function createSupabaseStepUpProvider(configuration){
  ...
  async function verifyTotp(candidate){
    ...
    if(!UUID.test(input.userId)||!UUID.test(input.factorId)||!strictString(input.accessToken,MAX_TOKEN)||!strictString(input.refreshToken,MAX_TOKEN)||typeof input.code!=='string'||!/^\d{6}$/.test(input.code))throw new TypeError('Exact TOTP verification input is required.');
