...
function createCredentialSaveRequest({origin,save=null,admit=null,admissionTimeoutMs=10000,maxConcurrent=8}={}){
 ...
 return async request=>{
  ...
  try{
   ...
   const admissionDeadline=performance.now()+admissionTimeoutMs;
   let decision;try{decision=await boundedAdmission(async(check,signal,deadline)=>{check();const result=await admit(request,credential,{signal,deadline});check();return result;},{signal:request.signal,timeoutMs:admissionTimeoutMs,deadline:admissionDeadline});}catch{return failure(503,'CREDENTIAL_ADMISSION_UNAVAILABLE');}
   ...
   if(request.signal?.aborted||performance.now()>=admissionDeadline)return failure(503,'CREDENTIAL_ADMISSION_UNAVAILABLE');
