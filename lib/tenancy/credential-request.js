'use strict';
const {boundedAdmission}=require('./credential-request-admission.js');
...
function createCredentialSaveRequest({origin,save=null,readMetadata=null,admit=null,admissionTimeoutMs=10000,maxConcurrent=8}={}){
 ...
 if(readMetadata!==null&&typeof readMetadata!=='function')throw TypeError('Trusted metadata reader required');
 if(admit!==null&&typeof admit!=='function'||!Number.isInteger(admissionTimeoutMs)||admissionTimeoutMs<1||admissionTimeoutMs>30000)throw TypeError('Bounded admission adapter required');
 let active=0;
 return async (request,options={})=>{
  ...
  try{
   ...
   if(request.method!=='POST'&&!reading)return failure(405,'METHOD_NOT_ALLOWED');
   ...
   if(!reading&&new URL(request.url).search)return failure(400,'INVALID_REQUEST');
   if(!reading&&!/^application\/json(?:\s*;\s*charset=utf-8)?$/i.test(request.headers.get('content-type')||''))return failure(415,'INVALID_CONTENT_TYPE');
   ...
   if(reading){
    ...
    if(request.body||query.size!==2||query.getAll('tenantId').length!==1||query.getAll('provider').length!==1||!UUID.test(query.get('tenantId')||'')||!Object.hasOwn(schemas,query.get('provider')||''))return failure(400,'INVALID_REQUEST');
    input={tenantId:query.get('tenantId'),provider:query.get('provider')};
   }else{
    try{input=await readBody(request);}catch{return failure(400,'INVALID_REQUEST');}
    if(!valid(input))return failure(400,'CREDENTIAL_INVALID');
   }
   if(!(reading?readMetadata:save)||!admit)return failure(503,'SETUP_REQUIRED');
   const admissionDeadline=Math.min(performance.now()+admissionTimeoutMs,options.deadline??Infinity);
   const signal=options.signal??request.signal;
   let decision;try{decision=await boundedAdmission(async(check,signal,deadline)=>{check();const result=await admit(request,credential,{signal,deadline});check();return result;},{signal,timeoutMs:admissionTimeoutMs,deadline:admissionDeadline});}catch{return failure(503,'CREDENTIAL_ADMISSION_UNAVAILABLE');}
   if(!decision||typeof decision.allowed!=='boolean')return failure(503,'CREDENTIAL_ADMISSION_UNAVAILABLE');
   if(!decision.allowed)return failure(429,'CREDENTIAL_RATE_LIMITED');
   if(request.signal?.aborted||signal?.aborted||performance.now()>=admissionDeadline)return failure(503,'CREDENTIAL_ADMISSION_UNAVAILABLE');
   if(reading){
    const result=await readMetadata(credential,input,{signal,deadline:admissionDeadline});
    if(request.signal?.aborted||signal?.aborted||performance.now()>=admissionDeadline)throw Error('Read deadline');
    if(result?.tenantId!==input.tenantId.toLowerCase()||result.provider!==input.provider||!Number.isInteger(result.revision)||result.revision<0||result.revision>2147483647||result.status!==(result.revision===0?'NOT_SAVED':'SAVED_UNVERIFIED'))throw Error('Invalid metadata');
   ...
   writeStarted=true;
   const result=await save(credential,input,{signal,deadline:admissionDeadline});
