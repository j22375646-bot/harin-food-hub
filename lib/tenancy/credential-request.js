...
async function readBody(request){
 if(!request.body)throw Error('Invalid body');const reader=request.body.getReader();let size=0,timer;
 try{
  const chunks=[];const deadline=new Promise((_,reject)=>{timer=setTimeout(()=>{reader.cancel().catch(()=>{});reject(Error('Body timeout'));},5000);});
  while(true){request.signal?.throwIfAborted();const item=await Promise.race([reader.read(),deadline]);if(item.done)break;size+=item.value.byteLength;if(size>16384){await reader.cancel();throw Error('Body limit');}chunks.push(item.value);}
  return JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(Buffer.concat(chunks)));
 }finally{clearTimeout(timer);reader.releaseLock();}
}
function createCredentialSaveRequest({origin,save=null}={}){
 ...
 return async request=>{
  ...
  }catch(error){const codes={CREDENTIAL_ACCESS_DENIED:403,CREDENTIAL_CONFLICT:409,CREDENTIAL_INVALID:400};return Object.hasOwn(codes,error?.code||'')?failure(codes[error.code],error.code):failure(503,'CREDENTIAL_STORAGE_UNAVAILABLE');}
 };
}
