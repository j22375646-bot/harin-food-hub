...
function createAccountRecovery({provider,profiles,sessionStore,reviewStore,requestLimit,randomUUID,diagnostic=()=>{},timeoutMs=10_000,now=Date.now}={}){
  const explicitReview=reviewStore!==undefined;
  if(typeof window!=='undefined'||typeof document!=='undefined'||!provider||!['requestRecoveryEmail','confirmEmail','openRecovery'].every(k=>typeof provider[k]==='function')
    ...
    ||(explicitReview&&(!reviewStore||!['start','markRequired','complete','reject','list'].every(k=>typeof reviewStore[k]==='function')))
  ...
  function reviewDiagnostic(){try{diagnostic(Object.freeze({event:'RECOVERY_REVIEW_RECORD_FAILED'}));}catch{}}
  async function reviewMutation(method,args,state){
    let value;try{value=await step(()=>reviewStore[method](args),state);}catch{throw unavailable();}
    if(value!==true)throw unavailable();return true;
  }
  async function requireReview(userId,operationId,stage){
    if(explicitReview){
      try{await reviewMutation('markRequired',{userId,operationId,stage});}catch{reviewDiagnostic();}
    }
    return Object.freeze({status:'REVIEW_REQUIRED'});
  }
  return Object.freeze({
    ...
    async completeRecovery({tokenHash,newPassword}={}){
      ...
      const state={cancelled:false};let handle=null,fenceRisk=false,operationId,stage;
      try{
        ...
        if(explicitReview)await reviewMutation('start',{userId:handle.identity.id,operationId},state);
        stage='FENCE_BEGIN';
        fenceRisk=true;
        try{
          const began=await step(()=>sessionStore.beginPasswordChange({userId:handle.identity.id,operationId}),state);
          if(explicitReview&&began!==true)throw unavailable();
        }
        catch(error){
          if(error?.code==='AUTH_TRANSITION_REJECTED'){
            fenceRisk=false;
            if(explicitReview){try{await reviewMutation('reject',{userId:handle.identity.id,operationId});}catch{reviewDiagnostic();}}
            throw rejected();
          }
          throw error;
        }
        ...
        stage='FENCE_COMPLETE';
        const completed=await step(()=>sessionStore.completePasswordChange({userId:current.id,operationId}),state);
        if(explicitReview&&completed!==true)throw unavailable();
        if(explicitReview)await reviewMutation('complete',{userId:current.id,operationId},state);
        return Object.freeze({status:'COMPLETED',requiresFreshLogin:true});
      }catch(error){
        if(fenceRisk)return requireReview(handle.identity.id,operationId,stage);
