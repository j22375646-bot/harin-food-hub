...
// Undefined preserves the earlier candidate-store call shape. The composed
// fenced dashboard login always supplies the complete verified snapshot.
function expectedProfileArgs(value) {
  if (value === undefined) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)
    || typeof value.email !== 'string' || value.email !== value.email.trim().toLowerCase()
    || value.email.length > 320 || !EMAIL.test(value.email)
    || typeof value.username !== 'string' || !USERNAME.test(value.username)
    || typeof value.displayName !== 'string' || !ROLES.has(value.role)) {
    throw new TypeError('A complete verified profile snapshot is required.');
  }
  return {
    p_expected_email:value.email,
    p_expected_username:value.username,
    p_expected_display_name:value.displayName,
    p_expected_role:value.role,
  };
}
...
function sessionWindow(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    || !Object.hasOwn(value,'issuedAt') || !Object.hasOwn(value,'expiresAt')) {
    throw new AuthSessionStoreError();
  }
  const {issuedAt,expiresAt}=value;
  if (typeof issuedAt !== 'string' || typeof expiresAt !== 'string'
    || !UTC_MILLISECONDS.test(issuedAt) || !UTC_MILLISECONDS.test(expiresAt)) {
    throw new AuthSessionStoreError();
  }
  ...
  if (!Number.isFinite(issued) || !Number.isFinite(expires)
    ...
    || expires-issued !== SESSION_LIFETIME_MS) {
    throw new AuthSessionStoreError();
  }
...
function createAuthSessionStore({rpcClient, timeoutMs = 10000} = {}) {
  ...
  return Object.freeze({
    ...
    async getSessionWindow({userId, ticketId}) {
      const args={p_user_id:uuid(userId),p_ticket_id:uuid(ticketId)};
      let timer;
      try {
        const result=await Promise.race([
          Promise.resolve().then(()=>rpc('moaon_get_session_window',args)),
          new Promise((_,reject)=>{timer=setTimeout(()=>reject(new AuthSessionStoreError()),timeoutMs);}),
        ]);
        if (result?.error?.code === 'P0001' && result.error.message === 'AUTH_TRANSITION_REJECTED') {
          throw new AuthSessionStoreError('AUTH_TRANSITION_REJECTED');
        }
        if (result?.error) throw new AuthSessionStoreError();
        return sessionWindow(result?.data);
      } catch (error) {
        if (error instanceof AuthSessionStoreError) throw error;
        throw new AuthSessionStoreError();
      } finally { clearTimeout(timer); }
    },
    async issueSession({userId, ticketId, sessionId, tokenHash, expiresAt, expectedProfile}) {
      ...
      const expected=expectedProfileArgs(expectedProfile);
