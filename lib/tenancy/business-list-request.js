...
function guardBusinessListRequest(request){
 if(request.method!=='GET')return {response:failure(405,'METHOD_NOT_ALLOWED')};
 const url=new URL(request.url),origin=request.headers.get('origin');
 if((origin&&origin!==url.origin)||request.headers.get('sec-fetch-site')==='cross-site')return {response:failure(403,'ORIGIN_DENIED')};
 if(url.search)return {response:failure(400,'INVALID_REQUEST')};
 const sessionCredential=credential(request);
 if(!sessionCredential)return {response:failure(401,'AUTH_REQUIRED')};
...
function createBusinessListRequest({listBusinesses=null,timeoutMs=10000}={}){
 ...
 try{
   ...
   const {sessionCredential}=guarded;
...
module.exports={createBusinessListRequest,guardBusinessListRequest};
