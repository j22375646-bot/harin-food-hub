...
function sessionSnapshot(value, now) {
  ...
  const expiry = ISO_TIMESTAMP_PATTERN.test(value.expiresAt) ? Date.parse(value.expiresAt) : Number.NaN;
...
function optionalTimestamp(value) {
  ...
  if (typeof value !== 'string' || !ISO_TIMESTAMP_PATTERN.test(value)) failAuthentication();
...
function createDashboardIdentityVerifier({
  ...
  return async function verifySession(opaqueCredential) {
    ...
    try {
      return await bounded(async () => {
        const firstResult = await validateSession(opaqueCredential, { db, touch: false });
        ...
        const secondResult = await validateSession(opaqueCredential, { db, touch: false });
