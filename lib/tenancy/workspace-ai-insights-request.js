...
function createWorkspaceAiInsightsRequest({resolveContext,createService,configuration=()=>({status:'DISABLED'}),timeoutMs=25000}={}){
 ...
 return async request=>{
  ...
  try{return await Promise.race([
   (async()=>{
    current();const before=await timed('clova.context',()=>resolveContext(guarded));current();assertLegacyHarinWorkspaceOwner(before,guarded.tenantId);
    const assertCurrentContext=async()=>{current();const after=await timed('clova.context',()=>resolveContext(guarded));current();assertLegacyHarinWorkspaceOwner(after,guarded.tenantId);if(['tenantId','userId','sessionId','role','membershipVersion'].some(k=>before[k]!==after[k]))fail('WORKSPACE_CHANGED');return true;};
    const context={...before,actorId:before.userId,verified:true};
    const service=await createService({assertCurrentContext,signal:controller.signal,deadlineAt});current();let result;
    if(request.method==='GET'){
     ...
     result={configuration:await (service.configuration||configuration)(),runs};current();
    }
