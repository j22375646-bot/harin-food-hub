...
function secretSource() {
  const value = process.env.DASHBOARD_SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!value) throw new Error('DASHBOARD_SESSION_SECRET or SUPABASE_SERVICE_ROLE_KEY is required');
  return value;
}
function signingKey(scope) {
  return crypto.createHash('sha256').update(`harin-dashboard-${scope}-v2\0${secretSource()}`).digest();
}
function signature(scope, value) {
  return crypto.createHmac('sha256', signingKey(scope)).update(String(value)).digest('base64url');
}
...
function parseSession(token, now = Date.now()) {
  ...
  if (!encoded || !provided || extra || !safeEqual(provided, signature('session', encoded))) return null;
  try {
...
function verifySession(token, now) { return Boolean(parseSession(token, now)); }
...
function sessionCookieOptions(maxAge = SESSION_HOURS * 60 * 60) {
...
async function validateSession(token, { db = supabaseModule.getSupabase(), touch = false } = {}) {
  const local = parseSession(token);
  if (!local) return null;
  const found = await db.from('dashboard_sessions')
  ...
  if (touch && Date.now() - new Date(row.last_seen_at).getTime() > 5 * 60 * 1000) {
    await db.from('dashboard_sessions').update({ last_seen_at:new Date().toISOString() }).eq('id', row.id).is('revoked_at', null);
  }
...
async function authenticateAccount({ account, password, ip, userAgent }, db = supabaseModule.getSupabase()) {
  ...
  const authResult = await client.auth.signInWithPassword({ email:loginEmail, password:String(password) });
...
function verifyFinancialTrust(token, now = Date.now()) {
  ...
  if (!payload || !provided || extra || !safeEqual(provided, signature('financial-trust', payload))) return null;
  try {
...
function verifyAiSnapshot(token, now = Date.now()) {
  const [payload, provided, extra] = String(token || '').split('.');
  if (!payload || !provided || extra || !safeEqual(provided, signature('ai-analysis', payload))) return null;
  try {
    const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
...
function verifyBidProposalSnapshot(token, now = Date.now()) {
  const [payload, provided, extra] = String(token || '').split('.');
  if (!payload || !provided || extra || !safeEqual(provided, signature('naver-bid-proposal', payload))) return null;
  try {
    const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
...
module.exports = {
  ...
  signAiSnapshot, verifyAiSnapshot, signBidProposalSnapshot, verifyBidProposalSnapshot,
