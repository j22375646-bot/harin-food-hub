...
const ACCOUNT_LIMIT_UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
function createSessionToken({ sessionId = crypto.randomUUID(), userId = crypto.randomUUID(), username = 'test-owner', displayName = '테스트 OWNER', role = 'OWNER', expiresAt } = {}, {issuedAt} = {}) {
  const now = issuedAt === undefined ? Date.now() : Date.parse(issuedAt);
  if (!Number.isFinite(now)) throw new TypeError('A valid session issue time is required.');
  const payload = {
    v:2,
    ...
    exp:Math.floor((expiresAt ? new Date(expiresAt).getTime() : now + SESSION_LIFETIME_MS) / 1000)
  };
...
function fencedLoginConfig(sessionFence, fenceTimeoutMs) {
  ...
  if (!sessionFence || typeof sessionFence.beginLogin !== 'function'
    || typeof sessionFence.getSessionWindow !== 'function' || typeof sessionFence.issueSession !== 'function') {
...
function requestLimitConfig(requestLimit, fenceTimeoutMs) {
  if (requestLimit === undefined) return null;
  if (typeof requestLimit !== 'function') throw new TypeError('An explicit server request limit is required.');
  return {requestLimit, timeoutMs:fenceTimeoutMs ?? LOGIN_FENCE_TIMEOUT_MS};
}
function accountLimitConfig(accountLimit, requestLimit, fenceTimeoutMs) {
  if (accountLimit === undefined) return null;
  if (typeof accountLimit !== 'function' || typeof requestLimit !== 'function') {
    throw new TypeError('Explicit server request and account limits are required.');
  }
  return {accountLimit, timeoutMs:fenceTimeoutMs ?? LOGIN_FENCE_TIMEOUT_MS};
}
function exactLimitResult(value) {
  return value && typeof value === 'object' && !Array.isArray(value)
    && Object.keys(value).length === 1 && typeof value.allowed === 'boolean'
    ? value.allowed : null;
}
function exactAccountLimitResult(value) {
  try {
    const keys=value && typeof value === 'object' && !Array.isArray(value)
      ? Reflect.ownKeys(value) : [];
    if (keys.length !== 1 || keys[0] !== 'allowed' || !Object.hasOwn(value, 'allowed')
      || Object.keys(value).length !== 1) return null;
    const allowed=value.allowed;
    return typeof allowed === 'boolean' ? allowed : null;
  } catch { return null; }
}
function validSessionWindow(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    || Object.keys(value).length !== 2
    || !Object.hasOwn(value,'issuedAt') || !Object.hasOwn(value,'expiresAt')) return null;
  const {issuedAt,expiresAt}=value;
  if (typeof issuedAt !== 'string' || typeof expiresAt !== 'string'
    || !UTC_MILLISECONDS.test(issuedAt) || !UTC_MILLISECONDS.test(expiresAt)) return null;
  const issued=Date.parse(issuedAt),expires=Date.parse(expiresAt);
  if (!Number.isFinite(issued) || !Number.isFinite(expires)
    || new Date(issued).toISOString() !== issuedAt || new Date(expires).toISOString() !== expiresAt
    || expires-issued !== SESSION_LIFETIME_MS) return null;
...
async function createDatabaseSession(profile, requestMeta = {}, db = supabaseModule.getSupabase(), {sessionFence, ticketId, fenceTimeoutMs} = {}) {
  ...
  const window=fence ? await boundedFencedDependency(async()=>{
    const value=await fence.sessionFence.getSessionWindow({userId:profile.user_id,ticketId});
    const valid=validSessionWindow(value);
    if (!valid) throw fencedLoginError();
    return valid;
  },fence.timeoutMs) : null;
  const sessionId = crypto.randomUUID();
  const expiresAt = window?.expiresAt || new Date(Date.now() + SESSION_LIFETIME_MS).toISOString();
  ...
  const token = createSessionToken({
    ...
    role:profile.role,
    expiresAt
  },window ? {issuedAt:window.issuedAt} : undefined);
  if (fence) {
    const session=parseSession(token);
    if (!session) throw fencedLoginError();
...
async function authenticateAccount({ account, password, ip, userAgent }, db = supabaseModule.getSupabase(), { authClient, sessionFence, fenceTimeoutMs, requestLimit, accountLimit } = {}) {
  const fence=fencedLoginConfig(sessionFence, fenceTimeoutMs);
  const limiter=requestLimitConfig(requestLimit, fenceTimeoutMs);
  const accountLimiter=accountLimitConfig(accountLimit, requestLimit, fenceTimeoutMs);
  const normalized = text(account).toLowerCase();
  if ((['president','vice-president'].includes(normalized)&&!/^\d{4}$/.test(String(password))) || !/^[a-z0-9][a-z0-9._@+-]{2,119}$/.test(normalized) || text(password).length < (['president','vice-president'].includes(normalized)?4:6) || String(password).length > 200) {
    throw Object.assign(new Error('계정 또는 비밀번호를 확인해주세요.'), { status:401, code:'INVALID_CREDENTIALS' });
  }
  if (limiter) {
    const answer=exactLimitResult(await boundedFencedDependency(
      () => limiter.requestLimit({kind:'LOGIN',subject:normalized}),
      limiter.timeoutMs
    ));
    if (answer === false) {
      throw Object.assign(new Error('로그인 시도가 잠시 차단되었습니다. 15분 뒤 다시 시도해주세요.'), {status:429,code:'LOGIN_RATE_LIMITED'});
    }
    if (answer !== true) throw fencedLoginError();
  }
  ...
  if (accountLimiter && profile?.active && profile.active !== true) throw fencedLoginError();
  if (accountLimiter && profile?.active === true) {
    const profileEmail=typeof profile.email === 'string' ? profile.email.trim().toLowerCase() : '';
    const selected=normalized.includes('@') ? profileEmail
      : typeof profile.username === 'string' ? profile.username.trim().toLowerCase() : '';
    if (typeof profile.user_id !== 'string' || !ACCOUNT_LIMIT_UUID.test(profile.user_id)
      || !AUTH_EMAIL.test(profileEmail)
      || selected !== normalized) throw fencedLoginError();
    const answer=exactAccountLimitResult(await boundedFencedDependency(
      () => accountLimiter.accountLimit({userId:profile.user_id}),
      accountLimiter.timeoutMs
    ));
    if (answer === false) {
      throw Object.assign(new Error('로그인 시도가 잠시 차단되었습니다. 15분 뒤 다시 시도해주세요.'), {status:429,code:'LOGIN_RATE_LIMITED'});
    }
  ...
  let authResult;
  try {
    authResult=await signInWithTimeout(client, { email:loginEmail, password:['president','vice-president'].includes(normalized)?`Moaon-PIN-v1:${normalized}:${String(password)}`:String(password) }, fence?.timeoutMs);
  } catch (error) {
  ...
  if (fence && authResult.error == null
    && (!authResult.data || typeof authResult.data !== 'object'
      || !authResult.data.user || typeof authResult.data.user !== 'object'
      || !authResult.data.session || typeof authResult.data.session !== 'object')) {
    throw fencedLoginError();
  }
  if (authResult.error) {
