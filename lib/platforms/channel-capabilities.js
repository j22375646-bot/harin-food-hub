...
function coupangChannel(syncs, counts = {}) {
  ...
  const csEvidence=newer(csSync,fullSync);
  const csMeta=csEvidence?.metadata||{};
  const failedCsDatasets=new Set((csMeta.errors||[]).map(item=>String(item.dataset||'').toLowerCase()));
  const csDatasetReady=datasets=>{
    if(!['SUCCESS','PARTIAL'].includes(csEvidence?.status))return false;
    if(datasets.some(dataset=>failedCsDatasets.has(dataset))||failedCsDatasets.has('sync')||failedCsDatasets.has('customer_service'))return false;
    // A partial run proves only the datasets with their own successful counts.
    return csEvidence.status==='SUCCESS'||datasets.every(dataset=>
      hasOwn(csMeta.counts,dataset)&&csMeta.counts[dataset]!=null&&Number.isFinite(Number(csMeta.counts[dataset]))
    );
  };
  const inquiryWarnings=csMeta.inquiryVerificationWarnings||csMeta.counts?.inquiryVerificationWarnings||[];
  const inquiryRead=csDatasetReady(['inquiries'])&&inquiryWarnings.length===0;
  const claimsRead=csDatasetReady(['returns','exchanges']);
  const csRead=inquiryRead&&claimsRead;
  const datasetReady = {
  ...
  const capabilities = capabilityRows(key => {
    const read = datasetReady[key] ? ready('서울 고정 IP 서버의 조회 기록이 있습니다.') : verify(
      key==='inquiries'?'최근 문의 수집과 처리 상태 재확인 결과를 확인하세요.':
      key==='claims'?'최근 취소·반품·교환 수집 결과를 확인하세요.':'고정 IP 전체 수집을 실행해 조회 권한을 확인하세요.'
    );
