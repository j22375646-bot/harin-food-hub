...
async function event(db,row){const result=await db.from('provider_request_runs').insert(row).select('id').single();if(result.error)throw result.error;return result.data;}
async function mark(db,id,row){const result=await db.from('provider_request_runs').update(row).eq('id',id);if(result.error)throw result.error;}
