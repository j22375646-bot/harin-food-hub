...
async function protectedRead({db,provider,requestInput={},execute,ttlMs=15*60*1000,cacheResponse=true,allowStaleFallback=true,killSwitchEnabled=true,missingFields=[],now=new Date()}={}){
  ...
  try{
    ...
    await mark(db,activeId,{status,response_summary:summary,quota_summary:quotaSummary(result),source_timestamp:source,finished_at:finished,expires_at:cacheResponse?expires:null,metadata:{provider_isolated:true,response_cached:Boolean(cacheResponse),contains_customer_data:false}});
    return {...result,runtime:runtime('LIVE',{id:activeId,source_timestamp:source,expires_at:cacheResponse?expires:null},{cached:false,deduplicated:false,staleFallback:false})};
  }catch(error){
