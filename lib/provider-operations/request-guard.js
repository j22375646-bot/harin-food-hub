...
function ledgerWarning(error){return String(error?.code||error?.message||'RUNTIME_LEDGER_UNAVAILABLE').replace(/\s+/g,' ').slice(0,120);}
...
async function protectedRead({db,provider,requestInput={},execute,ttlMs=15*60*1000,cacheResponse=true,allowStaleFallback=true,killSwitchEnabled=true,missingFields=[],now=new Date()}={}){
  ...
  if(cacheResponse){
    const cached=await latestFresh(db,provider,hash,now).catch(error=>{ledgerError=ledgerWarning(error);return null;});
    if(cached){
  ...
  const running=await latestRunning(db,provider,hash).catch(error=>{ledgerError=ledgerError||ledgerWarning(error);return null;});
  if(running){
  ...
  try{active=await event(db,{provider,request_hash:hash,status:'RUNNING',started_at:iso(now),metadata:{provider_isolated:true,response_cached:Boolean(cacheResponse)}});}
    ...
    if(String(error?.code||'')==='23505'){
      const duplicate=await latestRunning(db,provider,hash).catch(()=>null);
      if(duplicate)return {provider,status:'RUNNING',runtime:runtime('DEDUPLICATED',duplicate,{cached:false,deduplicated:true,staleFallback:false})};
    }
    // The runtime ledger is observability, not the provider itself. A missing
    // migration/returned row must never stop a safe read-only connector.
    ledgerError=ledgerWarning(error);
  }
  try{
    ...
    await mark(db,activeId,{status,response_summary:summary,quota_summary:quotaSummary(result),source_timestamp:source,finished_at:finished,expires_at:cacheResponse?expires:null,metadata:{provider_isolated:true,response_cached:Boolean(cacheResponse),contains_customer_data:false}}).catch(error=>{ledgerError=ledgerError||ledgerWarning(error);});
    return {...result,runtime:runtime('LIVE',{id:activeId,source_timestamp:source,expires_at:cacheResponse?expires:null},{cached:false,deduplicated:false,staleFallback:false,ledgerRecorded:Boolean(activeId&&!ledgerError),ledgerWarning:ledgerError})};
  }catch(error){
    const code=String(error?.code||'PROVIDER_READ_FAILED').slice(0,100);
    await mark(db,active?.id||null,{status:'FAILED',finished_at:iso(now),error_code:code,metadata:{provider_isolated:true,response_cached:false,contains_customer_data:false}}).catch(()=>{});
