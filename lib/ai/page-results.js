...
function latestByPage(rows = []) {
  ...
  for(const row of rows){
    ...
    if(ANALYSIS_TYPES[page]&&!map[page])map[page]=publicRecord(row);
  }
  return map;
}
...
async function saveServerPreview(db,{snapshot,result,actor='owner'}={}){
  ...
  const existing=await db.from('ai_analysis_results')
    .select('id,page_key,status,result_mode,data_status,period_label,formula_version,result,created_at,model,knowledge_versions')
    .eq('analysis_type',analysisType).eq('input_fingerprint',inputFingerprint)
    .eq('result_mode','SERVER_PREVIEW').order('created_at',{ascending:false}).limit(1).maybeSingle();
  ...
  }).select('id,page_key,status,result_mode,data_status,period_label,formula_version,result,created_at,model,knowledge_versions').single();
