...
const operatingRules=require('./operating-rules.js');
...
async function loadPhase28KnowledgeSnapshot({db,now=new Date(),configuration=openaiClient.configuration,contracts=analysisContracts.listContracts}={}){
  ...
  const [found,operatingRuleSet]=await Promise.all([
    db.from('ai_knowledge_documents').select(KNOWLEDGE_FIELDS).order('updated_at',{ascending:false}).limit(120),
    operatingRules.loadOperatingRuleSet(db,{historyLimit:20})
  ]);
  ...
  return {
    ...
    operatingRules:operatingRuleSet,
    guard:{
