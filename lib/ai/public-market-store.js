...
function createPublicMarketStore({db}) {
  ...
  return {
    ...
    async reserve({tenantId,requestId,accountId,fingerprint,requester,model}) {
      return value(db.rpc(model?'moaon_public_market_ai_model_reserve':'moaon_public_market_ai_reserve',{...(model?{p_model:checked(model,/^(gemini-3\.5-flash-lite|gemini-3\.8-flash)$/)}:{}),p_tenant_id:checked(tenantId,UUID),p_request_id:checked(requestId,UUID),
