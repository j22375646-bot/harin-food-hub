...
function buildAiPagePanels(input = {}) {
  ...
  const rocketGrowthInventory=Array.isArray(input.rocketGrowthInventory)?input.rocketGrowthInventory:[];
  const inventoryActions = rocketGrowthInventory.filter(item=>['OUT_OF_STOCK','CRITICAL','LOW'].includes(String(item.stock_status||'').toUpperCase())).length;
  ...
  const panels = {
    ...
    inventory:panel({
      id:'inventory', title:'로켓그로스 재고 위험 자동분석', summary:'쿠팡 로켓그로스의 품절과 과잉재고를 찾아 입고·판매촉진 순서를 제안할 자리입니다.',
      metricLabel:'지금 확인할 상품', metricValue:`${inventoryActions.toLocaleString('ko-KR')}개`,
      metricNumber:inventoryActions, period,
      sources:['로켓그로스 판매가능 수량','최근 30일 판매속도','로켓그로스 기준시각'], tasks:['품절 예상일 설명','과잉재고 구분','입고·촉진 순서 제안'],
      ready:healthy('COUPANG'), schedule:'매일 오전 7:10'
    }),
    settlement:panel({
