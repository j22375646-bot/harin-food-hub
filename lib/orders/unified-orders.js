...
function normalizeCafe24Orders(orders = [], items = []) {
  ...
  return orders.flatMap(order => {
    ...
    return [shippingWorkbench.enrichOrder({
      ...
      status, stage:stageFor('CAFE24', status), amount,
...
function buildUnifiedOrders(input = {}) {
  ...
  const historyOrders = [...cafe24, ...coupang, ...naver].sort((a, b) => String(b.orderedAt || '').localeCompare(String(a.orderedAt || '')));
  const asOf=input.asOf?new Date(input.asOf):new Date();
  const windowEndDate=koreaDate(asOf);
  const windowStart=new Date(`${windowEndDate}T00:00:00Z`);windowStart.setUTCDate(windowStart.getUTCDate()-30);
  const windowStartDate=windowStart.toISOString().slice(0,10);
  const orders=historyOrders.filter(order=>dateOnly(order.orderedAt)>=windowStartDate);
  const channels = [
...
module.exports = {
  ...
  isActiveClaimStatus, normalizeCafe24Orders, normalizeCoupangOrders, normalizeNaverOrders, stageFor
};
