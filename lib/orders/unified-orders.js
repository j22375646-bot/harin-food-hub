...
const coupangOperationQueue = require('../coupang/operation-queue.js');
const coupangMappers = require('../coupang/mappers.js');
const cafe24OrderOrigin = require('../cafe24/order-origin.js');
const koreaDateFormatter = new Intl.DateTimeFormat('en-CA', { timeZone:'Asia/Seoul', year:'numeric', month:'2-digit', day:'2-digit' });
const koreaOrderTimeFormatter = new Intl.DateTimeFormat('en-US',{
...
const CAFE24_STAGE = Object.freeze({
  ...
  N22:'WAITING_FOR_CARRIER', N30:'SHIPPING', N40:'DELIVERED', N50:'DELIVERED',
...
function koreaOrderTime(value) {
  ...
  const parts=Object.fromEntries(koreaOrderTimeFormatter.formatToParts(date).filter(part=>part.type!=='literal').map(part=>[part.type,part.value]));
...
function fulfillmentTiming(order, asOf=new Date(),calendar={}) {
  ...
  const active=['PAID','PREPARING','READY_TO_SHIP','WAITING_FOR_CARRIER'].includes(order.stage);
...
function hubOrderId(platform, externalOrderId, shipmentId='') {
  const prefix = { CAFE24:'C24', COUPANG:'CP', NAVER:'NV' }[platform] || 'ETC';
  const identity=platform==='COUPANG'&&shipmentId?JSON.stringify([platform,externalOrderId,shipmentId]):`${platform}:${externalOrderId}`;
  const digest = crypto.createHash('sha1').update(identity).digest('hex').slice(0, 8).toUpperCase();
...
function resolveOrderTarget(orders=[],id){
  const matches=orders.filter(order=>order.hubOrderId===id||order.legacyHubOrderId===id);
  if(matches.length>1||matches.some(order=>order.legacyHubOrderId===id&&order.legacyTargetAmbiguous))throw Object.assign(new Error('여러 배송묶음에 해당하는 주문입니다. 목록을 새로고침하고 배송묶음을 다시 선택하세요.'),{status:409,code:'AMBIGUOUS_ORDER_TARGET'});
...
function imageFromList(images) {
  if(!Array.isArray(images))return '';
  const ordered=[
    ...images.filter(image=>upper(image?.imageType)==='REPRESENTATION'),
    ...images.filter(image=>upper(image?.imageType)!=='REPRESENTATION')
  ];
  for(const image of ordered){
    const direct=[image?.url,image?.imageUrl].map(safeProductImage).find(Boolean);
    if(direct)return direct;
    const coupang=[image?.cdnPath,image?.vendorPath].map(safeCoupangCdnImage).find(Boolean);
...
function productImageFromRaw(raw = {}) {
  ...
  const directCoupang=[
    ...
    raw.image?.vendorPath,raw.images?.[0]?.vendorPath
  ].map(safeCoupangCdnImage).find(Boolean)||'';
  const lists=[raw.images,raw.itemImages,raw.productImages];
  for(const item of [...(Array.isArray(raw.items)?raw.items:[]),...(Array.isArray(raw.data?.items)?raw.data.items:[])]){
    lists.push(item?.images,item?.itemImages,item?.productImages);
  }
  return lists.map(imageFromList).find(Boolean)||directCoupang;
}

function coupangProductDetailCatalogRows(rawResponses = []) {
  const byVendorItem=new Map();
  for(const response of rawResponses||[]){
    for(const item of coupangMappers.mapProductDetailItems(response?.response_json||response)){
      if(!item.vendor_item_id||byVendorItem.has(String(item.vendor_item_id)))continue;
      byVendorItem.set(String(item.vendor_item_id),{
  ...
  return [...byVendorItem.values()];
}

async function coupangImageFallbackRows({ db, orderItems = [], imageCatalog = new Map() }) {
  const missingSellerIds=[...new Set(attachOrderImages(orderItems,'COUPANG','seller_product_id',imageCatalog)
    .filter(item=>!item.image_url)
    .map(item=>text(item.seller_product_id))
    .filter(Boolean))];
  if(!missingSellerIds.length)return [];
  const endpoints=missingSellerIds.map(id=>`/v2/providers/seller_api/apis/api/v1/marketplace/seller-products/${id}`);
  const rawDetails=await isolatedQuery(db.from('raw_api_responses')
    .select('endpoint,response_json,created_at')
    .eq('platform','COUPANG')
    .in('endpoint',endpoints)
    .gte('http_status',200)
    .lt('http_status',300)
    .order('created_at',{ascending:false})
    .limit(Math.min(300,Math.max(20,endpoints.length*3))));
  return coupangProductDetailCatalogRows(rawDetails.rows);
}
...
function attachOrderImages(rows = [], platform, externalField, catalog = new Map()) {
  return (rows||[]).map(row=>{
    ...
    const aliases=(platform==='COUPANG'?[row.vendor_item_id,raw.vendorItemId,row[externalField]]:[row[externalField]])
      .concat([row.product_id,row.original_product_id,row.seller_product_id,row.vendor_item_id,row.external_product_no,
...
function normalizeCafe24Orders(orders = [], items = []) {
  ...
  return orders.flatMap(order => {
    ...
    if(!cafe24OrderOrigin.isCafe24StorefrontOrder(order))return [];
    ...
    // Original actual payment fixes legacy default-zero columns and preserves
    // free orders. Nominal order price is never evidence of payment.
    const amount=[raw.actual_payment_amount,raw.actual_order_amount?.payment_amount,raw.payment_amount,order.paid_amount]
      .find(value=>value!==null&&value!==undefined&&String(value).trim()!==''&&Number.isFinite(Number(value))&&Number(value)>=0);
    ...
    return [shippingWorkbench.enrichOrder({
      hubOrderId:hubOrderId('CAFE24', externalOrderId), platform:'CAFE24', channelLabel:cafe24OrderOrigin.cafe24OrderChannelLabel(order),
      ...
      status, stage, amount:amount===undefined?null:Number(amount), invoiceNumber, deliveryCompanyCode,
...
function normalizedCoupangReceiver(receiver={}){
  return {
...
function successfulCoupangReceiverIndex(rows=[],openResult=coupangOperationQueue.open){
  const receivers=new Map();
  const newest=[...(rows||[])].sort((left,right)=>String(right.created_at||'').localeCompare(String(left.created_at||'')));
  for(const row of newest){
    if(upper(row.operation_type)!=='ORDER_DETAIL'||upper(row.target_type)!=='ORDER'||upper(row.status)!=='SUCCESS'||!row.result_json)continue;
    const shipmentId=text(row.target_id);
    if(!shipmentId||receivers.has(shipmentId))continue;
    try{
      const receiver=normalizedCoupangReceiver(openResult(row.result_json)?.order?.receiver||{});
      if(receiver.name&&receiver.contact&&receiver.address)receivers.set(shipmentId,receiver);
    }catch{}
  }
  return receivers;
}
...
function normalizeCoupangOrders(orders = [], items = [], returns = [], rgOrders = [], rgItems = [], orderDetailTerminals = []) {
  ...
  const terminalShipmentIds = new Set((orderDetailTerminals || [])
    ...
    .filter(Boolean));
  const receiverByShipment=successfulCoupangReceiverIndex(orderDetailTerminals);
  const seller = orders.filter(order => !rgOrderIds.has(text(order.order_id))).map(order => {
    ...
    return shippingWorkbench.enrichOrder({
      hubOrderId:hubOrderId('COUPANG', externalOrderId,text(order.shipment_box_id)), legacyHubOrderId:hubOrderId('COUPANG',externalOrderId), platform:'COUPANG', channelLabel:'쿠팡',
      externalOrderId, shipmentId:text(order.shipment_box_id), orderedAt:firstValue(order.ordered_at, order.paid_at),
      status, stage, amount:number(order.gross_amount), invoiceNumber:text(firstValue(order.raw_data?.invoiceNumber,order.raw_data?.invoice_number)), deliveryCompanyCode:text(firstValue(order.raw_data?.deliveryCompanyCode,order.raw_data?.delivery_company_code)), ...products, receiver:receiverByShipment.get(text(order.shipment_box_id))||undefined, cancelled, cancellationRequested,
...
function buildUnifiedOrders(input = {}) {
  ...
  const pendingIssueIds=new Set(input.pendingIssueIds||[]);
  const normalizedOrders=[...cafe24,...coupang,...naver];
  const legacyCounts=new Map();
  for(const order of normalizedOrders)if(order.legacyHubOrderId)legacyCounts.set(order.legacyHubOrderId,(legacyCounts.get(order.legacyHubOrderId)||0)+1);
  const historyOrders = normalizedOrders
    .map(order=>{
      const legacyId=legacyCounts.get(order.legacyHubOrderId)===1?order.legacyHubOrderId:null;
      const trackingCandidate=trackingStates[order.hubOrderId]||(legacyId?trackingStates[legacyId]:null)||null;
      const transfer=successfulTransfers.get(channelTransfer.successfulTransferKey(order.platform,order));
      const issue=successfulIssues.get(order.hubOrderId)||(legacyId?successfulIssues.get(legacyId):null);
      const legacyHistoryAmbiguous=Boolean(!legacyId&&order.legacyHubOrderId&&(successfulIssues.has(order.legacyHubOrderId)||trackingStates[order.legacyHubOrderId]));
      const legacyIssuePending=Boolean(order.legacyHubOrderId&&pendingIssueIds.has(order.legacyHubOrderId));
      const historyUnavailable=['CAFE24','COUPANG'].includes(order.platform)&&input.shippingHistoryReady===false;
      ...
      // A channel can mark an order shipped as soon as its invoice is uploaded.
      // Only a successful trace for the current invoice proves physical movement.
      // Channel completion/purchase confirmation is terminal evidence, however:
      // missing, failed, or older tracking must never reopen completed history.
      const tracking=trackingCandidate?.trackingNo&&text(trackingCandidate.trackingNo)!==invoiceNumber?null:trackingCandidate;
      const deliveryCode=tracking?.status==='SUCCESS'?tracking.statusCode:null;
      const trackedStage=order.cancelled
        ?'CANCELLED'
        :order.stage==='DELIVERED'||deliveryCode==='DELIVERED'
        ?'DELIVERED'
        :deliveryCode==='IN_TRANSIT'
          ?'SHIPPING'
          :registeredInvoice||['SHIPPING','DELIVERED','WAITING_FOR_CARRIER'].includes(order.stage)
            ?'WAITING_FOR_CARRIER'
            :order.stage;
      ...
      return {
        ...
        legacyHistoryAmbiguous,legacyTargetAmbiguous:legacyCounts.get(order.legacyHubOrderId)>1,
        shippingHistoryStatus:historyUnavailable?'CHECK_REQUIRED':'READY',
        shippingEligible:eligibility.ok&&!legacyHistoryAmbiguous&&!legacyIssuePending&&!historyUnavailable,shippingBlockedReason:historyUnavailable?'기존 송장 이력을 불러오지 못했습니다. 중복 발급 방지를 위해 이력 확인 후 다시 시도하세요.':legacyIssuePending?'기존 주문번호로 송장 발급이 진행 중입니다. 완료 후 목록을 새로고침하세요.':legacyHistoryAmbiguous?'기존 주문번호에 여러 배송묶음의 송장 이력이 연결되어 있습니다. 기존 송장을 확인하세요.':eligibility.reason,
...
async function pagedQuery(factory){
  try{
    const rows=[];
    for(let offset=0;;offset+=500){
      ...
      if((result.data||[]).length<500)return {rows,unavailable:false};
    }
  }catch(error){return {rows:[],unavailable:true,error:{code:String(error?.code||'QUERY_ERROR'),message:'채널 주문을 불러오지 못했습니다.'}};}
}

async function loadUnifiedOrders({ db, channelConnections = [], shippingSafetyPlatform = null, summaryRead = false }) {
  const [cafe24Orders, cafe24Items, coupangOrders, coupangItems, coupangReturns, rgOrders, rgItems, naverOrders, naverItems, trackingStates, cafe24Products, channelProducts, coupangProducts, coupangProductItems, shippingReferenceSnapshots, coupangOrderDetailTerminals, successfulTransferRows, successfulIssueRows] = await Promise.all([
    pagedQuery(()=>db.from('cafe24_orders').select('order_id,order_date,payment_status,order_price,paid_amount,cancel_amount,raw_data').order('order_id',{ascending:true})),
    pagedQuery(()=>db.from('cafe24_order_items').select('order_id,external_item_id,external_product_no,product_name,option_name,quantity,unit_price,paid_amount,raw_data').order('external_item_id',{ascending:true})),
    pagedQuery(()=>db.from('coupang_orders').select('shipment_box_id,order_id,ordered_at,paid_at,status,gross_amount,raw_data').order('shipment_box_id',{ascending:true})),
    pagedQuery(()=>db.from('coupang_order_items').select('shipment_box_id,order_id,vendor_item_id,seller_product_id,product_name,quantity,unit_price,paid_amount,status,raw_data').order('external_item_key',{ascending:true})),
    pagedQuery(()=>db.from('coupang_returns').select('order_id,status,requested_at').order('id',{ascending:true})),
    pagedQuery(()=>db.from('coupang_rg_orders').select('order_id,status,paid_at,total_amount,item_count').order('order_id',{ascending:true})),
    pagedQuery(()=>db.from('coupang_rg_order_items').select('order_id,product_name,quantity,amount').order('id',{ascending:true})),
    pagedQuery(()=>db.from('naver_commerce_orders').select('order_id,order_date,payment_date,status,paid_amount,receiver_name,receiver_phone,receiver_address,shipping_memo,shipment_id,invoice_no,delivery_company,raw_data').order('order_id',{ascending:true})),
    pagedQuery(()=>db.from('naver_commerce_order_items').select('product_order_id,order_id,product_id,original_product_id,product_name,option_name,quantity,unit_price,paid_amount,status,raw_data').order('product_order_id',{ascending:true})),
    ...
    isolatedQuery(db.from('coupang_operation_requests')
      .select('operation_type,target_type,target_id,status,result_json,error_message,created_at')
      .eq('operation_type','ORDER_DETAIL')
      .eq('target_type','ORDER')
      .in('status',['SUCCESS','CANCELLED','FAILED'])
      .order('created_at',{ascending:false})
      .limit(5000)),
    pagedQuery(()=>db.from('coupang_operation_requests')
    ...
    pagedQuery(()=>db.from('coupang_operation_requests')
      ...
      .in('status',['SUCCESS','PENDING','RUNNING','EXECUTING'])
      .order('created_at',{ascending:false}).order('id',{ascending:false}))
  ]);
  if (shippingSafetyPlatform !== null) {
    const sources = shippingSafetyPlatform === 'CAFE24' ? [cafe24Orders,cafe24Items]
      : shippingSafetyPlatform === 'COUPANG' ? [coupangOrders,coupangItems,coupangReturns,coupangOrderDetailTerminals,rgOrders,rgItems] : null;
    if (!sources || [...sources,successfulTransferRows,successfulIssueRows].some(source=>source.unavailable) || trackingStates.__error) {
      throw Object.assign(new Error('출고 판단에 필요한 주문·취소·송장 자료를 모두 확인하지 못했습니다.'),{code:'EPOST_ORDER_RECHECK_REQUIRED',status:409});
    }
  }
  let coupangCatalogRows=[
  ...
  let imageCatalog=buildOrderImageCatalog(cafe24Products.rows,[...channelProducts.rows,...coupangCatalogRows]);
  const fallbackRows=await coupangImageFallbackRows({db,orderItems:coupangItems.rows,imageCatalog});
  if(fallbackRows.length){
    coupangCatalogRows=[...coupangCatalogRows,...fallbackRows];
    imageCatalog=buildOrderImageCatalog(cafe24Products.rows,[...channelProducts.rows,...coupangCatalogRows]);
  }
  const coupangOrderItems=attachOrderImages(coupangItems.rows,'COUPANG','seller_product_id',imageCatalog);
  ...
  return buildUnifiedOrders({
    ...
    coupangOrders:coupangOrders.rows, coupangOrderItems, coupangReturns:coupangReturns.rows,
    ...
    pendingIssueIds:successfulIssueRows.rows.filter(row=>['PENDING','RUNNING','EXECUTING'].includes(row.status)).map(row=>row.target_id),
    shippingHistoryReady:!successfulIssueRows.unavailable&&!successfulTransferRows.unavailable&&!trackingStates.__error,
    ...
    unavailable:{ CAFE24:cafe24Orders.unavailable || (summaryRead && (cafe24Items.unavailable || successfulTransferRows.unavailable || successfulIssueRows.unavailable || Boolean(trackingStates.__error))), COUPANG:coupangOrders.unavailable || rgOrders.unavailable || (summaryRead && (coupangItems.unavailable || coupangReturns.unavailable || coupangOrderDetailTerminals.unavailable || successfulTransferRows.unavailable || successfulIssueRows.unavailable || Boolean(trackingStates.__error))), NAVER:naverOrders.unavailable || (summaryRead && (naverItems.unavailable || successfulTransferRows.unavailable || successfulIssueRows.unavailable || Boolean(trackingStates.__error))) }
  });
}

async function loadCoupangShipmentIdentities(db){
  return pagedQuery(()=>db.from('coupang_orders').select('shipment_box_id,order_id').order('shipment_box_id',{ascending:true}));
}

module.exports = {
  STAGES, buildUnifiedOrders, filterUnifiedOrders, hubOrderId, resolveOrderTarget, loadUnifiedOrders,loadCoupangShipmentIdentities,
  attachOrderImages, buildOrderImageCatalog, fulfillmentTiming, isActiveClaimStatus, isCoupangTerminalOrderDetail,
  coupangImageFallbackRows, coupangProductDetailCatalogRows, normalizeCafe24Orders, normalizeCoupangOrders, normalizeNaverOrders, productImageFromRaw, safeProductImage, stageFor, successfulCoupangReceiverIndex
};
