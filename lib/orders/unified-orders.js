...
function buildUnifiedOrders(input = {}) {
  ...
  const historyOrders = [...cafe24, ...coupang, ...naver]
    .map(order=>{
      ...
      const trackedStage=tracking?.statusCode==='DELIVERED'?'DELIVERED':tracking?.statusCode==='IN_TRANSIT'&&!['DELIVERED'].includes(order.stage)?'SHIPPING':order.stage;
...
async function loadUnifiedOrders({ db, channelConnections = [] }) {
  const [cafe24Orders, cafe24Items, coupangOrders, coupangItems, coupangReturns, rgOrders, rgItems, naverOrders, naverItems, trackingStates] = await Promise.all([
  ...
  return buildUnifiedOrders({
    ...
    naverOrders:naverOrders.rows, naverOrderItems:naverItems.rows,
