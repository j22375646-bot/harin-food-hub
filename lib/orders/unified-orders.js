...
function productImageFromRaw(raw = {}) {
  return safeProductImage(firstValue(
    ...
    raw.representativeImage?.url,raw.representative_image?.url,raw.product?.imageUrl,raw.images?.[0]?.url
  ));
}
...
function buildOrderImageCatalog(cafe24Products = [], channelProducts = []) {
  ...
  for(const product of channelProducts||[]){
    ...
    const aliases=[external,raw.channelProductNo,raw.productNo,raw.originProductNo,raw.groupProductNo,raw.id].map(text).filter(Boolean);
