...
function productImageFromRaw(raw = {}) {
  return safeProductImage(firstValue(
    ...
    raw.representativeImage?.url,raw.representative_image?.url,raw.images?.[0]?.url
  ));
}
...
function buildOrderImageCatalog(cafe24Products = [], channelProducts = []) {
  ...
  for(const product of channelProducts||[]){
    ...
    const directImage=productImageFromRaw(product.raw_data||{});
    const image=directImage||(platform==='CAFE24'?cafeImages.get(external):'');
    if(platform&&external&&image)direct.set(`${platform}:${external}`,image);
...
function attachOrderImages(rows = [], platform, externalField, catalog = new Map()) {
  return (rows||[]).map(row=>({
    ...row,
    image_url:productImageFromRaw(row.raw_data||{})||catalog.get(`${platform}:${text(row[externalField])}`)||''
  }));
}
...
function normalizeNaverOrders(orders = [], items = []) {
  ...
  return orders.flatMap(order => {
    ...
    const completedClaim = [status, ...itemStatuses].some(value => ['CANCELED','RETURNED','EXCHANGED','CANCELED_BY_NOPAYMENT'].includes(value));
    ...
    return [shippingWorkbench.enrichOrder({
      ...
      receiver, actionRequired:cancellationRequested || ['PAID','PREPARING','READY_TO_SHIP'].includes(stage), fulfillment:'SELLER', source:'naver_commerce_orders'
...
function buildUnifiedOrders(input = {}) {
  ...
  const historyOrders = [...cafe24, ...coupang, ...naver]
    .map(order=>{
      ...
      const registeredInvoice=/^\d{13}$/.test(text(order.invoiceNumber));
      const trackedStage=order.stage==='DELIVERED'||tracking?.statusCode==='DELIVERED'
        ?'DELIVERED'
      ...
      return {
        ...order,stage:trackedStage,tracking,
        actionRequired:order.cancellationRequested||['PAID','PREPARING','READY_TO_SHIP'].includes(trackedStage),
        shippingEligible:eligibility.ok,shippingBlockedReason:eligibility.reason,
        ...fulfillmentTiming({...order,stage:trackedStage},asOf)
      };
    })
...
async function loadUnifiedOrders({ db, channelConnections = [] }) {
  const [cafe24Orders, cafe24Items, coupangOrders, coupangItems, coupangReturns, rgOrders, rgItems, naverOrders, naverItems, trackingStates, cafe24Products, channelProducts] = await Promise.all([
    ...
    isolatedQuery(db.from('naver_commerce_order_items').select('product_order_id,order_id,product_id,product_name,option_name,quantity,unit_price,paid_amount,status,raw_data').limit(15000)),
    ...
    isolatedQuery(db.from('channel_products').select('platform,external_product_id,master_product_id,raw_data').limit(10000))
  ]);
  const imageCatalog=buildOrderImageCatalog(cafe24Products.rows,channelProducts.rows);
...
module.exports = {
  ...
  attachOrderImages, buildOrderImageCatalog, fulfillmentTiming, isActiveClaimStatus,
