...
function fulfillmentTiming(order, asOf=new Date(),calendar={}) {
  ...
  if(estimate.status==='OVERDUE'&&order.stage==='PREPARING'){
...
function productImageFromRaw(raw = {}) {
  return safeProductImage(firstValue(
    ...
    raw.image?.url,raw.image?.imageUrl,raw.image?.cdnPath,raw.image?.vendorPath,
    raw.product?.imageUrl,raw.product?.representativeImage?.url,
    raw.images?.[0]?.url,raw.images?.[0]?.imageUrl,raw.images?.[0]?.cdnPath,raw.images?.[0]?.vendorPath,
    raw.itemImages?.[0]?.url,raw.itemImages?.[0]?.cdnPath,raw.productImages?.[0]?.url,raw.productImages?.[0]?.cdnPath
  ));
}
...
function attachOrderImages(rows = [], platform, externalField, catalog = new Map()) {
  return (rows||[]).map(row=>{
    ...
    const aliases=[row[externalField],row.product_id,row.original_product_id,row.seller_product_id,row.external_product_no,
      raw.productId,raw.originalProductId,raw.channelProductNo,raw.productNo,raw.originProductNo,raw.groupProductNo]
...
async function loadUnifiedOrders({ db, channelConnections = [] }) {
  const [cafe24Orders, cafe24Items, coupangOrders, coupangItems, coupangReturns, rgOrders, rgItems, naverOrders, naverItems, trackingStates, cafe24Products, channelProducts, coupangProducts, coupangProductItems, shippingReferenceSnapshots] = await Promise.all([
