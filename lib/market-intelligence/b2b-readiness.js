...
function buildB2BReadiness({project={},product={},env=process.env}={}){
  ...
  const connectionStatus=providerStatus(provider);
  const checks=[
    ...
    {id:'PROVIDER',label:'나라장터 읽기 연결',status:connectionStatus,detail:connectionStatus==='NOT_STARTED'?'B2B 사업 시작 확인 전에는 호출하지 않음':connectionStatus==='READ_PROBE_REQUIRED'?'공식 API 읽기 확인만 남음':'최종 키 입력·안전 스위치 확인 필요'}
  ];
  ...
  return {
    ...
    nextAction:connectionStatus==='NOT_STARTED'?'상품·서류·공급 조건부터 준비하고, 실제 B2B 납품을 시작할 때 연결을 여세요.':connectionStatus==='READ_PROBE_REQUIRED'?'공식 공고를 읽기 전용으로 한 번 확인할 단계입니다.':'최종 환경 키 입력 때 나라장터 서비스 키와 안전 스위치를 함께 확인하세요.',
