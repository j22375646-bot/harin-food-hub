...
async function searchEvidence({db,projectId,input={},actor='OWNER'}){
  ...
  const requestCount=settled.reduce((total,item)=>total+(item.status==='rejected'&&item.reason?.code==='NAVER_API_HUB_CONFIG_REQUIRED'?0:1),0);
  const finalQuota={...quota,used:quota.used+requestCount,remaining:Math.max(0,quota.remaining-requestCount)};
  await finishLog(db,logId,{status:deduped.length?errors.length?'PARTIAL':'SUCCESS':errors.length?'FAILED':'SUCCESS',rows_received:deduped.length,error_message:errors.length?errors.map(item=>`${item.source_label}: ${item.message}`).join(' | ').slice(0,500):null,metadata:{project_id:project.id,master_product_id:project.master_product_id,query,types,display,sort,result_count:deduped.length,error_count:errors.length,request_count:requestCount,cache_hits:cacheSummary.hits,stale_fallbacks:cacheSummary.stale,search_enabled:enabled,daily_budget:quota.budget}});
