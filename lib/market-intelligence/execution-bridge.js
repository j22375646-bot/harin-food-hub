...
async function createDraftExperiment({db,projectId,planId,actor='OWNER'}){
  ...
  const test=await experiments.createTest({name:`[${loaded.product?.name||'선택 상품'}] ${plan.title}`,master_product_id:loaded.project.master_product_id,market_project_id:loaded.project.id,platform:plan.platform,hypothesis:plan.hypothesis,target_type:experimentTarget(plan.source_type),source_type:'MANUAL',metric:plan.metric,start_date:plan.start_date,end_date:plan.end_date,status:'DRAFT',minimum_sample_size:plan.minimum_sample_size,variants:[{name:plan.control_label},{name:plan.variant_label}]});
