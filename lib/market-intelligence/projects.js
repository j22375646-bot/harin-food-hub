...
const WORKSPACES=new Set(['data','market','competition','conversion']);
...
function buildProjectHome({products=[],projects=[]}={}){
  ...
  return {
    products:activeProducts.map(product=>({...product,project:latestByProduct.get(product.id)||null})),
    projects:safeProjects,
    summary:{saleable_products:activeProducts.length,active_projects:safeProjects.filter(item=>item.status!=='ARCHIVED').length,versions:safeProjects.reduce((sum,item)=>sum+item.active_version,0)}
  };
}
...
async function loadProjectHome({db}){
  ...
  return buildProjectHome({products:productsResult.data||[],projects:projectsResult.data||[]});
}
...
module.exports={
  ACTIVE_STATUSES,WORKSPACES,MarketProjectError,requiredUuid,projectHref,buildProjectHome,
