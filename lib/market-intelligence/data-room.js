...
async function createDownloadUrl({db,projectId,sourceId}){
  ...
  const found=await db.from('market_sources').select('source_kind,source_url,file_name,storage_bucket,storage_path').eq('id',id).eq('project_id',project.id).maybeSingle();
  if(found.error)throw found.error;
  if(['API','URL'].includes(found.data?.source_kind)&&found.data?.source_url){
    let url;try{url=new URL(found.data.source_url);}catch{throw new MarketDataRoomError('원문 주소를 확인하지 못했습니다.',400,'SOURCE_URL_INVALID');}
    if(!['http:','https:'].includes(url.protocol))throw new MarketDataRoomError('안전한 원문 주소가 아닙니다.',400,'SOURCE_URL_INVALID');
    return {signed_url:url.toString(),expires_in:null,external:true};
  }
  if(!found.data?.storage_path)throw new MarketDataRoomError('보관된 원본 파일이 없습니다.',404,'SOURCE_FILE_NOT_FOUND');
