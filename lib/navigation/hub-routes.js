...
const HUB_WORKSPACES = {
  insight:[
    ...
    { id:'channels', label:'채널 상세', description:'플랫폼별 상세 성과', href:'/insights/channels' }
  ],
  keyword:[
    { id:'search-terms', label:'실제 검색어', description:'고객이 입력한 검색어', href:'/keywords/search-terms' },
    { id:'registered', label:'등록 키워드', description:'광고 키워드 성과와 조치', href:'/keywords/registered' },
    { id:'diagnosis', label:'기회·낭비 진단', description:'확장·감액 후보와 AI 분석', href:'/keywords/diagnosis' }
  ],
  product:[
...
function normalizeHubState(input = {}) {
  ...
  const platform = COUPANG_OPERATION_VIEWS.has(view) ? 'coupang' : view==='product'&&workspace!=='catalog' ? 'all' : CHANNEL_SCOPED_VIEWS.has(view) ? requestedPlatform : 'all';
