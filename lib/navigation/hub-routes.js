...
const HUB_NAV_GROUPS = [
  ...
  { id:'development', label:'개발', icon:'B', description:'상품개발·실험·결과 검증', items:['market','validation','experiments'] },
...
function parseHubHref(input = '/') {
  ...
  return normalizeHubState({
    view:parsedView,
    workspace:VIEW_IDS.has(queryView)?url.searchParams.get('workspace'):workspaceForPath(parsedView,url.pathname),
...
module.exports = { HUB_NAV, HUB_NAV_GROUPS, HUB_WORKSPACES, HUB_LEGACY_ROUTES, normalizeHubState, buildHubHref, parseHubHref, canonicalLegacyHubHref, routeFor, viewForPath, workspaceForView, groupForView, navigationContext };
