...
const customerServiceOperationModule=require('../customer-service/operation-summary.js');

const inFlightSources=new WeakMap();
...
function buildLiveNavigationOperationSnapshot(input={}){
  ...
  const customerService=customerServiceOperationModule.buildCustomerServiceOperationSummary({
    customerServiceRows:input.customerServiceRows,
    coupangInquiries:input.coupangInquiries,coupangReturns:input.coupangReturns,coupangExchanges:input.coupangExchanges,
    available:customerServiceKnown
  });
...
async function queryNavigationSources(db,mallId){
  ...
  return Promise.all([
    ...
    guarded(db.from('coupang_returns').select('receipt_id,order_id,status,cancel_type,requested_at').order('requested_at',{ascending:false}).limit(1000),'coupang_returns'),
    ...
    guarded(db.from('customer_service_items').select('id,source_key,platform,kind,completed').or('completed.eq.false,completed.is.null').limit(1000),'customer_service_items'),
    ...
    guarded(db.from('coupang_inquiries').select('inquiry_key,inquiry_id,answered').or('answered.eq.false,answered.is.null').limit(1000),'coupang_inquiries'),
    guarded(db.from('coupang_exchanges').select('exchange_id,status').order('requested_at',{ascending:false}).limit(1000),'coupang_exchanges')
  ]);
}

function loadNavigationSources(db,mallId){
  let pendingByMall=inFlightSources.get(db);
  if(!pendingByMall){
    pendingByMall=new Map();
    inFlightSources.set(db,pendingByMall);
  }
  if(pendingByMall.has(mallId))return pendingByMall.get(mallId);
  // Share only overlapping reads from the same client and mall. Never cache
  // completed data: the next refresh must see new work and retry failed sources.
  const pending=Promise.resolve().then(()=>queryNavigationSources(db,mallId)).finally(()=>{
    pendingByMall.delete(mallId);
    if(pendingByMall.size===0)inFlightSources.delete(db);
  });
  pendingByMall.set(mallId,pending);
  return pending;
}
...
async function loadLiveNavigationOperationSnapshot({db,now=new Date()}={}){
  ...
  const results=await loadNavigationSources(db,mallId);
  const [cafe24Orders,cafe24OrderItems,coupangOrders,coupangOrderItems,coupangReturns,coupangRgOrders,coupangOrderDetailTerminals,naverOrders,naverOrderItems,customerServiceRows,inventoryRows,alerts,syncs,cafe24Token,coupangProductCount,coupangInquiries,coupangExchanges]=results;
  ...
  const availability={
    ...
    customerService:[customerServiceRows,coupangInquiries,coupangReturns,coupangExchanges].every(result=>result.available),
  ...
  const snapshot=buildLiveNavigationOperationSnapshot({
    ...
    customerServiceRows:customerServiceRows.data,coupangInquiries:coupangInquiries.data,coupangExchanges:coupangExchanges.data,
    inventoryRows:inventoryRows.data,alerts:alerts.data
  });
  return {
    snapshot,
