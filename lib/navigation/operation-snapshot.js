...
function selectFetchedNavigationOperationSnapshot(current,fetched,{partial=false,now=Date.now()}={}) {
  ...
  if(partial&&currentSnapshot)return currentSnapshot;
  return selectNavigationOperationSnapshot(currentSnapshot,fetchedSnapshot);
}

module.exports={
  SNAPSHOT_VERSION,NAVIGATION_SNAPSHOT_KEY,NAVIGATION_SNAPSHOT_COOKIE,MAX_AGE_MS,REFRESH_AFTER_MS,DISPLAY_MAX_AGE_MS,buildNavigationOperationSnapshot,
  ...
  navigationOperationSnapshotFreshness,serializeNavigationOperationSnapshotCookie,parseNavigationOperationSnapshotCookie
};
