...
const NAVIGATION_SNAPSHOT_COOKIE='harin_hub_navigation_operation_snapshot';
const REFRESH_AFTER_MS=10*60*1000;
const SNAPSHOT_RECHECK_MS=60*1000;
...
const REQUIRED_BADGE_KEYS=['orders','cs','inventory','notifications'];
...
function knownBadgeCount(value) {
  return REQUIRED_BADGE_KEYS.filter(key=>Number.isFinite(value?.badges?.[key])&&value.badges[key]>=0).length;
}

function isNavigationOperationSnapshotComplete(value) {
  return knownBadgeCount(value)===REQUIRED_BADGE_KEYS.length;
}
...
function serializeNavigationOperationSnapshotCookie(value,now=Date.now()) {
  return isNavigationOperationSnapshot(value,now)?encodeURIComponent(JSON.stringify(value)):'';
}

function parseNavigationOperationSnapshotCookie(raw,now=Date.now()) {
  if(typeof raw!=='string'||!raw)return null;
  try{return parseNavigationOperationSnapshot(decodeURIComponent(raw),now);}
  catch{return null;}
}

function selectNavigationOperationSnapshot(...values) {
  return values.filter(Boolean).sort((left,right)=>{
    const completeness=Number(isNavigationOperationSnapshotComplete(right))-Number(isNavigationOperationSnapshotComplete(left));
    return completeness||snapshotTime(right)-snapshotTime(left);
  })[0]||null;
}

function selectFetchedNavigationOperationSnapshot(current,fetched,{partial=false,now=Date.now()}={}) {
  const currentSnapshot=parseNavigationOperationSnapshot(current,now);
  const fetchedSnapshot=parseNavigationOperationSnapshot(fetched,now);
  if(!fetchedSnapshot)return currentSnapshot;
  if(partial&&currentSnapshot){
    if(isNavigationOperationSnapshotComplete(currentSnapshot))return currentSnapshot;
    return knownBadgeCount(fetchedSnapshot)>knownBadgeCount(currentSnapshot)?fetchedSnapshot:currentSnapshot;
  }
  return selectNavigationOperationSnapshot(currentSnapshot,fetchedSnapshot);
}

module.exports={
  SNAPSHOT_VERSION,NAVIGATION_SNAPSHOT_KEY,NAVIGATION_SNAPSHOT_COOKIE,MAX_AGE_MS,REFRESH_AFTER_MS,SNAPSHOT_RECHECK_MS,DISPLAY_MAX_AGE_MS,buildNavigationOperationSnapshot,
  isNavigationOperationSnapshot,parseNavigationOperationSnapshot,selectNavigationOperationSnapshot,selectFetchedNavigationOperationSnapshot,
  isNavigationOperationSnapshotComplete,navigationOperationSnapshotFreshness,serializeNavigationOperationSnapshotCookie,parseNavigationOperationSnapshotCookie
};
