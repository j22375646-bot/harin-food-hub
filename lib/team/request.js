...
function createHandler({database=getSupabase,validate=auth.validateSession}={}){return async request=>{
 ...
 const input=request.method==='GET'?{action:'READ'}:await readJson(request,{maxBytes:192*1024});if(!validInput(input)||request.method==='POST'&&input.action==='READ')return reply(400,{ok:false,code:'TEAM_INVALID'});
 if(input.action.startsWith('CS_REPLY_')){
 const allowed=await db.rpc('moaon_team_command',{p_actor:session.userId,p_tenant:TENANT,p_session:session.id,p_hash:auth.tokenHash(token),p_input:{action:'CS_READ'}});if(allowed.error)return reply(403,{ok:false,code:'TEAM_ACCESS_DENIED'});
 try{return reply(200,{ok:true,value:await require('../customer-service/desktop-reply.js').command({db,input})});}catch(e){return reply(e.code==='TEAM_CONFLICT'?409:503,{ok:false,code:e.code||'TEAM_UNAVAILABLE'});}
 }
 ...
 if(result.error){const code=['RESERVATION_EXPIRY','RESERVATION_EXCEEDS_STOCK','TEAM_AUTH_REQUIRED','TEAM_ACCESS_DENIED','TEAM_CONFLICT','TEAM_NOT_FOUND','TEAM_ASSIGNEE_INVALID','TEAM_INVALID'].find(c=>result.error.message?.includes(c));return reply(({RESERVATION_EXPIRY:409,RESERVATION_EXCEEDS_STOCK:409,TEAM_AUTH_REQUIRED:401,TEAM_ACCESS_DENIED:403,TEAM_CONFLICT:409,TEAM_NOT_FOUND:404,TEAM_ASSIGNEE_INVALID:409,TEAM_INVALID:400})[code]||503,{ok:false,code:code||'TEAM_UNAVAILABLE'});}
