...
function createHandler({database=getSupabase,validate=auth.validateSession}={}){return async request=>{
 ...
 const input=request.method==='GET'?{action:'READ'}:await readJson(request,{maxBytes:16000});if(!validInput(input)||request.method==='POST'&&input.action==='READ')return reply(400,{ok:false,code:'TEAM_INVALID'});
