...
// The Korea Post contract-parcel OpenAPI manual publishes this legacy API on
// HTTP. The HTTPS virtual host currently presents a certificate for an IP
// address, so standards-compliant Node clients reject it before any API call.
const BASE_URL = 'http://ship.epost.go.kr';
...
async function requestXml(message, plainText, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  const query = new URLSearchParams({ key:config.apiKey, regData:seed128.encryptRegData(config.securityKey, plainText), option:'001' });
  const requestUrl = new URL(`${BASE_URL}/${message}`);
  ...
  try {
    ...
    const causeCode = text(error?.cause?.code || error?.code || 'NETWORK');
    throw Object.assign(new Error(`우체국 OpenAPI 통신 실패(${causeCode}). 잠시 뒤 자동 재시도합니다.`), {
      code:'EPOST_NETWORK_ERROR', status:503, retryable:true, cause:error
    });
  }
  const responseText = await response.text();
  if (!response.ok) {
    const maintenance = response.status === 404
      && /전산시스템\s*작업\s*안내|서비스가\s*일시\s*중단/.test(responseText);
    if (maintenance) {
      throw Object.assign(new Error('우체국 전산시스템 점검 중입니다. 작업 종료 후 자동 재시도합니다.'), {
        code:'EPOST_MAINTENANCE', status:503, retryable:true
      });
    }
    const retryable = response.status === 408 || response.status === 429 || response.status >= 500;
    throw Object.assign(new Error(`우체국 OpenAPI 응답 오류(HTTP ${response.status})`), {
      code:'EPOST_HTTP_ERROR', status:502, retryable
    });
  }
...
async function listOffices(options = {}) {
  if(!options.env)options={...options,env:await require('../integrations/managed-keys.js').environment('EPOST')};
