...
function validateShipment(input = {}, { testOnly = false } = {}) {
  ...
  if (!['CAFE24','COUPANG','NAVER'].includes(text(input.platform).toUpperCase())) errors.push('지원하지 않는 판매 채널입니다.');
...
function application(input = {}, config = {}, { testOnly = false } = {}) {
  ...
  const fields = [
    ...
    ['insuYn', 'N'],
    ['ordCompNm', field(`하린식품 ${{ COUPANG:'쿠팡', CAFE24:'Cafe24', NAVER:'네이버' }[shipment.platform]}`, 100)],
