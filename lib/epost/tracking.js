...
function classifyEvent(event = {}) {
  ...
  if(/배달준비|배달출발|배송중|운송중|발송|도착|접수|인수|delivery|transit/i.test(description))return 'IN_TRANSIT';
  return 'UNKNOWN';
}
...
function parseTrackingResponse(xml, expectedTrackingNo = '') {
  ...
  const events=blocks(xml).map(block=>({
    ...
    name:tag(block,'eventnm','eventNm','eventName','status'),
  ...
  const statusCode=deliveredEvent?'DELIVERED':events.some(event=>classifyEvent(event)==='IN_TRANSIT')?'IN_TRANSIT':events.length?'UNKNOWN':'NOT_FOUND';
  const statusLabel={DELIVERED:'배달완료',IN_TRANSIT:'배송중',UNKNOWN:'배송상태 확인',NOT_FOUND:'우체국 접수 확인 전'}[statusCode];
  return {
