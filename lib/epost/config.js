...
function readiness({ env = process.env, actualIp = '' } = {}) {
  ...
  const checks = {
    ...
    liveWrite: {
      label: '실제 송장 자동발급',
      configured: true,
      ok: values.liveWritesEnabled,
      detail: values.liveWritesEnabled ? '실제 계약소포 접수 허용' : '워커에서 실제 접수 잠금 해제 필요'
    },
    tracking: {
      label: '배송추적 인증키',
  ...
  const readyForLive = ['fixedIp','apiKey','securityKey','contract','encryption','liveWrite'].every(key=>checks[key].ok);
  const readyForTracking = checks.fixedIp.ok && checks.tracking.ok;
  return {
    status: readyForLive ? 'READY_FOR_LIVE' : readyForTest ? 'READY_FOR_TEST' : 'SETUP_REQUIRED',
    readyForTest,
    readyForLive,
    readyForTracking,
