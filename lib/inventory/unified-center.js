...
function cafe24Quantity(product) {
  ...
  return sumRows(raw.variants || raw.options || [], ['inventory_quantity','stock_quantity','quantity','available_quantity']);
}
...
function stockState(quantity, { missing = false, reference = false, stale = false } = {}) {
  if (missing) return reference ? 'REFERENCE' : 'MISSING';
...
function buildUnifiedInventoryCenter({
  ...
  coupangItemInventory = [], coupangRgInventory = [], now = new Date(), staleHours = 6
} = {}) {
  ...
  const items = masterProducts.filter(item => item.is_active !== false).map(master => {
    ...
    const cafeQuantityValue = cafeProduct ? cafe24Quantity(cafeProduct) : null;
    ...
    const channels = {
      CAFE24:{
        state:stockState(cafeQuantityValue,{ missing:!cafeLink, stale:cafeStale }), quantity:cafeQuantityValue,
        label:stockLabel(stockState(cafeQuantityValue,{ missing:!cafeLink, stale:cafeStale })), updated_at:cafeUpdatedAt,
        detail:cafeLink ? cafeQuantityValue == null ? '상품은 연결됐지만 재고 수량 응답이 없습니다.' : 'Cafe24 판매 가능 수량' : 'Cafe24 상품 연결이 필요합니다.'
      },
      NAVER:{
    ...
    const known = Object.values(channels).filter(channel => channel.quantity != null).length;
    return {
  ...
  }).sort((a,b) => b.priority-a.priority || b.issues.length-a.issues.length || a.name.localeCompare(b.name,'ko'));
...
module.exports = { buildUnifiedInventoryCenter, cafe24Quantity, naverQuantity, stockState, isStale };
