...
function cafe24Inventory(product) {
  ...
  if (direct != null) return { quantity:Math.max(0,direct), unmanaged:false };
  const variantQuantity = sumRows(variants, ['inventory_quantity','stock_quantity','quantity','available_quantity','inventories.quantity','inventory.quantity']);
  return {
    quantity:variantQuantity == null ? null : Math.max(0,variantQuantity),
    unmanaged:false
  };
}
...
function naverQuantity(link) {
  ...
  const optionQuantity = sumRows(raw.optionCombinations || raw.options || raw.variants || [], ['stockQuantity','stock_quantity','inventory_quantity','quantity']);
  return optionQuantity == null ? null : Math.max(0,optionQuantity);
}
...
function buildUnifiedInventoryCenter({
  ...
  const items = masterProducts.filter(item => item.is_active !== false).map(master => {
    ...
    const marketplaceQuantity = marketRows.length ? Math.max(0,marketRows.reduce((sum, row) => sum + Number(row.quantity || 0), 0)) : null;
    const rocketGrowthQuantity = rgRows.length ? Math.max(0,rgRows.reduce((sum, row) => sum + Number(row.total_orderable_quantity || 0), 0)) : null;
