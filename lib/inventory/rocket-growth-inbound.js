...
function buildDestinationDirectory({costTransactions=[],savedDestinations=[],openReceiver=()=>({})}={}){
  ...
  const codes=new Set([...hints,...savedByCode.keys()]);
  return [...codes].sort((a,b)=>a.localeCompare(b,'ko')).map(centerCode=>{
    ...
    const normalized=normalizeDestination({
      centerCode,label:saved?.label||centerCode,...receiver
    });
    const validation=validateDestination(normalized);
    const source=saved&&hints.has(centerCode)?'SAVED_AND_API':saved?'SAVED':'COUPANG_API_HINT';
...
function prepareShipmentDrafts({inventory=[],drafts=[]}={}){
  ...
  for(const raw of (drafts||[]).slice(0,50)){
    ...
    if(!item){invalid.push({vendorItemId,error:'최신 로켓그로스 재고에서 상품을 찾지 못했습니다.'});continue;}
    if(seen.has(vendorItemId)){invalid.push({vendorItemId,error:'같은 상품이 중복 선택됐습니다.'});continue;}
...
module.exports={buildDestinationDirectory,buildRocketGrowthProductDirectory,normalizeDestination,prepareShipmentDrafts,validateDestination};
