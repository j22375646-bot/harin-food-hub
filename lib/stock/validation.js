...
function validate(row){
 ...
 if(Object.hasOwn(row,'packLabel')){if(!['봉지','개'].includes(row.packLabel))throw Error('포장 표시는 봉지 또는 개를 선택하세요.');out.packLabel=row.packLabel;}
 if(Object.hasOwn(row,'receivedDate')){if(typeof row.receivedDate!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(row.receivedDate)||!Number.isFinite(Date.parse(row.receivedDate))||new Date(row.receivedDate).toISOString().slice(0,10)!==row.receivedDate)throw Error('입고일을 확인하세요.');out.receivedDate=row.receivedDate;}
 if(Object.hasOwn(row,'portionSize')){if(!out.packLabel)throw Error('봉지 또는 개를 선택하세요.');const allowed=out.unit==='티백'?[10,20,30,40,50]:out.unit==='KG'?Array.from({length:19},(_,i)=>100+i*50):[];if(row.portionSize!==null&&!allowed.includes(row.portionSize))throw Error('소분량을 다시 선택하세요.');out.portionSize=row.portionSize;}
