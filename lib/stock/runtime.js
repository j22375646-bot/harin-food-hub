...
if(request.method==='GET'){const result=await db.from('moaon_stock_lots').select('id,revision,data,updated_at').eq('tenant_id',context.tenantId).order('updated_at',{ascending:false}).limit(5001);if(result.error||!Array.isArray(result.data)||result.data.length>5000)throw Error('DB');let catalog={products:[],productsStatus:'UNAVAILABLE'};if(context.tenantId===HARIN){try{catalog={products:await readProducts(),productsStatus:'READY'};}catch{}}const after=await resolveContext(input);if(['userId','tenantId','membershipVersion','sessionId','role'].some(k=>context[k]!==after[k]))return reply(409,{ok:false,code:'WORKSPACE_CHANGED'});return reply(200,{ok:true,...catalog,value:result.data.map(r=>({...r.data,id:r.id,revision:r.revision,updatedAt:r.updated_at}))});}
