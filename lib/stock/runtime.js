...
if(request.method==='GET'){const result=await db.from('moaon_stock_lots').select('id,revision,data,updated_at').eq('tenant_id',context.tenantId).order('updated_at',{ascending:false}).limit(5001);if(result.error||!Array.isArray(result.data)||result.data.length>5000)throw Error('DB');let rocket={rocket:[],rocketStatus:'UNAVAILABLE'};if(context.tenantId===HARIN){try{rocket={rocket:await loadRocket({db}),rocketStatus:'READY'};}catch{}}let catalog={products:[],productsStatus:'UNAVAILABLE'};if(context.tenantId===HARIN){try{catalog={products:await readProducts(),productsStatus:'READY'};}catch{}}const after=await resolveContext(input);if(['userId','tenantId','membershipVersion','sessionId','role'].some(k=>context[k]!==after[k]))return reply(409,{ok:false,code:'WORKSPACE_CHANGED'});return reply(200,{ok:true,...catalog,...rocket,value:result.data.map(r=>({...r.data,id:r.id,revision:r.revision,updatedAt:r.updated_at}))});}
let value,fields;try{value=await body(request);}catch{return reply(400,{ok:false,code:'INVALID_INPUT'});}
if(['CHOOSE_PRODUCT','REFRESH_ROCKET'].includes(value.action)){
 ...
 try{if(value.action==='REFRESH_ROCKET'){const queued=await db.rpc('moaon_queue_rocket_refresh');if(queued.error)throw Error('DB');return reply(200,{ok:true,value:queued.data});}await saveProductChoice({db,tenantId:context.tenantId,value});}catch(e){if(e.message==='INVALID_CHOICE')return reply(400,{ok:false,code:'INVALID_INPUT'});throw e;}
