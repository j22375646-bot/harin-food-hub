...
function buildCustomerRetention({ orders = [], items = [], referrers = [], asOf = new Date() } = {}) {
  ...
  const historyDays=historyStart&&historyEnd ? dayDiff(historyStart,historyEnd)+1 : 0;
  ...
  return {
    ...
    period:{start:historyStart?dateOnly(historyStart.toISOString()):null,end:historyEnd?dateOnly(historyEnd.toISOString()):null,days:historyDays},
    summary:{
...
function buildRetentionValidation(input={}){
  return {customer:buildCustomerRetention(input),execution:buildExecutionValidation(input)};
}

module.exports={median,dayDiff,buildCustomerRetention,actionExpectation,resultAt,buildExecutionValidation,buildRetentionValidation};
