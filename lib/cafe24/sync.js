  ...
  try {
    ...
    try {
      ...
      errors.push({
        dataset:"salesDaily",
        status:error.status || null,
        code:error.status===403?'SETUP_REQUIRED':'COLLECTION_FAILED',
        message:error.status===403
          ? 'Cafe24 매출통계 API 승인 또는 mall.read_salesreport 재연결이 필요합니다.'
          : error.message
      });
    }
    ...
    await finishSync(logId, {
      ...
      metadata: {
        ...
        capabilities:{
          settlement:errors.some(error=>error.dataset==='salesDaily')?'SETUP_REQUIRED':'READY',
