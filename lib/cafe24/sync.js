...
const koreaDate = d => new Intl.DateTimeFormat('en-CA', {
...
async function syncOrdersRealtime(config, { days = 90 } = {}) {
  ...
  const start = new Date(`${endDate}T00:00:00Z`); start.setUTCDate(start.getUTCDate() - Math.max(1, Math.min(Number(days) || 31, 90)) + 1);
  ...
  try {
    ...
    await finishSync(logId, {
      ...
      metadata:{ counts, period:{ start:isoDate(start), end:endDate }, realtime:true, embeddedItems:true, businessTimezone:'Asia/Seoul' }
    });
    return { syncLogId:logId, status:'SUCCESS', counts, finishedAt, period:{ start:isoDate(start), end:endDate } };
  } catch (error) {
