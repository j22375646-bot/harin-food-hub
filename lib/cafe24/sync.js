...
async function syncAll(config) {
  ...
  const orderPeriod = { start_date:isoDate(historyStart), end_date:isoDate(end) };
  ...
  try {
    ...
    for (const range of dateRanges(historyStart, end)) {
      const rangeOrders = await fetchAdminCollection(config, '/orders', 'orders', range);
    ...
    await finishSync(logId, { status, rows_received: Object.values(counts).reduce((a, b) => a + b, 0), error_message: errors.length ? JSON.stringify(errors) : null, metadata: { counts, errors, period, order_period:orderPeriod } });
    return { syncLogId: logId, status, period, orderPeriod, counts, errors };
  } catch (error) {
...
module.exports = { syncAll, analyticsTotal, analyticsByDate, dateRanges };
