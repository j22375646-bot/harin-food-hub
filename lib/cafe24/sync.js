...
async function syncAll(config) {
  ...
  try {
    ...
    try {
      ...
      const classification=financeCapability.classifyFinanceError(error);
      errors.push({
        dataset:"salesDaily",
        status:classification.evidence.http_status,
        code:classification.status,
        message:classification.action,
        scope:financeCapability.SALES_REPORT_SCOPE,
        docs_url:classification.docsUrl,
