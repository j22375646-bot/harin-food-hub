...
async function tokenRequest(config, body) {
  const env=await require('../integrations/managed-keys.js').environment('CAFE24');
  if(env!==process.env)config={...config,clientId:env.CAFE24_CLIENT_ID,clientSecret:env.CAFE24_CLIENT_SECRET};
...
async function adminGet(config, pathname, params = {}, requestOptions = {}) {
  ...
  try { return await requestJson(url, token, requestOptions); }
  catch (error) {
    if (error.status !== 401) throw error;
    token = await getAccessToken(config, true);
    return requestJson(url, token, requestOptions);
  }
}

function buildAdminWriteRequest(config, method, pathname, request) {
  const url = new URL(`https://${config.mallId}.cafe24api.com/api/v2/admin${pathname}`);
  const options = { method:String(method || 'POST').toUpperCase(), body:{ shop_no:config.shopNo, request } };
  return { url, options };
}

async function adminRequest(config, method, pathname, request) {
  const { url, options } = buildAdminWriteRequest(config, method, pathname, request);
  let token = await getAccessToken(config);
  try { return await requestJson(url, token, options); }
  catch (error) {
    if (error.status !== 401) throw error;
    token = await getAccessToken(config, true);
    return requestJson(url, token, options);
  }
}
...
module.exports = { exchangeCode, getAccessToken, adminGet, adminRequest, analyticsGet, buildAdminWriteRequest };
