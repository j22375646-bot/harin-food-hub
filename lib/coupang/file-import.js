...
const { sanitize, mapSettlementSummary } = require('./mappers.js');
const ledger = require('./settlement-ledger.js');

const MAX_ROWS = 50000;
const BATCH_SIZE = 400;
const DATASETS = new Set(['AUTO', 'ORDERS', 'PRODUCTS', 'SETTLEMENTS', 'PAYOUTS']);
const aliases = {
  deliveryFamily: ['deliveryfamily', '배송유형'],
  sourceRecordId: ['sourcerecordid', '정산원본id'],
  recognitionMonth: ['recognitionmonth', '매출인식월'],
  periodStart: ['periodstart', 'revenuerecognitiondatefrom', '매출인식시작일'],
  periodEnd: ['periodend', 'revenuerecognitiondateto', '매출인식종료일'],
  settlementType: ['settlementtype', '지급유형'],
  finalAmount: ['finalamount', '최종지급액'],
  settlementTargetAmount: ['settlementtargetamount', '정산대상액'],
  lastAmount: ['lastamount', '유보금액'],
  pendingReleasedAmount: ['pendingreleasedamount', '보류해제금액'],
...
function detectedDataset(lookup) {
  if (lookup.recognitionMonth >= 0 || lookup.finalAmount >= 0) return 'PAYOUTS';
...
function mapPayoutRow(row, lookup) {
  const periodStart = dateOnly(value(row, lookup, 'periodStart'));
  const periodEnd = dateOnly(value(row, lookup, 'periodEnd'));
  const month = text(value(row, lookup, 'recognitionMonth')) || periodStart?.slice(0, 7);
  if (!month || !/^\d{4}-(0[1-9]|1[0-2])$/.test(month) || !periodStart || !periodEnd || periodStart > periodEnd) return null;
  return mapSettlementSummary({
    sourceRecordId: text(value(row, lookup, 'sourceRecordId')),
    settlementType: text(value(row, lookup, 'settlementType')) || text(value(row, lookup, 'saleType')),
    settlementDate: dateOnly(value(row, lookup, 'settlementDate')),
    revenueRecognitionDateFrom: periodStart, revenueRecognitionDateTo: periodEnd,
    status: text(value(row, lookup, 'status')),
    finalAmount: value(row, lookup, 'finalAmount'), settlementAmount: value(row, lookup, 'settlementAmount'),
    settlementTargetAmount: value(row, lookup, 'settlementTargetAmount'),
    lastAmount: value(row, lookup, 'lastAmount'), pendingReleasedAmount: value(row, lookup, 'pendingReleasedAmount'),
    totalSale: value(row, lookup, 'saleAmount'), serviceFee: value(row, lookup, 'serviceFee')
  }, month, { source: 'WING_PAYOUT_FILE', deliveryFamily: value(row, lookup, 'deliveryFamily') });
}
...
function mapSettlementRow(row, lookup, rowIndex) {
  ...
  const settlementAmount = ledger.nullableAmount(value(row, lookup, 'settlementAmount'));
  const family = ledger.ledgerDeliveryFamily(value(row, lookup, 'deliveryFamily'));
  const sourceRecordId = text(value(row, lookup, 'sourceRecordId'));
  if (!sourceRecordId && (!orderId || orderId === '0')) return null;
  const source = 'WING_SETTLEMENT_FILE';
  const amounts = Object.fromEntries(['saleAmount', 'serviceFee', 'serviceFeeVat', 'quantity'].map(field => [field, ledger.nullableAmount(value(row, lookup, field))]));
  const provenance = ledger.ledgerProvenance({ family, source, sourceRecordId, periodStart: recognitionDate, periodEnd: recognitionDate, amounts: [settlementAmount, amounts.saleAmount, amounts.serviceFee, amounts.serviceFeeVat] });
  if (!sourceRecordId || sourceRecordId === '0') provenance.reconciliation_status = 'AMBIGUOUS_IDENTITY';
  const raw = sanitize(Object.fromEntries(Object.entries(lookup).filter(([, index]) => index >= 0).map(([field, index]) => [field, row[index]])));
  return { ...provenance, settlement_key: ledger.stableLedgerKey('LINE', family, source, sourceRecordId ? ['ID', sourceRecordId] : ['NATURAL', orderId, vendorItemId, recognitionDate, saleType]), order_id: orderId, vendor_item_id: vendorItemId, sale_type: saleType, recognition_date: recognitionDate, settlement_date: dateOnly(value(row, lookup, 'settlementDate')), sale_amount: amounts.saleAmount, service_fee: amounts.serviceFee, service_fee_vat: amounts.serviceFeeVat, settlement_amount: settlementAmount, quantity: amounts.quantity, raw_data: raw, updated_at: new Date().toISOString() };
}
...
async function importFile({ buffer, fileName, fileType, dataset = 'AUTO', db = getSupabase() }) {
  ...
  const counts = { products: 0, orders: 0, orderItems: 0, settlements: 0, payouts: 0, invalidRows: 0, inputRows: totalRows };
  const periods = [];
  try {
    const products = [], orders = [], orderItems = [], settlements = [], payouts = [], detected = [];
    for (const table of tables) {
      ...
      table.rows.forEach((row, index) => {
        if (kind === 'ORDERS') {
          ...
          const mapped = mapSettlementRow(row, table.lookup, index);
          if (!mapped) throw new Error('INVALID_LEDGER_ROW: 정산 행의 원본 ID와 인식일을 확인해주세요.');
          settlements.push(mapped); periods.push(mapped.recognition_date);
        } else if (kind === 'PAYOUTS') {
          const mapped = mapPayoutRow(row, table.lookup);
          if (!mapped) throw new Error('INVALID_LEDGER_ROW: 지급 행의 원본 ID와 인식 기간을 확인해주세요.');
          payouts.push(mapped); periods.push(mapped.period_start, mapped.period_end);
        }
      });
    }
    ledger.assertUniqueLedgerRows(settlements, 'settlement_key');
    ledger.assertUniqueLedgerRows(payouts, 'summary_key');
    await ledger.assertNoLegacyOverlap(db, 'coupang_settlements', settlements);
    await ledger.assertNoLegacyOverlap(db, 'coupang_settlement_summaries', payouts);
    for (const row of [...settlements, ...payouts]) row.provenance = { ...row.provenance, source_file: fileName, import_id: logId };
    ...
    counts.payouts = await upsertMany(db, 'coupang_settlement_summaries', payouts, 'summary_key');
    ...
    const rowsReceived = counts.products + counts.orders + counts.orderItems + counts.settlements + counts.payouts;
    ...
    const unresolvedFamilyRows = [...settlements, ...payouts].filter(row => row.delivery_family === 'UNKNOWN').length;
    const missingAmountRows = [...settlements, ...payouts].filter(row => row.reconciliation_status === 'MISSING_AMOUNT').length;
    return { syncLogId: logId, status, counts, detected, ledger: { unresolvedFamilyRows, missingAmountRows, coverage: 'UNVERIFIED', action: unresolvedFamilyRows ? 'delivery_family에 COUPANG 또는 ROCKET_GROWTH를 명시한 원본을 확인해주세요.' : missingAmountRows ? '누락된 정산 금액을 원본에서 확인해주세요.' : null }, period: { start: periods.sort()[0] || null, end: periods.sort().at(-1) || null } };
  } catch (error) {
...
module.exports = { importFile, extractTables, detectedDataset, mapOrderRow, mapProductRow, mapSettlementRow, mapPayoutRow, normalize, parseCsvLine };
