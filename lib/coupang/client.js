...
async function throttle(startRequest, minInterval = 250) {
  let release;
  ...
  try {
    let remaining;
    while (lastRequestAt !== null && (remaining = minInterval - (performance.now() - lastRequestAt)) > 0) {
      await wait(remaining);
    }
    try {
      return startRequest();
    } finally {
      // Record the invocation even if fetch throws synchronously.
      // Release before awaiting the response so network I/O can overlap.
...
async function request(method, path, params = {}, options = {}) {
  const config = getConfig(await require('../integrations/managed-keys.js').environment('COUPANG'));
