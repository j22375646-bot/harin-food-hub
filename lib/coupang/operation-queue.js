...
function validateRequest(input = {}) {
  ...
  if (!['ORDER', 'RETURN', 'EXCHANGE', 'INQUIRY'].includes(targetType)) throw Object.assign(new Error('쿠팡 작업 대상이 올바르지 않습니다.'), { status:400 });
  if (!/^\d+$/.test(targetId)) throw Object.assign(new Error('쿠팡 작업 대상 번호가 올바르지 않습니다.'), { status:400 });
...
async function queueOperation(db, input = {}) {
  ...
  const expiresInMs = request.operationType === 'ORDER_DETAIL' ? 10 * 60 * 1000 : 90 * 1000;
