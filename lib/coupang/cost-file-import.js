...
const number = ledger.nullableAmount;
const product = (left, right) => left == null || right == null ? null : left * right;
...
function mappedRow(type, row, sourceFile, sheetName, context = {}) {
  const common = {
    source_type: type,
  ...
  if (type === 'WAREHOUSING') Object.assign(common, {
    order_id: text(row[6]), reference_id: text(row[7]), seller_product_id: text(row[9]), vendor_item_id: text(row[10]), sku_id: text(row[11]), product_name: text(row[12]), option_name: text(row[13]), quantity: number(row[19]), gross_sales: product(number(row[14]), number(row[19])), cost_amount: number(row[24]),
  ...
  const family = ledger.ledgerDeliveryFamily(context.family);
  const sourceRecordId = text(context.sourceRecordId);
  const provenance = ledger.ledgerProvenance({ family, source: 'WING_COST_FILE', sourceRecordId, periodStart: common.recognition_date || common.event_date, periodEnd: common.settlement_end_date || common.recognition_date || common.event_date, amounts: [type === 'INVENTORY_COMPENSATION' ? common.credit_amount : common.cost_amount] });
  if (!sourceRecordId || sourceRecordId === '0') provenance.reconciliation_status = 'AMBIGUOUS_IDENTITY';
  const identity = sourceRecordId ? ['ID', sourceRecordId] : [type, common.transaction_type, common.event_date, common.recognition_date, common.settlement_end_date, common.order_id, common.reference_id, common.vendor_item_id, common.sku_id];
  return { ...common, ...provenance, transaction_key: ledger.stableLedgerKey('COST', family, `WING_COST_FILE:${type}`, identity) };
}
...
async function parseCostWorkbook(buffer, fileName) {
  ...
  for (const sheet of workbook.worksheets) {
    ...
    const headers = rowValues(sheet.getRow(header)).map(value => String(value ?? '').trim().toLowerCase().replace(/[_\s]/g, ''));
    const familyColumn = headers.findIndex(value => value === 'deliveryfamily' || value === '배송유형');
    const recordColumn = headers.findIndex(value => value === 'sourcerecordid' || value === '정산원본id');
    let rows = 0;
    for (let rowNumber = header + 1; rowNumber <= sheet.rowCount; rowNumber += 1) {
      ...
      const mapped = mappedRow(type, values, fileName, sheet.name, { family: values[familyColumn], sourceRecordId: values[recordColumn] });
      if (!mapped) throw new Error('INVALID_LEDGER_ROW: 비용 행의 원본 ID와 인식일을 확인해주세요.');
  ...
  ledger.assertUniqueLedgerRows(transactions, 'transaction_key');
...
async function importCostFile({ buffer, fileName, db = getSupabase() }) {
  const fileHash = hash(buffer);
  const prior = await db.from('coupang_cost_imports').select('*').eq('file_hash', fileHash).maybeSingle();
  if (prior.error) throw prior.error;
  if (prior.data && ['SUCCESS', 'PARTIAL'].includes(prior.data.status)) return { skipped: true, import: prior.data, counts: { inputRows: prior.data.input_rows, storedRows: 0, duplicateRows: prior.data.input_rows, invalidRows: prior.data.invalid_rows } };
  ...
  await ledger.assertNoLegacyOverlap(db, 'coupang_cost_transactions', unique);
  const periods = unique.flatMap(row => [row.event_date, row.recognition_date].filter(Boolean)).sort();
  const total = field => unique.length && unique.every(row => row[field] != null) ? unique.reduce((sum, row) => sum + row[field], 0) : null;
  const totals = { grossSales: total('gross_sales'), costAmount: total('cost_amount'), costVat: total('cost_vat'), creditAmount: total('credit_amount') };
  const record = {
    file_hash: fileHash, file_name: fileName, source_types: [...new Set(parsed.sheets.map(item => item.sourceType))], status: 'FAILED',
  ...
  const inserted = prior.data ? { data: { ...prior.data, ...record } } : await db.from('coupang_cost_imports').insert(record).select('*').single();
  ...
  const status = parsed.invalidRows ? 'PARTIAL' : 'SUCCESS';
  const finished = await db.from('coupang_cost_imports').update({ ...record, status, stored_rows: unique.length - existingDuplicates, duplicate_rows: duplicateRows }).eq('id', inserted.data.id);
  if (finished.error) throw finished.error;
  const calibration = { status: 'PENDING_TYPED_EVIDENCE', autoApplied: false };
  const calibrationWarning = '배송 유형별 전체 원장 대조가 완료될 때까지 자동 비용 보정을 보류합니다.';
  return { skipped: false, import: { ...inserted.data, status, stored_rows: unique.length - existingDuplicates, duplicate_rows: duplicateRows }, counts: { inputRows: parsed.transactions.length + parsed.invalidRows, storedRows: unique.length - existingDuplicates, duplicateRows, invalidRows: parsed.invalidRows }, sheets: parsed.sheets, totals, calibration, calibrationWarning };
}
