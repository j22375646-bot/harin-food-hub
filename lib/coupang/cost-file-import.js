...
async function importCostFile({ buffer, fileName }) {
  ...
  let calibration = null; let calibrationWarning = null;
  try {
    const refreshed = await costCalibration.refreshCoupangCostCalibration({ db, triggerType: 'COST_IMPORT' });
    calibration = { status:refreshed.status, confidence:refreshed.confidence, autoApplied:refreshed.auto_applied, periodStart:refreshed.period_start, periodEnd:refreshed.period_end };
  } catch (error) { calibrationWarning = error.message || '비용 자동 보정 이력 저장 실패'; }
  return { skipped: false, import: { ...inserted.data, stored_rows: unique.length - existingDuplicates, duplicate_rows: duplicateRows }, counts: { inputRows: parsed.transactions.length + parsed.invalidRows, storedRows: unique.length - existingDuplicates, duplicateRows, invalidRows: parsed.invalidRows }, sheets: parsed.sheets, totals, calibration, calibrationWarning };
}
