...
async function syncAll() {
  ...
  try {
    ...
    const finishedAt = new Date().toISOString();
    const updated = await db.from('sync_logs').update({ status, finished_at: finishedAt, rows_received: rowsReceived, error_message: errors.length ? JSON.stringify(errors) : null, metadata: { counts, errors, period } }).eq('id', logId);
    if (updated.error) throw updated.error;
