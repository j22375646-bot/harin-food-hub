...
async function syncSellerOrdersRealtime() {
  const config = getConfig(); const db = supabaseModule.getSupabase(); const end = koreaDate();
  const startDate = new Date(`${end}T00:00:00Z`); startDate.setUTCDate(startDate.getUTCDate() - 30);
  const period = { start:isoDate(startDate), end };
  const started = await db.from('sync_logs').insert({ platform:'COUPANG', job_type:'ORDERS_REALTIME', status:'RUNNING' }).select('id').single();
  if(started.error)throw started.error;
  try {
    ...
    const counts = {
      orders:await upsertMany(db, 'coupang_orders', orderRows, 'shipment_box_id'),
      orderItems:await upsertMany(db, 'coupang_order_items', itemRows, 'external_item_key')
    };
    try { Object.assign(counts, await operations.syncReturns(config, period, db)); }
    catch (error) { counts.claimWarning = error.message; }
    const finishedAt = new Date().toISOString();
    const saved = await db.from('sync_logs').update({
      status:counts.claimWarning?'PARTIAL':'SUCCESS', finished_at:finishedAt,
      rows_received:counts.orders + counts.orderItems + Number(counts.returns || 0),
      error_message:counts.claimWarning || null, metadata:{ counts, period, realtime:true, sellerDeliveryOnly:true }
    }).eq('id', started.data.id);
    if(saved.error)throw saved.error;
    return { syncLogId:started.data.id, status:counts.claimWarning?'PARTIAL':'SUCCESS', counts, period, finishedAt };
  } catch(error) {
    await db.from('sync_logs').update({ status:'FAILED', finished_at:new Date().toISOString(), error_message:error.message }).eq('id', started.data.id);
    throw error;
  }
...
module.exports = { syncAll, syncProducts, syncProductDetails, syncOrders, syncSettlements, syncRocketGrowthInventory, syncRocketGrowthInventoryOnly, syncRocketGrowthRealtime, syncSellerOrdersRealtime, saveRocketGrowthInventory, collection, nextToken, koreaDate };
