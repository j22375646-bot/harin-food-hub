...
async function syncAll() {
  ...
  try {
    ...
    const detailItemsById = new Map(detailResult.rows.map(item => [item.vendor_item_id,item]));
    const productItems = [
      ...new Map(
        orderItemRows
          .filter((item) => item.vendor_item_id)
          .map((item) => {
            const detailItem=detailItemsById.get(item.vendor_item_id);
            return [item.vendor_item_id,{
    ...
    const operationResult = await operations.syncOperations({
      config,
      period,
      vendorItemIds: productItems.map((item) => item.vendor_item_id),
    });
