...
async function syncItemInventory(vendorItemIds, db) {
  const rows = [];
  for (const id of [...new Set(vendorItemIds.filter(Boolean))].slice(0, 300)) {
...
module.exports = { collection, token, syncOperations, syncRocketGrowthOrders, syncReturns, syncExchanges, syncInquiries, syncItemInventory, syncSettlementSummaries, syncShippingCenters, syncBudgetsAndBrands };
