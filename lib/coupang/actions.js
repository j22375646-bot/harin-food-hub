...
async function executeOrderAction(action, input) {
  const config = configModule.getConfig();
  const shipmentBoxId = numericId(input.shipmentBoxId, '묶음배송번호');
  const orderId = numericId(input.orderId, '주문번호');
  const audit = await auditStart(action, 'ORDER', shipmentBoxId, input);
  try {
    let result;
    ...
    await auditFinish(audit, 'SUCCESS', result.data);
    return { requestId: audit.id, response: result.data, order: publicOrderDetail(refreshed) };
  } catch (error) {
    await auditFinish(audit, 'FAILED', error.response, error.message);
    throw normalizeCoupangError(error);
  }
}

async function executeCsAction(action, input) {
  const config = configModule.getConfig();
  ...
  const audit = await auditStart(action, 'INQUIRY', inquiryId, input);
  try {
    let path; let body;
...
async function executeCaseAction(action, input) {
  const config = configModule.getConfig();
  ...
  const audit = await auditStart(action, targetType, targetId, input);
  try {
    ...
    await auditFinish(audit, 'FAILED', normalized.response || error.response, normalized.message);
    throw normalized;
  }
}

module.exports = { getOrderDetail, executeOrderAction, executeCsAction, executeCaseAction, normalizeCoupangError };
