...
const inquiryOperations = require('./operations.js');
const { inquiryState } = require('./inquiry-state.js');
...
async function executeCsAction(action, input, options = {}) {
  ...
  const manageAudit = !options.audit;
  try {
    let path; let body; let currentInquiry;
    if (action === 'REPLY_ONLINE') {
      ...
      if (content.length < 2 || content.length > 1000) throw Object.assign(new Error('답변은 2~1,000자로 입력해주세요.'), { status: 400 });
      const current = await inquiryOperations.refreshCallCenterInquiry(inquiryId, audit.db, options);
      currentInquiry = current.inquiry;
      const state = inquiryState(current.item, 'CALL_CENTER');
      if (!state.canReply) throw Object.assign(new Error('현재 판매자 답변이 필요한 문의가 아닙니다. 최신 상태를 확인해주세요.'), { status: 409, code: 'COUPANG_INQUIRY_NOT_REPLYABLE' });
      if (state.parentAnswerId !== parentAnswerId) throw Object.assign(new Error('문의 이관 내역이 바뀌었습니다. 새 문의 내용을 확인한 뒤 전송해주세요.'), { status: 409, code: 'COUPANG_INQUIRY_CHANGED' });
      ...
      currentInquiry = current.inquiry;
      const state = inquiryState(current.item, 'CALL_CENTER');
      if (!state.confirmationRequired || state.completed || text(current.item.inquiryStatus).toLowerCase() !== 'progress') {
        throw Object.assign(new Error('현재 판매자 확인이 필요한 문의가 아닙니다. 최신 상태를 확인해주세요.'), { status: 409, code: 'COUPANG_INQUIRY_NOT_CONFIRMABLE' });
      }
    ...
    // A response timeout must never cause the client to repeat a customer reply.
    const result = await (options.request || client.request)('POST', path, {}, { body, maxAttempts: 1 });
    if (result.data?.code != null && !['200', 'SUCCESS'].includes(String(result.data.code).toUpperCase())) {
      throw Object.assign(new Error('쿠팡이 문의 처리를 승인하지 않았습니다. 원본 채널 상태를 확인해주세요.'), { status: 502, code: 'COUPANG_CS_WRITE_REJECTED' });
    }
    let verificationRequired = false;
    if (currentInquiry) {
      try {
        const item = await inquiryOperations.getCallCenterInquiry(inquiryId, options);
        const state = inquiryState(item, 'CALL_CENTER');
        const parentReply = action === 'REPLY_CALL_CENTER' && (Array.isArray(item.replies) ? item.replies : []).some(reply =>
          text(reply.answerType).toLowerCase() === 'vendor' && text(reply.parentAnswerId) === text(body.parentAnswerId));
        if (!state.answered && !state.completed && !parentReply) throw new Error('Reply completion has not been verified');
        const saved = await audit.db.from('coupang_inquiries').upsert(map.mapInquiry(item, 'CALL_CENTER', options), { onConflict: 'inquiry_key' });
        if (saved.error) throw saved.error;
      } catch (error) {
        verificationRequired = true;
        // The POST already succeeded. A failed read/save cannot turn it into a retryable write failure.
        await inquiryOperations.markInquiryUnverified(audit.db, currentInquiry, error, options).catch(() => {});
      }
    }
    if (manageAudit) await auditFinish(audit, 'SUCCESS', result.data);
    const operationLabel = action === 'CONFIRM_CALL_CENTER' ? '문의 확인 요청' : '답변 전송';
    return { requestId: audit.id, response: result.data, sent: true, verificationRequired,
      ...(verificationRequired ? { warning: `${operationLabel}은 완료됐습니다. 최신 처리 상태는 재수집 후 확인해주세요. 같은 요청을 다시 전송하지 마세요.` } : {}) };
  } catch (error) {
    if (manageAudit) await auditFinish(audit, 'FAILED', error.response, error.message);
    throw normalizeCoupangError(error);
  }
