...
function mapProductDetailItems(detail) {
  ...
  return (data.items || []).flatMap((item, index) => {
    ...
    if (!rg?.vendorItemId) return [];
    const price = rg.priceData || {};
    return [{
      vendor_item_id: text(rg.vendorItemId),
      ...
      raw_data: sanitize({ productName: data.sellerProductName, displayProductName: data.displayProductName, itemName: item.itemName, sellerProductItemId: rg.sellerProductItemId, itemId: rg.itemId, externalVendorSku: rg.externalVendorSku, priceData: price, barcode: rg.barcode, skuInfo: rg.skuInfo }),
      updated_at: new Date().toISOString()
    }];
  });
}
