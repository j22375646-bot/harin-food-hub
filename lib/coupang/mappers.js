...
const inquiryThread = require('./inquiry-thread.js');
...
function mapProductDetailItems(detail) {
  ...
  return (data.items || []).flatMap((item, index) => {
    ...
    const variants = [
      rg && { fulfillmentType:'ROCKET_GROWTH', value:rg },
      marketplace && { fulfillmentType:'MARKETPLACE', value:marketplace },
      !rg && !marketplace && { fulfillmentType:'ITEM', value:item }
    ].filter(Boolean);
    const rows = variants.flatMap(({ fulfillmentType, value }) => {
      const vendorItemId = value.vendorItemId ?? item.vendorItemId;
      if (!vendorItemId) return [];
      const price = value.priceData || item.priceData || {};
      return [{
        ...
        raw_data: sanitize({
          ...
          fulfillmentType,
          sellerProductItemId: value.sellerProductItemId ?? item.sellerProductItemId,
          itemId: value.itemId ?? item.itemId,
          vendorItemId,
          externalVendorSku: value.externalVendorSku ?? item.externalVendorSku,
          priceData: price,
          barcode: value.barcode ?? item.barcode,
          skuInfo: value.skuInfo ?? item.skuInfo,
    ...
    return [...new Map(rows.map(row=>[row.vendor_item_id,row])).values()];
  });
}
...
function mapSettlementRows(entry) {
  const details = entry.items || entry.orderItems;
  const items = Array.isArray(details) && details.length ? details : [entry];
  return items.map((item, index) => {
    ...
    const source = 'COUPANG_REVENUE_API';
    const amounts = Object.fromEntries(['saleAmount', 'serviceFee', 'settlementAmount'].map(field => [field, ledger.nullableAmount(item[field])]));
    const feeVat = ledger.nullableAmount(item.serviceFeeVat ?? item.serviceFeeVAT);
    const provenance = ledger.ledgerProvenance({ family: 'UNKNOWN', source, periodStart: recognitionDate, periodEnd: recognitionDate, amounts: [...Object.values(amounts), feeVat] });
    if (!orderId || orderId === '0' || !vendorItemId || vendorItemId === '0' || !saleType) provenance.reconciliation_status = 'AMBIGUOUS_IDENTITY';
    provenance.provenance.identity_version = 1;
    provenance.provenance.identity_evidence = 'UNVERIFIED_API_PROJECTION';
    return {
      ...provenance,
      // Preserve the exact original API projection key; v2 is file-ledger only.
      ...
      sale_amount: amounts.saleAmount,
      service_fee: amounts.serviceFee,
      service_fee_vat: feeVat,
      settlement_amount: amounts.settlementAmount,
      quantity: ledger.nullableAmount(item.quantity ?? item.saleCount),
...
function mapInquiry(item, type, options = {}) {
  ...
  return {
    ...
    question_text: type === 'CALL_CENTER' ? '상담이력' : text(item.content ?? item.question ?? item.inquiryContent ?? item.title),
    parent_answer_id: state.parentAnswerId,
    inquired_at: item.inquiryAt || item.createdAt || null, raw_data: raw, updated_at: updatedAt
  };
}
...
function mapSettlementSummary(item, month, context = {}) {
  const source = context.source === 'WING_PAYOUT_FILE' ? context.source : 'COUPANG_PAYOUT_API';
  const family = source === 'WING_PAYOUT_FILE' ? ledger.ledgerDeliveryFamily(context.deliveryFamily) : 'UNKNOWN';
  const sourceRecordId = source === 'WING_PAYOUT_FILE' ? text(item.sourceRecordId) : null;
  const periodStart = item.revenueRecognitionDateFrom || null;
  const periodEnd = item.revenueRecognitionDateTo || null;
  const key = source === 'COUPANG_PAYOUT_API'
    ? hash(`${month}:${item.settlementType}:${item.settlementDate}:${item.revenueRecognitionDateFrom}:${item.revenueRecognitionDateTo}`)
    : ledger.stableLedgerKey('PAYOUT', family, source, sourceRecordId ? ['ID', sourceRecordId] : [month, item.settlementType, item.settlementDate, periodStart, periodEnd]);
  const provenance = ledger.ledgerProvenance({ family, source, sourceRecordId, periodStart, periodEnd, amounts: [ledger.nullableAmount(item.finalAmount), ledger.nullableAmount(item.settlementTargetAmount)] });
  if (source === 'WING_PAYOUT_FILE' && (!sourceRecordId || sourceRecordId === '0')) provenance.reconciliation_status = 'AMBIGUOUS_IDENTITY';
  if (source === 'COUPANG_PAYOUT_API') {
    provenance.provenance.identity_version = 1;
    provenance.provenance.identity_evidence = 'UNVERIFIED_API_PROJECTION';
    if (!month || !item.settlementType || !item.settlementDate || !periodStart || !periodEnd) provenance.reconciliation_status = 'AMBIGUOUS_IDENTITY';
  }
  const { bankAccountHolder, bankName, bankAccount, ...safeItem } = item;
  return {
    ...provenance,
    ...
    status: text(item.status), total_sale: ledger.nullableAmount(item.totalSale), service_fee: ledger.nullableAmount(item.serviceFee),
    settlement_target_amount: ledger.nullableAmount(item.settlementTargetAmount), settlement_amount: ledger.nullableAmount(item.settlementAmount),
    last_amount: ledger.nullableAmount(item.lastAmount), pending_released_amount: ledger.nullableAmount(item.pendingReleasedAmount), final_amount: ledger.nullableAmount(item.finalAmount),
    seller_discount_coupon: ledger.nullableAmount(item.sellerDiscountCoupon), downloadable_coupon: ledger.nullableAmount(item.downloadableCoupon),
    raw_data: sanitize(safeItem), updated_at: new Date().toISOString()
  };
}
