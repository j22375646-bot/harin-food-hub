...
function flattenProducts(payload, collectedAt = new Date().toISOString()) {
  ...
  for (const origin of collection(payload)) {
    const originProduct = origin.originProduct || origin.origin_product || origin;
    ...
    for (const product of channels) {
      ...
      const representativeImage = product.representativeImage
        || product.representative_image
        || originProduct.representativeImage
        || originProduct.representative_image
        || null;
      const images = product.images || originProduct.images || null;
      rows.push({
        externalProductId,
        externalProductName:text(product.name || originProduct.name) || `네이버 상품 ${externalProductId}`,
        sellingPrice:number(product.discountedPrice ?? product.salePrice),
        isActive:text(product.statusType).toUpperCase() === 'SALE'
          && text(product.channelProductDisplayStatusType).toUpperCase() === 'ON'
          && number(product.stockQuantity) > 0,
        rawData:{
          ...product,
          representativeImage,
          images,
          originProductNo:product.originProductNo ?? originProduct.originProductNo ?? null,
          groupProductNo:product.groupProductNo ?? originProduct.groupProductNo ?? null,
...
async function sync({ db, now = new Date(), days = 31, paymentMaxPagesPerRun = 20, paymentMaxDurationMs = 20_000 } = {}) {
  ...
  try {
    ...
    await run('payment_period', async () => {
      // Runs on the existing fixed-IP worker; the HTTP route only queues this job.
      const paymentPeriod = await require('./payment-period.js').collectRecentPaymentPeriod({db,config,now,days,
        maxPagesPerRun:paymentMaxPagesPerRun,maxDurationMs:paymentMaxDurationMs});
      diagnostics.paymentPeriod = {status:paymentPeriod.status,syncLogId:paymentPeriod.syncLogId,
        order_coverage:paymentPeriod.order_coverage,failure:paymentPeriod.failure};
      if (paymentPeriod.status !== 'SUCCESS') errors.push({dataset:'payment_period',
        code:paymentPeriod.failure?.code || 'NAVER_PAYMENT_COLLECTION_FAILED',
        message:paymentPeriod.failure?.message || '네이버 결제기간 수집이 아직 완료되지 않았습니다.'});
    });
    const paymentInProgress = errors.length > 0 && errors.every(error => error.dataset === 'payment_period' && error.code === 'NAVER_PAYMENT_BUDGET_EXHAUSTED');
    const status = errors.length ? (paymentInProgress || Object.values(counts).some(Boolean)) ? 'PARTIAL' : 'FAILED' : 'SUCCESS';
