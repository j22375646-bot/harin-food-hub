...
function nextMoreSequence(payload) {
  return payload?.data?.moreSequence ?? payload?.moreSequence ?? null;
}
...
async function fetchOrderChanges(config, { start, end }) {
  ...
  for (let cursor = new Date(start); cursor < end; cursor = new Date(cursor.getTime() + DAY_MS)) {
    ...
    for (let page = 0; page < 100; page += 1) {
      ...
      const batch = collection(result.data);
      rows.push(...batch);
      const next = nextMoreSequence(result.data);
      if (!next || next === moreSequence || batch.length < 300) break;
      moreSequence = next;
    }
  }
  return rows;
...
async function fetchSettlements(config, { startDate, endDate }, maxPages = 100) {
  const rows = [];
  for (let pageNumber = 1; pageNumber <= maxPages; pageNumber += 1) {
    const result = await client.request('GET', '/v1/pay-settle/settle/daily', {
      config,
      query:{ startDate, endDate, pageNumber, pageSize:1000 },
    });
    const batch = collection(result.data);
    rows.push(...batch);
    if (batch.length < 1000 || result.data?.last === true) break;
  }
  return rows;
}
...
async function sync({ db, now = new Date(), days = 31 } = {}) {
  ...
  try {
    ...
    await run('settlements', async () => {
      const rows = await fetchSettlements(config, { startDate:dateOnly(start), endDate:dateOnly(now) });
    ...
    const updated = await db.from('sync_logs').update({
      ...
      metadata:{ counts, errors, fixedIp:true, sourceIp:process.env.COUPANG_ALLOWED_SOURCE_IP || null, writeEnabled:false },
    }).eq('id', started.data.id);
    if (updated.error) throw updated.error;
    return { syncLogId:started.data.id, status, counts, errors, fixedIp:true };
  } catch (error) {
