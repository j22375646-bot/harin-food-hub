...
async function getAccessToken(config, options = {}) {
  if(process.env.MOAON_MANAGED_KEYS_ENABLED==='1')config=getConfig(await require('../integrations/managed-keys.js').environment('NAVER'));
  config ||= getConfig(await require('../integrations/managed-keys.js').environment('NAVER'));
  options.signal?.throwIfAborted();
  config = { ...config };
  // Credentials, not just SELF/SELLER, identify the token owner. Keep no raw secret in the cache key.
  const cacheKey = createHash('sha256').update(JSON.stringify([config.clientId, config.clientSecret, config.tokenType, config.accountId])).digest('hex');
  if (options.force) resetTokenCache();
  const generation = cacheGeneration;
  const now = Date.now();
  if (cachedToken?.key === cacheKey && cachedToken.expiresAt - now > 60_000) return cachedToken.value;
  ...
  if (generation === cacheGeneration) cachedToken = {
    key:cacheKey,
...
async function request(method, pathname, options = {}) {
  const config = process.env.MOAON_MANAGED_KEYS_ENABLED==='1'?getConfig(await require('../integrations/managed-keys.js').environment('NAVER')):options.config || getConfig(await require('../integrations/managed-keys.js').environment('NAVER'));
  const url = new URL(`${BASE_URL}${pathname}`);
  for (const [key, value] of Object.entries(options.query || {})) {
    if (value !== undefined && value !== null && value !== '') url.searchParams.set(key, String(value));
  }
  let accessToken = await getAccessToken(config,{signal:options.signal});
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    if (response.status === 401 && !refreshedAuth) {
      refreshedAuth = true;
      accessToken = await getAccessToken(config, { force:true,signal:options.signal });
      continue;
    }
...
function resetTokenCache() { cacheGeneration++; cachedToken = null; }
