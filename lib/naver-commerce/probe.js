...
async function probeReadAccess({ db, now = new Date() } = {}) {
  ...
  try {
    ...
    const [products, orderClaims, inquiries] = await Promise.all([
      ...
      checked(db, '/v1/contents/qnas', () => client.request('GET', '/v1/contents/qnas', { config }))
    ]);
    const checks = [products, orderClaims, inquiries];
