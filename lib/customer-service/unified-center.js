...
function buildChannelStates(channelConnections = [], rows = []) {
  return ['NAVER','COUPANG','CAFE24'].map(platform => {
    ...
    return {
      platform, label:PLATFORM_LABELS[platform], status:inquiries.status === 'READY' || claims.status === 'READY' ? 'READY' : inquiries.status,
      statusLabel:inquiries.status === 'READY' || claims.status === 'READY' ? '수집 가능' : inquiries.label,
      message:inquiries.status === 'READY' || claims.status === 'READY'
        ? `${rows.filter(row=>row.platform===platform).length}건 통합 표시`
