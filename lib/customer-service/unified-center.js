...
const COMPLETED_CASE_STATUS =
  /(?:COMPLETE|COMPLETED|SUCCESS|DONE|CANCELLED|CANCELED|REJECT|WITHDRAW|REFUND)/i;
...
const REPLY_TEMPLATES = [
  {
    ...
    label: "확인 후 안내",
    content:
      "안녕하세요, 하린식품입니다. 문의하신 내용을 확인 중이며 확인되는 대로 빠르게 안내드리겠습니다. 잠시만 기다려 주세요. 감사합니다.",
  },
  {
    id: "SHIPPING",
    label: "배송 확인",
    content:
      "안녕하세요, 하린식품입니다. 주문하신 상품의 출고 및 배송 상태를 확인한 뒤 정확한 내용을 안내드리겠습니다. 이용에 불편을 드려 죄송합니다.",
  },
  {
    id: "RETURN",
    label: "반품 접수",
    content:
      "안녕하세요, 하린식품입니다. 반품 요청 내용을 확인했습니다. 상품 회수 및 입고 상태를 확인한 뒤 환불 절차를 안내드리겠습니다.",
  },
  {
    id: "EXCHANGE",
    label: "교환 접수",
    content:
      "안녕하세요, 하린식품입니다. 교환 요청 내용을 확인했습니다. 회수 및 교환상품 준비 상태를 확인한 뒤 진행 상황을 안내드리겠습니다.",
  },
  {
    id: "THANKS",
    label: "처리 완료",
    content:
      "안녕하세요, 하린식품입니다. 요청하신 처리가 완료되었습니다. 이용해 주셔서 감사드리며, 추가 문의가 있으시면 언제든 남겨 주세요.",
  },
];
...
function dueState({ occurredAt, completed = false, now = new Date() }) {
  ...
  if (kstDay(occurredAt) === kstDay(now))
    return {
      code: "TODAY",
      label: "오늘 처리",
      priority: 1,
...
function channelCapability(channel, key) {
  const capability = (channel?.capabilities || []).find(
    (item) => item.key === key,
  );
  return (
    capability?.read || {
...
function buildOrderIndex({
  ...
  for (const order of coupangOrders)
    index.set(text(order.order_id), {
      orderId: text(order.order_id),
      platform: "COUPANG",
      status: order.status || null,
      orderedAt: order.ordered_at || order.paid_at || null,
      shipmentBoxId: text(order.shipment_box_id),
      amount: Number(order.gross_amount || 0),
      products: [],
    });
  for (const item of coupangOrderItems) {
    ...
    order.products.push({
      name: item.product_name || "상품명 확인 필요",
      option: item.raw_data?.vendorItemName || null,
  ...
  for (const order of cafe24Orders)
    index.set(text(order.order_id), {
      ...
      amount: Number(order.paid_amount || order.order_price || 0),
      products: [],
    });
...
function auditIndex(audits = []) {
  ...
  for (const audit of [...audits].sort(
    (a, b) =>
      dateMs(b.executed_at || b.created_at || b.requested_at) -
      dateMs(a.executed_at || a.created_at || a.requested_at),
  )) {
    ...
    if (!result.has(key))
      result.set(key, {
        ...
        requestedAt: audit.created_at || audit.requested_at || null,
        executedAt: audit.executed_at || null,
...
function normalizeInquiry(item, orderIndex, audits, now) {
  ...
  return {
    id: `COUPANG:INQUIRY:${item.inquiry_key || item.inquiry_id}`,
    platform: "COUPANG",
    kind: "INQUIRY",
    kindLabel:
      item.inquiry_type === "CALL_CENTER" ? "고객센터 문의" : "상품 문의",
    sourceId: text(item.inquiry_id),
    occurredAt: item.inquired_at || null,
    completed,
    due,
    status: item.status || (completed ? "ANSWERED" : "WAITING"),
    orderId: text(item.order_id) || null,
    productId:
      text(item.vendor_item_id || item.seller_product_id || item.product_id) ||
      null,
    title:
...
function normalizeCase(item, fallbackKind, orderIndex, audits, now) {
  ...
  return {
    id: `COUPANG:${kind}:${sourceId}`,
    platform: "COUPANG",
    kind,
    kindLabel: { CANCEL: "취소", RETURN: "반품", EXCHANGE: "교환" }[kind],
    sourceId,
    occurredAt: item.requested_at || null,
    completed,
    due: dueState({ occurredAt: item.requested_at, completed, now }),
    status: item.status || "REQUESTED",
    orderId: text(item.order_id) || null,
    title: item.reason_text || item.cancel_type || `${kind} 요청`,
    content: item.reason_text || item.cancel_type || "요청 사유 확인 필요",
    order: orderIndex.get(text(item.order_id)) || null,
    audit:
      audits.get(`${isReturn ? "RETURN" : "EXCHANGE"}:${sourceId}`) || null,
    source: item,
  };
}

function normalizeChannelItem(item, orderIndex, now) {
  ...
  const occurredAt = item.occurred_at || item.occurredAt || null;
  const completed = Boolean(item.completed);
  return {
    id: item.source_key || `${platform}:${kind}:${sourceId}`,
    platform,
    kind,
    kindLabel: {
      INQUIRY: item.source_subtype || "문의",
      CANCEL: "취소",
      RETURN: "반품",
      EXCHANGE: "교환",
    }[kind],
    ...
    occurredAt,
    completed,
    due: dueState({ occurredAt, completed, now }),
    status: item.status || (completed ? "COMPLETED" : "WAITING"),
    orderId: text(item.order_id || item.orderId) || null,
    productId: text(item.product_id || item.productId) || null,
    title: item.title || `${PLATFORM_LABELS[platform] || platform} ${kind}`,
    content: item.content || "수집된 상세 내용이 없습니다.",
    order: orderIndex.get(text(item.order_id || item.orderId)) || null,
    audit: null,
    source: {
      ...(item.raw_summary || item.rawSummary || {}),
      collector: "UNIFIED_CUSTOMER_SERVICE",
    },
  };
}
...
function buildChannelStates(
  ...
  return ["NAVER", "COUPANG", "CAFE24"].map((platform) => {
    ...
    const permissionReady =
      inquiries.status === "READY" || claims.status === "READY";
    const collectedCount = rows.filter(
      (row) => row.platform === platform,
    ).length;
    const liveCollectorConnected =
      platform === "COUPANG" ||
      collectorPlatforms.includes(platform) ||
      collectedCount > 0;
    if (permissionReady && !liveCollectorConnected) {
      return {
        ...
        message:
          "읽기 권한은 확인됐고 실제 문의·클레임 수집 연결을 준비 중입니다.",
    ...
    return {
      platform,
      label: PLATFORM_LABELS[platform],
      status: permissionReady ? "READY" : inquiries.status,
      statusLabel: permissionReady ? "수집 가능" : inquiries.label,
      message: permissionReady
        ? liveCollectorConnected
          ? `실수집 연결됨 · 현재 ${collectedCount}건`
...
function buildUnifiedCustomerService(input = {}) {
  ...
  const rows = [
    ...(input.coupangInquiries || []).map((item) =>
      normalizeInquiry(item, orders, audits, now),
    ),
    ...(input.coupangReturns || []).map((item) =>
      normalizeCase(item, "RETURN", orders, audits, now),
    ),
    ...(input.coupangExchanges || []).map((item) =>
      normalizeCase(item, "EXCHANGE", orders, audits, now),
    ),
    ...(input.channelItems || []).map((item) =>
      normalizeChannelItem(item, orders, now),
    ),
  ].sort(
    (a, b) =>
      a.due.priority - b.due.priority ||
      dateMs(b.occurredAt) - dateMs(a.occurredAt),
  );
  ...
  return {
    ...
    rows,
    active,
    channelStates: buildChannelStates(
      input.channelConnections || [],
      rows,
      input.collectorPlatforms || [],
    ),
...
function filterRows(rows = [], filters = {}) {
  ...
  return rows.filter((row) => {
    ...
    if (filters.kind && filters.kind !== "ALL" && row.kind !== filters.kind)
      return false;
    if (filters.due && filters.due !== "ALL" && row.due.code !== filters.due)
      return false;
