...
const COUPANG_EPOST_COURIER_CODE = 'EPOST';
const CAFE24_EPOST_COURIER_CODE = '0012';
const NAVER_EPOST_COURIER_CODE = 'EPOST';
const PLATFORM_EPOST_COURIER_CODES = Object.freeze({
  COUPANG:COUPANG_EPOST_COURIER_CODE,
  CAFE24:CAFE24_EPOST_COURIER_CODE,
  NAVER:NAVER_EPOST_COURIER_CODE
});
const EPOST_COURIER_ALIASES = new Set(['EPOST', '0012', '0013', '00004']);
...
function courierCode(platform, value) {
  const channel = text(platform).toUpperCase();
  const normalized = text(value).toUpperCase();
  if (normalized && !/^[A-Z0-9_-]{2,20}$/.test(normalized)) {
    throw Object.assign(new Error('배송업체 코드를 확인하세요.'), { status:400 });
  }
  const expected = PLATFORM_EPOST_COURIER_CODES[channel];
  if (!expected) {
    throw Object.assign(new Error('송장 전송을 지원하지 않는 채널입니다.'), { status:409 });
  }
  if (normalized && !EPOST_COURIER_ALIASES.has(normalized)) {
    throw Object.assign(new Error(`${channel} 우체국택배 배송사 코드를 확인하세요.`), { status:400, code:`${channel}_EPOST_COURIER_INVALID` });
  }
  return expected;
}
...
module.exports = {
  CAFE24_OPERATION,
  COUPANG_EPOST_COURIER_CODE,
  CAFE24_EPOST_COURIER_CODE,
  NAVER_EPOST_COURIER_CODE,
  PLATFORM_EPOST_COURIER_CODES,
