...
// Harin Food's Cafe24 mall has Korea Post registered as 0012. Cafe24 validates
// shipment carriers against the mall's registered carrier list, so the public
// carrier-table value alone is not sufficient for this write API.
...
function courierCode(platform, value) {
  ...
  if (platform === 'CAFE24') {
    const configured = text(process.env.CAFE24_EPOST_SHIPPING_COMPANY_CODE).toUpperCase();
    const candidate = normalized || configured || CAFE24_EPOST_COURIER_CODE;
    if ([CAFE24_EPOST_COURIER_CODE, '0013', '00004'].includes(candidate)) return CAFE24_EPOST_COURIER_CODE;
    throw Object.assign(new Error('Cafe24 우체국택배 배송사 코드는 하린식품 쇼핑몰 등록값(0012)이어야 합니다.'), { status:400, code:'CAFE24_EPOST_COURIER_INVALID' });
  }
