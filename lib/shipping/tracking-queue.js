...
function trackingStatesFromRows(rows=[]) {
  ...
  for(const row of rows) {
    ...
    for(const hubOrderId of ids.map(text).filter(Boolean)) {
      ...
      else if(latest[hubOrderId].status!=='SUCCESS'&&row.status==='SUCCESS'){
        const observed=publicState(row,result,payload,hubOrderId);
        const refresh=latest[hubOrderId];
        if(observed.trackingNo===refresh.trackingNo&&observed.statusCode){
          latest[hubOrderId]={...observed,refreshStatus:refresh.status,refreshError:refresh.error,
            statusLabel:`${observed.statusLabel} · ${refresh.status==='FAILED'?'최근 재조회 실패':'재조회 중'} (마지막 확인 결과)`};
        }
      }
    }
  }
...
async function latestTrackingRows(db,{metadataLimit=1000,batchSize=100}={}) {
  ...
  const needsObservation=new Set();
  for(const row of metadata.data||[]){
    const targetId=text(row.target_id);
    if(!row.id||!targetId)continue;
    if(seenTargets.has(targetId)){
      if(needsObservation.has(targetId)&&row.status==='SUCCESS'){
        latestIds.push(row.id);needsObservation.delete(targetId);
      }
      continue;
    }
    seenTargets.add(targetId);
    latestIds.push(row.id);
    if(row.status!=='SUCCESS')needsObservation.add(targetId);
  }
