...
- 상품명/판매처/가격 직접 입력 또는 엑셀 3열 표 붙여넣기로 최대 6개 상품을 비교한다. 가격순 정렬, 확인된 가격의 최저/최고/중간 값을 표시한다. 개인화된 쇼핑 순위, 시장 전체 평균, 경쟁사 판매량·실매출로 표현하지 않는다.
- 공식 공지 https://developers.naver.com/notice/article/32564 및 /32530을 추가 확인했다. 쇼핑 검색 API는 2026-07-31 종료되고 API HUB 이관 대상에서도 제외되었다. 0.143.0의 별도 키를 통한 호출 설명은 오류였다. 0.143.1에서 네트워크 호출 자체와 키 준비 안내를 제거하고 서비스 종료를 명시했다.
...
- 기존 검색 API 레퍼런스는 남아 있지만 종료 공지가 우선한다. 종료된 엔드포인트에 키가 있어도 요청하지 않는 것을 테스트했다.
- 변경 범위 20개 PASS. 표 붙여넣기·가격 정렬·종료 안내를 포함한 소스 가시 격리 UI PASS. 수정본 설치/공개 검증 후 아래 기록한다.
- 0.143.0 공개 배포 뒤 종료 공지를 확인했으므로 최종 배포는 0.143.1로 교체한다. 0.143.0을 최종 인수 버전으로 표시하지 않는다.
