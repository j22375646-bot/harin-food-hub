...
Controller clarification: reject-side locking alone cannot stop a future begin after rejection commits. Candidate recovery-review.sql may therefore add a journal-aware BEFORE INSERT guard on password_changes. Under the existing begin account lock, it locks/checks the corresponding journal before allowing the insert; terminal/cross-user journal reuse must fail. No-journal legacy begin remains unchanged. The guard does not mutate account/session values; the prior fence SQL file remains untouched. Native tests must cover begin-first and reject-first orderings and legacy no-journal begin in a database with this guard installed.
