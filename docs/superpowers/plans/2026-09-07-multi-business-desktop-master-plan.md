...
> **최종 갱신: 2026-09-08. 이 문서가 전체 개발계획과 진행 현황의 기준 문서다.** 제품명은 모아온(MOAON). Windows 조회용 EXE는 인도되었지만 다사업장 실사용·앱 송장 발급·실제 출력은 아직 미개방이다. 개별 코드 완료를 전체 단계 완료로 표시하지 않는다.
...
| P5 EXE·발급·출력 | 진행 중 | 조회·내용 확인용 0.11.0 인도 기록. P5-09 주문별 작업 기록/중복 실행 차단 보완, 실제 앱 발급·프린터·업데이트 검수/G5는 미완료 |
...
**현재 집중:** P5-09 주문별 작업 관리. 사업장당 한 건이던 기록을 사업장+주문별로 분리하고, 같은 주문의 동시 실행은 하나의 작업으로 모은다. 이전 기록은 삭제하지 않으며 결과 불명 주문을 새 작업으로 재발급하지 않는다. 서버 안전장치의 기존 운영 적용 기록과 별개로, PC 발급 코어/통신/작업 관리는 아직 실제 발급 버튼에 연결하지 않았다.

0.11.0에는 선택 주문 재조회와 정보 확인용 Windows 창이 있다. 이번 주문별 기록/작업 관리자 변경은 내부 코드이며 새 EXE를 생성하거나 설치하지 않았다. 발급 통신의 인증 세션 연결과 실제 발급 확인·진행 UI는 남아 있다.

**다음 순서:** Main 호스트의 인증/단일 작업/고정 통신 연결 → 명시적 발급 확인창과 진행 UI → 연결 해제·재시작·오류 통합 검증 → 새 EXE 인도 → 별도 확인 후 실제 송장·프린터 검증. 타 사업장 개방은 P1/P2/P3 게이트를 우회하지 않는다.

**바로가기:** [최신 개발 보고서 / 주문별 작업 관리](./2026-09-08-moaon-p5-09-order-journal-report.md) · [0.11.0 확인창·설치파일 기록](./2026-09-08-moaon-p5-09-confirmation-report.md) · [Windows 세부 계획](./2026-09-07-windows-private-hub-roadmap.md)
...
| P5-09 호스트·확인 UI·실통신 | 인증된 주문을 명시적 확인 후 발급하고 진행 표시 | 다음 개발, 미완료 |
