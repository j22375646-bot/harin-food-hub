...
**우선순위 변경 — 앱 업무 묶음 먼저:** 사용자가 사업장 분리 작업의 지연을 지적했다. 앞으로 기반 코드/설치/검증만 별도 기능처럼 나눠 인도하지 않고, 실제 앱 업무 단위로 구현·검증·설치까지 묶는다. 다사업장 외부 개방은 기존 안전 조건을 유지한 채 보류한다. 다음 앱 작업은 ‘오늘’의 실제 저장 주문 요약·상태별 바로가기를 우선한다. 전체 웹 메뉴 이전은 여전히 미완료다.
