...
**최신 P4-12: 기존 OWNER와 하린식품 사업장 등록·목록 API 서버 연결.** 기존 활성/인증 계정 대조 후 전용 제어 테이블에 하린식품과 OWNER 소속을 원자적으로 등록했다. API는 제한 DB와 실제 인증 검증을 사용하도록 연결했고 초기화 재사용·실패 정리·재시도를 추가했다. 기존 OWNER-only proxy와 업무 데이터는 유지한다. 새 사업장 선택 UI·타 사업자 접근·전체 업무 격리는 아직 미완료다. [P4-12 코드·운영 반영·검증 결과](./2026-09-08-moaon-p4-12-business-runtime-report.md)
