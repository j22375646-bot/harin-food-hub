...
**최신: P5-11 / 0.13.1 인쇄 상태·재출력 보호.** 104개 테스트 및 패키징 코드 화면 시험 통과, 설치파일 생성. 이 PC는 설치 등록상 0.9.0이며 실물 프린터가 목록에 없어 설치/로그인 유지·실제 출력 인수는 남아 있다. [P5-11 보고서·설치 안내](./2026-09-08-moaon-p5-11-print-retry-report.md)

**최신 개발 작업: P5-10 / 0.13.0 송장 미리보기·인쇄 연결.** 문서 검사/이동 오류 해결, 99개 단위 테스트 및 실제 Electron/패키징 코드 화면 시험 통과. **설치파일 인도 준비 완료**, 사용자 앱 자동 교체·실제 프린터 인수는 미실시다. [P5-10 개발 내역·검증 결과·다음 작업](./2026-09-08-moaon-p5-10-label-preview-report.md)
...
| P5 EXE·발급·출력 | 진행 중 | 0.12.0 인증·확인창·우체국 요청·상태 UI 연결 및 격리 시험. 실제 발급·프린터·업데이트 인수/G5는 미완료 |
...
**현재 집중:** P5-10 / 0.13.0. 기존 P5-09 발급 연결을 유지하고 등록 송장 조회·미리보기·명시적 인쇄 설정 연결을 추가했다. 수취인·송장번호·주문 상태를 대조하고 연결 해제 시 창을 닫는다. 다음은 설치/재실행 및 실제 용지·프린터 인수다. 기존 결과 불명 기록을 지우거나 POST를 자동 재시도하지 않는다.
...
**바로가기:** [최신 개발 보고서 / 0.12.0 발급 연결](./2026-09-08-moaon-p5-09-shipment-host-report.md) · [이전 주문별 작업 관리](./2026-09-08-moaon-p5-09-order-journal-report.md) · [Windows 세부 계획](./2026-09-07-windows-private-hub-roadmap.md)
