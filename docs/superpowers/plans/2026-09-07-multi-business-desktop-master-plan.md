...
**최신: P5-15 / 0.15.1 실제 실행 버전 표시.** 설정의 고정된 0.9.0 표시를 제거하고 로그인·설정이 호스트 버전을 읽도록 연결했다. 112개 테스트 및 패키징 코드 로그인 흐름 시험 통과. 설치파일 준비 완료이며 현재 PC·출고 PC 설치는 이번 단계에서 수행하지 않았다. [P5-15 보고서](./2026-09-08-moaon-p5-15-version-info-report.md)
...
**현재 집중:** P5-15 / 0.15.1 패키징까지 진행했다. 실제 설치 인수는 P5-12 / 0.13.1까지 확인된 상태다. 다음은 최신 설치본 업데이트, 실제 로그인 유효성과 출고 장비의 용지·프린터 인수다. 기존 발급/등록 송장 검증과 재출력 보호를 유지하며, 결과 불명 기록을 지우거나 POST를 자동 재시도하지 않는다.
...
**바로가기:** [최신 개발 보고서 / 0.15.1](./2026-09-08-moaon-p5-15-version-info-report.md) · [0.12.0 발급 연결](./2026-09-08-moaon-p5-09-shipment-host-report.md) · [Windows 세부 계획](./2026-09-07-windows-private-hub-roadmap.md)
