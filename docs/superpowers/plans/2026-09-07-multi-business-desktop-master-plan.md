...
**최신 개발 작업: P5-10 송장 미리보기·인쇄 연결.** 99개 단위 테스트 통과. 실제 Electron 문서 검사와 전체 앱 이동 시험에 실패가 있어 **개발 중·배포 보류**다. 0.12.0 이후 새 설치파일은 인도하지 않았다. [P5-10 개발 내역·검증 결과·다음 작업](./2026-09-08-moaon-p5-10-label-preview-report.md)
...
**현재 집중:** P5-09 호스트 통합 / 0.12.0. 인증된 카페24·쿠팡 판매자배송 한 주문을 네이티브 확인창 전후에 다시 검증하고, 주문별 기록을 남긴 뒤 고정 우체국 API로 요청한다. 앱 상세에서 접수/처리/완료/확인 필요 상태를 표시한다. 기존 결과 불명 기록을 지우거나 POST를 자동 재시도하지 않는다.
