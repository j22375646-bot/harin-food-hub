...
**현재 집중:** P4-63A 자동 업데이트 클라이언트·UI·업무 중 재시작 보호. 개발 단계마다 재설치하지 않고 소스 앱으로 확인하며 설치본0.52.1을 유지한다. 다음 배포 단계에는 실제 HTTPS 배포 주소와 Windows 코드 서명, 첫 updater 포함 버전의 설치 및 구버전→신버전 인수가 필요하다. 로그인 유지·실제 프린터 인수도 별도 미완료로 유지한다.
