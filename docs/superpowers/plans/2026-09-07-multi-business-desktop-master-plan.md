...
**최신: P5-14 / 0.15.0 로그인 우선 진입·디자인.** 샘플 메인보다 로그인 화면을 먼저 표시하고 저장된 세션 확인 후 업무 화면으로 연결한다. 취소/만료/403 복구와 라이트·다크 디자인, 110개 테스트 및 패키징 화면 시험 완료. 실행 중인 사용자 앱이 있어 현재 PC 업데이트는 중단했으며 설치파일은 준비됐다. [P5-14 보고서·설치 상태](./2026-09-08-moaon-p5-14-login-first-report.md)
...
**현재 집중:** P5-12 / 0.13.1 실제 설치·재실행 인수까지 진행했다. 다음은 실제 로그인 유효성과 출고 장비의 용지·프린터 인수다. 기존 발급/등록 송장 검증과 재출력 보호를 유지하며, 결과 불명 기록을 지우거나 POST를 자동 재시도하지 않는다.
