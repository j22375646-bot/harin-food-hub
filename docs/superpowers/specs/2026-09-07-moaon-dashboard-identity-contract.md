...
- 매 호출은 기존 `dashboard-auth.validateSession(credential, { db, touch:false })`로 서명·DB 세션을 두 번 확인한다. `resolveRequestSession`, proxy 신뢰 헤더, global profile role은 사용하지 않는다.
...
- 익명·삭제·현재 차단 계정, 변형/폐기/만료 세션, identity 변경은 거부한다. 두 세션 만료 중 이른 값을 사용하며 결과는 `{id,userId,email,emailVerified,expiresAt}`만 담은 frozen 객체다.
