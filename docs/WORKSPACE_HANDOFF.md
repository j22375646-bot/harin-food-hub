...
- 자동 전체수집은 Vercel이 매일 오전 5시 30분 큐에 등록하고 서울 고정 IP 서버가 실행한다.
- 5분 간격 워커는 허브의 수동 요청을 확인하는 역할만 하며, 요청이 없을 때 쿠팡 API를 호출하지 않는다.
- 로컬 예약작업은 PowerShell 숨김 창으로 실행하여 콘솔창을 띄우지 않는다.
- 수동 버튼은 `/api/coupang/sync`에 요청을 저장하고 서울 고정 IP 워커가 Realtime 이벤트로 즉시 처리한다.
- Vercel 오전 5시 30분 Cron은 Cafe24·네이버만 수집하며 쿠팡 API를 호출하지 않는다.
