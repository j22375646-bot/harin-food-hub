...
Release note: GitHub release exceeded 100 assets. The publisher now paginates asset listings and offers explicit --resume only when an existing uploaded installer matches the already signature-verified local installer by size and GitHub SHA-256 digest. This recovered the interrupted 0.174.0 metadata publication without replacing the installer. Installer and signed public manifest were verified separately.
