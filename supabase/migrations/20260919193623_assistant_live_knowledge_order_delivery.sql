-- Keep current authorization wrappers; change only notification timing.
do $patch$
declare src text; dst text;
begin
 select pg_get_functiondef(coalesce(to_regprocedure('public.moaon_assistant_preferences_before_chat(uuid,uuid,text,jsonb,text)'),to_regprocedure('public.moaon_assistant_preferences(uuid,uuid,text,jsonb,text)'))) into src;
 dst:=replace(replace(src,' and extract(hour from ts) between 8 and 19',''),' or extract(hour from ts) not between 8 and 19','');
 if dst=src then raise exception 'ORDER_WINDOW_PATCH_NOT_FOUND';end if;
 execute dst;
 select pg_get_functiondef('public.moaon_assistant_cases(uuid,uuid,text,jsonb,text)'::regprocedure) into src;
 dst:=replace(src,$v$s.checked_at>now()-interval '10 minutes'$v$,$v$s.checked_at>now()-interval '2 minutes'$v$);
 if dst=src then raise exception 'ORDER_INTERVAL_PATCH_NOT_FOUND';end if;
 execute dst;
end $patch$;
create or replace function public.moaon_assistant_learning(p_actor uuid,p_session uuid,p_hash text,p_input jsonb,p_worker_hash text default null) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 ...
 if a='LEARN_SUBMIT' then
  ...
  insert into public.moaon_learning_events(proposal_id,actor_id,kind,snapshot) values(p.id,p_actor,'SUBMIT',to_jsonb(p));
   ...
   update public.moaon_learning_proposals set status='APPROVED',published_id=newid,reviewed_by=null,revision=revision+1,updated_at=now() where id=p.id returning * into p;
  insert into public.moaon_learning_events(proposal_id,actor_id,kind,snapshot) values(p.id,p_actor,'AUTO_PUBLISH',to_jsonb(p));
  return jsonb_build_object('id',p.id,'status',p.status);
