update public.moaon_ads_settings set settings='{"monthly":false,"changes":false,"minClicks":30,"minCost":10000,"changePercent":30,"watchdog":false}'::jsonb||settings;
alter table public.moaon_ads_settings alter column settings set default '{"monthly":false,"changes":false,"minClicks":30,"minCost":10000,"changePercent":30,"watchdog":false,"daily":false,"weekly":false,"time":"09:00","notify":true,"failures":true}';
...
begin
 if p_worker_hash is null then
  ...
  if a is null or a not in ('ADS_READ','ADS_SAVE','ADS_REQUEST','ADS_CHANGE_TEST') then raise exception 'ASSISTANT_INVALID';end if;
 else
 ...
 return jsonb_build_object('revision',s.revision,'settings',s.settings,'watchdog',(select jsonb_build_object('status',status,'checkedAt',checked_at,'seenAt',seen_at,'delivery',delivery) from public.moaon_hermes_watchdog where tenant_id=tenant),'jobs',(select coalesce(jsonb_agg(to_jsonb(q)),'[]') from (select id,start_date,end_date,fresh,status,report_id,error_code,created_at,finished_at,delivery,summary from public.moaon_ads_jobs where tenant_id=tenant order by created_at desc limit 30) q));
end $$;
...
create table public.moaon_hermes_watchdog(tenant_id uuid primary key references moaon_control.tenants(id),status text not null default 'HEALTHY',checked_at timestamptz,seen_at timestamptz,delivery text not null default 'SKIPPED');
alter table public.moaon_hermes_watchdog enable row level security;
revoke all on public.moaon_hermes_watchdog from public,anon,authenticated,service_role;
create function public.moaon_assistant_watchdog(p_result text default null) returns jsonb language plpgsql security definer set search_path='' as $$
declare tenant uuid:='a3452bca-e259-40ed-a93d-b8bcc5c1b9e0';s public.moaon_hermes_watchdog;b public.moaon_assistant_bots;seen timestamptz;next_status text;
begin
 perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('moaon-watchdog:'||tenant::text,0));
 insert into public.moaon_hermes_watchdog(tenant_id) values(tenant) on conflict do nothing;
 if p_result is not null then
  if p_result not in ('SENT','FAILED','UNKNOWN') then raise exception 'ASSISTANT_INVALID';end if;
  update public.moaon_hermes_watchdog set delivery=p_result where tenant_id=tenant and delivery='UNKNOWN';return '{}';
 end if;
 if not coalesce((select (settings->>'watchdog')::boolean from public.moaon_ads_settings where tenant_id=tenant),false) then return '{"status":"DISABLED"}';end if;
 select * into s from public.moaon_hermes_watchdog where tenant_id=tenant;
 select worker_seen_at into seen from public.moaon_assistant_settings where tenant_id=tenant;
 next_status:=case when seen is null or seen<now()-interval '15 minutes' then 'STALE' else 'HEALTHY' end;
 update public.moaon_hermes_watchdog set checked_at=now(),seen_at=seen where tenant_id=tenant;
 if s.status=next_status then return jsonb_build_object('status',next_status);end if;
 select * into b from public.moaon_assistant_bots where tenant_id=tenant and slot='SUP';
 if not coalesce((b.settings->>'enabled')::boolean,false) or b.settings->>'chatId' !~ '^[1-9][0-9]{0,18}$' or not coalesce(b.settings->'allowedUsers' ? (b.settings->>'chatId'),false) then return '{"status":"RECIPIENT_REQUIRED"}';end if;
 update public.moaon_hermes_watchdog set status=next_status,delivery='UNKNOWN' where tenant_id=tenant;
 return jsonb_build_object('status',next_status,'seenAt',seen,'bot',to_jsonb(b));
end $$;
revoke all on function public.moaon_assistant_watchdog(text) from public,anon,authenticated;
grant execute on function public.moaon_assistant_watchdog(text) to service_role;
