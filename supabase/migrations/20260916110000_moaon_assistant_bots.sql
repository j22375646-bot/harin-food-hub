create table public.moaon_assistant_bots (
 tenant_id uuid not null, slot text not null check(slot in ('WORK','SOLO')), revision integer not null check(revision>0),
 username text not null, envelope jsonb not null, settings jsonb not null,
 applied_revision integer, runtime_status text not null default 'PENDING', checked_at timestamptz,
 test_id uuid, test_status text, test_at timestamptz, updated_at timestamptz not null default now(), primary key(tenant_id,slot)
);
alter table public.moaon_assistant_bots enable row level security;
revoke all on public.moaon_assistant_bots from public,anon,authenticated,service_role;
...
begin
 ...
 if a<>'BOT_LIST' then
  if (p_input->>'slot') is null or (p_input->>'slot') not in ('WORK','SOLO') then raise exception 'ASSISTANT_INVALID';end if;
