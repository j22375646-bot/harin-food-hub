create or replace function public.moaon_assistant_automation(p_actor uuid,p_session uuid,p_hash text,p_input jsonb,p_worker_hash text default null) returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
 if p_worker_hash is null then
  ...
  if a not in ('CONFIG','PULSE','CATALOG','ARTICLE','PROPOSE','CLAIM','RESULT') then raise exception 'ASSISTANT_INVALID';end if;
 end if;
 ...
 if a in ('CATALOG','ARTICLE') then
  if not coalesce((s.settings->>'knowledge')::boolean,false) then raise exception 'ASSISTANT_DISABLED';end if;
  if a='CATALOG' then return jsonb_build_object('items',(select coalesce(jsonb_agg(jsonb_build_object('id',id,'title',title,'updatedAt',updated_at)),'[]') from public.moaon_assistant_knowledge where tenant_id=tenant and deleted_at is null));end if;
  select * into k from public.moaon_assistant_knowledge where tenant_id=tenant and id=(p_input->>'id')::uuid and deleted_at is null;
  if not found then raise exception 'ASSISTANT_INVALID';end if;
  return jsonb_build_object('id',k.id,'title',k.title,'body',k.body,'updatedAt',k.updated_at);
 end if;
