-- Claim all eligible updates atomically, then send one bounded summary.
alter table public.moaon_case_events add column delivery_batch_id uuid;
...
begin
 ...
 if a='CASE_SAVE' then
  ...
  update public.moaon_case_events e1 set delivery='SKIPPED' where e1.tenant_id=tenant and e1.delivery='PENDING' and (
   not s.enabled or not coalesce((b.settings->>'enabled')::boolean,false) or e1.bot_revision is distinct from b.revision or e1.chat_id is distinct from b.settings->>'chatId'
