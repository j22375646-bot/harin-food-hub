alter table public.moaon_tasks add column deleted_at timestamptz, add column deleted_by uuid references public.dashboard_users(user_id);
...
begin
 ...
 'tasks',(select coalesce(jsonb_agg(to_jsonb(x) order by x.due_date,x.created_at),'[]') from (select * from public.moaon_tasks where tenant_id=p_tenant and deleted_at is null and (status='OPEN' or updated_at>now()-interval '90 days') order by due_date,id limit 1000) x),
 'truncated',(select count(*)>1000 from public.moaon_tasks where tenant_id=p_tenant and deleted_at is null and (status='OPEN' or updated_at>now()-interval '90 days')),
 'events',(select coalesce(jsonb_agg(to_jsonb(x) order by x.id),'[]') from (select e.id,e.task_id,e.kind,e.created_at from public.moaon_task_events e where e.tenant_id=p_tenant and e.recipient_id=p_actor and e.actor_id<>p_actor and e.kind='ASSIGNED' and exists(select 1 from public.moaon_tasks mt where mt.id=e.task_id and mt.tenant_id=p_tenant and mt.deleted_at is null) and e.created_at>now()-interval '7 days' order by e.id desc limit 100) x));
 ...
 elsif a in ('CHECK','COMPLETE','REOPEN','DELETE') then
 ...
 if a='DELETE' then return jsonb_build_object('saved',true,'id',t.id);end if;
 raise exception 'TEAM_NOT_FOUND';end if;
 if t.revision<>(p_input->>'revision')::integer then raise exception 'TEAM_CONFLICT';end if;
 if a='DELETE' then
 update public.moaon_tasks set deleted_at=now(),deleted_by=p_actor,revision=revision+1,updated_at=now() where id=t.id;
 insert into public.moaon_task_events(task_id,tenant_id,actor_id,recipient_id,kind) values(t.id,p_tenant,p_actor,t.assigned_to,'DELETE');
 return jsonb_build_object('saved',true,'id',t.id);
 end if;
 if a='CHECK' then
