...
alter table public.ai_analysis_results
  add constraint ai_analysis_results_analysis_type_check
    check (analysis_type in (
      ...
      'PAGE_INVENTORY','PAGE_SETTLEMENT','PAGE_COLLECTION','PAGE_NOTIFICATIONS',
  ...
  add constraint ai_analysis_results_page_scope_match_check
    check (
      ...
      (page_key = 'collection' and analysis_type = 'PAGE_COLLECTION') or
      (page_key = 'notifications' and analysis_type = 'PAGE_NOTIFICATIONS') or
...
comment on constraint ai_analysis_results_page_scope_match_check on public.ai_analysis_results is
  'Each Harin AI result belongs to exactly one page-specific analysis contract, including collection and notification operations.';
