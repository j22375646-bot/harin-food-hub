...
const OPTIONAL_MONEY=['advertisingStats','advertisingCharged'];
...
function project(payload,days){
 ...
 for(const row of payload.channels)if(!object(row)||!PLATFORMS.includes(row.platform)||!STATES.has(row.stateCode)||typeof row.label!=='string'||!row.label||!text(row.label)||typeof row.stateLabel!=='string'||!row.stateLabel||!text(row.stateLabel)||!MONEY.every(key=>number(row[key]))||!OPTIONAL_MONEY.every(key=>row[key]===undefined||number(row[key]))||!(row.advertisingSource===undefined||text(row.advertisingSource))||!text(row.basis)||!text(row.payoutBasis)||!timestamp(row.asOf))throw Error('Channel');
