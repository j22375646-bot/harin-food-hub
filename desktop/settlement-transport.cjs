...
function project(payload){
 if(!object(payload)||payload.ok!==true||payload.writePolicy!=='READ_ONLY'||!timestamp(payload.generatedAt)||!object(payload.period)||payload.period.days!==30||!timestamp(payload.period.start)||!timestamp(payload.period.end)||payload.period.start&&payload.period.end&&Date.parse(payload.period.start)>Date.parse(payload.period.end))throw Error('Payload');
...
function createSettlementTransport({fetch,timeoutMs=30000}={}){
 ...
 const run=async()=>{try{
  const response=await fetch(SETTLEMENT_URL,{method:'GET',credentials:'include',cache:'no-store',redirect:'error',signal:controller.signal});
  if(response.status!==200)return empty(({401:'LOGIN_REQUIRED',403:'FORBIDDEN',504:'TIMEOUT'})[response.status]||'UNAVAILABLE');
  if(response.redirected||response.url&&response.url!==SETTLEMENT_URL||Number(response.headers.get('content-length'))>262144)throw Error('Response');
...
module.exports={createSettlementTransport};
