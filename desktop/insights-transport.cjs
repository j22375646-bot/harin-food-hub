...
function project(p){
 ...
 return {status:'READY',writePolicy:'READ_ONLY',generatedAt:p.generatedAt,channel:{...Object.fromEntries(['platform','name','reportCount','revenue','profit','changeRate','cause','causeNote','action','actionNote'].map(k=>[k,c[k]])),currentPeriod:c.currentPeriod===null?null:Object.fromEntries(['start','end','createdAt'].map(k=>[k,c.currentPeriod[k]]))},reports:p.reports.map(row=>Object.fromEntries(['id','title','periodStart','periodEnd','createdAt'].map(k=>[k,row[k]]))),caveats:[...p.caveats]};
}
