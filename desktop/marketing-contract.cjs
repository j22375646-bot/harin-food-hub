...
function projectMarketing(value){
 ...
 const projectWorkbench=w=>w==null?null:{status:text(w.status,30),start:date(w.start),end:date(w.end),total:number(w.total),rows:list(w.rows,200,r=>({id:text(r.id,120),name:text(r.name,120),type:text(r.type,120),campaignId:text(r.campaignId??'',120),campaignName:text(r.campaignName??'',120),adgroupId:text(r.adgroupId??'',120),bid:number(r.bid??null),...m(r)}))};
 const workbench=projectWorkbench(value.workbench),searchTerms=projectWorkbench(value.searchTerms);
 const telegram=value.telegram==null?null:Object.fromEntries(['configured','enabled','sendingEnabled'].map(k=>{if(typeof value.telegram[k]!=='boolean')throw Error('Telegram state');return [k,value.telegram[k]];}));
 return {version:1,workbench,searchTerms,converting:projectWorkbench(value.converting),telegram,asOf:date(value.asOf),sourceAt:date(value.sourceAt),conversionBasis:value.conversionBasis,notes:list(value.notes,8,v=>text(v)),windows:list(value.windows,3,w=>{
