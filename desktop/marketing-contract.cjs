...
function projectMarketing(value){
 ...
 const projectWorkbench=w=>w==null?null:{status:text(w.status,30),start:date(w.start),end:date(w.end),total:number(w.total),rows:list(w.rows,200,r=>({id:text(r.id,120),name:text(r.name,120),type:text(r.type,120),...m(r)}))};
 ...
 return {version:1,workbench,searchTerms,telegram,asOf:date(value.asOf),sourceAt:date(value.sourceAt),conversionBasis:value.conversionBasis,notes:list(value.notes,8,v=>text(v)),windows:list(value.windows,3,w=>{
