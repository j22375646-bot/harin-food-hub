...
function metadata(row){return {business:row.business,provider:row.provider,status:'SAVED_UNVERIFIED',...(row.tenantId?{tenantId:row.tenantId}:{})};}
...
function createDraftStore({directory,safeStorage,platform='win32'}){
 ...
 async function read(){
  ...
  const rows=document.records.map(validate),keys=new Set(rows.map(identity));
