...
function prepare({sourceRoot,stage,channel=null,dependencyRoot}){
 ...
 fs.writeFileSync(path.join(stage,'package.json'),JSON.stringify(pkg,null,2));if(channel)fs.writeFileSync(path.join(stage,'update-channel.json'),JSON.stringify({enabled:true,url:channel.url,publisherName:channel.publisherName},null,2));
