...
async function publish(directory,{resume=false}={}){
 ...
 const existing=[];for(let page=1;;page++){const assets=await api(`/releases/${release.id}/assets?per_page=100&page=${page}`);existing.push(...assets);if(assets.length<100)break;}const uploaded=existing.find(a=>a.name===p.file);if(uploaded&&(!resume||uploaded.state!=='uploaded'||uploaded.size!==p.size||uploaded.digest?.toLowerCase()!=='sha256:'+require('node:crypto').createHash('sha256').update(fs.readFileSync(path.join(directory,p.file))).digest('hex')))throw Error('Version already published or resume digest mismatch');
 const upload=release.upload_url.replace(/\{.*$/,'');if(!uploaded)await api(upload+'?name='+encodeURIComponent(p.file),'POST',fs.readFileSync(path.join(directory,p.file)),true);
...
if(require.main===module)publish(path.resolve(process.argv[2]||''),{resume:process.argv.includes('--resume')}).then(v=>console.log(JSON.stringify(v))).catch(e=>{console.error(e.message);process.exitCode=1;});
