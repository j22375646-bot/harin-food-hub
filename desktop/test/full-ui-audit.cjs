...
(async()=>{
 ...
 for(const theme of ['light','dark'])for(const width of [1040,1440]){
  ...
  for(const route of routes){
   ...
   for(const group of groups){assert.deepEqual(group.border,['1px','1px','1px','1px'],'selection group has a balanced enclosing border');for(const button of group.buttons){assert.ok(Math.round(button.height*100)/100>=44,theme+'/'+width+'/'+route+' selection control '+JSON.stringify(button));assert.equal(button.contained,true,'selection stays inside its box');assert.ok(button.font>=14,'selection label readability');}}
