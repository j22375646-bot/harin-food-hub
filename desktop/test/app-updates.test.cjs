...
test('automatic checks download without restart and stop scheduling after disposal',async()=>{const {startAutomaticUpdates}=require('../app-updates.cjs');const timers=[];let checks=0,downloads=0,cleared;const stop=startAutomaticUpdates({updates:{check:async()=>{checks++;return {status:'AVAILABLE'};},download:async()=>{downloads++;}},setTimer:(fn,delay)=>{const t={fn,delay};timers.push(t);return t;},clearTimer:t=>cleared=t});assert.equal(timers[0].delay,10000);await timers[0].fn();assert.equal(checks,1);assert.equal(downloads,1);assert.equal(timers[1].delay,21600000);stop();assert.equal(cleared,timers[1]);});
