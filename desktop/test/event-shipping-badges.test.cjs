...
test('shipping badge respects strict 15:00 cutoff and never treats registration as departure',()=>{
 assert.equal(projectVisual(base).timing.label,'당일배송 대상');
 ...
 assert.equal(projectVisual({...base,invoiceNumber:'1234567890123'}).timing.label,'당일배송 대상');
 assert.equal(projectVisual({...base,shippingEstimate:{...base.shippingEstimate,confidence:'PARTIAL'}}).timing.type,'CHECK_REQUIRED');
 const delayed={...base,timingBadge:{type:'DELAYED'}};assert.equal(projectVisual(delayed).timing.label,'배송지연');assert.equal(projectVisual({...delayed,shippingEstimate:{confidence:'PARTIAL'}}).timing.label,'출고일 확인');
 assert.equal(projectVisual({...base,orderedAt:'2026-09-10T16:00:00+09:00'}).timing.label,'오늘 배송출발 예정');
});
