...
async function main(){
 ...
 try{
  ...
  assert.equal(await visualRow.locator('.delivery-badge').innerText(),'결제완료');
  assert.match(await visualRow.locator('.product-option').innerText(),/옵션: 30T 1상자 · 수량 1개/);assert.equal(await visualRow.locator('.product-thumbnail').evaluate(e=>e.getBoundingClientRect().width),64);await visualRow.click();assert.equal(await visualRow.locator('..').locator('.order-select').isChecked(),true);assert.equal(await page.locator('#order-selection').isVisible(),true);
