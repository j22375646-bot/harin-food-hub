...
(async()=>{
 ...
 try{
  ...
  const sourcePage=await page.evaluate(()=>connectionResult);
  await app.evaluate(({ipcMain},payload)=>{ipcMain.removeHandler('moaon-hub:find-order');ipcMain.handle('moaon-hub:find-order',()=>({status:'FOUND',page:payload}));},sourcePage);
  ...
  await followup.getByRole('button',{name:'주문 찾기',exact:true}).click();
  await page.waitForFunction(()=>document.querySelector('#shipping-history-status').textContent.includes('주문을 찾았습니다'));
  ...
  await page.evaluate(()=>document.querySelector('#server-history-load').click());
  await page.waitForFunction(()=>document.querySelector('#server-history-status').textContent.includes('2건'));
  const serverPanel=page.locator('#server-shipping-history');assert.equal(await serverPanel.getAttribute('open'),'','explicit refresh opens history');
