...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  const result=await preview.open({hubOrderId:'HR-C24-1234ABCD',trackingNo:'1234567890123',goodsName:'시험 상품',quantity:1,expectedReceiver:{name:'TEST',contact:'01012345678',postCode:'12345',address:'TEST ADDRESS'},validate:async()=>true});
  ...
  const oversized=await preview.open({hubOrderId:'HR-C24-1234ABCD',trackingNo:'1234567890123',goodsName:'가'.repeat(1500),quantity:1,expectedReceiver:{name:'TEST',contact:'01012345678',postCode:'12345',address:'TEST ADDRESS'},validate:async()=>true});
  assert.equal(oversized.status,'PRINT_UNAVAILABLE');console.log('PASS: oversized label refused; no physical print');
