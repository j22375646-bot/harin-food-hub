...
test('detail projection includes delivery allowlist without raw labels or image URLs', () => {
  ...
  assert.deepEqual(detail, {externalOrderId:'CHANNEL-123',receiver:{name:'SECRET',contact:'',postCode:'',address:'',addressDetail:'',message:''},items:[{name:'차',option:'3봉',quantity:3}],invoice:{status:'REGISTERED',number:'1234567890123'},delivery:{status:'IN_TRANSIT',source:'EPOST'},cancelled:false,cancellationRequested:true});
  assert.equal(JSON.stringify({...detail,receiver:null}).includes('SECRET'), false);
...
test('missing or malformed detail never becomes cleared cancellation or known shipment status', () => {
  for (const source of [{}, {items:'bad',invoice:{status:'REGISTERED',number:'123'},listDeliveryBadge:{status:'DELIVERED',source:'unknown'},cancelled:'false',cancellationRequested:0}]) {
    assert.deepEqual(project(source),{externalOrderId:'',receiver:{name:'',contact:'',postCode:'',address:'',addressDetail:'',message:''},items:[],invoice:null,delivery:null,cancelled:null,cancellationRequested:null});
  }
...
test('detail consumes actual server adapter delivery contract', () => {
  ...
  assert.equal(row.details.receiver.name,'PRIVATE');
});
