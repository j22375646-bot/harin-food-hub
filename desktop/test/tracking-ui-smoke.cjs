...
(async()=>{
 ...
 try{
  ...
  assert.equal(await app.evaluate(()=>globalThis.trackingReads),1,'opening registered detail reads tracking once automatically');
  assert.equal(await app.evaluate(()=>globalThis.trackingRefreshes),0,'opening detail must not enqueue tracking');
  ...
  console.log('PASS: automatic tracking read, explicit refresh, truthful pending, no stage mutation, compact results, logout');
