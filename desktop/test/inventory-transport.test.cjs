...
test('INVENTORY rejects invalid payloads, excessive rows, duplicates and crossed channel identities',async()=>{
 ...
 for(const [status,expected] of [[401,'LOGIN_REQUIRED'],[403,'FORBIDDEN'],[503,'SERVER_UNAVAILABLE']])assert.equal((await createInventoryTransport({fetch:async()=>new Response('',{status})})()).status,expected);
});
