...
async function main() {
  ...
  try {
      ...
      ses.webRequest.onBeforeRequest = (filter, handler) => register(filter, (details, callback) => {
        handler(details, decision => {
          if (details.method === 'POST' && details.url === 'https://harin-cafe24-sync.vercel.app/api/dashboard/login') {
            globalThis.testPostDecisions.push({cancel:decision.cancel});
    ...
    if (count !== 1) console.log(JSON.stringify({submitted,decisions:await app.evaluate(()=>globalThis.testPostDecisions),dom:await login.evaluate(()=>({state:document.readyState,connected:globalThis.emptyTestForm?.isConnected})).catch(()=>({closed:true}))}));
