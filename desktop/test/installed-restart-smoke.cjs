...
async function main(){
 ...
 for(let run=0;run<2;run++){
  ...
  try{
   ...
   await page.locator('[data-app-version]').first().filter({hasText:'v0.37.0'}).waitFor();
   ...
   assert.equal(state.version,'0.37.0');assert.equal(path.resolve(state.profile),path.resolve(profile));
