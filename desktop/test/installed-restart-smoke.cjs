...
async function main(){
 ...
 for(let run=0;run<2;run++){
  ...
  try{
   ...
   await page.locator('[data-app-version]').first().filter({hasText:'v'+require('../package.json').version}).waitFor();
   ...
   assert.equal(state.version,require('../package.json').version);assert.equal(path.resolve(state.profile),path.resolve(profile));
