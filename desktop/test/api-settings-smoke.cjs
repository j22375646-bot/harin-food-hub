...
(async()=>{const app=await launchDesktop({root:path.resolve(__dirname,'..'),executablePath:require('electron'),packaged:process.argv.includes('--packaged'),override:-1});try{
 ...
 await page.locator('#api-provider').selectOption('EPOST');
 for(const field of ['customerId','approvalNo','officeSerial','apiKey'])await page.locator(`[name="${field}"]`).fill('EPOST_TEST_ONLY');
 await page.locator('[name="securityKey"]').fill('가'.repeat(16));
 assert.equal(await page.locator('[name="securityKey"]').evaluate(el=>el.checkValidity()),false);
 await page.locator('[name="securityKey"]').fill('1234567890abcdef');
 assert.equal(await page.locator('[name="securityKey"]').getAttribute('type'),'password');
 assert.equal(await page.locator('[name="trackingApiKey"]').evaluate(el=>el.required),false);
 assert.doesNotMatch(await page.locator('#api-provider-note').innerText(),/Cafe24/);
 await page.locator('#api-draft-save').click();await page.waitForFunction(()=>document.querySelector('#api-draft-status').textContent.includes('저장됨'));
 assert.equal(await page.locator('[name="securityKey"]').inputValue(),'');
 assert.equal((await page.evaluate(()=>window.moaonHub.listApiDrafts()))[0].provider,'EPOST');
 await page.locator('#api-settings').scrollIntoViewIfNeeded();await page.screenshot({path:path.resolve(__dirname,'../dist/p461c-epost-settings.png')});
 await page.evaluate(()=>window.moaonHub.removeApiDraft({business:'시험 사업장',provider:'EPOST'}));
 await page.locator('#api-provider').selectOption('COUPANG');
