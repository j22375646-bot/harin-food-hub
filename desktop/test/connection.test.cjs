...
test('post-dialog authorization or snapshot replacement refuses the native export write',async()=>{for(const mode of ['auth','snapshot']){const changed='e'.repeat(64),book=new ExcelJS.Workbook();book.addWorksheet('주문').addRow(['ok']);const zip=Buffer.from(await book.xlsx.writeBuffer());let dialogOpen=false,writes=0;const remote=makeRemoteSession(async url=>{if(url.endsWith('&format=xlsx'))return new Response(zip,{headers:{'content-type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','x-moaon-search-contract':'1','x-moaon-export-count':'1','x-moaon-export-snapshot':TEST_SNAPSHOT}});if(dialogOpen&&mode==='auth')return new Response('',{status:401});return Response.json(makePagePayload({snapshot:dialogOpen&&mode==='snapshot'?changed:TEST_SNAPSHOT}));});const {connection}=makeConnection(remote,{saveOrderExport:async(_bytes,current)=>{dialogOpen=true;if(!await current())return 'DOCUMENT_CHANGED';writes++;return 'XLSX_SAVED';}});await connection.refresh();assert.equal((await connection.exportOrdersXlsx()).status,'DOCUMENT_CHANGED');assert.equal(writes,0);}});
