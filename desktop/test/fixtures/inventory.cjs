...
function database(data={},failure=null){const operations=[];return {operations,from(table){const q={};for(const m of ['select','in','order','limit'])q[m]=(...args)=>{operations.push([table,m,...args]);return q;};q.then=(resolve,reject)=>Promise.resolve({data:data[table]||[],error:table===failure?{message:'PRIVATE_SQL'}:null}).then(resolve,reject);return q;}};}
