...
async function main(){
 ...
 try {
  ...
  assert.ok((await page.locator('#order-detail').textContent()).includes('송장 이력 조회 상태 확인 필요'));
  assert.equal(await page.getByRole('button',{name:'주문 정보 새로고침',exact:true}).count(),1,'optional refresh remains available');
  await page.getByRole('button',{name:'주문 정보 새로고침',exact:true}).click();
  await page.getByRole('button',{name:'주문 정보 새로고침',exact:true}).waitFor();
  assert.ok((await page.locator('#order-detail').textContent()).includes('RECHECK-1'));
  for(const theme of ['light','dark']){
   ...
   for(const [width,height] of [[1040,720],[1920,1080],[1440,960]]) {
    ...
    await page.locator('.review-actions button').first().scrollIntoViewIfNeeded();
