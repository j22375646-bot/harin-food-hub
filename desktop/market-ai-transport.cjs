'use strict';
const {validCommand,projectResponse,empty}=require('./market-ai-contract.cjs');
const MARKET_AI_URL='https://harin-cafe24-sync.vercel.app/api/moaon/businesses/a3452bca-e259-40ed-a93d-b8bcc5c1b9e0/market-ai';
const METHODS={CONFIG:'GET',GENERATE:'POST'};
function createMarketAiTransport({fetch,timeoutMs=28000}={}){
 ...
 return async(command,{signal}={})=>{
  ...
  const run=async()=>{try{
   const method=METHODS[command.operation],body=method==='GET'?undefined:JSON.stringify(command.input);
   const res=await fetch(MARKET_AI_URL,{method,credentials:'include',cache:'no-store',redirect:'error',signal:controller.signal,headers:{Accept:'application/json',...(body?{'Content-Type':'application/json','Origin':'https://harin-cafe24-sync.vercel.app'}:{})},...(body?{body}:{})});
   if(res.redirected||res.url&&res.url!==MARKET_AI_URL||Number(res.headers.get('content-length'))>131072)throw Error('Response');
...
module.exports={MARKET_AI_URL,METHODS,createMarketAiTransport};
