...
function projectConfiguration(v){if(!object(v)||v.provider!=='GEMINI_FREE'||v.model!=='gemini-3.5-flash-lite'||typeof v.enabled!=='boolean'||typeof v.ready!=='boolean'||!['DISABLED','SETUP_REQUIRED','READY'].includes(v.status)||![20,500].includes(v.dailyLimit))throw Error('Invalid config');return {provider:v.provider,model:v.model,enabled:v.enabled,ready:v.ready,status:v.status,freeConfirmedAt:v.freeConfirmedAt==null?null:date(v.freeConfirmedAt),dailyLimit:v.dailyLimit};}
