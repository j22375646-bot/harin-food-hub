...
function projectConfiguration(v){if(!object(v)||v.provider!=='GEMINI_FREE'||v.model!=='gemini-2.5-flash-lite'||typeof v.enabled!=='boolean'||typeof v.ready!=='boolean'||!['DISABLED','SETUP_REQUIRED','READY'].includes(v.status)||v.dailyLimit!==20)throw Error('Invalid config');return {provider:v.provider,model:v.model,enabled:v.enabled,ready:v.ready,status:v.status,freeConfirmedAt:v.freeConfirmedAt==null?null:date(v.freeConfirmedAt),dailyLimit:20};}
function projectRun(v){
 if(!object(v)||!uuid(v.id)||v.scope!=='PUBLIC_SEARCH_TREND'||v.provider!=='GEMINI_FREE'||v.model!=='gemini-2.5-flash-lite'||v.status!=='SUCCEEDED'||!KINDS.includes(v.kind)||!['COMPLETE','PARTIAL','STALE','BLOCKED'].includes(v.dataState)||!/^([0-9a-f]{64})$/.test(v.snapshotHash)||!object(v.period)||!object(v.metrics)||Object.keys(v.metrics).length!==METRICS.length)throw Error('Invalid run');
