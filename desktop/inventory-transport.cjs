...
function project(p){
 ...
 if(p?.ok!==true||p.status!=='READY'||p.writePolicy!=='READ_ONLY'||typeof p.generatedAt!=='string'||!date(p.generatedAt)||p.truncated!==false||!Array.isArray(p.items)||p.items.length>200)throw Error('Invalid inventory');
 const ids=new Set();return {status:'READY',generatedAt:p.generatedAt,truncated:false,items:p.items.map(r=>{
  ...
  const keys=new Set();const channels=r.channels.map(c=>{
   ...
   return {platform:c.platform,family:c.family,state:c.state,quantity:c.quantity,updatedAt:c.updatedAt,stale:c.stale,stopped:c.stopped,unmanaged:c.unmanaged,detail:c.detail};
...
function createInventoryTransport({fetch,timeoutMs=30000}={}){
 ...
 return async({signal}={})=>{
  ...
  const run=async()=>{try{
   ...
   if(res.redirected||res.url&&res.url!==INVENTORY_URL||Number(res.headers.get('content-length'))>524288)throw Error('Response');
   reader=res.body?.getReader();if(!reader)throw Error('Body');let size=0;const chunks=[];
   while(true){const {done,value}=await reader.read();if(done)break;size+=value.byteLength;if(size>524288)throw Error('Size');chunks.push(value);}
