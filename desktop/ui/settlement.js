...
(()=>{
 ...
 function render(){
  ...
  const period=value?.period;select('settlement-period').textContent=period?.start&&period?.end?`${formatTime(period.start)} — ${formatTime(period.end)} · 최근 30일`:'최근 30일 · 조회 전';
  ...
  select('settlement-channels').replaceChildren(...(value?.channels||[]).map(channel=>{
   ...
   card.append(header,body,node('p','settlement-basis',channel.basis||'근거 자료 확인 필요'));return card;
  }));
