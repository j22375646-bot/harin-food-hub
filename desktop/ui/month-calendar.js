...
(()=>{
 ...
 function openEditor(){if(saving)return;el('month-create-date').value=selected;el('month-create-end').value=selected;el('month-compose').open=true;if(!editor.open)editor.showModal();el('month-create-title').focus();}
 el('month-add-selected').onclick=openEditor;
 function typeChanged(){const event=el('month-create-type').value==='EVENT',memo=el('month-create-type').value==='MEMO';el('month-event-fields').hidden=!event;el('month-event-fields').querySelectorAll('input,select,button').forEach(el=>el.disabled=!event);el('month-end-label').hidden=memo;el('month-create-time').disabled=event||memo;el('month-create-time').closest('label').hidden=event||memo;el('month-create-body').maxLength=event?2000:4000;el('month-create-save').textContent=event?'이벤트 등록':'일정 등록';}
 ...
 const remove=node('button','','삭제');remove.type='button';remove.onclick=()=>{row.remove();el('month-add-tier').disabled=false;};row.append(remove);el('month-gift-tiers').append(row);el('month-add-tier').disabled=el('month-gift-tiers').children.length>=10;};
