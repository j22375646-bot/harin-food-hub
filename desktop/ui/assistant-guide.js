...
(()=>{
 ...
 function render(){choices.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.guideRole===role)));body.replaceChildren();const steps=el('div','','guide-steps');body.append(steps);
  if(role==='user'){
   ...
   step(steps,1,'업무비서를 열고 시작을 눌러요','아래 업무비서 열기를 누르면 텔레그램으로 이동해요. 처음에는 대화창 아래 “시작”을 누르세요. 시작 버튼이 없으면 메시지 칸에 /start를 입력해 보내세요. 인증 코드는 업무비서가 보내므로, 다른 비서만 사용할 때도 업무비서는 먼저 시작해 주세요.');
   ...
   step(steps,5,'AI와 대화하고 싶다면 사용 승인을 받아요','알림 수신 인증과 Hermes AI 대화 권한은 별도예요. 봇에 인사를 보냈는데 승인 코드가 나오거나 답이 없으면 운영자에게 알려 주세요. 알림을 연결했다고 개인 AI 기억이나 대화 권한이 자동으로 분리되는 것은 아닙니다. 운영자 확인 후 봇에 “메뉴”를 보내 사용할 기능을 고르세요.',[['메뉴 문구 복사',()=>copy('메뉴')]]);
  }else if(role==='owner'){
