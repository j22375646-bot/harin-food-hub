...
(()=>{
 ...
 function render(){choices.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.guideRole===role)));body.replaceChildren();const steps=el('div','','guide-steps');body.append(steps);
  if(role==='user'){
   ...
   step(steps,5,'사용할 봇의 메뉴·대화를 켜요','내 알림 → 봇 메뉴·AI 대화 연결에서 사용할 비서를 체크하고 저장하세요. 약 2분 후 각 봇에 /start 또는 메뉴를 보내면 공용 메뉴가 나타나요. 새 사용자 AI 대화·기억은 별도 프로필로 준비됩니다. 주문·업무·광고 자료는 메뉴에서 조회하고, AI 대화에서는 질문과 개인 기억을 사용해요.',[['메뉴 문구 복사',()=>copy('메뉴')]]);
  }else if(role==='owner'){
 ...
 ['개인화가 어디까지 되어 있나요?','개인 수신처, 받을 비서, 시간·요일, 신규 주문 알림과 개인 브리핑의 배정 업무가 계정별로 구분됩니다. 봇 연결과 메뉴, 공용 제품 지식, 광고 자료 수집은 공용입니다. 새 사용자의 AI 대화·기억은 사용자별 프로필로 나누며 기존 대화를 복사하지 않아요. 응답 지침은 봇 공용입니다. 개인비서 메뉴는 인증한 모아온 계정의 업무만 조회해요.','preferences'],
 ...
 ['시험 이미지는 오는데 AI 대화가 안 돼요','내 알림에서 수신처 인증 후 봇 메뉴·AI 대화 연결을 켜고 저장하세요. 약 2분 후 각 봇에 메뉴를 보내세요. 메뉴는 되는데 AI만 답하지 않으면 Hermes 모델 로그인·사용량·서버 상태를 관리자가 확인해야 해요. 시험 이미지 성공은 AI 응답 성공을 뜻하지 않습니다.','preferences'],
