...
(()=>{
 ...
 function render(){choices.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.guideRole===role)));body.replaceChildren();const steps=el('div','','guide-steps');body.append(steps);
  if(role==='user'){
   step(steps,1,'필요한 봇을 골라요','업무비서는 주문·문의·브리핑, 개인비서는 내 업무·아이디어, 학습비서는 제품·운영 자료 정리, 관리비서는 연결 상태 점검, 광고비서는 네이버 광고 보고서 검토에 사용해요. 아래 사용자명을 텔레그램에서 검색하세요.');
   const bots=el('div','','guide-bot-list');for(const [title,handle] of [['업무비서','moaon_hub_bot'],['개인비서','moaon_solo_bot'],['학습비서','moaon_study_bot'],['관리비서','moaon_sup_bot'],['광고비서','moaon_ad_bot']]){const card=el('article');card.append(el('strong',title),el('code','@'+handle));button(card,'사용자명 복사',()=>copy('@'+handle));bots.append(card);}steps.append(bots);
 ...
 ['엄마나 동료도 같은 봇을 쓰게 하고 싶어요','업무비서는 운영자가 허용 사용자와 개인·그룹 수신처를 설정합니다. 그룹에서는 봇을 추가한 뒤 /메뉴 대신 /moaon@moaon_hub_bot 같은 봇 지정 명령을 사용하면 명확해요. 개인·학습·관리·광고비서는 현재 각각 한 개인 계정만 연결하므로 다른 사용자로 바꾸면 기존 사용자의 접근이 바뀝니다. 다른 사람의 개인 대화가 자동 공유되는 것은 아닙니다.','bots'],
 ['관리비서·광고비서는 어떻게 시작하나요?','각 봇에서 /start를 보낸 뒤 텔레그램 봇 설정에 토큰과 본인 숫자 ID를 저장하고 Hermes 대화 연결을 켜세요. 적용 후 메뉴를 보내면 관리비서는 연결·자료 상태, 광고비서는 저장 네이버 보고서와 수익 검토를 볼 수 있어요. 광고 보고서는 연결 설정의 보고서 조회 권한이 필요합니다. 전용 예약 발송과 Hermes 외부 장애 감시는 별도 개발 대상이며 현재 메뉴는 조회·안내 기능입니다.','bots'],
