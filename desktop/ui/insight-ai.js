...
(()=>{
 ...
 function renderChat(draft,hadFocus){
  ...
  function bubble(role,text){const item=el('article',`insight-ai-message is-${role}`);item.append(el('strong','',role==='user'?'나':'CLOVA'),el('p','',text));transcript.append(item);return item;}
  ...
  if(!chain.length){const welcome=el('div','general-chat-welcome');const name=document.querySelector('.settings-profile-identity strong')?.textContent;welcome.append(el('h3','',name&&!['내 계정','프로필'].includes(name)?name+'님, 안녕하세요':'안녕하세요'),el('p','','보고서에서 무엇이 궁금하세요?'));transcript.append(welcome);transcript.append(el('p','insight-ai-chat-empty',configuration?.ready===false?'AI 연결 준비 필요 · CLOVA 계정 연결 후 답변할 수 있어요.':configuration?.enabled===false?'분석 AI 사용이 꺼져 있어요. 기존 보고서는 계속 볼 수 있어요.':'선택한 네이버 광고 보고서에서 궁금한 점을 질문해 주세요. 답변은 자료 확인 후 생성됩니다.'));}
  if(busy&&pendingQuestion)bubble('user',pendingQuestion);
  if(busy){const thinking=el('div','general-chat-thinking');thinking.setAttribute('role','status');thinking.append(el('span','','CLOVA가 자료를 살펴보고 있어요'));for(let i=0;i<3;i++)thinking.append(el('i'));transcript.append(thinking);}
  panel.append(transcript);
  const composer=el('div','insight-ai-composer'),inputShell=el('div','general-chat-input-shell'),suggestions=el('div','insight-ai-suggestions');suggestions.setAttribute('aria-label','질문 예시');
  ...
  const label=el('label','','이 자료에 질문하기');label.htmlFor='insight-ai-question';const input=el('textarea');input.id='insight-ai-question';input.dataset.aiQuestion='';input.maxLength=500;input.rows=2;input.value=busy?'':draft;input.disabled=busy;input.placeholder='네이버 광고 자료에 대해 질문해 주세요';input.setAttribute('aria-describedby','insight-ai-question-help');
  const help=el('p','insight-ai-meta');help.id='insight-ai-question-help';const send=button('질문 보내기',()=>generate('QUESTION'));send.dataset.aiSend='';send.className='general-chat-send';send.textContent='';send.setAttribute('aria-label','질문 보내기');send.title='질문 보내기';const arrow=document.createElementNS('http://www.w3.org/2000/svg','svg');arrow.setAttribute('viewBox','0 0 24 24');arrow.setAttribute('aria-hidden','true');const arrowPath=document.createElementNS('http://www.w3.org/2000/svg','path');arrowPath.setAttribute('d','M12 19V5m-6 6 6-6 6 6');arrow.append(arrowPath);send.append(arrow);const unavailable=configuration?.enabled===false||configuration?.ready===false;
  const update=()=>{const length=Array.from(input.value.trim()).length;help.textContent=`${length} / 500자 · ${run?.turn||0} / 6턴 · 개인정보는 입력하지 마세요`;send.disabled=busy||historyBusy||!ids.length||!length||length>500||(run?.turn||0)>=6||unavailable;};input.oninput=()=>{pendingQuestion='';draftText=input.value;update();};input.onkeydown=event=>{if(event.key==='Enter'&&!event.shiftKey&&!event.isComposing&&event.keyCode!==229){event.preventDefault();if(!send.disabled)void generate('QUESTION');}};update();
  label.className='clova-input-label';inputShell.append(label,input);const state=el('p','insight-ai-chat-status',status);state.setAttribute('role','status');const controls=el('div','insight-ai-actions');controls.append(el('span','clova-provider-label','CLOVA · 네이버 보고서'));if(busy)controls.append(button('대기 취소',cancel));controls.append(send);inputShell.append(controls);composer.append(inputShell,help,state);panel.append(composer);if(!panel.isConnected)document.querySelector('.app-area').append(panel);
 ...
 const launcherIcon=document.createElementNS('http://www.w3.org/2000/svg','svg');launcherIcon.setAttribute('viewBox','0 0 24 24');launcherIcon.setAttribute('fill','none');launcherIcon.setAttribute('stroke','currentColor');launcherIcon.setAttribute('stroke-width','1.7');launcherIcon.setAttribute('stroke-linecap','round');launcherIcon.setAttribute('stroke-linejoin','round');launcherIcon.setAttribute('aria-hidden','true');const launcherPath=document.createElementNS('http://www.w3.org/2000/svg','path');launcherPath.setAttribute('d','M20 11.5a8 8 0 0 1-8 8H5l-3 2v-10a8 8 0 0 1 8-8h2M17 2l1.3 3.7L22 7l-3.7 1.3L17 12l-1.3-3.7L12 7l3.7-1.3L17 2');launcherIcon.append(launcherPath);launcher.prepend(launcherIcon);
 ...
 function render(){
  ...
  const draft=busy?'':($('insight-ai-question')?.value??draftText);if(!busy)draftText=draft;const hadFocus=document.activeElement?.id==='insight-ai-question';transcriptResize?.disconnect();if(!questionOpen)$('insight-ai-chat')?.remove();document.querySelector('.app-area')?.classList.toggle('has-global-ai',questionOpen);
