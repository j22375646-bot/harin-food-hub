...
(()=>{
 ...
 let historyGeneration=0,pendingQuestion='',transcriptResize=null;
 ...
 function restorePendingDraft(){const input=$('insight-ai-question');if(input&&pendingQuestion){input.value=pendingQuestion;pendingQuestion='';}}
 function cancel(){restorePendingDraft();generation++;busy=false;void window.moaonHub?.cancelInsightAi?.();notice('CANCELLED');render();}
 function discardDraft(){pendingQuestion='';const input=$('insight-ai-question');if(input)input.value='';}
 ...
 async function generate(kind){
  ...
  const expected=++generation;busy=true;pendingQuestion=question;notice('PENDING');render();
  try{const response=await window.moaonHub.insightAi({operation:'GENERATE',input});if(expected!==generation)return;
   if(response?.ok&&response.run){run=response.run;pendingQuestion='';notice(run.status);if(run.id)runs=[run,...runs.filter(r=>r.id!==run.id)].slice(0,50);}
 ...
 function renderChat(draft,hadFocus){
  ...
  const head=el('header','insight-ai-chat-heading'),title=el('h2','','네이버 자료와 대화');title.id='insight-ai-chat-title';head.append(title,button('닫기',()=>closeQuestion(true)));panel.append(head);
  panel.append(el('p','insight-ai-chat-scope',ids.map(id=>{const report=reports.find(r=>r.id===id);return `${date(report?.periodStart)} — ${date(report?.periodEnd)}`;}).join(' / ')));
