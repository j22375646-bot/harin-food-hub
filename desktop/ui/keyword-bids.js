...
(()=>{
 ...
 function mount(parent,row,search){
  ...
  async function call(input){state.busy=true;render();let r;try{r=await window.moaonHub.keywordBid(input);}catch{r={ok:false,code:'BID_RESULT_UNKNOWN'};}if(epoch!==generation)return;state.busy=false;if(!r.ok)state.message=errors[r.code]||'입찰 상태를 확인하지 못했습니다. 연결과 광고 설정을 확인해 주세요.';return r;}
  ...
  function render(){
   ...
   body.append(el('p','','원가·상품별 허용 광고비가 확인되기 전에는 감액만 가능합니다. 기존 웹허브 정책에 따라 7일 내 중복 변경을 제한합니다.'));
