...
(()=>{
 ...
 function render(){
  ...
  rows.sort((a,b)=>{if($('cs-sort').value==='DEADLINE')return (tools.deadline(a).due??Infinity)-(tools.deadline(b).due??Infinity)||a.id.localeCompare(b.id);if(!a.occurredAt||!b.occurredAt)return Number(!a.occurredAt)-Number(!b.occurredAt)||a.id.localeCompare(b.id);return ($('cs-sort').value==='OLDEST'?1:-1)*(Date.parse(a.occurredAt)-Date.parse(b.occurredAt))||a.id.localeCompare(b.id);});
 ...
 $('cs-reset').onclick=()=>{tools.reset();for(const id of ['cs-platform','cs-kind','cs-content-filter'])$(id).value='ALL';$('cs-sort').value='NEWEST';$('cs-search').value='';pageIndex=0;selected=null;render();$('cs-search').focus({preventScroll:true});};
