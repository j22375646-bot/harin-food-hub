...
(() => {
  ...
  let rows = [], editing = null, busy = false, generation = 0, ready = false, mode = 'edit';
  ...
  function render() {
    ...
    if(ready) for(const row of visible) {
      ...
      const product=node('div','');product.append(node('strong',row.name),node('small',row.lot?'제조번호 '+row.lot:'제조번호 미입력'));
  ...
  function setMode(value) {
    mode=value;const adjustment=value!=='edit';$('stock-adjustment').hidden=!adjustment;
  ...
  function previewAdjustment() {
    ...
    $('stock-delta').setCustomValidity(delta<=0?'변경 수량을 입력하세요.':result<0?'보유 수량보다 많이 출고할 수 없습니다.':result>1e9?'보유 수량은 10억 이하로 입력하세요.':'');
  }
  ...
  window.moaonStock={ensure:refresh,clear:()=>{generation++;ready=false;rows=[];editing=null;$('stock-status').textContent='사업장 연결 후 재고를 사용할 수 있습니다.';$('stock-dialog').close();render();}};render();
})();
