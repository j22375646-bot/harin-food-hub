...
(()=>{
 ...
 function rows(){
  ...
  const source=snapshot()?.rows||[],filtered=source.filter(r=>(r.name.toLocaleLowerCase().includes(term))&&(type==='ALL'||r.type===type)&&(filter==='ALL'||filter==='ZERO'&&r.cost>0&&r.conversions===0||filter==='CONVERTED'&&r.conversions>0||filter==='UNKNOWN'&&(r.cost===null||r.conversions===null)));
  ...
  if(selected){detail.append(node('h2','',selected.name),node('p','',`${snapshot().start} ~ ${snapshot().end} · ${selected.type}`));const grid=node('div','keyword-detail-grid');for(const [k,label,unit]of [['impressions','노출','회'],['clicks','클릭','회'],['ctr','클릭률','%'],['cpc','클릭당 비용','원'],['revenue','전환매출','원'],['cvr','전환율','%'],['cpa','전환당 비용','원'],['roas','ROAS','%']]){const cell=node('div');cell.append(node('small','',label),node('strong','',fmt(selected[k],unit,unit==='%'?2:0)));grid.append(cell);}detail.append(grid,node('p','','전환에는 구매 외 행동이 포함될 수 있습니다. 전환 지연·표본·원가를 확인한 뒤 광고 변경을 판단하세요.'));
   const close=node('button','','상세 닫기');close.type='button';close.onclick=()=>{const id=chosen;chosen=null;rows();[...$('keyword-rows').querySelectorAll('button')].find(n=>n.dataset.keyword===id)?.focus();};detail.append(close);
  }
 }
 function render(){
  const w=snapshot();$('keyword-refresh').disabled=busy||displayMode!=='live';$('keyword-page').setAttribute('aria-busy',String(busy));
  $('keyword-period').textContent=w?.start?`${w.start} ~ ${w.end} · 가장 최근 저장 기간`:'기간 확인 필요';
 ...
 window.moaonKeywords={set:(next,state={})=>{data=next;busy=!!state.busy;failed=!!state.failed;if(!next)chosen=null;render();}};
 ...
 $('keyword-reset').onclick=()=>{$('keyword-search').value='';$('keyword-type').value='ALL';$('keyword-filter').value='ALL';$('keyword-sort').value='cost';chosen=null;rows();$('keyword-search').focus();};
 $('keyword-detail').onkeydown=e=>{if(e.key==='Escape')$('keyword-detail').querySelector('button')?.click();};render();
})();
