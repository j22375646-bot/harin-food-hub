...
(()=>{const root=document.querySelector('[data-page=assistant]');if(!root)return;
 ...
 function render(){enabled.checked=state.enabled;list.replaceChildren();list.append(el('h3','진행 중 '+state.activeCount+'건 · 최근 최대 100건'),el('p','마지막 점검: '+(state.checkedAt?new Date(state.checkedAt).toLocaleString('ko-KR'):'아직 없음'),'assistant-help'));
  ...
  for(const c of state.cases){const card=el('details','','assistant-card');const summary=el('summary','','assistant-case-summary');summary.append(el('span',({NAVER:'네이버',CAFE24:'카페24',COUPANG:'쿠팡'})[c.platform]||c.platform,'assistant-channel-tag'),el('strong',c.kind==='ORDER'?'송장 발급 전 주문':'미답변 문의'),el('span',names[c.status]||'확인 필요','assistant-case-state'));card.dataset.caseState=c.status;card.append(summary,el('p','참조: '+c.source_id),el('p','자료: '+(c.observed_state==='UNKNOWN'?'확인 필요 · 마지막 관측 이후 누락 또는 조회 실패':'모아온 저장 상태')+' · 마지막 관측 '+new Date(c.last_seen_at).toLocaleString('ko-KR'),'assistant-help'));
