...
test('phase 21-2 maps the current execution owners with automatic single-path protection',()=>{
  ...
  assert.equal(center.phase,'21-2');assert.equal(center.mode,'AUTO_GUARD');assert.equal(center.summary.lanes,6);assert.equal(center.worker.ready,true);
...
test('active duplicate idempotency keys block the topology instead of looking successful',()=>{
  ...
  assert.equal(center.summary.collisionKeys,1);assert.equal(center.summary.switchReady,false);assert.equal(center.collisions[0].count,2);assert.ok(center.lanes.every(lane=>lane.state==='COLLISION_RISK'));
});
...
test('execution path route, one-click check and service-role-only registry exist',()=>{
  ...
  assert.match(ui,/작업 실행 경로·전환센터/);assert.match(ui,/자동 보호/);assert.match(ui,/지금 중복 확인/);assert.match(css,/@media\(max-width:760px\).*executionFlow/s);
