...
const readyChannels = ["NAVER", "COUPANG", "CAFE24"].map((platform) => ({
  platform,
  capabilities: [
    { key: "inquiries", read: { status: "READY", label: "가능" } },
    { key: "claims", read: { status: "READY", label: "가능" } },
  ],
}));
...
test("미답변 문의를 오늘 처리와 기한 초과로 구분한다", () => {
  ...
  assert.equal(
    center.dueState({ occurredAt: "2026-08-14T08:00:00+09:00", now }).code,
    "TODAY",
  );
  assert.equal(
    center.dueState({ occurredAt: "2026-08-13T08:00:00+09:00", now }).code,
    "OVERDUE",
  );
  assert.equal(
    center.dueState({
      occurredAt: "2026-08-12T08:00:00+09:00",
      completed: true,
      now,
    }).code,
    "COMPLETED",
...
test("문의와 주문 상품 배송상태를 연결한다", () => {
  const result = center.buildUnifiedCustomerService({
    ...
    channelConnections: readyChannels,
    coupangOrders: [
      {
        order_id: "O1",
        shipment_box_id: "S1",
    ...
    coupangOrderItems: [
      { order_id: "O1", product_name: "작두콩차", quantity: 2 },
    ],
    coupangInquiries: [
      {
        inquiry_key: "Q1",
        ...
        order_id: "O1",
        answered: false,
        inquired_at: "2026-08-14T09:00:00+09:00",
      },
    ],
  });
    ...
    coupangInquiries: [
      {
        ...
        inquiry_id: "1",
        answered: true,
        inquired_at: "2026-08-13",
      },
    ],
    coupangReturns: [
      { receipt_id: "R1", status: "SUCCESS", requested_at: "2026-08-13" },
    ],
  });
...
test("취소와 반품을 구분하고 교환을 공통 단계로 만든다", () => {
  const result = center.buildUnifiedCustomerService({
    ...
    coupangExchanges: [
      { exchange_id: "E1", status: "PROGRESS", requested_at: "2026-08-14" },
    ],
  });
  assert.deepEqual(
    new Set(result.active.map((row) => row.kind)),
    new Set(["CANCEL", "RETURN", "EXCHANGE"]),
  );
...
test("권한만 확인되고 실제 수집기가 없는 채널은 수집 연결 대기로 표시한다", () => {
  ...
  const cafe24 = result.channelStates.find(
    (item) => item.platform === "CAFE24",
  );
  const coupang = result.channelStates.find(
    (item) => item.platform === "COUPANG",
  );
...
test("카페24·네이버 공통 수집 자료를 주문과 연결하고 수집기 0건도 정상으로 표시한다", () => {
  const result = center.buildUnifiedCustomerService({
    now: new Date("2026-08-14T12:00:00+09:00"),
    channelConnections: readyChannels,
    collectorPlatforms: ["CAFE24", "NAVER"],
    cafe24Orders: [
      {
        order_id: "O24",
    ...
    channelItems: [
      {
        source_key: "CAFE24:INQUIRY:6:1",
        platform: "CAFE24",
        kind: "INQUIRY",
        source_id: "6:1",
        source_subtype: "질문과답변",
        status: "WAITING",
        completed: false,
        order_id: "O24",
  ...
  assert.equal(
    result.rows.find((item) => item.platform === "CAFE24").order.amount,
    18000,
  );
  assert.equal(
    result.channelStates.find((item) => item.platform === "CAFE24").status,
    "READY",
  );
  assert.match(
    result.channelStates.find((item) => item.platform === "NAVER").message,
    /실수집 연결됨/,
  );
});
