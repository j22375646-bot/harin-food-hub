...
test('13-1 limits heavy route payloads and long client lists',()=>{
  ...
  assert.match(read('app/_products/harin-product-workbench.js'),/open\?<CatalogCards items=\{items\}\/\>:null/);
  assert.match(read('app/_products/harin-product-workbench.js'),/items\.slice\(0,visibleCount\)/);
});
