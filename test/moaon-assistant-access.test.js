...
test('Telegram reads resolve the verified member and recheck after loading private tasks',async()=>{
 const issuer={tenantId:A,userId:U,scopes:['tasks']},member={...issuer,userId:S};let checks=0,revoked=false;
 const database=()=>({rpc:async name=>name==='moaon_assistant_chat_context'?(++checks>1&&revoked?{error:{message:'ASSISTANT_AUTH_REQUIRED'}}:{data:member}):{data:issuer}});
 const request=()=>new Request('https://hub.example/api/moaon/assistant/read',{headers:{authorization:'Bearer moaon_ro_'+'a'.repeat(43),'x-moaon-bot-slot':'WORK','x-moaon-telegram-user':'101','x-moaon-telegram-chat':'101'}});
 const load=async({context})=>{assert.equal(context.userId,S);return {sources:{tasks:{scope:'ASSIGNED_TO_ME'}},caveats:[]};};
 assert.equal((await createRead({database,load})(request())).status,200);assert.equal(checks,2);
 checks=0;revoked=true;assert.equal((await createRead({database,load})(request())).status,401);
});
