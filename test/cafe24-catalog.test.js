...
test('화면과 서버 저장 경로가 판매중 상품만 매칭·원가 대상으로 사용한다', () => {
  ...
  const productWorkbench = fs.readFileSync(path.join(root,'app','_products','harin-product-workbench.js'),'utf8');
  ...
  assert.match(productWorkbench,/sellableMasterProducts/);
  assert.match(productWorkbench,/품절 상품/);
  assert.match(productWorkbench,/판매중단 상품/);
