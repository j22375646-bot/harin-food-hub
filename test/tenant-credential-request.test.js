...
test('simultaneous requests are bounded and the slot is released after completion',async()=>{
 ...
 const handle=createCredentialSaveRequest({origin,maxConcurrent:1,save:async()=>{entered();await new Promise(r=>release=r);return {tenantId,provider:'NAVER',revision:1,status:'SAVED_UNVERIFIED'};}});
...
test('aborted stalled body releases the request without starting a write',async()=>{
 ...
 const handle=createCredentialSaveRequest({origin,save:async()=>{writes++;}}),pending=handle(req);controller.abort();
...
test('unknown errors after write starts return an uncertain result rather than a retryable storage failure',async()=>{
 const result=await createCredentialSaveRequest({origin,save:async()=>{throw Error('private database detail');}})(request());
