...
test('V106 navigation exposes all 17 primary routes once after diagnoses moved into insights',()=>{
  ...
  assert.equal(model.items.find(item=>item.id==='calendar').description,'일정·메모 관리');
});

test('저빈도 기록 페이지는 메뉴 클릭 전 자동으로 받지 않는다',()=>{
  const model=navigation.buildPhase28Navigation();
  const onDemandIds=['changes','validation','experiments','knowledge'];
  assert.deepEqual(
