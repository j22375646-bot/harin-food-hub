...
test('loader follows stable inclusive pages beyond Supabase first 1000 rows',async()=>{
 ...
 const m=await loadNaverMarketing({db,now});assert.deepEqual(ranges,[[0,999],[1000,1999]]);assert.equal(m.windows[1].metrics.cost,1001000);assert.equal(m.windows[1].campaigns.length,50);assert.equal(m.windows[1].campaignCount,1001);
});
test('a failed later page discards partial sums instead of presenting an understated total',async()=>{
 const {loadNaverMarketing}=require('../lib/dashboard/naver-marketing-loader.js');const db={from(table){const q={};for(const method of ['select','eq','gte','lte','order','limit'])q[method]=()=>q;q.range=a=>{q.page=a;return q;};q.then=(ok,fail)=>Promise.resolve({data:table==='naver_stats_daily'?Array.from({length:1000},()=>row('2026-09-11')):[],error:q.page?{message:'private'}:null}).then(ok,fail);return q;}};const m=await loadNaverMarketing({db,now});assert.equal(m.windows[1].metrics.cost,null);assert.equal(m.windows[1].status,'UNAVAILABLE');
});
