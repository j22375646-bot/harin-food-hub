...
test('loader scopes both dates, deterministic cost ordering, bounded rows and exact count',async()=>{
 ...
 const db={from(){const index=count++,q={};for(const method of ['select','order','limit','eq'])q[method]=(...args)=>{ops.push([index,method,...args]);return q;};q.then=(yes,no)=>Promise.resolve({data:index===0?[row({})]:[row({salesAmt:2})],count:256,error:null}).then(yes,no);return q;}};
 const w=await loadKeywordWorkbench(db);assert.equal(w.total,256);assert.equal(w.rows.length,1);assert.deepEqual(ops.filter(r=>r[1]==='eq'),[[1,'eq','period_start','2026-09-01'],[1,'eq','period_end','2026-09-07']]);assert.ok(ops.some(r=>r[1]==='limit'&&r[2]===200));
...
test('search-term samples preserve missing conversions and capped samples cannot understate spend',async()=>{
 ...
 for(const size of [1,5]){let index=0;const db={from(table){assert.equal(table,'naver_search_terms');const call=index++,q={};for(const m of ['select','eq','order','limit'])q[m]=()=>q;q.then=(yes,no)=>Promise.resolve({data:call===0?[row({})]:[{...row({samples:Array.from({length:size},()=>({impCnt:20,clkCnt:2,salesAmt:100}))}),id:'term-id',search_term:'티백'}],count:1}).then(yes,no);return q;}};const result=await loadKeywordWorkbench(db,'SEARCH_TERMS');assert.equal(result.rows[0].name,'티백');assert.equal(result.rows[0].cost,size===1?100:null);assert.equal(result.rows[0].conversions,null);}
});
