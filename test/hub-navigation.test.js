...
test('phase 13-6 focused workspaces have real addresses and restore their exact state', () => {
  ...
  assert.deepEqual(HUB_WORKSPACES.collection.map(item=>item.id),['overview','naver-api','advertising','provider-fallback','optional-providers','provider-runtime','owned-site','shipping-reference','operations-health']);
